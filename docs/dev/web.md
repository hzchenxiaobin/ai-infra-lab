...
│   └── pages/
│       ├── HomePage.tsx            # 组卷页（/start，方向/范围/题量 + 最近场次）
│       ├── LandingPage.tsx         # 首页落地页（/，对齐 ai-infra-notes 版式；landing.css 同目录）
...
| 路由 | 页面 | 状态 |
|---|---|---|
| `/` | 首页落地页（版式对齐 ai-infra-notes 首页：hero / 统计带 / 学习路线 / 专题 / 资源） | 已有（2026-09-22 改版；原学习路径总览移驻 `/learn`） |
| `/start` | 组卷首页（方向/范围/题量 + 最近场次） | 已有（2026-09-11 自 `/` 迁入） |
