...
- **判题数据**（judge 数据源，2026-09-10 第六批）：`judge-extract.ts` 从 algo 题解
  机器解析示例用例（`testcases`）与参考签名（`judge_meta`：方法名/C++ 参数类型/
  语言可用性）；`judge_type` 按解析能力推导（有示例 + 有签名 → internal；
  frontmatter 的 leetgpu-com 显式保留）。当前 4229 题中 3549 题可站内评测。
...
- **内容正文不入库**：只同步元数据与索引（02 关键取舍）；题解参考代码同样不入库，
  判题只需签名元数据（完整题解在 docs 站渲染）。
