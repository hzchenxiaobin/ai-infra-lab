...
import { BackArrowIcon, Button, Card, DifficultyBadge, ErrorBox, Loading, SegmentedControl } from "../components/ui";
...
export default function JudgePage() {
  ...
  if (isLegacyNumeric) {
    return (
      <div className="itv py-20 text-center text-sm text-muted">
        评测入口已升级为统一题目 ID，请从
  ...
  if (problem.error) {
    return (
      <div className="itv space-y-3 pt-24">
  ...
  return (
    <div className="itv">
      {/* 标题区：与首页同款 hero（工作台紧凑版） */}
      <section className="hero hero--slim">
        <div className="hero-inner">
          <div className="hero-eyebrow">面试 · 在线评测</div>
          <h1 className="hero-title">
            {data.problem.number > 0 && (
              <span className="mr-2 align-middle font-mono text-base font-normal text-muted">
                #{data.problem.number}
              </span>
            )}
            <span className="hero-title-accent">{data.problem.title}</span>
          </h1>
          <p className="hero-subtitle">
            <DifficultyBadge difficulty={data.problem.difficulty} /> · 评测用例为题面示例（LeetCode
            不公开完整测试集）
          </p>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                {submit.error && <span className="err-text text-xs">{submit.error.message}</span>}
              </div>
            </Card>
