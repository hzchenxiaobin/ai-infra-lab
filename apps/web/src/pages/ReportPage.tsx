  ...
  if (progress.stage === "failed") {
    return (
      <Card className="py-8 text-center">
        <p className="err-text text-sm">评估失败：{progress.error ?? "未知错误"}</p>
        <Button
          className="mt-4"
        ...
        {regenerate.error && (
          <p className="err-text mt-3 text-sm">生成失败：{regenerate.error.message}</p>
        )}
      </Card>
    );
  }
    ...
    <div className="itv">
      ...
      <section className="hero">
        <div className="hero-inner">
          <div className="hero-eyebrow">面试 · 评估报告</div>
          <h1 className="hero-title">
            <span className="hero-title-accent">{session.title}</span>
          </h1>
          <p className="hero-subtitle">
            {`方向：${cats} · 题数：${session.questionIds.length}${duration != null ? ` · 时长：${duration} 分钟` : ""}`}
          </p>
        </div>
      </section>
