...
function buildEvaluationMessages(transcript: GroupedTranscript): ChatMessage[] {
  ...
  const system = [
    ...
    `{"overallGrade":"A|B|C|D","summary":"一句话总评","questions":[{"questionId":数字,"dimensions":[{"name":"维度名","grade":"A|B|C|D"}],"diagnosis":"诊断","suggestion":"改进建议","answers":["主问题的参考答案","第1个追问的参考答案"]}],"weakDimensions":["薄弱维度Top2"]}`,
    "questions 数组必须覆盖每一道题，questionId 与输入一致；diagnosis/suggestion/answers 用中文、具体、可执行。",
    "answers 字段：与对话中面试官的每一次提问（主问题 + 每一次追问）一一对应、顺序一致、数量一致；每条答案针对对应问题完整作答，包含关键知识点、正确思路和要点（若为算法题需包含最优解法与复杂度）；答案内部不要用行首 '-' 的无序列表。",
  ].join("\n");
      ...
      try {
        ...
        for (const g of transcript.groups) {
          ...
          questions.push({
            ...
            answers: [g.question.keyPoints || "（无参考答案）"],
          });
        }
        questions.sort(
          (a, b) =>
