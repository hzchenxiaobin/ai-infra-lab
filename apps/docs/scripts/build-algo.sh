...
# 单批内存 ~6GB；如仍 OOM，调大 BATCH_TOTAL 或 NODE_OPTIONS。
set -euo pipefail
cd "$(dirname "$0")/.."
...
for i in $(seq 0 $((BATCH_TOTAL - 1))); do
  ...
  BATCH_TOTAL=$BATCH_TOTAL BATCH_INDEX=$i \
    ...
    ./node_modules/.bin/vitepress build algo
done
node --experimental-strip-types scripts/merge-dist.mts algo "$BATCH_TOTAL"
rm -rf algo/dist_*
