...
id: "lc:contest:506q4"
type: problem
title: "至多 K 次交换后最大子数组和"
tags: [前缀和, 枚举, 贪心, 顺序统计]
knowledge_points: [enumeration, greedy, prefix-sum, 顺序统计]
updated: 2026-09-03
source: contest
number: 3962
difficulty: hard
...
# LeetCode 至多 K 次交换后最大子数组和 题解

## 1. 题目概述

- **标题 / 题号**：至多 K 次交换后最大子数组和（#3962，hard）
- **链接**：https://leetcode.cn/problems/maximum-subarray-sum-after-at-most-k-swaps/
- **来源**：LeetCode 第 506 场周赛 Q4
- **难度**：困难
- **标签**：枚举、贪心、顺序统计、前缀和

**题意**：给定整数数组 `nums` 和整数 `k`。你可以对数组执行**至多** `k` **次**交换操作（每次选任意两个下标 `i, j` 交换 `nums[i]` 和 `nums[j]`）。返回交换后可能的**最大子数组和**。
...
- **时间复杂度**：`O(n³)`。枚举 `[l,r]` 共 `O(n²)` 个子数组，每个子数组维护有序数组（vector 插入/删除移位）`O(n)` + 计算增益 `O(min(n,k))`，因 `min(n,k) <= n`，维护项占优，每步 `O(n)`。`n=1500` 时约 `3.4×10^9`，C++ 勉强可通过（增益循环提前终止，实际远快于最坏）；Python 同算法必然超时，需用下方 Fenwick 优化版。
