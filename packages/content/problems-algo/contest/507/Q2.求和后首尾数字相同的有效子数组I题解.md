...
# LeetCode 求和后首尾数字相同的有效子数组 I 题解

## 1. 题目概述

- **标题 / 题号**：求和后首尾数字相同的有效子数组 I（#3969，medium）
- **链接**：https://leetcode.cn/problems/valid-subarrays-with-matching-sum-digits-i/
- **来源**：LeetCode 第 507 场周赛 Q2
- **难度**：中等
- **标签**：数组、前缀和、枚举

**题意**：给定整数数组 `nums` 和数字 `x`（`1 <= x <= 9`）。子数组 `nums[l..r]` 是**有效子数组**当且仅当其元素和的**首位数字**和**末位数字**都等于 `x`。返回有效子数组的数量。
