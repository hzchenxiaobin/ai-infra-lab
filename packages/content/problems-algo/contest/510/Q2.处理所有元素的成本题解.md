...
title: 处理所有元素的成本
tags: [数学, 等差数列, 贪心]
knowledge_points: [arithmetic-sequence, greedy, math, 等差数列]
updated: 2026-09-03
source: contest
number: 3987
difficulty: medium
...
# LeetCode 处理所有元素的成本 题解

## 1. 题目概述

- **标题 / 题号**：处理所有元素的成本（#3987，medium）
- **链接**：https://leetcode.cn/problems/minimum-total-cost-to-process-all-elements/
- **来源**：LeetCode 第 510 场周赛 Q2
- **难度**：中等
- **标签**：贪心、数学、等差数列

**题意**：给定数组 `nums` 和整数 `k`。初始有 `k` 单位资源，从左到右依次处理元素，处理第 `i` 个消耗 `nums[i]` 资源。若资源不足，可执行操作：每次操作增加 `k` 资源，第 `cnt` 次操作成本为 `cnt`（1, 2, 3, ...）。返回处理完所有元素的**最小总成本**，对 `10^9+7` 取模。
...
public:
    int minimumCost(vector<int>& nums, int k) {
        ...
        const long long INV2 = 500000004;  // 2 的模逆元 (mod 1e9+7)
        ...
        for (int x : nums) {
            if (cur < x) {
                ...
                //      = ops * (2*cnt + ops + 1) / 2
                // ops 与 (2*cnt+ops+1) 必一偶，故可先取模再用模逆元还原 /2
                long long a = ops % MOD;
                long long b = (2 * cnt % MOD + ops % MOD + 1) % MOD;
                long long cost = a * b % MOD * INV2 % MOD;
                ans = (ans + cost) % MOD;
                cur += ops * k;
                cnt += ops;
            }
