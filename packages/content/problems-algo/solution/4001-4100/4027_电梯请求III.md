...
type: problem
title: 电梯请求III
tags: [位运算, 动态规划, 数组, 状态压缩]
knowledge_points: [array, bit-manipulation, bitmask-dp, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 4027
difficulty: hard
...
- **标题 / 题号**：电梯请求III（#4027，hard）
- **链接**：https://leetcode.cn/problems/elevator-requests-iii/
- **难度**：困难
- **标签**：位运算、数组、动态规划、状态压缩

**题意**：给你一个整数 `n` 表示一栋建筑的楼层数，楼层编号从 `0` 到 `n - 1`。同时给你一个整数 `start`，表示电梯的起始楼层，以及一个二维整数数组 `requests`，其中 `requests[i] = [arrival_i, floor_i]` 表示在时间 `arrival_i` 发出了一个前往楼层 `floor_i` 的请求。
