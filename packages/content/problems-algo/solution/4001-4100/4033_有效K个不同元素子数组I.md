...
type: problem
title: 有效K个不同元素子数组I
tags: [前缀异或, 哈希表, 数组, 滑动窗口, 随机化]
knowledge_points: [array, hash-table, prefix-xor, randomized, sliding-window]
updated: 2026-09-03
source: leetcode
number: 4033
difficulty: hard
...
# LeetCode 有效K个不同元素子数组I 题解

## 1. 题目概述

- **标题 / 题号**：有效K个不同元素子数组I（#4033，hard）
- **链接**：https://leetcode.cn/problems/valid-k-unique-subarrays-i/
- **难度**：困难
- **标签**：数组、哈希表、前缀异或、滑动窗口、随机化

**题意**：给定一个整数数组 `nums` 和一个整数 `k`，同时给定一个二维整数数组 `queries`，其中 `queries[i] = [li, ri]` 表示子数组 `nums[li..ri]`。
