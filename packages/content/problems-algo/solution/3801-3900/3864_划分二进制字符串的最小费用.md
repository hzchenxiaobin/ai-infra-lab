...
title: 划分二进制字符串的最小费用
tags: [分治, 前缀和, 字符串]
knowledge_points: [divide-and-conquer, prefix-sum, string]
updated: 2026-09-03
source: leetcode
number: 3864
difficulty: hard
...
# LeetCode 划分二进制字符串的最小费用 题解

## 1. 题目概述

- **标题 / 题号**：划分二进制字符串的最小费用（#3864，hard）
- **链接**：https://leetcode.cn/problems/minimum-cost-to-partition-a-binary-string/
- **难度**：困难
- **标签**：字符串、分治、前缀和

**题意**：给你一个二进制字符串 `s` 和两个整数 `encCost` 与 `flatCost`。
