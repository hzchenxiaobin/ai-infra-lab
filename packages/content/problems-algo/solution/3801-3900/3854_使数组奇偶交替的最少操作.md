...
# LeetCode 使数组奇偶交替的最少操作 题解

## 1. 题目概述

- **标题 / 题号**：使数组奇偶交替的最少操作（#3854，medium）
- **链接**：https://leetcode.cn/problems/minimum-operations-to-make-array-parity-alternating/
- **难度**：中等
- **标签**：贪心、数组

**题意**：给你一个整数数组 `nums`。如果对每个下标 $i$（$0 \le i < n-1$）都有 `nums[i]` 与 `nums[i+1]` **奇偶性不同**（一奇一偶），则称数组是**奇偶交替**的。一次操作可以任选下标 $i$，将 `nums[i]` **加 1 或减 1**。返回长度为 2 的数组 `answer`：
