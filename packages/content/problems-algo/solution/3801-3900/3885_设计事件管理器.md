...
type: problem
title: 设计事件管理器
tags: [哈希表, "堆（优先队列）", 有序集合, 设计]
knowledge_points: [design, hash-table, heap, ordered-set]
updated: 2026-09-03
source: leetcode
number: 3885
difficulty: medium
...
- **标题 / 题号**：设计事件管理器（#3885，medium）
- **链接**：https://leetcode.cn/problems/design-event-manager/
- **难度**：中等
- **标签**：设计、堆（优先队列）、哈希表、有序集合

**题意**：管理一组事件，每个事件有唯一的 `eventId` 和一个 `priority`（优先级）。实现 `EventManager` 类：
