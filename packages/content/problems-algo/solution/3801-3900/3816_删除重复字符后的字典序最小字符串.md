...
id: "lc:3816"
type: problem
title: 删除重复字符后的字典序最小字符串
tags: [单调栈, 哈希表, 字符串, 栈, 贪心]
knowledge_points: [greedy, hash-table, monotonic-stack, stack, string]
updated: 2026-09-03
source: leetcode
number: 3816
difficulty: hard
...
# LeetCode 删除重复字符后的字典序最小字符串 题解

## 1. 题目概述

- **标题 / 题号**：删除重复字符后的字典序最小字符串（#3816，hard）
- **链接**：https://leetcode.cn/problems/lexicographically-smallest-string-after-deleting-duplicate-characters/
- **难度**：困难
- **标签**：贪心、字符串、栈、哈希表、单调栈

**题意**：给你一个字符串 `s`，它由小写英文字母组成。你可以进行如下操作**任意次**（可能为零次）：
...
**约束**：$1 \le s.length \le 10^5$，`s` 仅包含小写英文字母。
