...
id: "lc:2633"
type: problem
title: "将对象转换为 JSON 字符串"
tags: [JSON, 字符串, 类型分派, 递归]
knowledge_points: [json, recursion, string, type-dispatch]
updated: 2026-09-03
source: leetcode
number: 2633
difficulty: medium
...
# LeetCode 将对象转换为 JSON 字符串 题解

## 1. 题目概述

- **标题 / 题号**：将对象转换为 JSON 字符串（#2633，medium）
- **链接**：https://leetcode.cn/problems/convert-object-to-json-string/
- **难度**：中等
- **标签**：递归、JSON、类型分派、字符串

**题意**：给定一个 **JSON 值** `object`，将其转换为 JSON 字符串。**不能**使用语言内置的 `JSON.stringify` 方法，需要手动实现序列化逻辑。
