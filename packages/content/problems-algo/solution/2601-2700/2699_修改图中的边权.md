...
title: 修改图中的边权
tags: [图, 堆/优先队列, 最短路径]
knowledge_points: [graph, heap, shortest-path]
updated: 2026-09-03
source: leetcode
number: 2699
difficulty: hard
...
- **标题 / 题号**：修改图中的边权（#2699，hard）
- **链接**：https://leetcode.cn/problems/modify-graph-edge-weights/
- **难度**：困难
- **标签**：图、最短路径、堆/优先队列

**题意**：给你一个 $n$ 个节点的**无向带权连通**图，节点编号 $0$ 到 $n-1$，再给你一个整数数组 `edges`，其中 `edges[i] = [a_i, b_i, w_i]` 表示节点 $a_i$ 和 $b_i$ 之间有一条权为 $w_i$ 的边。
...
![示例1](https://assets.leetcode.com/uploads/2023/04/18/graph.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/04/18/graph-2.png)
...
![示例3](https://assets.leetcode.com/uploads/2023/04/19/graph-3.png)
