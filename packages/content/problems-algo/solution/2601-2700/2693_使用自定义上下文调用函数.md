...
title: 使用自定义上下文调用函数
tags: ["this 绑定", 函数, 原型]
knowledge_points: [function, prototype, this-binding, 函数]
updated: 2026-09-03
source: leetcode
number: 2693
difficulty: medium
...
# LeetCode 使用自定义上下文调用函数 题解

## 1. 题目概述

- **标题 / 题号**：使用自定义上下文调用函数（#2693，medium）
- **链接**：https://leetcode.cn/problems/call-function-with-custom-context/
- **难度**：中等
- **标签**：函数、原型、this 绑定

**题意**：增强所有函数，使其具有 `callPolyfill` 方法。该方法接收一个对象 `obj` 作为第一个参数，以及任意数量的附加参数。`obj` 成为函数的 `this` 上下文。附加参数将传递给该函数（即 `callPolyfill` 方法所属的函数）。
