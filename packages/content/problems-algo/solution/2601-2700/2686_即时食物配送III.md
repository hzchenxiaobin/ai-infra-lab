...
type: problem
title: "即时食物配送 III"
tags: [AVG, "GROUP BY", ROUND, SQL, 布尔表达式聚合, 数据库, 百分比计算]
knowledge_points: [avg, database, group-by, round, sql, 布尔表达式聚合, 百分比计算]
updated: 2026-09-03
source: leetcode
number: 2686
difficulty: medium
...
# LeetCode 即时食物配送 III 题解

## 1. 题目概述

- **标题 / 题号**：即时食物配送 III（#2686，medium）
...
**题意**：给定一张 `Delivery` 表，记录客户的食物配送订单。每行包含订单日期（`order_date`）和客户期望配送日期（`customer_pref_delivery_date`）。若**期望配送日期与订单日期相同**，则该订单称为**即时订单（immediate）**；否则称为**计划订单（scheduled）**。
