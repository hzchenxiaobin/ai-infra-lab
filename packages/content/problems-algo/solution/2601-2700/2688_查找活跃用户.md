...
type: problem
title: 查找活跃用户
tags: ["COUNT() 聚合", "GROUP BY", "HAVING 组级过滤", "ORDER BY 排序", SQL, 数据库]
knowledge_points: [count-聚合, database, group-by, having-组级过滤, order-by, order-by-排序, sql]
updated: 2026-09-03
source: leetcode
number: 2688
difficulty: medium
...
**题意**：给定 `Users` 购买记录表，每行记录一次用户购买行为：用户编号、商品名称、购买时间、金额。编写 SQL 查询，找出**活跃用户**——即**至少购买过 2 次**（同一 `user_id` 在表中出现 ≥ 2 行）的用户。结果只包含 `user_id` 一列，按 `user_id` 升序排列。
