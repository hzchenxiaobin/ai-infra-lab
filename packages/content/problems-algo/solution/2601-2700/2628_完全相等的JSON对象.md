...
type: problem
title: "完全相等的 JSON 对象"
tags: [JSON, 深度比较, 类型判断, 递归]
knowledge_points: [json, recursion, type-checking, 深度比较]
updated: 2026-09-03
source: leetcode
number: 2628
difficulty: medium
...
# LeetCode 完全相等的 JSON 对象 题解

## 1. 题目概述

- **标题 / 题号**：完全相等的 JSON 对象（#2628，medium）
- **链接**：https://leetcode.cn/problems/json-deep-equal/
- **难度**：中等
- **标签**：递归、深度比较、JSON、类型判断

**题意**：给定两个 **JSON 值** `o1` 与 `o2`，判断它们是否「深度相等」。JSON 值的文法定义为：
