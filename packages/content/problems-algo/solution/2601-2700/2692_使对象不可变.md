...
type: problem
title: 使对象不可变
tags: [Proxy, 不可变性, 对象, 递归]
knowledge_points: [immutability, object, proxy, recursion, 对象]
updated: 2026-09-03
source: leetcode
number: 2692
difficulty: medium
...
**题意**：编写函数 `makeImmutable(obj)`，接收一个对象或数组 `obj`，返回其**不可变版本**：读取行为与原对象一致，但**任何修改尝试都应抛出错误**而非真正改变数据。具体要求（据 hints 与示例归纳）：
