...
type: problem
title: 找到最大开销的子字符串
tags: [动态规划, 数组, "最大子数组和（Kadane）"]
knowledge_points: [array, dynamic-programming, 最大子数组和-kadane]
updated: 2026-09-03
source: leetcode
number: 2606
difficulty: medium
...
# LeetCode 找到最大开销的子字符串 题解

## 1. 题目概述

- **标题 / 题号**：找到最大开销的子字符串（#2606，medium）
- **链接**：https://leetcode.cn/problems/find-the-substring-with-maximum-cost/
- **难度**：中等
- **标签**：数组、动态规划、最大子数组和（Kadane）

**题意**：给定字符串 `s`、一个**字符互不相同**的字符串 `chars` 和与 `chars` 等长的整数数组 `vals`。每个字符的**价值**定义如下：
...
| 918 | [环形子数组的最大和](https://leetcode.cn/problems/maximum-sum-circular-subarray/)（[题解](../0901-1000/918_最大环形子数组和.md)） | Kadane 进阶，环上最大 = `total - min_sum`，复用同一累加和思想 |
