...
type: problem
title: 使子数组元素和相等
tags: [gcd, 中位数, 前缀和, 排序, 数学, 数组, 数论]
knowledge_points: [array, gcd, math, median, number-theory, prefix-sum, sorting]
updated: 2026-09-03
source: leetcode
number: 2607
difficulty: medium
...
# LeetCode 使子数组元素和相等 题解

## 1. 题目概述

- **标题 / 题号**：使子数组元素和相等（#2607，medium）
- **链接**：https://leetcode.cn/problems/make-k-subarray-sums-equal/
- **难度**：中等
- **标签**：数组、数学、数论、排序、中位数、前缀和、gcd

**题意**：给你一个下标从 **0** 开始的整数数组 `arr` 和一个整数 `k`。数组 `arr` 是一个**循环数组**：最后一个元素的下一个是首元素，首元素的前一个是末元素。
