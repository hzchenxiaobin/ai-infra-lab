...
type: problem
title: 检查是否是类的对象实例
tags: [JavaScript, 原型链, 类型判断]
knowledge_points: [javascript, prototype-chain, type-checking]
updated: 2026-09-03
source: leetcode
number: 2618
difficulty: medium
...
# LeetCode 检查是否是类的对象实例 题解

## 1. 题目概述

- **标题 / 题号**：检查是否是类的对象实例（#2618，medium）
- **链接**：https://leetcode.cn/problems/check-if-object-instance-of-class/
- **难度**：中等
- **标签**：JavaScript、原型链、类型判断

**题意**：编写函数 `checkIfInstanceOf(obj, classFunction)`，判断 `obj` 是否是 `classFunction` **或其超类**的实例。传入的数据类型不做限制——`obj` 与 `classFunction` 都可能是 `undefined`、原始值、函数或对象。
