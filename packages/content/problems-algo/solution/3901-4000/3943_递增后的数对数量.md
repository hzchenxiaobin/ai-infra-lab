...
title: 递增后的数对数量
tags: [分块, 哈希表, 数组, 计数]
knowledge_points: [array, counting, hash-table, sqrt-decomposition]
updated: 2026-09-03
source: leetcode
number: 3943
difficulty: hard
...
# LeetCode 递增后的数对数量 题解

## 1. 题目概述

- **标题 / 题号**：递增后的数对数量（#3943，hard）
- **链接**：https://leetcode.cn/problems/number-of-pairs-after-increment/
- **难度**：困难
- **标签**：数组、哈希表、分块、计数

**题意**：给你两个整数数组 `nums1` 和 `nums2`，以及一个二维整数数组 `queries`。每个 `queries[i]` 属于以下两种类型之一：
