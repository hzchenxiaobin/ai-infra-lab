...
tags: [动态规划, 图, "堆（优先队列）", 最短路]
knowledge_points: [dynamic-programming, graph, heap, shortest-path]
updated: 2026-09-03
source: leetcode
number: 3977
difficulty: hard
...
# LeetCode 有限电量到达目标节点的最少时间 题解

## 1. 题目概述

- **标题 / 题号**：有限电量到达目标节点的最少时间（#3977，hard）
- **链接**：https://leetcode.cn/problems/minimum-time-to-reach-target-with-limited-power/
- **难度**：困难
- **标签**：图、最短路、堆（优先队列）、动态规划

**题意**：给你一个 $n$ 个节点的**有向加权图**，`edges[i] = [u_i, v_i, t_i]` 表示一条从 $u_i$ 到 $v_i$、耗时 $t_i$ 秒的有向边。信号在时间 $0$ 从 `source` 出发，携带 `power` 单位电量，行进规则：
...
![示例1](https://assets.leetcode.com/uploads/2026/02/09/g1.png)
...
![示例2](https://assets.leetcode.com/uploads/2026/02/09/g22.png)
...
![示例3](https://assets.leetcode.com/uploads/2026/02/09/g23.png)
