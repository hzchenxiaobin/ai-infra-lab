...
id: "lc:3911"
type: problem
title: "移除子数组元素后第 K 小偶数"
tags: [二分查找, 二分答案, 前缀和, 数组]
knowledge_points: [array, binary-search, binary-search-answer, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 3911
difficulty: hard
...
# LeetCode 移除子数组元素后第 K 小偶数 题解

## 1. 题目概述

- **标题 / 题号**：移除子数组元素后第 K 小偶数（#3911，hard）
- **链接**：https://leetcode.cn/problems/k-th-smallest-remaining-even-integer-in-subarray-queries/
- **难度**：困难
- **标签**：二分查找、二分答案、前缀和、数组

**题意**：给你一个**严格递增**的整数数组 $nums$，以及二维整数数组 $queries$，其中 $queries[i] = [l_i, r_i, k_i]$。
