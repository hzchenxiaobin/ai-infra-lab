...
type: problem
title: 通往最近公共祖先的不同门径
tags: [位运算, 倍增, 动态规划, 数学, 最近公共祖先, 树]
knowledge_points: [binary-lifting, bit-manipulation, dynamic-programming, lca, math, tree]
updated: 2026-09-03
source: leetcode
number: 3973
difficulty: hard
...
# LeetCode 通往最近公共祖先的不同门径 题解

## 1. 题目概述

- **标题 / 题号**：通往最近公共祖先的不同门径（#3973，hard）
- **链接**：https://leetcode.cn/problems/distinct-gate-paths-to-lca/
- **难度**：困难
...
**题意**：给定一棵以节点 $0$ 为根、共 $n$ 个节点的**有根树**，由 `parent` 数组描述（$parent[0] = -1$）。节点 $i$（$i \ge 1$）与它的父节点之间设有若干扇**门**，颜色分三类：$gates[i] = [r_i, b_i, w_i]$ 表示这条父边上有 $r_i$ 扇**红门**、$b_i$ 扇**蓝门**、$w_i$ 扇**白门**。
