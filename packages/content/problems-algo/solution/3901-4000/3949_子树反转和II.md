...
# LeetCode 子树反转和 II 题解

## 1. 题目概述

- **标题 / 题号**：子树反转和 II（#3949，hard）
- **链接**：https://leetcode.cn/problems/subtree-inversion-sum-ii/
- **难度**：困难
- **标签**：树、深度优先搜索、数组、动态规划
...
**题意**：给你一棵以节点 $0$ 为根、包含 $n$ 个节点的无向树，由边数组 `edges` 表示。同时给定整数 $k$ 和长度为 $n$ 的整数数组 `nums`，其中 $\text{nums}[i]$ 是节点 $i$ 的值。
