...
# LeetCode 任务完成时间 I 题解

## 1. 题目概述

- **标题 / 题号**：任务完成时间 I（#3965，medium）
- **链接**：https://leetcode.cn/problems/finish-time-of-tasks-i/
- **难度**：中等
- **标签**：树、深度优先搜索、数组、动态规划

**题意**：给定整数 `n`，表示项目中的任务数量，编号 `0` 到 `n-1`。任务构成一棵**以任务 0 为根的树**，由长度为 `n-1` 的二维数组 `edges` 给出，其中 `edges[i] = [u_i, v_i]` 表示任务 `u_i` 是任务 `v_i` 的**父节点**（即边已经从父指向子）。
