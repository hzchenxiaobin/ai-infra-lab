...
type: problem
title: 员工任务持续时间和并发任务
tags: [FLOOR, LEAD, SQL, 区间离散化, 差分与前缀和, 扫描线, 数据库, 窗口函数, "范围 JOIN"]
knowledge_points: [database, floor, lead, sql, sweep-line, window-function, 区间离散化, 差分与前缀和, 范围-join]
updated: 2026-09-03
source: leetcode
number: 3156
difficulty: hard
...
# LeetCode 员工任务持续时间和并发任务 题解

## 1. 题目概述

- **标题 / 题号**：员工任务持续时间和并发任务（#3156，hard）
...
**题意**：给定 `Tasks`（任务）表，每行记录一个任务的归属员工与起止时间。同一员工的多个任务**可能重叠**——就像一边写代码一边挂着会议。编写 SQL 查询，对**每名员工**统计：
