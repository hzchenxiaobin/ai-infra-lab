...
tags: [分治, 动态规划, 数组, 线段树]
knowledge_points: [array, divide-and-conquer, dynamic-programming, segment-tree]
updated: 2026-09-03
source: leetcode
number: 3165
difficulty: hard
...
# LeetCode 不包含相邻元素的子序列的最大和 题解

## 1. 题目概述

- **标题 / 题号**：不包含相邻元素的子序列的最大和（#3165，hard）
- **链接**：https://leetcode.cn/problems/maximum-sum-of-subsequence-with-non-adjacent-elements/
- **难度**：困难
- **标签**：线段树、数组、分治、动态规划

**题意**：给你一个整数数组 `nums` 和一个二维数组 `queries`，其中 `queries[i] = [pos_i, x_i]`。
