...
type: problem
title: 第二天验证
tags: [DATEDIFF, "DISTINCT 去重", "EXISTS 半连接", "JOIN 连接", SQL, 数据库, 日期口径]
knowledge_points: [database, datediff, distinct-去重, exists-半连接, join-连接, sql, 日期口径]
updated: 2026-09-03
source: leetcode
number: 3172
difficulty: easy
...
**题意**：用户注册后会收到验证短信。给定 `emails`（注册表）与 `texts`（短信表）两张表，编写一个解决方案，找到所有**第二天验证注册**（即在注册后**第二天**收到 `Verified` 短信）的用户 ID。
