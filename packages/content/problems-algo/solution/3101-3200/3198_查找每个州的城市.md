...
type: problem
title: 查找每个州的城市
tags: ["GROUP BY", GROUP_CONCAT, SQL, 字符串聚合, 数据库, 聚合函数]
knowledge_points: [aggregate-function, database, group-by, group-concat, sql, 字符串聚合]
updated: 2026-09-03
source: leetcode
number: 3198
difficulty: easy
...
# LeetCode 查找每个州的城市 题解

## 1. 题目概述

- **标题 / 题号**：查找每个州的城市（#3198，easy）
...
**题意**：给定 `cities` 表，每行记录一个州名 `state` 与该州的一个城市名 `city`。编写解决方案，**查找每个州的所有城市**，并将它们组合成**一条逗号分隔**的字符串；结果按 `state` 与 `city` **升序**返回。
