...
type: problem
title: 找出唯一性数组的中位数
tags: [二分查找, 哈希表, 数组, 滑动窗口]
knowledge_points: [array, binary-search, hash-table, sliding-window]
updated: 2026-09-03
source: leetcode
number: 3134
difficulty: hard
...
# LeetCode 找出唯一性数组的中位数 题解

## 1. 题目概述

- **标题 / 题号**：找出唯一性数组的中位数（#3134，hard）
- **链接**：https://leetcode.cn/problems/find-the-median-of-the-uniqueness-array/
- **难度**：困难
- **标签**：数组，哈希表，二分查找，滑动窗口

**题意**：给你一个整数数组 `nums`。数组 `nums` 的**唯一性数组**是一个按元素**从小到大排序**的数组，包含 `nums` 的所有**非空子数组**中不同元素的个数。
