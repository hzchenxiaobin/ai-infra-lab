...
# LeetCode 无需开会的工作日 题解

## 1. 题目概述

- **标题 / 题号**：无需开会的工作日（#3169，medium）
- **链接**：https://leetcode.cn/problems/count-days-without-meetings/
- **难度**：中等
- **标签**：数组、排序、区间合并

**题意**：给你一个正整数 `days`，表示员工可工作的总天数（从第 1 天开始）。另给你一个二维数组 `meetings`，长度为 `n`，其中 `meetings[i] = [start_i, end_i]` 表示第 `i` 次会议的开始和结束天数（**包含首尾**）。
