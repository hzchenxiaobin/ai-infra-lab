...
type: problem
title: 树中可以形成回文的路径数
tags: [位运算, 哈希表, 树, 深度优先搜索]
knowledge_points: [bit-manipulation, dfs, hash-table, tree]
updated: 2026-09-03
source: leetcode
number: 2791
difficulty: hard
...
# LeetCode 树中可以形成回文的路径数 题解

## 1. 题目概述

- **标题 / 题号**：树中可以形成回文的路径数（#2791，hard）
- **链接**：https://leetcode.cn/problems/count-paths-that-can-form-a-palindrome-in-a-tree/
- **难度**：困难
- **标签**：位运算、树、深度优先搜索、哈希表

**题意**：给你一棵**树**（连通、无向、无环），根节点为 `0`，由编号 `0` 到 `n-1` 的 `n` 个节点组成。树用一个数组 `parent` 表示，`parent[i]` 为节点 `i` 的父节点，`parent[0] == -1`。
...
![示例1](https://assets.leetcode.com/uploads/2023/07/15/treedrawio-8drawio.png)
