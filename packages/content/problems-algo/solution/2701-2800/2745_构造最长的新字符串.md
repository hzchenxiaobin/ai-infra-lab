...
type: problem
title: 构造最长的新字符串
tags: [动态规划, 数学, 脑筋急转弯, 贪心]
knowledge_points: [brainteaser, dynamic-programming, greedy, math]
updated: 2026-09-03
source: leetcode
number: 2745
difficulty: medium
...
# LeetCode 构造最长的新字符串 题解

## 1. 题目概述

- **标题 / 题号**：构造最长的新字符串（#2745，medium）
- **链接**：https://leetcode.cn/problems/construct-the-longest-new-string/
- **难度**：中等
- **标签**：贪心、脑筋急转弯、数学、动态规划

**题意**：给你三个整数 $x$、$y$、$z$，分别表示你有 $x$ 个 `"AA"` 字符串、$y$ 个 `"BB"` 字符串、$z$ 个 `"AB"` 字符串。你需要选择其中部分（可全选也可不选），按顺序连接得到一个新字符串，且新字符串**不能包含子串 `"AAA"` 或 `"BBB"`**。返回新字符串的最大可能长度。
