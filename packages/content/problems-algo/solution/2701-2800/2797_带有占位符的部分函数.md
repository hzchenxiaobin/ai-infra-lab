...
type: problem
title: 带有占位符的部分函数
tags: [函数式编程, 部分应用, 闭包, 高阶函数]
knowledge_points: [closure, functional-programming, higher-order-function, 部分应用]
updated: 2026-09-03
source: leetcode
number: 2797
difficulty: easy
...
# LeetCode 带有占位符的部分函数 题解

## 1. 题目概述

- **标题 / 题号**：带有占位符的部分函数（#2797，easy）
...
**题意**：请你实现一个函数 `partial(fn, args)`，它对 `fn` 做**部分应用（partial application）**：预先固定一部分参数 `args`，返回一个新函数。`args` 中可以包含一个特殊**占位符** `"_"`，表示「这个位置暂时留空，等调用时再填」。
