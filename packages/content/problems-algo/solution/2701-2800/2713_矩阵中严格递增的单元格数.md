...
type: problem
title: 矩阵中严格递增的单元格数
tags: [动态规划, 哈希表, 排序, 数组, 矩阵, 记忆化搜索]
knowledge_points: [array, dynamic-programming, hash-table, matrix, memoization, sorting, 矩阵]
updated: 2026-09-03
source: leetcode
number: 2713
difficulty: hard
...
# LeetCode 矩阵中严格递增的单元格数 题解

## 1. 题目概述

- **标题 / 题号**：矩阵中严格递增的单元格数（#2713，hard）
- **链接**：https://leetcode.cn/problems/maximum-strictly-increasing-cells-in-a-matrix/
- **难度**：困难
- **标签**：记忆化搜索、数组、动态规划、矩阵、排序、哈希表

**题意**：给定一个下标从 1 开始、大小为 $m \times n$ 的整数矩阵 `mat`，你可以选择任一单元格作为**初始单元格**。从当前单元格出发，你可以移动到**同一行或同一列**中的任何其他单元格，但前提是目标单元格的值**严格大于**当前单元格的值。你可以多次重复这一过程，直到无法再移动。求从某个单元格出发，所能访问的单元格的**最大数量**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2023/04/23/diag1drawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/04/23/diag3drawio.png)
...
![示例3](https://assets.leetcode.com/uploads/2023/04/23/diag4drawio.png)
