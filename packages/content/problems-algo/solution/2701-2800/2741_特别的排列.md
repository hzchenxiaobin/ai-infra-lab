...
- **标题 / 题号**：特别的排列（#2741，medium）
- **链接**：https://leetcode.cn/problems/special-permutations/
- **难度**：中等
- **标签**：位运算、数组、动态规划、状态压缩

**题意**：给你一个下标从 **0** 开始、含 `n` 个**互不相同**正整数的数组 `nums`。若 `nums` 的一个排列满足：对每个 `0 <= i < n - 1`，要么 `nums[i] % nums[i+1] == 0`，要么 `nums[i+1] % nums[i] == 0`，则称它是一个**特别的排列**。返回特别排列的总数，答案对 $10^9 + 7$ 取余。
