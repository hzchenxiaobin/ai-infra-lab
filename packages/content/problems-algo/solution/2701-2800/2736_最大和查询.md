...
type: problem
title: 最大和查询
tags: [二分查找, 单调栈, 排序, 数组, 栈, 树状数组, 线段树]
knowledge_points: [array, binary-search, fenwick-tree, monotonic-stack, segment-tree, sorting, stack]
updated: 2026-09-03
source: leetcode
number: 2736
difficulty: hard
...
- **标题 / 题号**：最大和查询（#2736，hard）
- **链接**：https://leetcode.cn/problems/maximum-sum-queries/
- **难度**：困难
- **标签**：栈、树状数组、线段树、数组、二分查找、排序、单调栈

**题意**：给定长度为 $n$、下标从 $0$ 开始的两个整数数组 `nums1` 与 `nums2`，再给定下标从 $1$ 开始的二维数组 `queries`，其中 `queries[i] = [x_i, y_i]`。
