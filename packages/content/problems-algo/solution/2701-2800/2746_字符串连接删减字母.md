...
# LeetCode 字符串连接删减字母 题解

## 1. 题目概述

- **标题 / 题号**：字符串连接删减字母（#2746，medium）
- **链接**：https://leetcode.cn/problems/decremental-string-concatenation/
- **难度**：中等
- **标签**：数组、字符串、动态规划

**题意**：给你一个下标从 0 开始的字符串数组 `words`，含 `n` 个字符串。定义连接操作 `join(x, y)` 表示把 `x` 和 `y` 拼成 `xy`；若 `x` 的最后一个字符与 `y` 的第一个字符相等，则拼接后这**两个相等字符中的一个被删除**（即只重叠删一个）。
