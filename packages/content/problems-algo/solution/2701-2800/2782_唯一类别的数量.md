...
title: 唯一类别的数量
tags: [交互, 并查集, 计数]
knowledge_points: [counting, interactive, union-find, 交互]
updated: 2026-09-03
source: leetcode
number: 2782
difficulty: medium
...
**题意**：给定 `n` 个元素（编号 `0` 到 `n-1`），每个元素属于一个**类别**。类别信息通过一个交互式接口 `CategoryHandler` 提供——你**无法直接读取**每个元素的类别值，只能调用：
