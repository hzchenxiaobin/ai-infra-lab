...
type: problem
title: 一个小组的最大实力值
tags: [位运算, 动态规划, 回溯, 排序, 数组, 枚举, 贪心]
knowledge_points: [array, backtracking, bit-manipulation, dynamic-programming, enumeration, greedy, sorting]
updated: 2026-09-03
source: leetcode
number: 2708
difficulty: medium
...
# LeetCode 一个小组的最大实力值 题解

## 1. 题目概述

- **标题 / 题号**：一个小组的最大实力值（#2708，medium）
- **链接**：https://leetcode.cn/problems/maximum-strength-of-a-group/
- **难度**：中等
- **标签**：贪心、位运算、数组、动态规划、回溯、枚举、排序

**题意**：给你一个下标从 **0** 开始的整数数组 `nums`，表示班级中所有学生在一次考试中的成绩。老师想选出一部分同学组成一个 **非空** 小组，使这个小组的 **实力值** 最大。小组实力值定义为选中学生成绩的乘积：`nums[i0] * nums[i1] * ... * nums[ik]`。
