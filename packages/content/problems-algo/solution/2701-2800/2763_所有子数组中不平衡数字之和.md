...
# LeetCode 所有子数组中不平衡数字之和 题解

## 1. 题目概述

- **标题 / 题号**：所有子数组中不平衡数字之和（#2763，hard）
- **链接**：https://leetcode.cn/problems/sum-of-imbalance-numbers-of-all-subarrays/
- **难度**：困难
- **标签**：数组、哈希表、枚举

**题意**：一个长度为 $n$、下标从 $0$ 开始的整数数组 $arr$ 的**不平衡数字**定义为：在 $sarr = \mathrm{sorted}(arr)$ 中，满足 $0 \le i < n-1$ 且 $sarr[i+1] - sarr[i] > 1$ 的下标 $i$ 的数目。换言之，**排序后相邻元素之差大于 1 的相邻对数**。
