...
type: problem
title: 排序方式
tags: [JavaScript, "Schwartzian 变换", 排序, 数组, 高阶函数]
knowledge_points: [array, higher-order-function, javascript, schwartzian-变换, sorting]
updated: 2026-09-03
source: leetcode
number: 2724
difficulty: easy
...
- **标题 / 题号**：排序方式（#2724，easy）
- **链接**：https://leetcode.cn/problems/sort-by/
- **难度**：简单
- **标签**：JavaScript、数组、排序、高阶函数、Schwartzian 变换

**题意**：给定一个数组 `arr` 和一个函数 `fn`，返回一个**排序后的数组** `sortedArr`。`fn` 接收数组元素并**返回一个数字**，这些数字决定了 `sortedArr` 的顺序——`sortedArr` 必须按 `fn` 的输出值**升序**排列。你可以假设对于给定数组，`fn` 不会返回重复的数字。
