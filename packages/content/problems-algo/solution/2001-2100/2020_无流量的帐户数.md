...
type: problem
title: 无流量的帐户数
tags: ["LEFT JOIN + IS NULL", "NOT EXISTS", "NOT IN", SQL, "YEAR() 函数", 区间重叠, "反连接（anti-join）", 数据库]
knowledge_points: [anti-join, database, interval-overlap, left-join-is-null, not-exists, not-in, sql, year-函数, 区间重叠]
updated: 2026-09-03
source: leetcode
number: 2020
difficulty: medium
...
**题意**：给定 `Subscriptions`（订阅表）和 `Streams`（流量会话表）两张表，编写 SQL 查询，报告**在 2021 年有订阅但没有任何会话**的帐户数。
