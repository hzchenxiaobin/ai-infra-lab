...
type: problem
title: "并行课程 III"
tags: ["DAG 最长路", 动态规划, 图, 拓扑排序]
knowledge_points: [dag-longest-path, dag-最长路, dynamic-programming, graph, topological-sort]
updated: 2026-09-03
source: leetcode
number: 2050
difficulty: hard
...
# LeetCode 并行课程 III 题解

## 1. 题目概述

- **标题 / 题号**：并行课程 III（#2050，hard）
- **链接**：https://leetcode.cn/problems/parallel-courses-iii/
- **难度**：困难
- **标签**：图、拓扑排序、动态规划、DAG 最长路

**题意**：有 `n` 门课程，编号 `1` 到 `n`。数组 `relations[i] = [prevCourse, nextCourse]` 表示 `prevCourse` 是 `nextCourse` 的**先修课**——必须先修完 `prevCourse` 才能开始 `nextCourse`。数组 `time[i]` 表示完成第 `i+1` 门课程需要的**月份数**。

在一个学期里，可以**同时修任意多门**先修条件已满足的课程（并行免费）。每门课一旦开始就需要 `time[i]` 个月才能修完。求修完所有 `n` 门课的**最少月份数**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/10/07/ex1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/10/07/ex2.png)
