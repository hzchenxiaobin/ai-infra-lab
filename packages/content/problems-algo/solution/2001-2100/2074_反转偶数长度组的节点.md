...
type: problem
title: 反转偶数长度组的节点
tags: [分段处理, 指针操作, 翻转, 链表]
knowledge_points: [linked-list, pointer-manipulation, reversal, segmented-processing, 分段处理, 指针操作, 翻转]
updated: 2026-09-03
source: leetcode
number: 2074
difficulty: medium
...
# LeetCode 反转偶数长度组的节点 题解

## 1. 题目概述

- **标题 / 题号**：反转偶数长度组的节点（#2074，medium）
- **链接**：https://leetcode.cn/problems/reverse-nodes-in-even-length-groups/
- **难度**：中等
- **标签**：链表、分段处理、翻转、指针操作

**题意**：给定单链表的头节点 `head`。链表节点**依次**被分入长度为 `1, 2, 3, 4, …` 的组（第 $i$ 组预期长度为 $i$）。**最后一组可能不满**——剩余节点数可能小于其预期长度。请将**实际长度为偶数**的组反转，返回修改后链表的头节点。不能改节点内部的值，只能改指针。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/10/25/eg1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/10/25/eg2.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/10/28/eg3.png)
...
| 61 | [旋转链表](https://leetcode.cn/problems/rotate-list/)（[题解](../0001-0100/61_旋转链表.md)） | 成环再断开的链表重构，对照本题的「分段 + 局部翻转 + 串接」另一种链表重组范式 |
