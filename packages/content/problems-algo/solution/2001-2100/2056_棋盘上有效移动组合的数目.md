...
title: 棋盘上有效移动组合的数目
tags: [回溯, 数组, 枚举, 模拟]
knowledge_points: [array, backtracking, enumeration, simulation]
updated: 2026-09-03
source: leetcode
number: 2056
difficulty: hard
...
# LeetCode 棋盘上有效移动组合的数目 题解

## 1. 题目概述

- **标题 / 题号**：棋盘上有效移动组合的数目（#2056，hard）
- **链接**：https://leetcode.cn/problems/number-of-valid-move-combinations-on-chessboard/
- **难度**：困难
- **标签**：数组、回溯、枚举、模拟

**题意**：在一个 `8×8` 棋盘上有 `n` 枚棋子（车 rook / 象 bishop / 后 queen），给出每枚棋子的类型 `pieces[i]` 与初始位置 `positions[i]`（下标从 1 开始）。所有棋子**同时**移动：每枚先选一个**方向**，再选一个**步数**，然后从时刻 0 起每秒沿该方向走 1 格，直到走完选定步数后停在终点。若某个时刻有两枚及以上棋子占据同一格，则该移动组合**无效**。求**有效**移动组合的数目。
...
![示例1](https://assets.leetcode.com/uploads/2021/09/23/a1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/09/23/a2.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/09/23/a3.png)
...
![示例4](https://assets.leetcode.com/uploads/2021/09/23/a4.png)
...
![示例5](https://assets.leetcode.com/uploads/2021/09/23/a5.png)
