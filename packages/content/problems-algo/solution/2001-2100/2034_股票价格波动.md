...
title: 股票价格波动
tags: [哈希表, 堆, 延迟删除, 设计]
knowledge_points: [design, hash-table, heap, 延迟删除]
updated: 2026-09-03
source: leetcode
number: 2034
difficulty: medium
...
- **标题 / 题号**：股票价格波动（#2034，medium）
- **链接**：https://leetcode.cn/problems/stock-price-fluctuation/
- **难度**：中等
- **标签**：设计、堆、哈希表、延迟删除

**题意**：给你一支股票价格的**数据流**，每条记录包含一个**时间戳** `timestamp` 和该时刻对应的**价格** `price`。记录不按时间顺序到达，且可能出现**重复时间戳**——此时**后到的记录更正前一条**（前一条视为错误记录）。
