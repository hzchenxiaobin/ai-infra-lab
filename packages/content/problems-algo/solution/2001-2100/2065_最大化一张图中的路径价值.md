...
title: 最大化一张图中的路径价值
tags: [DFS, 剪枝, 回溯, 图]
knowledge_points: [backtracking, dfs, graph, pruning]
updated: 2026-09-03
source: leetcode
number: 2065
difficulty: hard
...
# LeetCode 最大化一张图中的路径价值 题解

## 1. 题目概述

- **标题 / 题号**：最大化一张图中的路径价值（#2065，hard）
- **链接**：https://leetcode.cn/problems/maximum-path-quality-of-a-graph/
- **难度**：困难
- **标签**：图、回溯、DFS、剪枝

**题意**：给定一个 **无向图**，含 `n` 个节点（编号 `0` 到 `n-1`），每个节点 `i` 有价值 `values[i]`。边以 `edges[j] = [uj, vj, timej]` 表示，`timej` 是通过该边所需时间。一条**合法路径**从节点 `0` 出发、最终回到节点 `0`，且总时间不超过 `maxTime`。路径的**价值**定义为路径中**不同节点**的价值之和（每个节点至多算一次）。求合法路径的**最大价值**。

> ⚠️ **关键约束**：每个节点至多有 **4 条边**，`10 <= timej, maxTime <= 100`。这两个约束共同决定了搜索空间极小，是本题可用回溯的根本原因。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/10/19/ex1drawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/10/19/ex2drawio.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/10/19/ex31drawio.png)
...
![示例4](https://assets.leetcode.com/uploads/2021/10/21/ex4drawio.png)
