...
type: problem
title: 商店中每个成员的级别
tags: ["CASE WHEN 顺序短路", "COUNT 的 NULL 计数语义", "GROUP BY", "LEFT JOIN", SQL, 数据库, 除零防护]
knowledge_points: [case-when-顺序短路, count-的-null-计数语义, database, group-by, left-join, sql, 除零防护]
updated: 2026-09-03
source: leetcode
number: 2051
difficulty: medium
...
# LeetCode 商店中每个成员的级别 题解

## 1. 题目概述

- **标题 / 题号**：商店中每个成员的级别（#2051，medium）
...
**题意**：给定 `Members`（成员）、`Visits`（访问）、`Purchases`（购买）三张表，编写 SQL 查询，为每位成员计算**转化率**并据此分级，输出 `member_id`、`name`、`category` 三列。结果可按任意顺序返回。
