...
type: problem
title: 两个最好的不重叠活动
tags: [二分查找, 动态规划, 排序, 数组]
knowledge_points: [array, binary-search, dynamic-programming, sorting]
updated: 2026-09-03
source: leetcode
number: 2054
difficulty: medium
...
# LeetCode 两个最好的不重叠活动 题解

## 1. 题目概述

- **标题 / 题号**：两个最好的不重叠活动（#2054，medium）
- **链接**：https://leetcode.cn/problems/two-best-non-overlapping-events/
- **难度**：中等
- **标签**：数组、排序、二分查找、动态规划

**题意**：给定一个二维整数数组 `events`，其中 `events[i] = [startTime_i, endTime_i, value_i]`，表示第 $i$ 个活动的时间区间与参与价值。你**最多**可以参加**两个时间不重叠**的活动，使得它们的价值之和最大。

> ⚠️ **不重叠的定义**：如果你参加一个结束时间为 $t$ 的活动，下一个活动必须在 $t + 1$ 或之后开始。即两个活动不重叠的条件是 $\text{endTime}_j < \text{startTime}_i$（**严格小于**，不是 $\le$）。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2026/01/03/untitled-diagramdrawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2026/01/03/2054b.png)
...
![示例3](https://assets.leetcode.com/uploads/2026/01/03/2054c.png)
