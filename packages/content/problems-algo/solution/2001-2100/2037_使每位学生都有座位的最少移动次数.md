...
# LeetCode 使每位学生都有座位的最少移动次数 题解

## 1. 题目概述

- **标题 / 题号**：使每位学生都有座位的最少移动次数（#2037，easy）
- **链接**：https://leetcode.cn/problems/minimum-number-of-moves-to-seat-everyone/
- **难度**：简单
- **标签**：贪心、排序、数组

**题意**：一个房间里有 $n$ 个空闲座位和 $n$ 名站着的学生，房间用一个数轴表示。给定长度为 $n$ 的数组 `seats`（`seats[i]` 是第 $i$ 个座位的位置）和长度为 $n$ 的数组 `students`（`students[j]` 是第 $j$ 位学生的位置）。
