...
title: 找出数组排序后的目标下标
tags: [排序, 数组, 计数]
knowledge_points: [array, counting, sorting]
updated: 2026-09-03
source: leetcode
number: 2089
difficulty: easy
...
# LeetCode 找出数组排序后的目标下标 题解

## 1. 题目概述

- **标题 / 题号**：找出数组排序后的目标下标（#2089，easy）
- **链接**：https://leetcode.cn/problems/find-target-indices-after-sorting-array/
- **难度**：简单
- **标签**：数组、计数、排序

**题意**：给定一个下标从 0 开始的整数数组 `nums` 以及一个目标元素 `target`。**目标下标**是满足 `nums[i] == target` 的下标 `i`。将 `nums` 按**非递减**顺序排序后，返回由 `nums` 中目标下标组成的列表。如果不存在目标下标，返回**空**列表。返回的列表必须按**递增**顺序排列。
