...
id: "lc:2084"
type: problem
title: 为订单类型为0的客户删除类型为1的订单
tags: ["COUNT ... OVER", "NOT EXISTS", "NOT IN", SQL, WHERE, 相关子查询, 窗口函数]
knowledge_points: [correlated-subquery, count-...-over, not-exists, not-in, sql, where, where-filter, window-function]
updated: 2026-09-03
source: leetcode
number: 2084
difficulty: easy
...
# LeetCode 为订单类型为0的客户删除类型为1的订单 题解

## 1. 题目概述

- **标题 / 题号**：为订单类型为0的客户删除类型为1的订单（#2084，easy）
...
**题意**：给定一张 `Orders` 表，每行记录一笔订单的编号、客户编号与订单类型。`order_type` 取值 `0`（到店订单）或 `1`（在线订单）。现需做一次数据清洗：**对「至少有一笔类型 0 订单」的客户，丢弃其全部类型 1 订单**；对只有类型 1 订单的客户，则原样保留。要求返回清洗后的结果表，顺序不限。
