...
tags: [博弈, 字符串, 计数, 贪心]
knowledge_points: [counting, game-theory, greedy, string]
updated: 2026-09-03
source: leetcode
number: 2038
difficulty: medium
...
# LeetCode 如果相邻两个颜色均相同则删除当前颜色 题解

## 1. 题目概述

- **标题 / 题号**：如果相邻两个颜色均相同则删除当前颜色（#2038，medium）
- **链接**：https://leetcode.cn/problems/remove-colored-pieces-if-both-neighbors-are-the-same-color/
- **难度**：中等
- **标签**：贪心、博弈、字符串、计数

**题意**：共有 $n$ 个颜色片段排成一列，每个片段要么是 `'A'` 要么是 `'B'`。给定字符串 `colors`，Alice 和 Bob 轮流删除片段，**Alice 先手**：
