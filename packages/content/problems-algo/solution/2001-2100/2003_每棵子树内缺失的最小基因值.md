...
type: problem
title: 每棵子树内缺失的最小基因值
tags: [并查集, 枚举优化, 树, 深度优先搜索]
knowledge_points: [dfs, enumeration, tree, union-find, 枚举优化]
updated: 2026-09-03
source: leetcode
number: 2003
difficulty: hard
...
# LeetCode 每棵子树内缺失的最小基因值 题解

## 1. 题目概述

- **标题 / 题号**：每棵子树内缺失的最小基因值（#2003，hard）
- **链接**：https://leetcode.cn/problems/smallest-missing-genetic-value-in-each-subtree/
- **难度**：困难
- **标签**：树、深度优先搜索、并查集、枚举优化

**题意**：有一棵以节点 `0` 为根的家族树，共 `n` 个节点（编号 `0` 到 `n-1`）。给定 `parents`，其中 `parents[i]` 是节点 `i` 的父节点（`parents[0] == -1`）。每个节点有一个**互不相同**的基因值 `nums[i]`（取值 `[1, 10^5]`）。

对每个节点 `i`，求以 `i` 为根的**子树**内**缺失的最小正整数**基因值，返回长度为 `n` 的数组 `ans`。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/08/23/case-1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/08/23/case-2.png)
...
| 547 | [省份数量](https://leetcode.cn/problems/number-of-provinces/)（[题解](../0501-0600/547_省份数量.md)） | 并查集模板，承接第 5 节「值域并查集维护 mex」的扩展思路 |
| 236 | [二叉树的最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/)（[题解](../0201-0300/236_二叉树的最近公共祖先.md)） | 树上「祖先/路径」概念，与本题「`node1` 到根的路径」定位同源 |
