...
id: "lc:2018"
type: problem
title: 判断单词是否能放入填字游戏内
tags: [字符串匹配, 数组, 枚举, 矩阵]
knowledge_points: [array, enumeration, matrix, string-matching, 矩阵]
updated: 2026-09-03
source: leetcode
number: 2018
difficulty: medium
...
# LeetCode 判断单词是否能放入填字游戏内 题解

## 1. 题目概述

- **标题 / 题号**：判断单词是否能放入填字游戏内（#2018，medium）
- **链接**：https://leetcode.cn/problems/check-if-word-can-be-placed-in-crossword/
- **难度**：中等
- **标签**：数组、矩阵、字符串匹配、枚举

**题意**：给定一个 `m x n` 的矩阵 `board` 表示填字游戏的当前状态，格子取值为小写字母（已填入）、`' '`（空格）或 `'#'`（障碍）。判断字符串 `word` 能否被放入棋盘——可**水平**（左→右 或 右→左）或**竖直**（上→下 或 下→上）放置，需同时满足：
...
![示例1](https://assets.leetcode.com/uploads/2021/09/18/crossword-1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/09/18/c2.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/09/18/crossword-2.png)
