...
type: problem
title: 找出知晓秘密的所有专家
tags: [图, 并查集, 广度优先搜索, 排序]
knowledge_points: [bfs, graph, sorting, union-find]
updated: 2026-09-03
source: leetcode
number: 2092
difficulty: hard
...
# LeetCode 找出知晓秘密的所有专家 题解

## 1. 题目概述

- **标题 / 题号**：找出知晓秘密的所有专家（#2092，hard）
- **链接**：https://leetcode.cn/problems/find-all-people-with-secret/
- **难度**：困难
- **标签**：并查集、图、排序、广度优先搜索

**题意**：有 `n` 个专家编号 `0` 到 `n-1`。专家 `0` 持有一个秘密，在时间 `0` 分享给专家 `firstPerson`。之后每场会议 `meetings[i] = [xi, yi, timei]` 中，若**一方知晓秘密**则**瞬时**分享给另一方。秘密共享是**瞬时发生**的——同一时刻一个专家不仅能接收秘密，还能在其他同时间会议里继续传播。返回最终所有知晓秘密的专家。
