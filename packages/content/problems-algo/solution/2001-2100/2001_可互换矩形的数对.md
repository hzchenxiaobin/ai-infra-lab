...
title: 可互换矩形的数对
tags: [几何, 哈希表, 数学]
knowledge_points: [geometry, hash-table, math]
updated: 2026-09-03
source: leetcode
number: 2001
difficulty: medium
...
# LeetCode 可互换矩形的数对 题解

## 1. 题目概述

- **标题 / 题号**：可互换矩形的数对（#2001，medium）
- **链接**：https://leetcode.cn/problems/number-of-pairs-of-interchangeable-rectangles/
- **难度**：中等
- **标签**：哈希表、数学、几何

**题意**：给定一个下标从 **0** 开始的二维整数数组 `rectangles`，其中 `rectangles[i] = [width_i, height_i]` 表示第 `i` 个矩形的宽与高。
...
| 454 | [四数相加 II](https://leetcode.cn/problems/4sum-ii/)（[题解](../0401-0500/454_四数相加%20II.md)） | 分组 + 哈希计数（Meet in the Middle），同样是「先归约为可哈希的键再统计配对数」 |
