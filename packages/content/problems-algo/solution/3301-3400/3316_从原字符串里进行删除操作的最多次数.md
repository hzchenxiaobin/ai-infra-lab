...
tags: [动态规划, 双指针, 哈希表, 字符串, 数组]
knowledge_points: [array, dynamic-programming, hash-table, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 3316
difficulty: medium
...
- **标题 / 题号**：从原字符串里进行删除操作的最多次数（#3316，medium）
- **链接**：https://leetcode.cn/problems/find-maximum-removals-from-source-string/
- **难度**：中等
- **标签**：数组、哈希表、双指针、字符串、动态规划

**题意**：给你一个长度为 $n$ 的字符串 `source`，一个字符串 `pattern` 且它是 `source` 的**子序列**，和一个**有序**整数数组 `targetIndices`，数组中的元素是 $[0, n-1]$ 中**互不相同**的数字。
