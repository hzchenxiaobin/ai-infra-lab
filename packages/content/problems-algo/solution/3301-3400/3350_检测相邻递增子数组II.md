...
title: "检测相邻递增子数组 II"
tags: [一次遍历, 二分查找, 数组]
knowledge_points: [array, binary-search, one-pass, 一次遍历]
updated: 2026-09-03
source: leetcode
number: 3350
difficulty: medium
...
- **标题 / 题号**：检测相邻递增子数组 II（#3350，medium）
- **链接**：https://leetcode.cn/problems/adjacent-increasing-subarrays-detection-ii/
- **难度**：中等
- **标签**：数组、二分查找、一次遍历

**题意**：给你一个由 $n$ 个整数组成的数组 `nums`，请找出 $k$ 的**最大值**，使得存在**两个相邻**且长度为 $k$ 的**严格递增**子数组。具体来说，需要检查是否存在从下标 $a$ 和 $b$（$a < b$）开始的两个子数组，并满足下述全部条件：
