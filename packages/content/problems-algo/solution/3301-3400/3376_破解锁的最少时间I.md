...
knowledge_points: [backtracking, bit-manipulation, bitmask-dp, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 3376
difficulty: medium
...
# LeetCode 破解锁的最少时间 I 题解

- **关联**：3385（II：规模放大，官方提示转向匈牙利算法最小费用匹配）；526 / 847（位掩码 DP 家族）

## 1. 题目概述

- **标题 / 题号**：破解锁的最少时间 I（#3376，medium）
- **链接**：https://leetcode.cn/problems/minimum-time-to-break-locks-i/
- **难度**：中等
- **标签**：位运算、动态规划、状态压缩、回溯

**题意**：Bob 被困在地窖里，需要破解 $n$ 把锁才能逃出去。数组 `strength` 给出每把锁所需的**能量**：`strength[i]` 表示打开第 $i$ 把锁时，剑的能量**至少**要达到的值。
