...
type: problem
title: "零数组变换 II"
tags: [二分查找, "前缀和（差分）", 双指针, 数组]
knowledge_points: [array, binary-search, two-pointers, 前缀和-差分]
updated: 2026-09-03
source: leetcode
number: 3356
difficulty: medium
...
- **标题 / 题号**：零数组变换 II（#3356，medium）
- **链接**：https://leetcode.cn/problems/zero-array-transformation-ii/
- **难度**：中等
- **标签**：数组、双指针、二分查找、前缀和（差分）

**题意**：给你一个长度为 $n$ 的整数数组 `nums` 和一个二维数组 `queries`，其中 `queries[i] = [l_i, r_i, val_i]`。
