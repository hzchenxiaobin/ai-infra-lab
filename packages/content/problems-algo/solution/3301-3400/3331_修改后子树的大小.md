...
type: problem
title: 修改后子树的大小
tags: [哈希表, 字符串, 数组, 树, 深度优先搜索]
knowledge_points: [array, dfs, hash-table, string, tree]
updated: 2026-09-03
source: leetcode
number: 3331
difficulty: medium
...
# LeetCode 修改后子树的大小 题解

## 1. 题目概述

- **标题 / 题号**：修改后子树的大小（#3331，medium）
- **链接**：https://leetcode.cn/problems/find-subtree-sizes-after-changes/
- **难度**：中等
- **标签**：树、深度优先搜索、数组、哈希表、字符串

**题意**：给你一棵 $n$ 个节点、根节点编号为 $0$ 的**有根树**，节点编号 $0 \sim n-1$。树用长度为 $n$ 的数组 `parent` 表示，其中 `parent[i]` 是节点 $i$ 的父节点（`parent[0] == -1`）。
...
![示例1](https://assets.leetcode.com/uploads/2024/08/15/graphex1drawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2024/08/20/exgraph2drawio.png)
