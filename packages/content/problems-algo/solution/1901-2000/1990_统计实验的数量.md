...
type: problem
title: 统计实验的数量
tags: ["COUNT(column)", "CROSS JOIN", "GROUP BY", "LEFT JOIN", SQL, 数据库, 笛卡尔积骨架, 维度齐全]
knowledge_points: [count-column, cross-join, database, group-by, left-join, sql, 笛卡尔积骨架, 维度齐全]
updated: 2026-09-03
source: leetcode
number: 1990
difficulty: medium
...
**题意**：给定 `Experiments` 表，编写 SQL 查询，报告**三个实验平台中每种实验完成的次数**。注意，每一对（实验平台、实验名称）都应包含在输出中，**包括次数为零的组合**。结果可以任意顺序返回。
