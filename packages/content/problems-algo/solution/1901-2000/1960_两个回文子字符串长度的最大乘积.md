...
tags: [Manacher, 动态规划, 字符串]
knowledge_points: [dynamic-programming, manacher, string]
updated: 2026-09-03
source: leetcode
number: 1960
difficulty: hard
...
# LeetCode 两个回文子字符串长度的最大乘积 题解

## 1. 题目概述

- **标题 / 题号**：两个回文子字符串长度的最大乘积（#1960，hard）
- **链接**：https://leetcode.cn/problems/maximum-product-of-the-length-of-two-palindromic-substrings/
- **难度**：困难
- **标签**：字符串、Manacher、动态规划

**题意**：给定下标从 0 开始的字符串 `s`，找到两个**不重叠**的回文子字符串，它们的长度都必须为**奇数**，使得它们长度的**乘积最大**。
