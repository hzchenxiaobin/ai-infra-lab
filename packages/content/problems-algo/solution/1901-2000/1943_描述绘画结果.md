...
type: problem
title: 描述绘画结果
tags: [哈希表, 差分数组, 扫描线, 排序]
knowledge_points: [difference-array, hash-table, sorting, sweep-line]
updated: 2026-09-03
source: leetcode
number: 1943
difficulty: medium
...
- **标题 / 题号**：描述绘画结果（#1943，medium）
- **链接**：https://leetcode.cn/problems/describe-the-painting/
- **难度**：中等
- **标签**：差分数组、扫描线、排序、哈希表

**题意**：数轴上有一幅画，由若干**半开区间**线段 `[start_i, end_i)` 表示，每条线段有**独一无二**的颜色 `color_i`。重叠部分的颜色会被**混合**，用一个**集合**表示；为简化输出，只需用集合中所有元素的**和**来代表该混合颜色。

要求用**最少数目**的不重叠半开区间表示最终的画，返回 `painting = [[left_j, right_j, mix_j], ...]`，其中 `mix_j` 是该区间内所有颜色之和。未被涂色的部分不出现在结果中。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/06/18/1.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/06/18/2.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/07/04/c1.png)
