...
type: problem
title: 确认率
tags: [AVG, "GROUP BY", IF/CASE, IFNULL, "LEFT JOIN", ROUND, SQL, 布尔转比例, 除零兜底]
knowledge_points: [avg, bool-to-ratio, case-when, group-by, if-case, ifnull, left-join, round, sql, 布尔转比例, 除零兜底]
updated: 2026-09-03
source: leetcode
number: 1934
difficulty: medium
...
- **标题 / 题号**：确认率（#1934，medium）
- **链接**：https://leetcode.cn/problems/confirmation-rate/
- **难度**：中等
- **标签**：SQL、LEFT JOIN、GROUP BY、AVG、IF/CASE、布尔转比例、IFNULL、ROUND、除零兜底

**题意**：给定 `Signups`（用户注册记录）与 `Confirmations`（确认消息请求记录）两张表。每条确认消息的 `action` 为 `'confirmed'`（已确认）或 `'timeout'`（已过期）。用户的**确认率**定义为「`'confirmed'` 消息数 ÷ 请求的确认消息总数」；**没有任何确认请求**的用户确认率记为 `0`，结果**四舍五入到小数点后两位**。
