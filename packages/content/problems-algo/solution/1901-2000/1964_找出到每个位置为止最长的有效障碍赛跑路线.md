...
tags: [二分查找, 数组, 最长递增子序列, 贪心]
knowledge_points: [array, binary-search, greedy, lis]
updated: 2026-09-03
source: leetcode
number: 1964
difficulty: hard
...
# LeetCode 找出到每个位置为止最长的有效障碍赛跑路线 题解

## 1. 题目概述

- **标题 / 题号**：找出到每个位置为止最长的有效障碍赛跑路线（#1964，hard）
- **链接**：https://leetcode.cn/problems/find-the-longest-valid-obstacle-course-at-each-position/
- **难度**：困难
- **标签**：数组、二分查找、贪心、最长递增子序列

**题意**：给定一个下标从 0 开始的整数数组 `obstacles`，长度为 `n`，`obstacles[i]` 表示第 `i` 个障碍的高度。对于每个下标 `i`，在满足下述条件的前提下，找出 `obstacles` 能构成的最长障碍路线的长度：
