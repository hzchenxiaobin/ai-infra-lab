...
tags: [子串匹配, 字符串, 模拟]
knowledge_points: [simulation, string, 子串匹配]
updated: 2026-09-03
source: leetcode
number: 1967
difficulty: easy
...
# LeetCode 作为子字符串出现在单词中的字符串数目 题解

## 1. 题目概述

- **标题 / 题号**：作为子字符串出现在单词中的字符串数目（#1967，easy）
- **链接**：https://leetcode.cn/problems/number-of-strings-that-appear-as-substrings-in-word/
- **难度**：简单
- **标签**：字符串、子串匹配、模拟

**题意**：给你一个字符串数组 $patterns$ 和一个字符串 $word$，统计 $patterns$ 中有多少个字符串是 $word$ 的**子字符串**。返回字符串数目。
