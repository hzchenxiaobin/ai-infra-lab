...
title: 收集足够苹果的最小花园周长
tags: [二分查找, 数学]
knowledge_points: [binary-search, math]
updated: 2026-09-03
source: leetcode
number: 1954
difficulty: medium
...
# LeetCode 收集足够苹果的最小花园周长 题解

## 1. 题目概述

- **标题 / 题号**：收集足够苹果的最小花园周长（#1954，medium）
- **链接**：https://leetcode.cn/problems/minimum-garden-perimeter-to-collect-enough-apples/
- **难度**：中等
- **标签**：数学、二分查找

**题意**：在一个无限二维网格中，每个整数坐标 `(i, j)` 处有一棵苹果树，该树上有 `|i| + |j|` 个苹果。你要买一块以 `(0, 0)` 为中心的**正方形土地**（边与坐标轴平行），使得土地**内部及边缘上**的苹果总数 **至少** 为 `neededApples`。返回满足条件的最小**周长**。

**示例 1**：

![示例1](https://pic.leetcode.cn/1627790803-qcBKFw-image.png)
