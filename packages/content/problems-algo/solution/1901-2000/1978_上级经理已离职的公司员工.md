...
type: problem
title: 上级经理已离职的公司员工
tags: ["LEFT JOIN ... IS NULL", "NOT EXISTS", "NOT IN", "NULL 处理", SQL, "反连接（anti-join）", 数据库, "自连接（self-join）"]
knowledge_points: [anti-join, database, left-join-...-is-null, not-exists, not-in, null-handling, self-join, sql]
updated: 2026-09-03
source: leetcode
number: 1978
difficulty: easy
...
# LeetCode 上级经理已离职的公司员工 题解

## 1. 题目概述

- **标题 / 题号**：上级经理已离职的公司员工（#1978，easy）
...
**题意**：给定 `Employees` 表（员工 id、姓名、上级经理 id、薪水），编写 SQL 查询，找出**薪水严格少于 `$30000`** 且**上级经理已离职**的员工 `employee_id`。
