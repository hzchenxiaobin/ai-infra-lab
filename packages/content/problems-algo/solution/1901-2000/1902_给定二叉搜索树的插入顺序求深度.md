...
id: "lc:1902"
type: problem
title: 给定二叉搜索树的插入顺序求深度
tags: [二叉搜索树, 有序集合, 树]
knowledge_points: [bst, ordered-set, tree]
updated: 2026-09-03
source: leetcode
number: 1902
difficulty: medium
...
# LeetCode 给定二叉搜索树的插入顺序求深度 题解

## 1. 题目概述

- **标题 / 题号**：给定二叉搜索树的插入顺序求深度（#1902，medium）
- **链接**：https://leetcode.cn/problems/depth-of-bst-given-insertion-order/
- **难度**：中等
- **标签**：二叉搜索树、有序集合、树

**题意**：给定一个长度为 $n$ 的 **0 下标**整数数组 `order`，它是 $1$ 到 $n$ 的一个**排列**，表示依次将这些整数插入一棵**二叉搜索树（BST）**的顺序：
...
| 315 | [计算右侧小于当前元素的个数](https://leetcode.cn/problems/count-of-smaller-numbers-after-self/)（[题解](../0301-0400/315_计算右侧小于当前元素的个数.md)） | 有序集合统计右侧更小元素个数，同为「有序结构 + 逐元素插入」套路 |
