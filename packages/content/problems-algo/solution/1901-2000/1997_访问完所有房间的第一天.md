...
title: 访问完所有房间的第一天
tags: [前缀和, 动态规划, 数学]
knowledge_points: [dynamic-programming, math, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1997
difficulty: medium
...
# LeetCode 访问完所有房间的第一天 题解

## 1. 题目概述

- **标题 / 题号**：访问完所有房间的第一天（#1997，medium）
- **链接**：https://leetcode.cn/problems/first-day-where-you-have-been-in-all-the-rooms/
- **难度**：中等
- **标签**：动态规划、前缀和、数学

**题意**：有 $n$ 个房间，编号 $0$ 到 $n-1$。日期编号从 $0$ 开始，每天访问一个房间。第 $0$ 天访问 $0$ 号房间。给定长度为 $n$ 的数组 `nextVisit`，访问规则如下（设当天访问房间 $i$）：
