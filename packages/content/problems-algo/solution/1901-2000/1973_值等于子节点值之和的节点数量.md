...
# LeetCode 值等于子节点值之和的节点数量 题解

## 1. 题目概述

- **标题 / 题号**：值等于子节点值之和的节点数量（#1973，medium）
- **链接**：https://leetcode.cn/problems/count-nodes-equal-to-sum-of-descendants
- **难度**：中等
- **标签**：树、二叉树、深度优先搜索、后序遍历

**题意**：给定一棵二叉树的根节点 `root`，返回满足条件「节点的值等于该节点所有子节点值之和」的节点数量。
...
| 236 | [二叉树的最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/)（[题解](../0201-0300/236_二叉树的最近公共祖先.md)） | 后序返回「子树是否含 p/q」，在节点处聚合判定 LCA，同套后序返回值传递骨架 |
