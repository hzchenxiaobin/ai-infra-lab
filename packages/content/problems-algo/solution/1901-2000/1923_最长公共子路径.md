...
type: problem
title: 最长公共子路径
tags: [二分查找, 字符串匹配, 滚动哈希]
knowledge_points: [binary-search, rolling-hash, string-matching]
updated: 2026-09-03
source: leetcode
number: 1923
difficulty: hard
...
- **标题 / 题号**：最长公共子路径（#1923，hard）
- **链接**：https://leetcode.cn/problems/longest-common-subpath/
- **难度**：困难
- **标签**：二分查找、滚动哈希、字符串匹配

**题意**：一个国家有 $n$ 个编号为 $0$ 到 $n-1$ 的城市，任意两城市间都有道路相连。$m$ 个朋友各自走了一条路径，每条路径是一个城市序列（同一城市可在一条路径中重复出现但不连续）。求**所有朋友都走过的最长公共子路径**的长度——子路径指路径中**连续**的城市序列。不存在则返回 $0$。
