...
title: 三数之和
tags: [双指针, 排序, 数组]
knowledge_points: [array, sorting, two-pointers]
updated: 2026-09-03
source: leetcode
number: 15
difficulty: medium
...
judge: none
related_learn: ["learn:w01d02", "learn:w01d07"]
---
...
- **标题 / 题号**：三数之和（#15，medium）
- **链接**：https://leetcode.cn/problems/3sum/
- **难度**：中等
- **标签**：数组、双指针、排序

**题意**：给定一个整数数组 `nums`，判断是否存在三元组 `[nums[i], nums[j], nums[k]]` 满足 `i != j`、`i != k` 且 `j != k`，同时还满足 `nums[i] + nums[j] + nums[k] == 0`。请返回所有**和为 0 且不重复**的三元组。
...
> 💡 这是「双指针」的招牌题，与 [Day 1 两数之和](1_两数之和.md) 形成鲜明对照——同样是"找数对使和为定值"，两数之和用哈希表 `O(n)`，三数之和却用排序 + 双指针。差别在于：三数之和要求**去重**，哈希表去重繁琐且易错，排序后双指针让相同元素相邻，去重只需"跳过相邻重复"。掌握本题的"排序 + 固定一个 + 双指针扫剩余"模板，就能秒杀四数之和、最接近的三数之和等同构题。
...
| 1 | [两数之和](https://leetcode.cn/problems/two-sum/)（[题解](1_两数之和.md)） | 哈希一次遍历 |
