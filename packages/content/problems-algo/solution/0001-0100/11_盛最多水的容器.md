...
title: 盛最多水的容器
tags: [双指针, 数组, 贪心]
knowledge_points: [array, greedy, two-pointers]
updated: 2026-09-03
source: leetcode
number: 11
difficulty: medium
...
judge: none
related_learn: ["lc:topic:greedy", "learn:w01d02", "learn:w01d07"]
---
...
> 💡 这是 **左右夹逼双指针** 的招牌题。与 [Week2/Day3 三数之和](15_三数之和.md) 的"排序后双指针"不同，本题**无需排序**——双指针从数组两端向中间夹逼。核心贪心思想：每次移动**较矮**的一侧，因为面积由短板决定，移动高侧不可能让面积变大。掌握这个"夹逼 + 贪心"模板，就能秒杀分发糖果、接雨水的双指针解法等同构题。
...
> ⚠️ 双指针从两端向中间夹逼，每次移动 1 步，总共最多 `n-1` 步，所以 `O(n)`。与 [Week2/Day3 三数之和](15_三数之和.md) 的 `O(n²)` 不同——三数之和外层还有枚举 `i` 的 `n` 轮，本题没有外层循环。
...
[Week1/Day1 接雨水](42_接雨水.md) 也有双指针解法，与本题形似但神不同：
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 11 | [盛最多水的容器](https://leetcode.cn/problems/container-with-most-water/)（题解） | 双指针经典 |
| 42 | [接雨水](https://leetcode.cn/problems/trapping-rain-water/)（[题解](42_接雨水.md)） | 双指针/单调栈 |
| 15 | [三数之和](https://leetcode.cn/problems/3sum/)（[题解](15_三数之和.md)） | 排序 + 双指针 |
