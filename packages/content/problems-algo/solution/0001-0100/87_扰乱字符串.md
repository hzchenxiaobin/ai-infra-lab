...
type: problem
title: 扰乱字符串
tags: ["三维 DP", 动态规划, 字符串, 记忆化搜索, 递归]
knowledge_points: [dynamic-programming, memoization, recursion, string, 三维-dp]
updated: 2026-09-03
source: leetcode
number: 87
difficulty: hard
...
**题意**：给定两个长度相同的字符串 `s1` 和 `s2`，判断 `s2` 是否是 `s1` 的**扰乱字符串**（Scramble String）。
