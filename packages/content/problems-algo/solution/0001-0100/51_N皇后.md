...
**题意**：按照国际象棋规则，在 `n × n` 的棋盘上摆放 `n` 个皇后，使其**互不攻击**：任意两个皇后不能处于同一**行**、同一**列**或同一**对角线**。返回所有不同的摆法。每种方案用棋盘字符串表示。
...
| 46 | [全排列](https://leetcode.cn/problems/permutations/)（[题解](46_全排列.md)） | 回溯 + 单集合剪枝的基础版 |
| 131 | [分割回文串](https://leetcode.cn/problems/palindrome-partitioning/)（[题解](../0101-0200/131_分割回文串.md)） | 回溯枚举方案 |
