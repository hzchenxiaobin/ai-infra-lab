...
type: problem
title: 接雨水
tags: [动态规划, 单调栈, 双指针, 数组, 栈]
knowledge_points: [array, dynamic-programming, monotonic-stack, stack, two-pointers]
updated: 2026-09-03
source: leetcode
number: 42
difficulty: hard
...
judge: none
related_learn: ["lc:topic:monotonic-stack", "learn:w01d02", "learn:w01d07"]
---
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2018/10/22/rainwatertrap.png)
...
**关键定义**：维护一个**栈**，栈中存放下标，对应高度从栈底到栈顶**单调递减**（与 [每日温度](../0701-0800/739_每日温度.md) 同模板）。
...
> 💡 对比双指针：双指针在每个位置算"该列能接多少"（纵向），单调栈每次弹栈算"该层能接多少"（横向）。两种视角等价，总和相同，但单调栈把"凹槽"结构显式化，是 [柱状图最大矩形](84_柱状图中最大的矩形.md) 的姊妹解法。
...
3. **单调栈法和双指针法的区别？**
   ...
   - **迁移性不同**：单调栈的"凹槽"视角可直接迁移到 [柱状图最大矩形](84_柱状图中最大的矩形.md)（找左右第一个更矮），双指针则不能。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 11 | [盛最多水的容器](https://leetcode.cn/problems/container-with-most-water/)（[题解](11_盛最多水的容器.md)） | 双指针，两指针从两端向中间收拢 |
| 407 | [接雨水 II](https://leetcode.cn/problems/trapping-rain-water-ii/) | 3D 接雨水，BFS + 优先队列 |
| 84 | [柱状图中最大的矩形](https://leetcode.cn/problems/largest-rectangle-in-histogram/)（[题解](84_柱状图中最大的矩形.md)） | 单调栈求最大矩形面积，与接雨水单调栈同骨架 |
| 739 | [每日温度](https://leetcode.cn/problems/daily-temperatures/)（[题解](../0701-0800/739_每日温度.md)） | 单调栈招牌题，与接雨水同 `while+pop+push` 模板 |
| 496 | [下一个更大元素 I](https://leetcode.cn/problems/next-greater-element-i/)（[题解](../0401-0500/496_下一个更大元素 I.md)） | 单调栈入门，存值+哈希表查表 |
