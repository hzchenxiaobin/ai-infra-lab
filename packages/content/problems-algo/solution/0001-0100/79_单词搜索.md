...
title: 单词搜索
tags: [回溯, 数组, 深度优先搜索]
knowledge_points: [array, backtracking, dfs]
updated: 2026-09-03
source: leetcode
number: 79
difficulty: medium
...
- **标题 / 题号**：单词搜索（#79，medium）
- **链接**：https://leetcode.cn/problems/word-search/
- **难度**：中等
- **标签**：数组、回溯、深度优先搜索

**题意**：给定一个 `m × n` 的二维字符网格 `board` 和一个字符串单词 `word`，判断 `word` 是否存在于网格中。
...
> 💡 这是 **DFS + 回溯** 的招牌题。与 [46. 全排列](46_全排列.md) 的「枚举所有方案」不同，本题是在**二维网格上搜索路径**——每个位置可以向上下左右四个方向探索，且不能走重复格子。核心模板是「标记已访问 → 四方向递归 → 回溯撤销标记」，这个模板可迁移到岛屿数量、岛屿的最大面积等所有网格搜索题。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 200 | [岛屿数量](https://leetcode.cn/problems/number-of-islands/)（[题解](../0101-0200/200_岛屿数量.md)） | 网格 DFS 连通块计数，不回溯（感染法） |
