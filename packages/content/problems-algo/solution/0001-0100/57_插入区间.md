...
title: 插入区间
tags: [区间, 数组, 贪心]
knowledge_points: [array, greedy, interval]
updated: 2026-09-03
source: leetcode
number: 57
difficulty: medium
...
- **标题 / 题号**：插入区间（#57，medium）
- **链接**：https://leetcode.cn/problems/insert-interval/
- **难度**：中等
- **标签**：数组、区间、贪心

**题意**：给定一个**无重叠**的、按起始端点升序排列的区间列表 `intervals`，以及一个新区间 `newInterval`，将 `newInterval` 插入 `intervals` 并按需合并重叠区间，返回最终的不重叠区间列表。
