...
id: "lc:2925"
type: problem
title: 在树上执行操作以后得到的最大分数
tags: [动态规划, 树, "树形 DP", 深度优先搜索]
knowledge_points: [dfs, dynamic-programming, tree, tree-dp]
updated: 2026-09-03
source: leetcode
number: 2925
difficulty: medium
...
# LeetCode 在树上执行操作以后得到的最大分数 题解

## 1. 题目概述

- **标题 / 题号**：在树上执行操作以后得到的最大分数（#2925，medium）
- **链接**：https://leetcode.cn/problems/maximum-score-after-applying-operations-on-a-tree/
- **难度**：中等
- **标签**：树、深度优先搜索、动态规划、树形 DP

**题意**：有一棵 $n$ 个节点的无向树，节点编号 $0$ 到 $n-1$，根节点为 $0$，`edges` 给出全部 $n-1$ 条边，`values[i]` 表示第 $i$ 个节点的值。
...
![示例1](https://assets.leetcode.com/uploads/2023/10/11/graph-13-1.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/10/11/graph-14-2.png)
