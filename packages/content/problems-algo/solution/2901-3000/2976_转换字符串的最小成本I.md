...
type: problem
title: "转换字符串的最小成本 I"
tags: [Floyd-Warshall, 图, 字符串, 数组, 最短路]
knowledge_points: [array, floyd-warshall, graph, shortest-path, string]
updated: 2026-09-03
source: leetcode
number: 2976
difficulty: medium
...
# LeetCode 转换字符串的最小成本 I 题解

## 1. 题目概述

- **标题 / 题号**：转换字符串的最小成本 I（#2976，medium）
- **链接**：https://leetcode.cn/problems/minimum-cost-to-convert-string-i/
- **难度**：中等
- **标签**：图、最短路、Floyd-Warshall、数组、字符串

**题意**：给定两个下标从 0 开始、长度均为 $n$ 且由**小写英文字母**组成的字符串 `source` 和 `target`，再给定转换规则数组 `original`、`changed` 和整数数组 `cost`：`cost[i]` 表示把字符 `original[i]` 更改为 `changed[i]` 的成本。
