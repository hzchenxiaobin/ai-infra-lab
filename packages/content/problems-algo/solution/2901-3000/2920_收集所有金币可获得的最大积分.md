...
tags: [动态规划, 树, "树形 DP"]
knowledge_points: [dynamic-programming, tree, tree-dp]
updated: 2026-09-03
source: leetcode
number: 2920
difficulty: hard
...
# LeetCode 收集所有金币可获得的最大积分 题解

## 1. 题目概述

- **标题 / 题号**：收集所有金币可获得的最大积分（#2920，hard）
- **链接**：https://leetcode.cn/problems/maximum-points-after-collecting-coins-from-all-nodes/
- **难度**：困难
- **标签**：树、动态规划、树形 DP

**题意**：有一棵由 `n` 个节点组成的无向树，以 `0` 为根节点，节点编号从 `0` 到 `n - 1`。给你一个长度为 `n - 1` 的二维整数数组 `edges`，其中 `edges[i] = [ai, bi]` 表示节点 `ai` 和 `bi` 之间存在一条边。另给你一个下标从 **0** 开始、长度为 `n` 的数组 `coins` 和一个整数 `k`，其中 `coins[i]` 表示节点 `i` 处的金币数量。
...
![示例1](https://assets.leetcode.com/uploads/2023/09/18/ex1-copy.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/09/18/ex2.png)
