...
type: problem
title: 计算订单平均商品数量
tags: [ROUND, SQL, SUM, 加权平均, "压缩数据（RLE）", 聚合函数]
knowledge_points: [aggregate-function, round, sql, sum, weighted-average, 加权平均, 压缩数据-rle]
updated: 2026-09-03
source: leetcode
number: 2985
difficulty: easy
...
# LeetCode 计算订单平均商品数量 题解

## 1. 题目概述

- **标题 / 题号**：计算订单平均商品数量（#2985，easy）
...
**题意**：给定一张**压缩存储**的订单表 `Orders`：不逐笔记录订单，而是每个「商品数档位」一行——`item_count` 表示一笔订单含多少件商品，`order_occurrences` 表示**恰好有这么多件商品的订单共有多少笔**（这正是题名 *Calculate Compressed Mean* 的含义）。编写解决方案，计算**每笔订单的平均商品数量**，保留 `2` 位小数。以任意顺序返回结果。
