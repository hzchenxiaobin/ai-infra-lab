...
title: 回文串重排列查询
tags: [前缀和, 哈希表, 字符串, 数组]
knowledge_points: [array, hash-table, prefix-sum, string]
updated: 2026-09-03
source: leetcode
number: 2983
difficulty: hard
...
# LeetCode 回文串重排列查询 题解

## 1. 题目概述

- **标题 / 题号**：回文串重排列查询（#2983，hard）
- **链接**：https://leetcode.cn/problems/palindrome-rearrangement-queries/
- **难度**：困难
- **标签**：数组、哈希表、字符串、前缀和

**题意**：给你一个长度为**偶数** $n$、下标从 0 开始的字符串 $s$，再给一个二维整数数组 $queries$，其中 $queries[i] = [a_i, b_i, c_i, d_i]$。对每个查询**独立**执行：
