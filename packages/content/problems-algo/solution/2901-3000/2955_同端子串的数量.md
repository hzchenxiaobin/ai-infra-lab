...
type: problem
title: 同端子串的数量
tags: [前缀和, 哈希表, 字符串, 数组, 计数]
knowledge_points: [array, counting, hash-table, prefix-sum, string]
updated: 2026-09-03
source: leetcode
number: 2955
difficulty: medium
...
**题意**：给定一个**下标从 0 开始**的字符串 `s`，以及一个二维整数数组 `queries`，其中 `queries[i] = [l_i, r_i]` 表示 `s` 中从索引 `l_i` 开始到索引 `r_i` 结束的子串（**包括两端**），即 `s[l_i..r_i]`。
