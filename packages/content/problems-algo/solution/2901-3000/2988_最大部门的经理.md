...
type: problem
title: 最大部门的经理
tags: ["GROUP BY 聚合", JOIN, SQL, "子查询求 MAX", "并列最值（Argmax）", "窗口函数 RANK"]
knowledge_points: [group-by, group-by-聚合, join, sql, 子查询求-max, 并列最值-argmax, 窗口函数-rank]
updated: 2026-09-03
source: leetcode
number: 2988
difficulty: medium
...
**题意**：给定员工表 `Employees`，每行是一位员工及其所属部门与职位。编写解决方案，找出**最大部门**（员工数最多的部门）的**经理**（`position = 'Manager'` 的员工）的姓名。**当多个部门员工数相同且并列最大时，这些部门的经理都要返回**。结果按 `dep_id` **升序**排列。
