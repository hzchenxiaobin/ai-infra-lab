...
type: problem
title: "市场分析 III"
tags: ["COUNT(DISTINCT)", "GROUP BY 聚合", SQL, 多表连接, 子查询, 窗口函数]
knowledge_points: [count-distinct, group-by, group-by-聚合, multi-join, sql, subquery, window-function]
updated: 2026-09-03
source: leetcode
number: 2922
difficulty: medium
...
# LeetCode 市场分析 III 题解

## 1. 题目概述

- **标题 / 题号**：市场分析 III（#2922，medium）
...
**题意**：给定电商网站的 `Users`、`Items`、`Orders` 三张表。编写解决方案，找出售出「**品牌与自己偏好品牌 `favorite_brand` 不同**」的**去重商品（unique items）**数量最多的**最佳卖家（top seller）**；若多个卖家数量并列最多，则**全部返回**。结果按 `seller_id` **升序**排列。
