...
title: "找出满足差值条件的下标 I"
tags: [一次遍历, 双指针, 数组]
knowledge_points: [array, one-pass, two-pointers, 一次遍历]
updated: 2026-09-03
source: leetcode
number: 2903
difficulty: easy
...
# LeetCode 找出满足差值条件的下标 I 题解

## 1. 题目概述

- **标题 / 题号**：找出满足差值条件的下标 I（#2903，easy）
- **链接**：https://leetcode.cn/problems/find-indices-with-index-and-value-difference-i/
- **难度**：简单
- **标签**：数组、双指针、一次遍历

**题意**：给你一个下标从 **0** 开始、长度为 $n$ 的整数数组 `nums`，以及整数 `indexDifference` 和 `valueDifference`。
