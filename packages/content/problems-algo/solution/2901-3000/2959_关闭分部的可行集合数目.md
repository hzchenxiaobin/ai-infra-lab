...
type: problem
title: 关闭分部的可行集合数目
tags: [位运算, 图, "最短路（Floyd-Warshall）", 枚举]
knowledge_points: [bit-manipulation, enumeration, graph, 最短路-floyd-warshall]
updated: 2026-09-03
source: leetcode
number: 2959
difficulty: hard
...
# LeetCode 关闭分部的可行集合数目 题解

## 1. 题目概述

- **标题 / 题号**：关闭分部的可行集合数目（#2959，hard）
- **链接**：https://leetcode.cn/problems/number-of-possible-sets-of-closing-branches/
- **难度**：困难
- **标签**：位运算、图、枚举、最短路（Floyd-Warshall）

**题意**：一个公司在全国有 $n$ 个分部，分部之间由一些**无向道路**连接，一开始所有分部两两互相可达。公司决定关闭一些分部（**也可以不关闭任何分部**），要求剩余分部两两互相可达，且它们之间的**最远距离不超过** `maxDistance`。
...
![示例1](https://assets.leetcode.com/uploads/2023/11/08/example11.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/11/08/example22.png)
...
| 526 | [优美的安排](https://leetcode.cn/problems/beautiful-arrangement/)（[站内题解](../0501-0600/526_优美的排列.md)） | 同样的 $n \le 15$ bitmask 枚举甜区信号，训练「看见小 n 就想子集枚举」的肌肉记忆 |
