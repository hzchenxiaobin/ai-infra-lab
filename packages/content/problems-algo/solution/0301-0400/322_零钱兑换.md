...
type: problem
title: 零钱兑换
tags: [动态规划, 完全背包, 广度优先搜索]
knowledge_points: [bfs, dynamic-programming, unbounded-knapsack]
updated: 2026-09-03
source: leetcode
number: 322
difficulty: medium
...
judge: none
related_learn: ["learn:w08d03", "learn:w08d06", "learn:w08d07"]
---
...
- **标题 / 题号**：零钱兑换（#322，medium）
- **链接**：https://leetcode.cn/problems/coin-change/
- **难度**：中等
- **标签**：动态规划、完全背包、广度优先搜索

**题意**：给定硬币面额数组 `coins` 和总金额 `amount`，求凑成该金额所需的最少硬币数。无法凑出返回 `-1`。每种硬币可使用无限次。
...
> 💡 与 [Week8 Day3 面试基础篇](../../../learn/daily/week8/day3/README.md) 的 GPU **shared memory tile 复用**同构——DP 中 `dp[i]` 查 `dp[i-coin]`（已算好的子问题解，不重算），GPU kernel 从 shared memory 读 tile（已从 HBM 加载的数据，不重读）。两者都是"缓存已计算/已加载的结果，避免重复"的核心模式。DP 表 = shared memory tile = 缓存。
...
| 377 | [组合总和 IV](https://leetcode.cn/problems/combination-sum-iv/)（[题解](377_组合总和IV.md)） | 排列数 DP |
| 416 | [分割等和子集](https://leetcode.cn/problems/partition-equal-subset-sum/)（[题解](../0401-0500/416_分割等和子集.md)） | 0-1 背包 |
