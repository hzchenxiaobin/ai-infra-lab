...
# LeetCode 至多包含 K 个不同字符的最长子串 题解

## 1. 题目概述

- **标题 / 题号**：至多包含 K 个不同字符的最长子串（#340，medium）
- **链接**：https://leetcode.cn/problems/longest-substring-with-at-most-k-distinct-characters/
- **难度**：中等
- **标签**：哈希表、字符串、滑动窗口

**题意**：给定一个字符串 `s` 和一个整数 `k`，找出其中**至多包含 `k` 个不同字符**的最长子串的长度，并返回该长度。
...
这是第 146 题 [LRU 缓存](../0101-0200/146_LRU缓存.md) 的同款结构，也是本题在面试中常见的「follow-up：如果 `s` 是数据流怎么办」的标准答案——流式场景下无法回头读 `s[left]`，必须靠 `lastPos` 记录位置并支持快速淘汰。
