...
title: "岛屿数量 II"
tags: [图, 并查集, 数组]
knowledge_points: [array, graph, union-find]
updated: 2026-09-03
source: leetcode
number: 305
difficulty: hard
...
- **标题 / 题号**：岛屿数量 II（#305，hard）
- **链接**：https://leetcode.cn/problems/number-of-islands-ii/
- **难度**：困难
- **标签**：并查集、图、数组

**题意**：给定一个 `m × n` 的二维网格，初始**全为水**（`0`）。接下来依次执行若干次 `addLand` 操作，每次把指定位置 `(row, col)` 的水变成陆地。要求在**每次操作之后**，返回当前网格中岛屿的数量。
