...
type: problem
title: 计算右侧小于当前元素的个数
tags: [分治, 归并排序, 数组, 树状数组]
knowledge_points: [array, divide-and-conquer, fenwick-tree, merge-sort]
updated: 2026-09-03
source: leetcode
number: 315
difficulty: hard
...
# LeetCode 计算右侧小于当前元素的个数 题解

## 1. 题目概述

- **标题 / 题号**：计算右侧小于当前元素的个数（#315，hard）
- **链接**：https://leetcode.cn/problems/count-of-smaller-numbers-after-self/
- **难度**：困难
- **标签**：数组、分治、归并排序、树状数组

**题意**：给定一个整数数组 `nums`，返回一个新数组 `counts`，其中 `counts[i]` 表示 `nums[i]` 右侧**严格小于** `nums[i]` 的元素个数。
