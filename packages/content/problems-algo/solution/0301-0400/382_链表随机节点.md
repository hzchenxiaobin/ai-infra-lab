...
type: problem
title: 链表随机节点
tags: [概率与随机, 水塘/蓄水池抽样, 设计, 链表]
knowledge_points: [design, linked-list, probability, reservoir-sampling]
updated: 2026-09-03
source: leetcode
number: 382
difficulty: medium
...
- **标题 / 题号**：链表随机节点（#382，medium）
- **链接**：https://leetcode.cn/problems/linked-list-random-node/
- **难度**：中等
- **标签**：水塘/蓄水池抽样、链表、概率与随机、设计

**题意**：给定一个**单链表**的头节点 `head`，实现一个类，支持「随机返回链表中一个节点的值」的操作。链表中每个节点被选中的概率必须**相等**。可以多次调用 `getRandom()`。
...
![示例1](https://assets.leetcode.com/uploads/2021/03/16/getrand-linked-list.jpg)
