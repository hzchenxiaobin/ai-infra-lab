...
title: 回文对
tags: [哈希表, 回文, 字典树, 字符串]
knowledge_points: [hash-table, palindrome, string, trie]
updated: 2026-09-03
source: leetcode
number: 336
difficulty: hard
...
**题意**：给定一个**互不相同**的字符串数组 `words`，找出所有下标对 `(i, j)`，使得 `i != j` 且 `words[i] + words[j]`（即 `words[i]` 拼上 `words[j]`）是一个**回文串**。返回所有满足条件的下标对列表，顺序不限。
