...
title: 字符串解码
tags: [字符串, 栈, 递归]
knowledge_points: [recursion, stack, string]
updated: 2026-09-03
source: leetcode
number: 394
difficulty: medium
...
judge: none
related_learn: ["learn:w04d02", "learn:w04d07"]
---
...
- **标题 / 题号**：字符串解码（#394，medium）
- **链接**：https://leetcode.cn/problems/decode-string/
- **难度**：中等
- **标签**：栈、字符串、递归

**题意**：给定一个经过编码的字符串 `s`，返回它解码后的结果。编码规则是：`k[encoded_string]`，表示其中的 `encoded_string` **重复 `k` 次**。`k` 保证为正整数。**方括号可以嵌套**，如 `3[a2[c]]` 解码为 `accaccacc`。
