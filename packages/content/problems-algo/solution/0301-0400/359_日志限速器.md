...
title: 日志限速器
tags: [哈希表, 设计]
knowledge_points: [design, hash-table]
updated: 2026-09-03
source: leetcode
number: 359
difficulty: easy
...
- **标题 / 题号**：日志限速器（#359，easy）
- **链接**：https://leetcode.cn/problems/logger-rate-limiter/
- **难度**：简单
- **标签**：设计、哈希表

**题意**：设计一个日志系统，接收消息流（每条消息附带一个整数时间戳 `timestamp`）。**同一条消息在 `10` 秒内最多打印一次**——即如果某条消息在时间戳 `t` 打印过，那么在 $t \le t' < t + 10$ 的时间段内**不得再次打印**，只有当 $t' \ge t + 10$ 时才允许重新打印。
...
| 981 | [基于时间的键值存储](https://leetcode.cn/problems/time-based-key-value-store/)（[题解](../0901-1000/981_基于时间的键值存储.md)） | 哈希表 + 有序时间戳，按时间检索值，对比 359 的「按时间判定是否打印」 |
