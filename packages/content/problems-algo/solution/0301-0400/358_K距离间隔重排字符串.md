...
type: problem
title: "K 距离间隔重排字符串"
tags: [哈希表, "堆（优先队列）", 字符串, 贪心, 队列]
knowledge_points: [greedy, hash-table, heap, queue, string]
updated: 2026-09-03
source: leetcode
number: 358
difficulty: hard
...
# LeetCode K 距离间隔重排字符串 题解

## 1. 题目概述

- **标题 / 题号**：K 距离间隔重排字符串（#358，hard）
- **链接**：https://leetcode.cn/problems/rearrange-string-k-distance-apart/
- **难度**：困难
- **标签**：贪心、堆（优先队列）、队列、哈希表、字符串

**题意**：给定一个非空字符串 `s` 和一个整数 `k`，重排字符串使得**相同字符之间至少相隔 `k` 个位置**（即若同一字符先后出现在下标 `i`、`j`（`i < j`），则必须 `j - i >= k`）。如果无法重排，返回空串 `""`。
