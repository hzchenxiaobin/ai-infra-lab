...
title: 稀疏矩阵的乘法
tags: [数组, 矩阵, 稀疏存储]
knowledge_points: [array, matrix, 矩阵, 稀疏存储]
updated: 2026-09-03
source: leetcode
number: 311
difficulty: medium
...
- **标题 / 题号**：稀疏矩阵的乘法（#311，medium）
- **链接**：https://leetcode.cn/problems/sparse-matrix-multiplication/
- **难度**：中等
- **标签**：数组、矩阵、稀疏存储

**题意**：给定两个**稀疏矩阵** `mat1`（大小 `m × k`）和 `mat2`（大小 `k × n`），返回它们的矩阵乘积 `mat1 × mat2`。可假设乘法总是可行的（即 `mat1` 的列数等于 `mat2` 的行数）。
