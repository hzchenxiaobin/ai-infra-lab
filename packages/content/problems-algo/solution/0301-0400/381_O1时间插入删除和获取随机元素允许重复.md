...
tags: [哈希表, 数组, 设计, 随机化]
knowledge_points: [array, design, hash-table, randomized]
updated: 2026-09-03
source: leetcode
number: 381
difficulty: hard
...
# LeetCode O(1) 时间插入、删除和获取随机元素 - 允许重复 题解

## 1. 题目概述

- **标题 / 题号**：O(1) 时间插入、删除和获取随机元素 - 允许重复（#381，hard）
- **链接**：https://leetcode.cn/problems/insert-delete-getrandom-o1-duplicates-allowed/
- **难度**：困难
- **标签**：设计、哈希表、数组、随机化

**题意**：实现一个 `RandomizedCollection` 类，允许同一元素**出现多次**，且 `insert`、`remove`、`getRandom` 三个操作**平均时间复杂度均为 $O(1)$**：
...
| 460 | [LFU 缓存](https://leetcode.cn/problems/lfu-cache/)（[题解](../0401-0500/460_LFU缓存.md)） | 更复杂的「频率桶 + 哈希」$O(1)$ 设计，频率桶内部用双向链表维护同频元素，与 381 的「下标集合」异曲同工 |
