...
tags: [哈希表, 数组, 设计]
knowledge_points: [array, design, hash-table]
updated: 2026-09-03
source: leetcode
number: 380
difficulty: medium
...
judge: none
related_learn: ["learn:w04d01", "learn:w04d07"]
---

# LeetCode O(1) 时间插入、删除和获取随机元素 题解

## 1. 题目概述

- **标题 / 题号**：O(1) 时间插入、删除和获取随机元素（#380，medium）
- **链接**：https://leetcode.cn/problems/insert-delete-getrandom-o1/
- **难度**：中等
- **标签**：设计、哈希表、数组

**题意**：实现一个 `RandomizedSet` 类，要求 `insert`、`remove`、`getRandom` 三个操作**平均时间复杂度均为 $O(1)$**：
...
| 146 | [LRU 缓存](https://leetcode.cn/problems/lru-cache/)（[题解](../0101-0200/146_LRU缓存.md)） | 同样是「哈希表 + 另一结构」的 $O(1)$ 设计题，对比「哈希 + 双向链表」与本题「哈希 + 动态数组」的选型差异 |
| 155 | [最小栈](https://leetcode.cn/problems/min-stack/)（[题解](../0101-0200/155_最小栈.md)） | 辅助结构协同思想——用一个额外结构（辅助栈）补足主结构缺失的能力 |
