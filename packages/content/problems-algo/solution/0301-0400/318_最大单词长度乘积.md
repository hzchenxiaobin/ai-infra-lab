...
title: 最大单词长度乘积
tags: [位运算, 哈希表, 数组]
knowledge_points: [array, bit-manipulation, hash-table]
updated: 2026-09-03
source: leetcode
number: 318
difficulty: medium
...
# LeetCode 最大单词长度乘积 题解

## 1. 题目概述

- **标题 / 题号**：最大单词长度乘积（#318，medium）
- **链接**：https://leetcode.cn/problems/maximum-product-of-word-lengths/
- **难度**：中等
- **标签**：位运算、数组、哈希表

**题意**：给定一个字符串数组 `words`，找出并返回 `length(words[i]) * length(words[j])` 的最大值，且这两个单词**不含有任何公共字母**。如果不存在这样的两个单词，返回 `0`。
