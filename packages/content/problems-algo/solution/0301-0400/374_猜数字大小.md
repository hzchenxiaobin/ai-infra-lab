...
title: 猜数字大小
tags: [二分查找, 交互]
knowledge_points: [binary-search, interactive, 交互]
updated: 2026-09-03
source: leetcode
number: 374
difficulty: easy
...
> 💡 本题的核心是「**在有序值域 `[1, n]` 上做精确匹配的二分查找**」：`guess(num)` 是一个**三段式比较器**，把值域切成「比 pick 小 / 等于 pick / 比 pick 大」三段。这正是 [704. 二分查找](../0701-0800/704_二分查找.md) 的母题模板——只不过「比较」从 `nums[mid]` 与 `target` 的值比较，换成了 `guess(mid)` 与 `0` 的函数调用比较。
...
> 💡 **与 278 的对照**：[278. 第一个错误的版本](../0201-0300/278_第一个错误的版本.md) 的 `isBadVersion` 是**布尔判定**（找「首个 true」的边界），用 `left < right` + `right = mid` 模板保留候选；本题 `guess` 是**三段式比较**（找「等于 0」的精确命中），用 `left <= right` + `right = mid - 1` 模板命中即返回。两者都是二分查找，区别在于判定是布尔还是三段式、目标是找边界还是找精确点——母题都是 [704](../0701-0800/704_二分查找.md)。
...
本题使用「`left <= right` + 命中即返回 + `right = mid - 1` / `left = mid + 1`」闭区间模板（与 [704. 二分查找](../0701-0800/704_二分查找.md) 一致），而 [278. 第一个错误的版本](../0201-0300/278_第一个错误的版本.md) 使用「`left < right` + `right = mid`」模板找边界。两者都是二分查找，但目标不同：
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 704 | [二分查找](https://leetcode.cn/problems/binary-search/)（[题解](../0701-0800/704_二分查找.md)） | 闭区间二分母题，判定从 `guess(mid)` 换成 `nums[mid]` 与 `target` 的值比较，模板同构 |
