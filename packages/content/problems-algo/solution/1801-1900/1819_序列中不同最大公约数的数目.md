...
title: 序列中不同最大公约数的数目
tags: [数学, 数组, 数论, 枚举]
knowledge_points: [array, enumeration, math, number-theory]
updated: 2026-09-03
source: leetcode
number: 1819
difficulty: hard
...
# LeetCode 序列中不同最大公约数的数目 题解

## 1. 题目概述

- **标题 / 题号**：序列中不同最大公约数的数目（#1819，hard）
- **链接**：https://leetcode.cn/problems/number-of-different-subsequences-gcds/
- **难度**：困难
- **标签**：数组、数学、数论、枚举

**题意**：给你一个由正整数组成的数组 `nums`。子序列可以通过删除若干元素（或不删除）得到。求 `nums` 的所有**非空**子序列中，**不同**最大公约数（GCD）的数目。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/04/03/image-1.png)
...
| 204 | [计数质数](https://leetcode.cn/problems/count-primes/)（[题解](../0201-0300/204_计数质数.md)） | 埃氏筛的"对每个 $p$ 划掉倍数"与本题"对每个 $g$ 遍历倍数"同属调和级数筛法家族，复杂度同源 |
