...
type: problem
title: "跳跃游戏 VII"
tags: [前缀和, 动态规划, 字符串, 滑动窗口]
knowledge_points: [dynamic-programming, prefix-sum, sliding-window, string]
updated: 2026-09-03
source: leetcode
number: 1871
difficulty: medium
...
# LeetCode 跳跃游戏 VII 题解

## 1. 题目概述

- **标题 / 题号**：跳跃游戏 VII（#1871，medium）
- **链接**：https://leetcode.cn/problems/jump-game-vii/
- **难度**：中等
- **标签**：字符串、动态规划、滑动窗口、前缀和

**题意**：给你一个下标从 **0** 开始的二进制字符串 `s` 和两个整数 `minJump` 和 `maxJump`。一开始你在下标 `0` 处，且该位置的值一定为 `'0'`。当同时满足如下条件时，你可以从下标 `i` 移动到下标 `j` 处：
...
| 45 | [跳跃游戏 II](https://leetcode.cn/problems/jump-game-ii/)（[题解](../0001-0100/45_跳跃游戏%20II.md)） | 求最少跳跃次数，贪心 BFS 层序扩展，与本题的可达性判定形成对比 |
