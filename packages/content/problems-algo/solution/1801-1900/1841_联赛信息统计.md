...
type: problem
title: 联赛信息统计
tags: ["CASE WHEN", COALESCE, "GROUP BY + 聚合", "LEFT JOIN", SQL, "UNION ALL（逆透视 UNPIVOT）", 多列排序, 数据库]
knowledge_points: [case-when, coalesce, database, group-by-聚合, left-join, sql, union-all-逆透视-unpivot, 多列排序]
updated: 2026-09-03
source: leetcode
number: 1841
difficulty: medium
...
**题意**：给定 `Teams` 表（球队信息）和 `Matches` 表（比赛记录），编写 SQL 查询报告联赛统计。胜者得 3 分，败者得 0 分；平局双方各得 1 分。每行输出球队名、比赛场数、积分、进球数、失球数、净胜球，按积分降序、净胜球降序、球队名字典序排列。
