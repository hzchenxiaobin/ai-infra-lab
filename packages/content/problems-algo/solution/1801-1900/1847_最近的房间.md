...
type: problem
title: 最近的房间
tags: [二分查找, 排序, 有序集合, 离线处理]
knowledge_points: [binary-search, offline-processing, ordered-set, sorting]
updated: 2026-09-03
source: leetcode
number: 1847
difficulty: hard
...
- **标题 / 题号**：最近的房间（#1847，hard）
- **链接**：https://leetcode.cn/problems/closest-room/
- **难度**：困难
- **标签**：排序、有序集合、二分查找、离线处理

**题意**：一个酒店里有 `n` 个房间，用二维数组 `rooms` 表示，其中 `rooms[i] = [roomIdi, sizei]` 表示房间号为 `roomIdi`、面积为 `sizei`。每个房间号**互不相同**。
