...
title: 最高频元素的频数
tags: [前缀和, 排序, 数组, 滑动窗口]
knowledge_points: [array, prefix-sum, sliding-window, sorting]
updated: 2026-09-03
source: leetcode
number: 1838
difficulty: medium
...
# LeetCode 最高频元素的频数 题解

## 1. 题目概述

- **标题 / 题号**：最高频元素的频数（#1838，medium）
- **链接**：https://leetcode.cn/problems/frequency-of-the-most-frequent-element/
- **难度**：中等
- **标签**：数组、滑动窗口、排序、前缀和

**题意**：给定整数数组 `nums` 和整数 `k`。每次操作可选择一个下标，将该下标元素值增加 1。执行**最多 `k` 次操作**后，返回数组中最高频元素的**最大可能频数**。
