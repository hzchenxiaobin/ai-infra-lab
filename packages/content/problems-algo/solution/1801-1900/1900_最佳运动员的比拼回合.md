...
type: problem
title: 最佳运动员的比拼回合
tags: [博弈, 深度优先搜索, 记忆化搜索]
knowledge_points: [dfs, game-theory, memoization]
updated: 2026-09-03
source: leetcode
number: 1900
difficulty: hard
...
# LeetCode 最佳运动员的比拼回合 题解

## 1. 题目概述

- **标题 / 题号**：最佳运动员的比拼回合（#1900，hard）
- **链接**：https://leetcode.cn/problems/the-earliest-and-latest-rounds-where-players-compete/
- **难度**：困难
- **标签**：记忆化搜索、深度优先搜索、博弈

**题意**：$n$ 名运动员站成一排，按初始站位从 $1$ 到 $n$ 编号。锦标赛由多个回合组成，每一回合中**第 $i$ 名**（从前数）与**倒数第 $i$ 名**（从后数）配对比拼，获胜者进入下一回合。若当前回合人数为奇数，中间那位运动员**轮空**自动晋级。每回合结束后，获胜者按**原始编号升序**重新排成一排。
...
| 526 | [优美的排列](https://leetcode.cn/problems/beautiful-arrangement/)（[站内题解](../0501-0600/526_优美的排列.md)） | 状态压缩 DP——`bitmask` 记已用元素，$n \le 15$ 甜区，理解后本题的位掩码状态编码水到渠成 |
