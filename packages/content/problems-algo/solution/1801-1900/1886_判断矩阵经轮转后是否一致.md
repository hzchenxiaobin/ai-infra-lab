...
title: 判断矩阵经轮转后是否一致
tags: [数组, 矩阵]
knowledge_points: [array, matrix, 矩阵]
updated: 2026-09-03
source: leetcode
number: 1886
difficulty: easy
...
# LeetCode 判断矩阵经轮转后是否一致 题解

## 1. 题目概述

- **标题 / 题号**：判断矩阵经轮转后是否一致（#1886，easy）
- **链接**：https://leetcode.cn/problems/determine-whether-matrix-can-be-obtained-by-rotation/
- **难度**：简单
- **标签**：数组、矩阵

**题意**：给你两个大小为 $n \times n$ 的二进制矩阵 `mat` 和 `target`。现 **以 90 度顺时针轮转** 矩阵 `mat` 中的元素 **若干次**，如果能够使 `mat` 与 `target` 一致，返回 `true`；否则返回 `false`。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/05/20/grid3.png)
...
![示例2](https://assets.leetcode.com/uploads/2021/05/20/grid4.png)
...
![示例3](https://assets.leetcode.com/uploads/2021/05/26/grid4.png)
