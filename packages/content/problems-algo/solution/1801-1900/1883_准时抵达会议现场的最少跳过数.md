...
id: "lc:1883"
type: problem
title: 准时抵达会议现场的最少跳过数
tags: [动态规划, 数学, 数组]
knowledge_points: [array, dynamic-programming, math]
updated: 2026-09-03
source: leetcode
number: 1883
difficulty: hard
...
# LeetCode 准时抵达会议现场的最少跳过数 题解

## 1. 题目概述

- **标题 / 题号**：准时抵达会议现场的最少跳过数（#1883，hard）
- **链接**：https://leetcode.cn/problems/minimum-skips-to-arrive-at-meeting-on-time/
- **难度**：困难
- **标签**：动态规划、数学、数组

**题意**：给定整数数组 `dist` 表示 $n$ 段路程的距离，整数 `speed` 表示行驶速度（千米/小时），浮点数 `hoursBefore` 表示会议开始前可用的总时间。必须**按给定顺序**依次行驶完所有 $n$ 段路程。
...
| 198 | [打家劫舍](https://leetcode.cn/problems/house-robber/)（[题解](../0101-0200/198_打家劫舍.md)） | 一维滚动 DP + 从高到低遍历的同类空间优化思想 |
