...
type: problem
title: 得到新鲜甜甜圈的最多组数
tags: [数学, "状态压缩 DP", 记忆化搜索, 贪心]
knowledge_points: [bitmask-dp, greedy, math, memoization]
updated: 2026-09-03
source: leetcode
number: 1815
difficulty: hard
...
# LeetCode 得到新鲜甜甜圈的最多组数 题解

## 1. 题目概述

- **标题 / 题号**：得到新鲜甜甜圈的最多组数（#1815，hard）
- **链接**：https://leetcode.cn/problems/maximum-number-of-groups-getting-fresh-donuts/
- **难度**：困难
- **标签**：状态压缩 DP、记忆化搜索、贪心、数学

**题意**：甜甜圈商店每批次烤 `batchSize` 个甜甜圈，且烤新一批时之前所有甜甜圈必须全部售完。给定整数数组 `groups`，`groups[i]` 表示第 $i$ 批顾客的人数，每人恰好买一个甜甜圈。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 526 | [优美的排列](https://leetcode.cn/problems/beautiful-arrangement/)（[题解](../0501-0600/526_优美的排列.md)） | 状态压缩 DP 母题——`bitmask` 记已用元素，$n \le 15$ 甜区，理解后本题的状态编码水到渠成 |
