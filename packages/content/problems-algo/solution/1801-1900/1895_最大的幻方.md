...
title: 最大的幻方
tags: [前缀和, 数组, 枚举]
knowledge_points: [array, enumeration, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1895
difficulty: medium
...
- **标题 / 题号**：最大的幻方（#1895，medium）
- **链接**：https://leetcode.cn/problems/largest-magic-square/
- **难度**：中等
- **标签**：数组、前缀和、枚举

**题意**：一个 `k × k` 的**幻方**指的是一个填满整数的方格阵，且**每一行、每一列以及两条对角线的和全部相等**。幻方中的整数**不需要互不相同**。显然，每个 `1 × 1` 的方格都是一个幻方。

给你一个 `m × n` 的整数矩阵 `grid`，返回矩阵中**最大幻方**的**尺寸**（即边长 `k`）。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/05/29/magicsquare-grid.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/05/29/magicsquare2-grid.jpg)
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 304 | [二维区域和检索 - 数组不可变](https://leetcode.cn/problems/range-sum-query-2d-immutable/)（[站内题解](../0301-0400/304_二维区域和检索%20-%20数组不可变.md)） | 二维前缀和模板题，本题行/列前缀和是其降维版 |
