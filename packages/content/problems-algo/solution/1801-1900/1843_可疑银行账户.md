...
type: problem
title: 可疑银行账户
tags: [CTE, "GROUP BY", HAVING, SQL, 数据库, 日期处理, 自连接, 连续区间判定]
knowledge_points: [cte, database, date-function, gaps-and-islands, group-by, having, self-join, sql]
updated: 2026-09-03
source: leetcode
number: 1843
difficulty: medium
...
- **标题 / 题号**：可疑银行账户（#1843，medium）
- **链接**：https://leetcode.cn/problems/suspicious-bank-accounts/
- **难度**：中等
- **标签**：数据库、SQL、CTE、`GROUP BY`、`HAVING`、自连接、日期处理、连续区间判定

**题意**：给定 `Accounts`（账户阈值表）与 `Transactions`（交易流水表）两张表，编写 SQL 查询，报告所有**可疑银行账户**的 `account_id`。如果一个账户在**连续两个及以上月份**的**月总收入**超过 `max_income`，则认为该账户可疑。月总收入 = 当月所有 `type = 'Creditor'`（存入）交易的 `amount` 之和。
