...
type: problem
title: 每天的最大交易
tags: ["DATE() 函数", "GROUP BY", "MAX()", "RANK()", SQL, "元组 IN", 分组求最值, "反连接（anti-join）", 数据库, 窗口函数, 自连接]
knowledge_points: [anti-join, database, date-函数, group-by, group-extremum, max, rank, self-join, sql, window-function, 元组-in, 分组求最值]
updated: 2026-09-03
source: leetcode
number: 1831
difficulty: medium
...
**题意**：给定 `Transactions` 表（交易记录，含交易 ID、日期时间、金额），编写 SQL 查询报告**每天交易金额 `amount` 最大的交易 ID**。如果一天中有多个这样的交易（并列最大），返回这些交易的 ID。结果根据 `transaction_id` **升序排列**。
