...
type: problem
title: 寻找今年具有正收入的客户
tags: ["AND 逻辑", SQL, "WHERE 过滤", 复合主键, 行级筛选]
knowledge_points: [and-逻辑, sql, where-filter, where-过滤, 复合主键, 行级筛选]
updated: 2026-09-03
source: leetcode
number: 1821
difficulty: easy
...
# LeetCode 寻找今年具有正收入的客户 题解

## 1. 题目概述

- **标题 / 题号**：寻找今年具有正收入的客户（#1821，easy）
- **链接**：https://leetcode.cn/problems/find-customers-with-positive-revenue-this-year/
- **难度**：简单
- **标签**：SQL、WHERE 过滤、复合主键、AND 逻辑、行级筛选

**题意**：给定 `Customers` 表，每行记录某客户在某一年的收入。编写 SQL 查询，报告在 **2021 年**收入为**正**的客户编号 `customer_id`。结果按任意顺序返回。
