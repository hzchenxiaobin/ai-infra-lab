...
type: problem
title: 页面推荐Ⅱ
tags: [CTE, DISTINCT, JOIN, SQL, "UNION（双向好友展开）", "反连接（Anti-Join / NOT EXISTS / LEFT JOIN ... IS NULL）", 数据库]
knowledge_points: [cte, database, distinct, join, sql, union-双向好友展开, 反连接-anti-join-not-exists-left-join-...-is-null]
updated: 2026-09-03
source: leetcode
number: 1892
difficulty: hard
...
**题意**：给定 `Friendship` 表（好友关系，**双向/对称**）和 `Likes` 表（用户点赞的页面），编写 SQL 查询：**为每一个用户**推荐其**好友**点赞过、但**自己未点赞**的页面。结果输出 `user_id` 和 `page_id` 两列，顺序不限。
