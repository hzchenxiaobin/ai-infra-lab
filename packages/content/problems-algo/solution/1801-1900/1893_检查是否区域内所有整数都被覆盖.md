...
tags: [前缀和, 差分数组, 数组]
knowledge_points: [array, difference-array, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1893
difficulty: easy
...
# LeetCode 检查是否区域内所有整数都被覆盖 题解

## 1. 题目概述

- **标题 / 题号**：检查是否区域内所有整数都被覆盖（#1893，easy）
- **链接**：https://leetcode.cn/problems/check-if-all-the-integers-in-a-range-are-covered/
- **难度**：简单
- **标签**：数组、差分数组、前缀和

**题意**：给定一个二维整数数组 `ranges`，其中每个 `ranges[i] = [starti, endi]` 表示一个**闭区间** $[\text{start}_i, \text{end}_i]$。再给定两个整数 `left` 和 `right`，判断闭区间 $[\text{left}, \text{right}]$ 内**每个整数**是否都被 `ranges` 中**至少一个**区间覆盖。
