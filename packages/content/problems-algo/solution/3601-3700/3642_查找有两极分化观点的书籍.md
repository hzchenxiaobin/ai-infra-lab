...
type: problem
title: 查找有两极分化观点的书籍
tags: ["GROUP BY", HAVING, ROUND, SQL, pandas, 数据库, 条件聚合, 聚合函数]
knowledge_points: [aggregate-function, conditional-aggregation, database, group-by, having, pandas, round, sql]
updated: 2026-09-03
source: leetcode
number: 3642
difficulty: medium
...
# LeetCode 查找有两极分化观点的书籍 题解

## 1. 题目概述

- **标题 / 题号**：查找有两极分化观点的书籍（#3642，medium）
- **链接**：https://leetcode.cn/problems/find-books-with-polarized-opinions/
- **难度**：中等
- **标签**：数据库、SQL、`GROUP BY`、聚合函数、`HAVING`、条件聚合、`ROUND`、pandas

**题意**：给定 `books`（书籍信息）与 `reading_sessions`（阅读事件，含 1–5 分评分）两张表，找出**观点两极分化**的书籍并输出其极化统计。
