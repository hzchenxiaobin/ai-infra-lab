...
id: "lc:3624"
type: problem
title: 位计数深度为K的整数数目II
tags: [分治, 数组, 树状数组, 线段树]
knowledge_points: [array, divide-and-conquer, fenwick-tree, segment-tree]
updated: 2026-09-03
source: leetcode
number: 3624
difficulty: hard
...
# LeetCode 位计数深度为K的整数数目II 题解

## 1. 题目概述

- **标题 / 题号**：位计数深度为K的整数数目II（#3624，hard）
- **链接**：https://leetcode.cn/problems/number-of-integers-with-popcount-depth-equal-to-k-ii/
- **难度**：困难
- **标签**：树状数组、线段树、数组、分治

**题意**：对于任意正整数 $x$，定义序列 $p_0 = x$，$p_{i+1} = \text{popcount}(p_i)$（即反复取二进制表示中 1 的个数）。该序列最终收敛到 1，**popcount-depth（位计数深度）**定义为满足 $p_d = 1$ 的最小整数 $d \ge 0$。例如 $x = 7$：$7 \to 3 \to 2 \to 1$，故 $\text{depth}(7) = 3$。
