...
id: "lc:3608"
type: problem
title: "包含 K 个连通分量需要的最小时间"
tags: [二分查找, 图, 并查集, 排序]
knowledge_points: [binary-search, graph, sorting, union-find]
updated: 2026-09-03
source: leetcode
number: 3608
difficulty: medium
...
# LeetCode 包含 K 个连通分量需要的最小时间 题解

## 1. 题目概述

- **标题 / 题号**：包含 K 个连通分量需要的最小时间（#3608，medium）
- **链接**：https://leetcode.cn/problems/minimum-time-for-k-connected-components/
- **难度**：中等
- **标签**：并查集、图、二分查找、排序

**题意**：给定 $n$ 个节点（编号 $0$ 到 $n-1$）的无向图，边由二维数组 `edges` 给出，其中 `edges[i] = [u_i, v_i, time_i]` 表示一条连接 $u_i$ 与 $v_i$ 的无向边，**该边会在时刻 $time_i$ 被移除**。

另给整数 $k$。初始时图可能连通也可能不连通。求**最小**时刻 $t$，使得移除所有满足 $time_i \le t$ 的边之后，图中**至少有 $k$ 个连通分量**，返回该 $t$。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2025/05/31/screenshot-2025-06-01-at-022724.png)
...
![示例2](https://assets.leetcode.com/uploads/2025/05/31/screenshot-2025-06-01-at-022812.png)
...
![示例3](https://assets.leetcode.com/uploads/2025/05/31/screenshot-2025-06-01-at-022930.png)
