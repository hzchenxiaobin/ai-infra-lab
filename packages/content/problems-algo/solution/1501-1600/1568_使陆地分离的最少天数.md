...
type: problem
title: 使陆地分离的最少天数
tags: [二维网格, 割点, 图, 深度优先搜索, 脑筋急转弯]
knowledge_points: [articulation-point, brainteaser, dfs, graph, 二维网格]
updated: 2026-09-03
source: leetcode
number: 1568
difficulty: hard
...
# LeetCode 使陆地分离的最少天数 题解

## 1. 题目概述

- **标题 / 题号**：使陆地分离的最少天数（#1568，hard）
- **链接**：https://leetcode.cn/problems/minimum-number-of-days-to-disconnect-island/
- **难度**：困难
- **标签**：图、深度优先搜索、二维网格、割点、脑筋急转弯

**题意**：给定一个 $m \times n$ 的二维网格 `grid`，其中 `1` 表示陆地，`0` 表示水。**岛屿**由水平或竖直方向上相邻的 `1` 连接形成。如果**恰好只有一座岛屿**，则陆地是**连通的**；否则（$0$ 座或 $\ge 2$ 座）就是**分离的**。

每天可以将**任意一个**陆地单元改成水单元。返回使陆地分离的**最少天数**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/12/24/land1.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/12/24/land2.jpg)
