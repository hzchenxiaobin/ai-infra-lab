...
type: problem
title: 连接所有点的最小费用
tags: ["Prim 算法", 图, 堆/优先队列, 最小生成树]
knowledge_points: [graph, heap, mst, prim]
updated: 2026-09-03
source: leetcode
number: 1584
difficulty: medium
...
# LeetCode 连接所有点的最小费用 题解

## 1. 题目概述

- **标题 / 题号**：连接所有点的最小费用（#1584，medium）
- **链接**：https://leetcode.cn/problems/min-cost-to-connect-all-points/
- **难度**：中等
- **标签**：图、最小生成树、Prim 算法、堆/优先队列

**题意**：给定平面上 `n` 个点 `points[i] = [xi, yi]`，两点之间的**曼哈顿距离**为 $|x_i - x_j| + |y_i - y_j|$。求使所有点连通的**最小总费用**——即从完全图中选出一棵**最小生成树（MST）**，使其 `n - 1` 条边的权值之和最小。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2020/08/26/d.png)
...
![示例2](https://assets.leetcode.com/uploads/2020/08/26/c.png)
