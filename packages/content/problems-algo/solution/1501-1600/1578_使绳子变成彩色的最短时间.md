...
title: 使绳子变成彩色的最短时间
tags: [动态规划, 数组, 贪心]
knowledge_points: [array, dynamic-programming, greedy]
updated: 2026-09-03
source: leetcode
number: 1578
difficulty: medium
...
# LeetCode 使绳子变成彩色的最短时间 题解

## 1. 题目概述

- **标题 / 题号**：使绳子变成彩色的最短时间（#1578，medium）
- **链接**：https://leetcode.cn/problems/minimum-time-to-make-rope-colorful/
- **难度**：中等
- **标签**：贪心、数组、动态规划

**题意**：Alice 把 `n` 个气球排列在一根绳子上，给定字符串 `colors`（`colors[i]` 是第 `i` 个气球的颜色）与数组 `neededTime`（`neededTime[i]` 是移除第 `i` 个气球所需的时间）。Bob 可以移除若干气球，使绳子上**不存在两个连续同色气球**。返回所需的最少总时间。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/12/13/ballon1.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/12/13/balloon2.jpg)
...
![示例3](https://assets.leetcode.com/uploads/2021/12/13/balloon3.jpg)
