...
tags: [分治, 差分, 数组, 贪心]
knowledge_points: [array, difference-array, divide-and-conquer, greedy]
updated: 2026-09-03
source: leetcode
number: 1526
difficulty: hard
...
# LeetCode 形成目标数组的子数组最少增加次数 题解

## 1. 题目概述

- **标题 / 题号**：形成目标数组的子数组最少增加次数（#1526，hard）
- **链接**：https://leetcode.cn/problems/minimum-number-of-increments-on-subarrays-to-form-a-target-array/
- **难度**：困难
- **标签**：数组、贪心、差分、分治

**题意**：给定一个整数数组 `target`，初始数组 `initial` 全为 0，且与 `target` 等长。每次操作可以选择 `initial` 中**任意子数组**，将其中每个元素加 1。求从全 0 数组构造出 `target` 的**最少操作次数**。
