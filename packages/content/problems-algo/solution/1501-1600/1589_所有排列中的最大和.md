...
type: problem
title: 所有排列中的最大和
tags: [前缀和, 差分数组, 排序, 数组, 贪心]
knowledge_points: [array, difference-array, greedy, prefix-sum, sorting]
updated: 2026-09-03
source: leetcode
number: 1589
difficulty: medium
...
# LeetCode 所有排列中的最大和 题解

## 1. 题目概述

- **标题 / 题号**：所有排列中的最大和（#1589，medium）
- **链接**：https://leetcode.cn/problems/maximum-sum-obtained-of-any-permutation/
- **难度**：中等
- **标签**：贪心、数组、差分数组、排序、前缀和

**题意**：有一个整数数组 `nums` 和一个查询数组 `requests`，其中 `requests[i] = [starti, endi]`。第 `i` 个查询求 `nums[starti] + nums[starti + 1] + ... + nums[endi]` 的结果（下标从 0 开始）。你可以**任意排列** `nums` 中的数字，求所有查询结果之和的**最大值**。答案对 $10^9 + 7$ 取余。
