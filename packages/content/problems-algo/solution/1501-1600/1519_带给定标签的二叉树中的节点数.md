...
id: "lc:1519"
type: problem
title: 带给定标签的二叉树中的节点数
tags: [后序遍历, 哈希表, 树, 深度优先搜索]
knowledge_points: [dfs, hash-table, postorder-traversal, tree]
updated: 2026-09-03
source: leetcode
number: 1519
difficulty: medium
...
# LeetCode 带给定标签的二叉树中的节点数 题解

## 1. 题目概述

- **标题 / 题号**：带给定标签的二叉树中的节点数（#1519，medium）
- **链接**：https://leetcode.cn/problems/number-of-nodes-with-a-given-label-in-a-binary-tree/
- **难度**：中等
- **标签**：树、深度优先搜索、后序遍历、哈希表

**题意**：给定一棵 **`n` 个节点**的树（无向无环连通图），节点编号 `0` 到 `n - 1`，以节点 `0` 为根。树用边列表 `edges[i] = [ai, bi]` 表示一条无向边，每个节点 `i` 有一个小写字母标签 `labels[i]`。
