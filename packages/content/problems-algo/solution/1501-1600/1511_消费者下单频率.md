...
type: problem
title: 消费者下单频率
tags: ["GROUP BY", "HAVING 聚合过滤", SQL, "多表 JOIN", 日期筛选]
knowledge_points: [group-by, having-聚合过滤, multi-join, sql, 日期筛选]
updated: 2026-09-03
source: leetcode
number: 1511
difficulty: easy
...
- **标题 / 题号**：消费者下单频率（#1511，easy）
- **链接**：https://leetcode.cn/problems/customer-order-frequency/
- **难度**：简单
- **标签**：SQL、多表 JOIN、GROUP BY、HAVING 聚合过滤、日期筛选

**题意**：给定 `Customers`、`Product`、`Orders` 三张表，编写 SQL 查询，返回**同时满足以下两个条件**的客户的 `customer_id` 和 `name`：
