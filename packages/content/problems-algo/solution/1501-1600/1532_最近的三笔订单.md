...
type: problem
title: 最近的三笔订单
tags: [JOIN, ROW_NUMBER, SQL, "Top-N per group", 数据库, 相关子查询, 窗口函数, 自连接]
knowledge_points: [correlated-subquery, database, join, row-number, self-join, sql, top-n-per-group, window-function]
updated: 2026-09-03
source: leetcode
number: 1532
difficulty: medium
...
**题意**：给定 `Customers` 与 `Orders` 两张表，编写 SQL 查询，找到**每个用户最近的 3 笔订单**。如果用户的订单少于 3 笔，则返回他的全部订单。
