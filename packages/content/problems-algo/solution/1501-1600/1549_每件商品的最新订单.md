...
type: problem
title: 每件商品的最新订单
tags: ["GROUP BY + MAX", JOIN, RANK, SQL, "Top-1 per group", 并列保留, 数据库, 相关子查询, 窗口函数]
knowledge_points: [correlated-subquery, database, group-by-max, group-top1, join, rank, sql, top-1-per-group, window-function, 并列保留]
updated: 2026-09-03
source: leetcode
number: 1549
difficulty: medium
...
# LeetCode 每件商品的最新订单 题解

## 1. 题目概述

- **标题 / 题号**：每件商品的最新订单（#1549，medium）
...
**题意**：给定 `Customers`、`Orders`、`Products` 三张表，编写 SQL 查询，找到**每件商品的最新订单**。若同一商品在最新日期有多笔订单，则**全部返回**；无订单的商品不出现在结果中。
