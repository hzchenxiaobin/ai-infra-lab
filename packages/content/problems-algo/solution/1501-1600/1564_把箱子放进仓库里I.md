...
title: "把箱子放进仓库里 I"
tags: [双指针, 排序, 数组, 贪心]
knowledge_points: [array, greedy, sorting, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1564
difficulty: medium
...
# LeetCode 把箱子放进仓库里 I 题解

## 1. 题目概述

- **标题 / 题号**：把箱子放进仓库里 I（#1564，medium）
- **链接**：https://leetcode.cn/problems/put-boxes-into-the-warehouse-i/
- **难度**：中等
- **标签**：贪心、排序、数组、双指针

**题意**：给定两个正整数数组 `boxes` 和 `warehouse`，分别表示若干个**单位宽度箱子**的高度和仓库中 `n` 个**房间**的高度。仓库房间从左到右编号 `0` 到 `n−1`，`warehouse[i]` 为第 `i` 个房间的高度。
