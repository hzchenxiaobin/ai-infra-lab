...
type: problem
title: 王位继承顺序
tags: [哈希表, 树, 深度优先搜索, 设计]
knowledge_points: [design, dfs, hash-table, tree]
updated: 2026-09-03
source: leetcode
number: 1600
difficulty: medium
...
- **标题 / 题号**：王位继承顺序（#1600，medium）
- **链接**：https://leetcode.cn/problems/throne-inheritance/
- **难度**：中等
- **标签**：树、深度优先搜索、设计、哈希表

**题意**：一个王国里住着国王、他的孩子们、他的孙子们等等。每一个时间点，这个家庭里有人出生也有人死亡。王国的继承顺序第一继承人总是国王自己，题目用递归函数 `Successor(x, curOrder)` 描述了「给定一个人 `x` 和当前继承顺序，返回 `x` 的下一继承人」的逻辑：
