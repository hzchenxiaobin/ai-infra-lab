...
type: problem
title: 产品名称格式修复
tags: [CONCAT, COUNT, "GROUP BY", SQL, TRIM, UPPER/LOWER, 字符串清洗]
knowledge_points: [concat, count, group-by, sql, trim, upper-lower, 字符串清洗]
updated: 2026-09-03
source: leetcode
number: 1543
difficulty: easy
...
# LeetCode 产品名称格式修复 题解

## 1. 题目概述

- **标题 / 题号**：产品名称格式修复（#1543，easy）
- **链接**：https://leetcode.cn/problems/fix-product-name-format/
- **难度**：简单
- **标签**：SQL、字符串清洗、`TRIM`、`UPPER`/`LOWER`、`CONCAT`、`GROUP BY`、`COUNT`、LeetCode 锁题

**题意**：给定 `Sales` 表，其 `product_name` 列存在两类「脏数据」：① 首尾可能有多余空格；② 大小写混乱（应为「首字母大写、其余小写」）。编写 SQL 查询，**清洗产品名后**按 `(清洗名, sale_date)` 分组统计销售次数，并按 `product_name`、`sale_date` 升序输出。
