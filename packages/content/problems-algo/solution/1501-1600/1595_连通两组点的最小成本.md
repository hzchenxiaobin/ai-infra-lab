...
title: 连通两组点的最小成本
tags: [位运算, 动态规划, 状态压缩]
knowledge_points: [bit-manipulation, bitmask-dp, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 1595
difficulty: hard
...
# LeetCode 连通两组点的最小成本 题解

## 1. 题目概述

- **标题 / 题号**：连通两组点的最小成本（#1595，hard）
- **链接**：https://leetcode.cn/problems/minimum-cost-to-connect-two-groups-of-points/
- **难度**：困难
- **标签**：动态规划、状态压缩、位运算

**题意**：给定两组点：第一组 `size1` 个点，第二组 `size2` 个点，且 `size1 >= size2`。用一个 `size1 × size2` 的矩阵 `cost` 表示连接代价，`cost[i][j]` 是第一组点 `i` 与第二组点 `j` 的连接成本。要求**两组中的每个点都至少与另一组中的一个点相连**，求满足条件的**最小总成本**。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/09/20/ex1.jpg)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/09/20/ex2.jpg)
...
| 526 | [优美的排列](https://leetcode.cn/problems/beautiful-arrangement/)（[题解](../0501-0600/526_优美的排列.md)) | `mask` 表示已放数字集合，`popcount` 定位置——与本题"逐点推进 + mask 追踪覆盖"的骨架一致 |
