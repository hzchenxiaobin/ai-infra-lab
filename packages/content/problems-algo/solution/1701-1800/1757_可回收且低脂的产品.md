...
type: problem
title: 可回收且低脂的产品
tags: [AND, SQL, WHERE, 投影, 数据库, 谓词过滤]
knowledge_points: [and, database, sql, where, where-filter, 投影, 谓词过滤]
updated: 2026-09-03
source: leetcode
number: 1757
difficulty: easy
...
# LeetCode 可回收且低脂的产品 题解

## 1. 题目概述

- **标题 / 题号**：可回收且低脂的产品（#1757，easy）
- **链接**：https://leetcode.cn/problems/recyclable-and-low-fat-products/
- **难度**：简单
- **标签**：数据库、SQL、`WHERE`、`AND`、谓词过滤、投影

**题意**：给定 `Products` 表，每行记录一个产品的 `product_id`、是否低脂（`low_fats`，枚举 `'Y'`/`'N'`）、是否可回收（`recyclable`，枚举 `'Y'`/`'N'`）。要求**找出既是低脂又是可回收的产品编号**，只返回 `product_id` 列，结果无顺序要求。
