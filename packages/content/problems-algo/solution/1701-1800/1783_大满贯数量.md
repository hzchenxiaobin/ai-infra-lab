...
type: problem
title: 大满贯数量
tags: [COUNT, "GROUP BY", JOIN, SQL, "SUM 布尔累加", "UNION ALL", "UNPIVOT（列转行）", 数据库, 条件聚合]
knowledge_points: [conditional-aggregation, count, database, group-by, join, sql, sum-布尔累加, union-all, unpivot-列转行]
updated: 2026-09-03
source: leetcode
number: 1783
difficulty: medium
...
- **标题 / 题号**：大满贯数量（#1783，medium）
- **链接**：https://leetcode.cn/problems/grand-slam-titles/
- **难度**：中等
- **标签**：数据库、SQL、`UNION ALL`、UNPIVOT（列转行）、`JOIN`、`GROUP BY`、`COUNT`、条件聚合、`SUM` 布尔累加

**题意**：给定 `Players`（球员）与 `Championships`（大满贯赛事）两张表，编写 SQL 查询，找出**每个球员赢得大满贯比赛的总次数**。结果**不包含没有赢得任何比赛的球员**，顺序任意。
