...
id: "lc:1724"
type: problem
title: "检查边长度限制的路径是否存在 II"
tags: ["Kruskal 重构树", 二分提升, 在线查询, 并查集, "最近公共祖先 LCA"]
knowledge_points: [binary-lifting, kruskal-reconstruction-tree, lca, union-find, 在线查询]
updated: 2026-09-03
source: leetcode
number: 1724
difficulty: hard
...
# LeetCode 检查边长度限制的路径是否存在 II 题解

## 1. 题目概述

- **标题 / 题号**：检查边长度限制的路径是否存在 II（#1724，hard）
- **链接**：https://leetcode.cn/problems/checking-existence-of-edge-length-limited-paths-ii/
- **难度**：困难
- **标签**：并查集、Kruskal 重构树、最近公共祖先 LCA、二分提升、在线查询

**题意**：这是 [1697. 检查边长度限制的路径是否存在](../1601-1700/1697_检查边长度限制的路径是否存在.md) 的**在线版本**：查询不再一次性给出、不能排序，而是通过方法调用逐个到达，必须即时回答。
