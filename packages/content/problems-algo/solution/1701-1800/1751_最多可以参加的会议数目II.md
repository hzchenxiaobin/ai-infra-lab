...
tags: [二分查找, 动态规划, 排序]
knowledge_points: [binary-search, dynamic-programming, sorting]
updated: 2026-09-03
source: leetcode
number: 1751
difficulty: hard
...
# LeetCode 最多可以参加的会议数目 II 题解

## 1. 题目概述

- **标题 / 题号**：最多可以参加的会议数目 II（#1751，hard）
- **链接**：https://leetcode.cn/problems/maximum-number-of-events-that-can-be-attended-ii/
- **难度**：困难
- **标签**：动态规划、二分查找、排序

**题意**：给定 `events` 数组，`events[i] = [startDayi, endDayi, valuei]`，表示第 `i` 个会议从 `startDayi` 天开始、`endDayi` 天结束，参加可得价值 `valuei`。再给整数 `k` 表示**最多**能参加的会议数。
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/02/06/screenshot-2021-01-11-at-60048-pm.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/02/06/screenshot-2021-01-11-at-60150-pm.png)
...
![示例3](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/02/06/screenshot-2021-01-11-at-60703-pm.png)
...
> 💡 **关键直觉**：这是经典的**带权重区间调度**（Weighted Interval Scheduling）。把它和「[435. 无重叠区间](../0401-0500/435_无重叠区间.md)」对比：435 是「**最少**删几个使剩余不重叠」（贪心按右端点），而本题每条区间带**价值**、还要**限量 k 个**、目标是**最大价值和**——贪心失效，必须 DP。但「**按 endDay 排序**」这一骨架仍然继承，它让"前驱兼容区间"具备单调性，可二分定位。
...
| 435 | [无重叠区间](https://leetcode.cn/problems/non-overlapping-intervals/)（[题解](../0401-0500/435_无重叠区间.md)） | 无权重区间调度贪心母题，对照"为何带权重后贪心失效、需 DP" |
