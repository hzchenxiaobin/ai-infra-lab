...
tags: [Dijkstra, 动态规划, 图, 拓扑排序, 最短路径]
knowledge_points: [dijkstra, dynamic-programming, graph, shortest-path, topological-sort]
updated: 2026-09-03
source: leetcode
number: 1786
difficulty: medium
...
# LeetCode 从第一个节点出发到最后一个节点的受限路径数 题解

## 1. 题目概述

- **标题 / 题号**：从第一个节点出发到最后一个节点的受限路径数（#1786，medium）
- **链接**：https://leetcode.cn/problems/number-of-restricted-paths-from-first-to-last-node/
- **难度**：中等
- **标签**：图、最短路径、Dijkstra、动态规划、拓扑排序

**题意**：给定一个 $n$ 个节点的**加权无向连通图**，节点编号 $1$ 到 $n$。边列表 `edges` 中每条 `[u, v, weight]` 表示节点 $u$、$v$ 间一条权重为 `weight` 的边。
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/03/07/restricted_paths_ex1.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/03/07/restricted_paths_ex22.png)
