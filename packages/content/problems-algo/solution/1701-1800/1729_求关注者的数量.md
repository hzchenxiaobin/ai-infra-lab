...
type: problem
title: 求关注者的数量
tags: [COUNT, "GROUP BY", "ORDER BY", SQL, 数据库, 聚合]
knowledge_points: [aggregation, count, database, group-by, order-by, sql]
updated: 2026-09-03
source: leetcode
number: 1729
difficulty: easy
...
- **标题 / 题号**：求关注者的数量（#1729，easy）
- **链接**：https://leetcode.cn/problems/find-followers-count/
- **难度**：简单
- **标签**：数据库、SQL、`GROUP BY`、`COUNT`、`ORDER BY`、聚合

**题意**：给定 `Followers` 表，编写 SQL 查询，对于**每一个用户**，返回该用户的**关注者数量**。按 `user_id` 的顺序返回结果表。
