...
type: problem
title: 得到子序列的最少操作次数
tags: [二分查找, 哈希表, 数组, 最长公共子序列, 最长递增子序列]
knowledge_points: [array, binary-search, hash-table, lcs, lis]
updated: 2026-09-03
source: leetcode
number: 1713
difficulty: hard
...
# LeetCode 得到子序列的最少操作次数 题解

## 1. 题目概述

- **标题 / 题号**：得到子序列的最少操作次数（#1713，hard）
- **链接**：https://leetcode.cn/problems/minimum-operations-to-make-a-subsequence/
- **难度**：困难
- **标签**：数组、哈希表、二分查找、最长递增子序列、最长公共子序列

**题意**：给定一个**元素互不相同**的数组 `target`，以及另一个**可能含重复元素**的数组 `arr`。每次操作可以在 `arr` 的任意位置插入一个整数。求使 `target` 成为 `arr` 的子序列所需的**最少操作次数**。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 300 | [最长递增子序列](https://leetcode.cn/problems/longest-increasing-subsequence/)（[题解](../0201-0300/300_最长递增子序列.md)） | 耐心排序 + 二分求 LIS 的母题——掌握 `tails` 数组的单调性与 `lower_bound` 用法后再看本题只需叠加「LCS → LIS 归约」 |
