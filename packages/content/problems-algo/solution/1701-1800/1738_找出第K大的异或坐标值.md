...
title: "找出第 K 大的异或坐标值"
tags: [位运算, 前缀异或, 排序, 数组]
knowledge_points: [array, bit-manipulation, prefix-xor, sorting]
updated: 2026-09-03
source: leetcode
number: 1738
difficulty: medium
...
# LeetCode 找出第 K 大的异或坐标值 题解

## 1. 题目概述

- **标题 / 题号**：找出第 K 大的异或坐标值（#1738，medium）
- **链接**：https://leetcode.cn/problems/find-kth-largest-xor-coordinate-value/
- **难度**：中等
- **标签**：位运算、数组、前缀异或、排序

**题意**：给定一个 `m × n` 的**非负整数**矩阵 `matrix` 和一个整数 `k`。坐标 `(a, b)` 的**目标值**定义为：对所有满足 `0 <= i <= a` 且 `0 <= j <= b` 的元素 `matrix[i][j]` 执行异或（$\oplus$）运算的结果。
