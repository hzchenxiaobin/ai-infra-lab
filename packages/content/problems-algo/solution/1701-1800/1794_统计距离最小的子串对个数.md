...
title: 统计距离最小的子串对个数
tags: [哈希表, 字符串, 贪心]
knowledge_points: [greedy, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 1794
difficulty: medium
...
# LeetCode 统计距离最小的子串对个数 题解

## 1. 题目概述

- **标题 / 题号**：统计距离最小的子串对个数（#1794，medium）
- **链接**：https://leetcode.cn/problems/count-pairs-of-equal-substrings-with-minimum-difference/
- **难度**：中等
- **标签**：贪心、哈希表、字符串

**题意**：给定两个 **0-indexed** 字符串 `firstString` 和 `secondString`（仅含小写字母）。统计满足以下条件的四元组 $(i, j, a, b)$ 的个数：
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 205 | [同构字符串](https://leetcode.cn/problems/isomorphic-strings/)（[题解](../0201-0300/205_同构字符串.md)） | 两字符串间建立字符位置映射，同样依赖哈希表记录字符出现信息，对照「位置映射」与「位置极值」的不同用法 |
