...
tags: [哈希表, 图, 并查集, 数组]
knowledge_points: [array, graph, hash-table, union-find]
updated: 2026-09-03
source: leetcode
number: 1722
difficulty: medium
...
# LeetCode 执行交换操作后的最小汉明距离 题解

## 1. 题目概述

- **标题 / 题号**：执行交换操作后的最小汉明距离（#1722，medium）
- **链接**：https://leetcode.cn/problems/minimize-hamming-distance-after-swap-operations/
- **难度**：中等
- **标签**：并查集、图、哈希表、数组

**题意**：给你两个长度均为 $n$ 的整数数组 `source` 和 `target`，以及一个数组 `allowedSwaps`，其中 `allowedSwaps[i] = [ai, bi]` 表示你可以**交换** `source` 中下标 `ai` 和 `bi` 处的两个元素。每对下标的交换可以执行**任意次**、**任意顺序**。
