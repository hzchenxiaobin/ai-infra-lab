...
# LeetCode 交换链表中的节点 题解

## 1. 题目概述

- **标题 / 题号**：交换链表中的节点（#1721，medium）
- **链接**：https://leetcode.cn/problems/swapping-nodes-in-a-linked-list/
- **难度**：中等
- **标签**：链表、双指针

**题意**：给定一个链表的**头节点** `head` 和一个整数 `k`，交换链表中**倒数第** `k` **个节点**与**正数第** `k` **个节点**的**值**，返回链表的头节点。链表下标从 1 开始，且保证 `1 <= k <= 节点总数`。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2021/01/10/linked1.jpg)
