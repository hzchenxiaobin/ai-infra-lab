...
title: 最长的美好子字符串
tags: [分治, 字符串, 递归]
knowledge_points: [divide-and-conquer, recursion, string]
updated: 2026-09-03
source: leetcode
number: 1763
difficulty: easy
...
# LeetCode 最长的美好子字符串 题解

## 1. 题目概述

- **标题 / 题号**：最长的美好子字符串（#1763，easy）
- **链接**：https://leetcode.cn/problems/longest-nice-substring/
- **难度**：简单
- **标签**：字符串、分治、递归

**题意**：当一个字符串 `s` 包含的**每一种字母的大写和小写形式同时出现**在 `s` 中时，就称 `s` 是**美好**字符串。例如 `"abABB"` 是美好的（`'A'`/`'a'` 同时出现，`'B'`/`'b'` 同时出现），而 `"abA"` 不是（`'b'` 出现但 `'B'` 未出现）。
