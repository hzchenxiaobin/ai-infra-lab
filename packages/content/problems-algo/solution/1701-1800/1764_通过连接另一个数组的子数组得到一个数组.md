...
tags: [字符串匹配, 数组, 贪心]
knowledge_points: [array, greedy, string-matching]
updated: 2026-09-03
source: leetcode
number: 1764
difficulty: medium
...
# LeetCode 通过连接另一个数组的子数组得到一个数组 题解

## 1. 题目概述

- **标题 / 题号**：通过连接另一个数组的子数组得到一个数组（#1764，medium）
- **链接**：https://leetcode.cn/problems/form-array-by-concatenating-subarrays-of-another-array/
- **难度**：中等
- **标签**：数组、贪心、字符串匹配

**题意**：给定一个二维整数数组 `groups`（共 $n$ 个子数组）和一个整数数组 `nums`。问能否从 `nums` 中选出 $n$ 个**不相交**的子数组，使得第 $i$ 个子数组与 `groups[i]` 完全相同，且它们在 `nums` 中出现的先后顺序与 `groups` 中的顺序一致。
