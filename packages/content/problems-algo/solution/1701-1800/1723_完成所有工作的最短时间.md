...
type: problem
title: 完成所有工作的最短时间
tags: [二分查找, 剪枝, 回溯, 数组, "状态压缩 DP"]
knowledge_points: [array, backtracking, binary-search, bitmask-dp, pruning]
updated: 2026-09-03
source: leetcode
number: 1723
difficulty: hard
...
# LeetCode 完成所有工作的最短时间 题解

## 1. 题目概述

- **标题 / 题号**：完成所有工作的最短时间（#1723，hard）
- **链接**：https://leetcode.cn/problems/find-minimum-time-to-finish-all-jobs/
- **难度**：困难
- **标签**：数组、二分查找、回溯、剪枝、状态压缩 DP

**题意**：给定一个整数数组 `jobs`，其中 `jobs[i]` 是完成第 $i$ 项工作要花费的时间。你需要将这些工作分配给 $k$ 位工人——**每项工作恰好分给一位工人**，工人的**工作时间**是其所有分配工作的总时长。目标是**最小化最大工作时间**（即让最忙的工人尽可能轻松）。
