...
# LeetCode 任意子数组和的绝对值的最大值 题解

## 1. 题目概述

- **标题 / 题号**：任意子数组和的绝对值的最大值（#1749，medium）
- **链接**：https://leetcode.cn/problems/maximum-absolute-sum-of-any-subarray/
- **难度**：中等
- **标签**：数组、动态规划、Kadane 算法

**题意**：给你一个整数数组 `nums`。一个子数组 `[nums_l, nums_{l+1}, ..., nums_r]` 的**和的绝对值**为 `abs(nums_l + ... + nums_r)`。请找出 `nums` 中**和的绝对值**最大的任意子数组（**可能为空**），并返回该最大值。
