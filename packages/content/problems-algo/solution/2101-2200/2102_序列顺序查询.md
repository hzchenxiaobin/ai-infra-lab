...
type: problem
title: 序列顺序查询
tags: ["堆（优先队列）", 数据流, 有序集合, 设计]
knowledge_points: [data-stream, design, heap, ordered-set]
updated: 2026-09-03
source: leetcode
number: 2102
difficulty: hard
...
- **标题 / 题号**：序列顺序查询（#2102，hard）
- **链接**：https://leetcode.cn/problems/sequentially-ordinal-rank-tracker/
- **难度**：困难
- **标签**：设计、堆（优先队列）、有序集合、数据流

**题意**：一个观光景点由**名字** `name` 和**评分** `score` 组成，`name` 在所有景点中**唯一**，`score` 为整数。景点按「好到坏」排序，规则是：
