...
type: problem
title: "每辆车的乘客人数 I"
tags: [COUNT, "GROUP BY", LAG, "LEFT JOIN", SQL, 差分, 数据库, 窗口函数]
knowledge_points: [count, database, difference-array, group-by, lag, left-join, sql, window-function]
updated: 2026-09-03
source: leetcode
number: 2142
difficulty: medium
...
# LeetCode 每辆车的乘客人数 I 题解

## 1. 题目概述

- **标题 / 题号**：每辆车的乘客人数 I（#2142，medium）
- **链接**：https://leetcode.cn/problems/the-number-of-passengers-in-each-bus-i/
- **难度**：中等
- **标签**：SQL、数据库、窗口函数、`LAG`、`LEFT JOIN`、`GROUP BY`、`COUNT`、差分

**题意**：公交车和乘客先后到达 LeetCode 站点。若一辆公交车在时间 $t_{\text{bus}}$ 到站，乘客在时间 $t_{\text{passenger}}$ 到站，且 $t_{\text{passenger}} \leq t_{\text{bus}}$，**该乘客之前没有赶上任何公交车**，则该乘客将搭乘该公交车。编写 SQL 查询每辆公交车搭载的乘客数量，按 `bus_id` **升序**排序返回。
