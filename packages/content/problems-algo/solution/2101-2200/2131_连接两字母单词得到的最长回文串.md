...
tags: [哈希表, 字符串, 数组, 计数, 贪心]
knowledge_points: [array, counting, greedy, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 2131
difficulty: medium
...
# LeetCode 连接两字母单词得到的最长回文串 题解

## 1. 题目概述

- **标题 / 题号**：连接两字母单词得到的最长回文串（#2131，medium）
- **链接**：https://leetcode.cn/problems/longest-palindrome-by-concatenating-two-letter-words/
- **难度**：中等
- **标签**：贪心、数组、哈希表、字符串、计数

**题意**：给定一个字符串数组 `words`，其中每个元素都是一个**长度为 2** 的小写英文字母单词。请你从 `words` 中选择一些元素，按**任意顺序**连接，得到一个**尽可能长的回文串**。每个元素**至多**只能使用一次。返回你能得到的最长回文串的**长度**；若无法得到任何回文串，返回 `0`。
