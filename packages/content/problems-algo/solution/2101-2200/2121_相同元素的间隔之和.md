...
title: 相同元素的间隔之和
tags: [前缀和, 哈希表, 数组]
knowledge_points: [array, hash-table, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 2121
difficulty: medium
...
# LeetCode 相同元素的间隔之和 题解

## 1. 题目概述

- **标题 / 题号**：相同元素的间隔之和（#2121，medium）
- **链接**：https://leetcode.cn/problems/intervals-between-identical-elements/
- **难度**：中等
- **标签**：数组、哈希表、前缀和

**题意**：给定一个下标从 0 开始、长度为 `n` 的整数数组 `arr`。两个元素的**间隔**定义为其下标的绝对差 `|i - j|`。返回长度为 `n` 的数组 `intervals`，其中 `intervals[i]` 是 `arr[i]` 与 `arr` 中**每个相同值元素**的间隔之和（不含自身，但自身贡献为 0，等价于含自身）。
