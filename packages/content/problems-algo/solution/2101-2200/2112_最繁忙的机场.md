...
type: problem
title: 最繁忙的机场
tags: ["GROUP BY", SQL, SUM, "UNION ALL", 子查询, 数据库, "窗口函数 RANK()", 聚合, "逆透视（UNPIVOT）"]
knowledge_points: [aggregation, database, group-by, sql, subquery, sum, union-all, 窗口函数-rank, 逆透视-unpivot]
updated: 2026-09-03
source: leetcode
number: 2112
difficulty: medium
...
**题意**：给定 `Flights`（航班）表，每行记录一条航线的出发机场、到达机场与航班数量。编写 SQL 查询，找出**客流量最大**的机场 ID。客流量 = 从该机场**起飞**的航班数 + **抵达**该机场的航班数。若有多个机场并列最大，全部返回，顺序不限。
