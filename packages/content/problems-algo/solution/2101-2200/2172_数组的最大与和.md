...
- **标题 / 题号**：数组的最大与和（#2172，hard）
- **链接**：https://leetcode.cn/problems/maximum-and-sum-of-array/
- **难度**：困难
- **标签**：位运算、动态规划、状态压缩

**题意**：给定长度为 $n$ 的整数数组 `nums` 和整数 `numSlots`（满足 $2 \cdot \text{numSlots} \ge n$）。共有 `numSlots` 个篮子，编号 $1$ 到 $\text{numSlots}$。需要把所有 $n$ 个数分到这些篮子中，**每个篮子至多放 2 个数**。一种分配方案的**与和**定义为：每个数与它所在篮子编号的**按位与**结果之和。
