...
type: problem
title: 建立方程
tags: [ABS, "CASE 表达式", CONCAT, GROUP_CONCAT, IF/CASE, SQL, 字符串构建, 排序拼接, 数据库]
knowledge_points: [abs, case-when, case-表达式, concat, database, group-concat, if-case, sql, 字符串构建, 排序拼接]
updated: 2026-09-03
source: leetcode
number: 2118
difficulty: hard
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 1484 | [按日期分组销售产品](https://leetcode.cn/problems/group-sold-products-by-the-date/)（[题解](../1401-1500/1484_按日期分组销售产品.md)） | `GROUP_CONCAT(DISTINCT ... ORDER BY ... SEPARATOR)` 排序拼接招牌题，与 2118 共享"GROUP_CONCAT 排序+拼接"骨架，但多了去重维度 |
