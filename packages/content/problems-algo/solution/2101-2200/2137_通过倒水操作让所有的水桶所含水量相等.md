...
id: "lc:2137"
type: problem
title: 通过倒水操作让所有的水桶所含水量相等
tags: [二分查找, 二分答案, 数组]
knowledge_points: [array, binary-search, binary-search-answer]
updated: 2026-09-03
source: leetcode
number: 2137
difficulty: medium
...
# LeetCode 通过倒水操作让所有的水桶所含水量相等 题解

## 1. 题目概述

- **标题 / 题号**：通过倒水操作让所有的水桶所含水量相等（#2137，medium）
- **链接**：https://leetcode.cn/problems/pour-water-between-buckets-to-make-water-levels-equal/
- **难度**：中等
- **标签**：数组、二分查找、二分答案

**题意**：你有 `n` 个水桶，第 `i` 个水桶中有 `buckets[i]` 升水。你想让所有水桶中的水量相同。你可以从一个水桶向**任意另一个**水桶倒任意数量的水（可以是小数）。但每倒 `k` 升水，**百分之** `loss` 的水会洒掉（即只有 $k \cdot \frac{100-\text{loss}}{100}$ 升能到达目标桶）。
