...
title: 向字符串添加空格
tags: [双指针, 字符串, 数组, 模拟]
knowledge_points: [array, simulation, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 2109
difficulty: medium
...
# LeetCode 向字符串添加空格 题解

## 1. 题目概述

- **标题 / 题号**：向字符串添加空格（#2109，medium）
- **链接**：https://leetcode.cn/problems/adding-spaces-to-a-string/
- **难度**：中等
- **标签**：数组、双指针、字符串、模拟

**题意**：给定下标从 0 开始的字符串 `s` 和下标从 0 开始的整数数组 `spaces`。`spaces` 描述原字符串中需要添加空格的下标——每个空格都应插入到给定索引处的字符**之前**。返回添加空格后的字符串。
...
| 977 | [有序数组的平方](https://leetcode.cn/problems/squares-of-a-sorted-array/)（[题解](../0901-1000/977_有序数组的平方.md)） | 双指针归并两个有序段到结果数组，预分配 + 双写指针同思路 |
| 1768 | [交替合并字符串](https://leetcode.cn/problems/merge-strings-alternately/) | 两个字符串按位交替归并，最简的双指针归并模板，本题的"退化特例" |
| 986 | [区间列表的交集](https://leetcode.cn/problems/interval-list-intersections/)（[题解](../0901-1000/986_区间列表的交集.md)） | 两个有序区间列表取交集，双指针各单调推进，同属有序序列归并 |
