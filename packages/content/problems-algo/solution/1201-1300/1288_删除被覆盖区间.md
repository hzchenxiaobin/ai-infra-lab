...
title: 删除被覆盖区间
tags: [区间, 排序, 数组, 贪心]
knowledge_points: [array, greedy, interval, sorting]
updated: 2026-09-03
source: leetcode
number: 1288
difficulty: medium
...
- **标题 / 题号**：删除被覆盖区间（#1288，medium）
- **链接**：https://leetcode.cn/problems/remove-covered-intervals/
- **难度**：中等
- **标签**：数组、排序、贪心、区间

**题意**：给你一个区间列表 `intervals`，其中 `intervals[i] = [a_i, b_i]`。区间 $[a, b]$ 被 $[c, d]$ **覆盖**，当且仅当 $c \le a$ 且 $b \le d$（即 $[c,d]$ 把 $[a,b]$ 完全「包住」）。返回**不被任何其他区间覆盖**的区间数量。
