...
type: problem
title: 步进数
tags: [广度优先搜索, 数位构造, 数学, 深度优先搜索]
knowledge_points: [bfs, dfs, math, 数位构造]
updated: 2026-09-03
source: leetcode
number: 1215
difficulty: medium
...
- **标题 / 题号**：步进数（#1215，medium）
- **链接**：https://leetcode.cn/problems/stepping-numbers/
- **难度**：中等
- **标签**：广度优先搜索、深度优先搜索、数学、数位构造

**题意**：如果一个数的**相邻两位数字之差的绝对值恰好为 1**，则称该数为**步进数**（Stepping Number）。例如 `321` 是步进数（$|3-2|=1$，$|2-1|=1$），而 `425` 不是（$|4-2|=2$）。**一位数**（$0$ 到 $9$）没有相邻位，按定义均为步进数。
