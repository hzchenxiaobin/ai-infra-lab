...
type: problem
title: 子串的最大出现次数
tags: [位运算, 哈希表, 字符串, 滑动窗口]
knowledge_points: [bit-manipulation, hash-table, sliding-window, string]
updated: 2026-09-03
source: leetcode
number: 1297
difficulty: medium
...
# LeetCode 子串的最大出现次数 题解

## 1. 题目概述

- **标题 / 题号**：子串的最大出现次数（#1297，medium）
- **链接**：https://leetcode.cn/problems/maximum-number-of-occurrences-of-a-substring/
- **难度**：中等
- **标签**：字符串、滑动窗口、哈希表、位运算

**题意**：给你一个字符串 `s`，返回满足以下条件且**出现次数最大**的任意子串的出现次数：
...
4. **哈希表存子串字符串会不会有内存问题？**
   ...
   - 最坏 `O(n)` 个键、每键长 `minSize ≤ 26`，总内存 `O(26n)` 字节量级，`n = 10^5` 时约几 MB，完全可接受。若极致优化可对子串做滚动哈希（参考 [187. 重复的 DNA 序列](../0101-0200/187_重复的DNA序列.md) 的 2 位编码思路），但本题无此必要。
