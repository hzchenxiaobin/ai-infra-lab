...
title: 奇数值单元格的数目
tags: [数学, 数组, 模拟]
knowledge_points: [array, math, simulation]
updated: 2026-09-03
source: leetcode
number: 1252
difficulty: easy
...
# LeetCode 奇数值单元格的数目 题解

## 1. 题目概述

- **标题 / 题号**：奇数值单元格的数目（#1252，easy）
- **链接**：https://leetcode.cn/problems/cells-with-odd-values-in-a-matrix/
- **难度**：简单
- **标签**：数组、数学、模拟

**题意**：给定一个 $m \times n$ 的矩阵，初始时所有单元格的值为 $0$。再给定一个下标数组 `indices`，其中 `indices[i] = [ri, ci]`。对每个 `indices[i]`，执行以下操作：
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2019/11/06/e1.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2019/11/06/e2.png)
