...
type: problem
title: 近义词句子
tags: [哈希表, 回溯, 字符串, 并查集, 排序]
knowledge_points: [backtracking, hash-table, sorting, string, union-find]
updated: 2026-09-03
source: leetcode
number: 1258
difficulty: medium
...
- **标题 / 题号**：近义词句子（#1258，medium）
- **链接**：https://leetcode.cn/problems/synonymous-sentences/
- **难度**：中等
- **标签**：并查集、回溯、哈希表、字符串、排序

**题意**：给定一个近义词表 `synonyms` 和一个句子 `text`，`synonyms` 表中是一些近义词对。你可以将句子 `text` 中每个单词用它的近义词来替换。近义词关系**可传递**：若 `a` 与 `b` 是近义词、`b` 与 `c` 是近义词，则 `a` 与 `c` 也是近义词。
...
| 140 | [单词拆分 II](https://leetcode.cn/problems/word-break-ii/)（[题解](../0101-0200/140_单词拆分II.md)） | DFS 回溯枚举所有方案，与本题的 DFS 枚举所有替换句结构一致，加记忆化优化重复搜索 |
