...
type: problem
title: 红绿灯路口
tags: [互斥锁, 并发, 条件变量, 线程同步]
knowledge_points: [concurrency, condition-variable, mutex, thread-synchronization]
updated: 2026-09-03
source: leetcode
number: 1279
difficulty: easy
...
- **标题 / 题号**：红绿灯路口（#1279，easy）
- **链接**：https://leetcode.cn/problems/traffic-light-controlled-intersection/
- **难度**：简单
- **标签**：并发、互斥锁、线程同步、条件变量

**题意**：两条路在一个十字路口相交。**A 路**纵向，车辆可沿 **1 号方向**（北→南）或 **2 号方向**（南→北）行驶；**B 路**横向，车辆可沿 **3 号方向**（西→东）或 **4 号方向**（东→西）行驶。每条路在路口前各有一组红绿灯。
