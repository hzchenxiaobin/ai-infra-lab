...
title: 字母组合迭代器
tags: [回溯, 字符串, 组合枚举, 设计]
knowledge_points: [backtracking, combination-enumeration, design, string]
updated: 2026-09-03
source: leetcode
number: 1286
difficulty: medium
...
- **标题 / 题号**：字母组合迭代器（#1286，medium）
- **链接**：https://leetcode.cn/problems/iterator-for-combination/
- **难度**：中等
- **标签**：设计、回溯、组合枚举、字符串

**题意**：设计一个 `CombinationIterator` 类，用于按**字典序**逐一枚举字符串 `characters` 中长度为 `combinationLength` 的所有**组合**（不分排列顺序，即 `{a,b}` 与 `{b,a}` 视为同一组合）。
