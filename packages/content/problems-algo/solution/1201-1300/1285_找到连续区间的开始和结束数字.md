...
id: "lc:1285"
type: problem
title: 找到连续区间的开始和结束数字
tags: ["GROUP BY", "Gaps & Islands", SQL, 数据库, 窗口函数]
knowledge_points: [database, gaps-and-islands, group-by, sql, window-function]
updated: 2026-09-03
source: leetcode
number: 1285
difficulty: medium
...
# LeetCode 找到连续区间的开始和结束数字 题解

## 1. 题目概述

- **标题 / 题号**：找到连续区间的开始和结束数字（#1285，medium）
- **链接**：https://leetcode.cn/problems/find-the-start-and-end-number-of-continuous-ranges/
- **难度**：中等
- **标签**：数据库、SQL、窗口函数、Gaps & Islands、GROUP BY

**题意**：给定一张 `Logs` 表，只有一个整数列 `log_id`（主键）。编写 SQL 查询，找出 `log_id` 中所有**连续区间**的起始编号 `start_id` 和结束编号 `end_id`，结果按 `start_id` 升序排列。
