...
tags: [二分查找, 前缀和, 数组, 矩阵]
knowledge_points: [array, binary-search, matrix, prefix-sum, 矩阵]
updated: 2026-09-03
source: leetcode
number: 1292
difficulty: medium
...
# LeetCode 元素和小于等于阈值的正方形的最大边长 题解

## 1. 题目概述

- **标题 / 题号**：元素和小于等于阈值的正方形的最大边长（#1292，medium）
- **链接**：https://leetcode.cn/problems/maximum-side-length-of-a-square-with-sum-less-than-or-equal-to-threshold/
- **难度**：中等
- **标签**：数组、二分查找、矩阵、前缀和

**题意**：给你一个大小为 $m \times n$ 的矩阵 `mat` 和一个整数阈值 `threshold`。请你返回元素总和**小于或等于**阈值的**正方形区域**的**最大边长**；如果没有这样的正方形区域，则返回 `0`。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2019/12/15/e1.png)
