...
type: problem
title: 每个帖子的评论数
tags: ["COUNT DISTINCT", "GROUP BY", "LEFT JOIN", SQL, 去重]
knowledge_points: [count-distinct, dedup, group-by, left-join, sql]
updated: 2026-09-03
source: leetcode
number: 1241
difficulty: easy
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 1211 | [查询结果的质量和占比](https://leetcode.cn/problems/queries-quality-and-percentage/)（[站内题解](./1211_查询结果的质量和占比.md)） | `GROUP BY` + `AVG` 聚合，同为「分组后多指标统计」SQL 基础题，对比理解 `COUNT DISTINCT` 与 `AVG` 两种聚合 |
