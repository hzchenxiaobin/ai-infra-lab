...
title: 串联字符串的最大长度
tags: [位运算, 回溯, 字符串, 数组]
knowledge_points: [array, backtracking, bit-manipulation, string]
updated: 2026-09-03
source: leetcode
number: 1239
difficulty: medium
...
# LeetCode 串联字符串的最大长度 题解

## 1. 题目概述

- **标题 / 题号**：串联字符串的最大长度（#1239，medium）
- **链接**：https://leetcode.cn/problems/maximum-length-of-a-concatenated-string-with-unique-characters/
- **难度**：中等
- **标签**：位运算、数组、字符串、回溯

**题意**：给定一个字符串数组 `arr`，字符串 `s` 是将 `arr` 的含有**不同字母**的**子序列**字符串**连接**所得的字符串。请返回所有可行解 `s` 中最长长度。
...
> ⚠️ **为什么用「start 索引循环」而非「二叉选/不选」？** 本题只求最大长度、不求所有方案，且「选了 `j` 之后只能往后选」天然用 `start` 索引避免重复组合（与 [78. 子集](../0001-0100/78_子集.md) 的 start 循环版同构）。每个节点都更新 `ans`（而不是只在叶子），因为「空拼接」「只选前几个」都是合法方案。
...
> ⚠️ **必须用** `dp[:]` **快照**：在遍历 `dp` 的同时向其追加新项，若不切片会无限扩展。这是「滚动扩张」型 DP 的经典陷阱（与 [78. 子集](../0001-0100/78_子集.md) 迭代扩张法同源）。该方法最坏仍生成 $O(2^n)$ 个组合，但常数比递归大、且无法剪枝，面试中以方法一为主、方法二作为对照。
...
3. **回溯的「选/不选」和「start 索引循环」两种写法有什么区别？**
   ...
   - 「start 索引循环」版：每层从 `i` 往后枚举下一个要选的串，天然避免重复组合（`{un,iq}` 与 `{iq,un}` 不会都出现），与 [78. 子集](../0001-0100/78_子集.md) 的 start 版同构。
   - 本题只求最大长度、组合无序，两者等价；start 版代码更紧凑，是首选。

4. **时间复杂度为什么是 $O(2^n)$ 而不是 $O(n \cdot 2^n)$？**

   - 与 [78. 子集](../0001-0100/78_子集.md) 不同，本题不收集每个子集的完整内容（那需 $O(n)$ 拷贝），只用标量 `curLen` / `curMask` 累积，每个节点 $O(1)$。故总时间为节点数 $O(2^n)$，而非 $O(n \cdot 2^n)$。
...
| 187 | [重复的 DNA 序列](https://leetcode.cn/problems/repeated-dna-sequences/)（[站内题解](../0101-0200/187_重复的DNA序列.md)） | 4 碱基用 2 bit 编码 + 滚动哈希，同属「小字母表位压缩」家族 |
