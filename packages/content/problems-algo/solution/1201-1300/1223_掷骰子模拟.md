...
- **标题 / 题号**：掷骰子模拟（#1223，hard）
- **链接**：https://leetcode.cn/problems/dice-roll-simulation/
- **难度**：困难
- **标签**：动态规划、数组

**题意**：有一个骰子模拟器，每次投掷生成一个 1 到 6 的随机数。使用时有一个约束：**连续**掷出数字 `i` 的次数不能超过 `rollMax[i]`（`i` 从 1 开始编号，即 `rollMax[0]` 对应数字 1）。
...
5. **本题与 [198. 打家劫舍](../0101-0200/198_打家劫舍.md) 的状态设计有何异同？**
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 198 | [打家劫舍](https://leetcode.cn/problems/house-robber/)（[题解](../0101-0200/198_打家劫舍.md)） | 带约束的序列 DP，记录末尾状态决定下一步选择 |
