...
title: 替换子串得到平衡字符串
tags: [双指针, 字符串, 滑动窗口]
knowledge_points: [sliding-window, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1234
difficulty: medium
...
# LeetCode 替换子串得到平衡字符串 题解

## 1. 题目概述

- **标题 / 题号**：替换子串得到平衡字符串（#1234，medium）
- **链接**：https://leetcode.cn/problems/replace-the-substring-for-balanced-string/
- **难度**：中等
- **标签**：字符串、滑动窗口、双指针

**题意**：给定一个只含 `'Q'`、`'W'`、`'E'`、`'R'` 四种字符且长度为 `n`（`n` 是 4 的倍数）的字符串 `s`。若四种字符各出现恰好 `n/4` 次，则称其为「平衡字符串」。
