...
type: problem
title: 学生们参加各科测试的次数
tags: [COUNT, "CROSS JOIN", "GROUP BY", "LEFT JOIN", SQL, 数据库]
knowledge_points: [count, cross-join, database, group-by, left-join, sql]
updated: 2026-09-03
source: leetcode
number: 1280
difficulty: easy
...
# LeetCode 学生们参加各科测试的次数 题解

## 1. 题目概述

- **标题 / 题号**：学生们参加各科测试的次数（#1280，easy）
- **链接**：https://leetcode.cn/problems/students-and-examinations/
- **难度**：简单
- **标签**：数据库、SQL、CROSS JOIN、LEFT JOIN、GROUP BY、COUNT

**题意**：给定三张表：`Students`（学生）、`Subjects`（科目）、`Examinations`（考试记录）。编写 SQL 查询，统计**每个学生参加每个科目考试的次数**。结果须按 `student_id`、`subject_name` 升序排列。
