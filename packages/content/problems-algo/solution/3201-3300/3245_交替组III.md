...
title: "交替组 III"
tags: [数组, 有序集合, 树状数组]
knowledge_points: [array, fenwick-tree, ordered-set]
updated: 2026-09-03
source: leetcode
number: 3245
difficulty: hard
...
- **标题 / 题号**：交替组 III（#3245，hard）
- **链接**：https://leetcode.cn/problems/alternating-groups-iii/
- **难度**：困难
- **标签**：树状数组、有序集合、数组

**题意**：给你一个整数数组 `colors` 和一个二维整数数组 `queries`。`colors` 表示一个由红色和蓝色瓷砖组成的**环**，第 `i` 块瓷砖的颜色为 `colors[i]`：`0` 表示红色，`1` 表示蓝色。环中**连续若干块**瓷砖的颜色如果是**交替**颜色（除第一块和最后一块外，中间每块瓷砖与它左边、右边的颜色都不同），称为一个**交替组**。你需要处理两种查询：
...
![示例1](https://assets.leetcode.com/uploads/2024/06/03/screenshot-from-2024-06-03-20-20-25.png)
...
![示例2](https://assets.leetcode.com/uploads/2024/06/03/screenshot-from-2024-06-03-20-25-02-2.png)
