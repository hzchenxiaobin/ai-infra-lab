...
title: 将双链表转换为数组I
tags: [双向链表, 数组, 遍历, 链表]
knowledge_points: [array, doubly-linked-list, linked-list, traversal, 遍历]
updated: 2026-09-03
source: leetcode
number: 3263
difficulty: easy
...
# LeetCode 将双链表转换为数组I 题解

## 1. 题目概述

- **标题 / 题号**：将双链表转换为数组I（#3263，easy）
...
**题意**：给定一个**双链表**（doubly linked list）的头节点 `head`。双链表的每个节点除 `val` 外还带两个指针：`next` 指向后一个节点，`prev` 指向前一个节点（头节点的 `prev`、尾节点的 `next` 均为空）。
