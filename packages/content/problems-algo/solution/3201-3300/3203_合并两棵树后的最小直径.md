...
type: problem
title: 合并两棵树后的最小直径
tags: [图, 广度优先搜索, 树, 深度优先搜索]
knowledge_points: [bfs, dfs, graph, tree]
updated: 2026-09-03
source: leetcode
number: 3203
difficulty: hard
...
# LeetCode 合并两棵树后的最小直径 题解

## 1. 题目概述

- **标题 / 题号**：合并两棵树后的最小直径（#3203，hard）
- **链接**：https://leetcode.cn/problems/find-minimum-diameter-after-merging-two-trees/
- **难度**：困难
- **标签**：树、深度优先搜索、广度优先搜索、图

**题意**：给定两棵**无向**树，分别有 $n$ 和 $m$ 个节点，节点编号分别为 $0$ 到 $n-1$ 和 $0$ 到 $m-1$。边数组 `edges1`、`edges2` 长度分别为 $n-1$ 和 $m-1$。
...
![示例1](https://assets.leetcode.com/uploads/2024/04/22/example11-transformed.png)
...
![示例2](https://assets.leetcode.com/uploads/2024/04/22/example211.png)
