...
type: problem
title: 获取最近第二次的活动
tags: ["COUNT OVER", ROW_NUMBER, SQL, UNION, pandas, 子查询, 数据库, 窗口函数, 自连接]
knowledge_points: [count-over, database, pandas, row-number, self-join, sql, subquery, union, window-function]
updated: 2026-09-03
source: leetcode
number: 1369
difficulty: hard
...
**第一层：「最近第二次」= 每组按 `startDate` 降序的第 2 行**。这是「每组取第 K 大」的 K=2 版本，与 [176. 第二高的薪水](../0101-0200/176_第二高的薪水.md)（全局取第 2 大）同构，只是多了「按用户分组」。`ROW_NUMBER() OVER (PARTITION BY username ORDER BY startDate DESC)` 一行搞定排序编号：`rn = 1` 是最近，`rn = 2` 是最近第二次。
...
> 💡 若题意改为「不足 K 行时丢弃该组」（即严格要求取第 K 行，不够就空），则只需 `WHERE rn = K`，无 `cnt` 兜底——这是 [176. 第二高的薪水](../0101-0200/176_第二高的薪水.md) 的语义（无第二高时返回 NULL）。两种语义的差别全在「不足时兜底还是丢弃」，需审题确认。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 176 | [第二高的薪水](https://leetcode.cn/problems/second-highest-salary/)（[题解](../0101-0200/176_第二高的薪水.md)） | 全局取第 2 高薪——本题「每人取第 2 近」的单组退化版，对照 `LIMIT OFFSET` 与窗口函数两种范式 |
