...
type: problem
title: 活动参与者
tags: [CTE, "GROUP BY", HAVING, SQL, pandas, 子查询, 数据库, 窗口函数, 聚合]
knowledge_points: [aggregation, cte, database, group-by, having, pandas, sql, subquery, window-function]
updated: 2026-09-03
source: leetcode
number: 1355
difficulty: medium
...
- **标题 / 题号**：活动参与者（#1355，medium）
- **链接**：https://leetcode.cn/problems/activity-participants/
- **难度**：中等
- **标签**：数据库、SQL、GROUP BY、聚合、CTE、子查询、窗口函数、HAVING、pandas

**题意**：给定两张表：`Friends`（每位朋友及其参与的活动）和 `Activities`（活动清单）。请编写解决方案找出**参与人数既不是最多、也不是最少**的活动名称。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 176 | [第二高的薪水](https://leetcode.cn/problems/second-highest-salary/)（[题解](../0101-0200/176_第二高的薪水.md)） | `ORDER BY DESC LIMIT 1 OFFSET 1` 取次大——本题「剔除最大保留中间」的单边版，对照「排首尾」与「≠ 极值」两种范式 |
