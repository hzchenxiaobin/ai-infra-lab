...
id: "lc:1398"
type: problem
title: "购买了产品 A 和产品 B 却没有购买产品 C 的顾客"
tags: [EXISTS, "GROUP BY", HAVING, "NOT IN 的 NULL 陷阱", SQL, pandas, 数据库, 条件聚合, 集合包含判定]
knowledge_points: [conditional-aggregation, database, exists, group-by, having, not-in-null-trap, not-in-的-null-陷阱, pandas, sql, 集合包含判定]
updated: 2026-09-03
source: leetcode
number: 1398
difficulty: medium
...
# LeetCode 购买了产品A和产品B却没有购买产品C的顾客 题解

## 1. 题目概述

- **标题 / 题号**：购买了产品 A 和产品 B 却没有购买产品 C 的顾客（#1398，medium）
...
**题意**：给定 `Customers`（顾客）和 `Orders`（订单）两张表。`Orders` 直接记录了某顾客买了哪个产品（`product_name`）。编写 SQL，找出**既买了产品 A、又买了产品 B、却没有买产品 C** 的顾客，返回 `customer_id` 和 `customer_name`。
