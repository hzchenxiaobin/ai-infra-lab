...
type: problem
title: 阈值距离内邻居最少的城市
tags: [Floyd-Warshall, 动态规划, 图, 最短路径]
knowledge_points: [dynamic-programming, floyd-warshall, graph, shortest-path]
updated: 2026-09-03
source: leetcode
number: 1334
difficulty: medium
...
# LeetCode 阈值距离内邻居最少的城市 题解

## 1. 题目概述

- **标题 / 题号**：阈值距离内邻居最少的城市（#1334，medium）
...
**题意**：有 `n` 个城市，编号 `0` 到 `n-1`。给定边数组 `edges`，其中 `edges[i] = [from_i, to_i, weight_i]` 表示 `from_i` 与 `to_i` 之间一条**双向加权**边，边长为 `weight_i`。再给定整数 `distanceThreshold`。

定义城市 `i` 的「邻居数」为：从 `i` 出发、经过若干条边、**最短路径距离 $\le$ `distanceThreshold`** 可达的**其他城市数**（不含自身）。要求返回**邻居数最小**的城市；若并列，返回**编号最大**者。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2024/08/23/problem1334example1.png)
...
![示例2](https://assets.leetcode.com/uploads/2024/08/23/problem1334example0.png)
