...
title: 不同的循环子字符串
tags: [哈希表, 字符串, 滚动哈希]
knowledge_points: [hash-table, rolling-hash, string]
updated: 2026-09-03
source: leetcode
number: 1316
difficulty: hard
...
# LeetCode 不同的循环子字符串 题解

## 1. 题目概述

- **标题 / 题号**：不同的循环子字符串（#1316，hard）
- **链接**：https://leetcode.cn/problems/distinct-echo-substrings/
- **难度**：困难
- **标签**：字符串、哈希表、滚动哈希

**题意**：给你一个字符串 `text`，返回满足下述条件的**不同**非空子字符串的数目：可以写成某个字符串与其自身相连接的形式（即 `a + a`，其中 `a` 是某个非空字符串）。
...
| 718 | [最长重复子数组](https://leetcode.cn/problems/maximum-length-of-repeated-subarray/)（[题解](../0701-0800/718_最长重复子数组.md)） | DP 求两数组最长公共子串，也可用滚动哈希 + 二分，对照本题的「哈希判等」 |
