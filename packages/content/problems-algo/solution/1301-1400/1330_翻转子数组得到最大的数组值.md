...
title: 翻转子数组得到最大的数组值
tags: [数学, 数组, 贪心]
knowledge_points: [array, greedy, math]
updated: 2026-09-03
source: leetcode
number: 1330
difficulty: hard
...
# LeetCode 翻转子数组得到最大的数组值 题解

## 1. 题目概述

- **标题 / 题号**：翻转子数组得到最大的数组值（#1330，hard）
- **链接**：https://leetcode.cn/problems/reverse-subarray-to-maximize-array-value/
- **难度**：困难
- **标签**：数组、数学、贪心

**题意**：给你一个整数数组 `nums`。「数组值」定义为所有满足 $0 \le i < \text{nums.length} - 1$ 的 $|\text{nums}[i] - \text{nums}[i+1]|$ 的和。你可以选择任意子数组翻转一次，求可行的最大数组值。
...
| 152 | [乘积最大子数组](https://leetcode.cn/problems/maximum-product-subarray/)（[题解](../0101-0200/152_乘积最大子数组.md)） | 维护前缀极值（max/min 滚动）的同类技巧 |
