...
type: problem
title: 股票的资本损益
tags: ["GROUP BY", IF/CASE, SQL, SUM, pandas, 数据库, 条件聚合]
knowledge_points: [case-when, conditional-aggregation, database, group-by, if-case, pandas, sql, sum]
updated: 2026-09-03
source: leetcode
number: 1393
difficulty: medium
...
- **标题 / 题号**：股票的资本损益（#1393，medium）
- **链接**：https://leetcode.cn/problems/capital-gainloss/
- **难度**：中等
- **标签**：数据库、SQL、条件聚合、IF/CASE、GROUP BY、SUM、pandas

**题意**：给定 `Stocks` 表，每行记录某支股票在某一天的**买入**或**卖出**操作及其价格。编写 SQL，报告**每只股票的资本损益**（即所有买卖操作后的总收益或损失）。
