...
type: problem
title: 顾客的可信联系人数量
tags: [COUNT, "GROUP BY", "LEFT JOIN", SQL, pandas, 多表关联, 数据库, 条件聚合, 自连接]
knowledge_points: [conditional-aggregation, count, database, group-by, left-join, multi-join, pandas, self-join, sql]
updated: 2026-09-03
source: leetcode
number: 1364
difficulty: medium
...
# LeetCode 顾客的可信联系人数量 题解

## 1. 题目概述

- **标题 / 题号**：顾客的可信联系人数量（#1364，medium）
...
**题意**：给定三张表：`Customers`（顾客）、`Contacts`（联系人）和 `Invoices`（发票）。请编写解决方案，为**每张发票**返回以下信息：
