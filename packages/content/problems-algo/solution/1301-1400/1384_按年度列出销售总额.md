...
type: problem
title: 按年度列出销售总额
tags: [DATEDIFF, LEAST/GREATEST, SQL, 区间相交, 数据库, 日期函数, 维度表]
knowledge_points: [database, date-function, datediff, least-greatest, sql, 区间相交, 维度表]
updated: 2026-09-03
source: leetcode
number: 1384
difficulty: hard
...
# LeetCode 按年度列出销售总额 题解

## 1. 题目概述

- **标题 / 题号**：按年度列出销售总额（#1384，hard）
- **链接**：https://leetcode.cn/problems/total-sales-amount-by-year/
- **难度**：困难
- **标签**：数据库、SQL、区间相交、日期函数、维度表、DATEDIFF、LEAST/GREATEST

**题意**：给定 `Product` 表与 `Sales` 表。`Sales` 每行记录某产品在一段**日期区间** `[period_start, period_end]`（含端点）内的**日均销售额** `average_daily_sales`。请编写 SQL，报告**每个产品在每个自然年的销售总额**。
