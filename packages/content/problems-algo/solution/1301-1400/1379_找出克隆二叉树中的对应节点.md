...
type: problem
title: 找出克隆二叉树中的对应节点
tags: [二叉树, "广度优先搜索（BFS）", 树, "深度优先搜索（DFS）"]
...
![示例1](https://assets.leetcode.com/uploads/2020/02/21/e1.png)
...
![示例2](https://assets.leetcode.com/uploads/2020/02/21/e2.png)
...
![示例3](https://assets.leetcode.com/uploads/2020/02/21/e3.png)
...
5. **若 `target` 不在 `original` 中（题目保证存在，但面试可能追问），算法行为如何？**
   ...
   > `dfs` 会遍历完整棵 `original`，所有递归调用都返回 `None`，最终入口返回 `None`。代码无需改动即正确处理「不存在」情形——返回 `None` 表示未找到。这与 [236. 二叉树最近公共祖先](../0201-0300/236_二叉树的最近公共祖先.md) 中「搜索树中节点并返回引用」的失败语义一致：用 `None` 表示「子树中无目标」。
...
| 236 | [二叉树最近公共祖先](../0201-0300/236_二叉树的最近公共祖先.md) | 在树中搜索目标节点并返回其引用，用 `None` 表示「子树无目标」；对照 1379 同样是「树搜索 + 返回节点引用 + 短路上抛」，但 236 搜两个节点取 LCA、1379 搜一个节点取同位克隆 |
