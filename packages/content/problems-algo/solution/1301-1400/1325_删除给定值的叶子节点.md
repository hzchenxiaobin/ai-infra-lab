...
# LeetCode 删除给定值的叶子节点 题解

## 1. 题目概述

- **标题 / 题号**：删除给定值的叶子节点（#1325，medium）
- **链接**：https://leetcode.cn/problems/delete-leaves-with-a-given-value/
- **难度**：中等
- **标签**：树、二叉树、深度优先搜索（DFS）、后序遍历

**题意**：给你一棵以 `root` 为根的二叉树和一个整数 `target`，请你删除所有值为 `target` 的**叶子节点**。

注意，一旦删除值为 `target` 的叶子节点，它的父节点就可能变成叶子节点；如果新叶子节点的值恰好也是 `target`，那么这个节点也应该被删除。也就是说，你需要**重复此过程直到不能继续删除**。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/01/16/sample_1_1684.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/01/16/sample_2_1684.png)
...
![示例3](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/01/16/sample_3_1684.png)
