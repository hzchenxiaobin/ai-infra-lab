...
title: "每隔 n 个顾客打折"
tags: [哈希表, 模拟, 设计]
knowledge_points: [design, hash-table, simulation]
updated: 2026-09-03
source: leetcode
number: 1357
difficulty: medium
...
# LeetCode 每隔 n 个顾客打折 题解

## 1. 题目概述

- **标题 / 题号**：每隔 n 个顾客打折（#1357，medium）
- **链接**：https://leetcode.cn/problems/apply-discount-every-n-orders/
- **难度**：中等
- **标签**：设计、哈希表、模拟

**题意**：超市里正在举行打折活动，每隔 `n` 个顾客会得到 `discount` 的折扣。超市有若干商品，第 `i` 种商品为 `products[i]`，每件单品的价格为 `prices[i]`。
