...
type: problem
title: "T 秒后青蛙的位置"
tags: [图, "广度优先搜索（BFS）", 树, 概率, "深度优先搜索（DFS）"]
knowledge_points: [bfs, dfs, graph, probability, tree]
updated: 2026-09-03
source: leetcode
number: 1377
difficulty: hard
...
# LeetCode T 秒后青蛙的位置 题解

## 1. 题目概述

- **标题 / 题号**：T 秒后青蛙的位置（#1377，hard）
- **链接**：https://leetcode.cn/problems/frog-position-after-t-seconds/
- **难度**：困难
- **标签**：树、深度优先搜索（DFS）、广度优先搜索（BFS）、图、概率

**题意**：给你一棵由 `n` 个顶点组成的**无向树**，顶点编号从 `1` 到 `n`。青蛙从**顶点 1** 起跳，每秒按以下规则移动：
...
![示例1](https://assets.leetcode.com/uploads/2021/12/21/frog1.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/12/21/frog2.jpg)
