...
tags: [前缀和, 数组, 滑动窗口]
knowledge_points: [array, prefix-sum, sliding-window]
updated: 2026-09-03
source: leetcode
number: 1343
difficulty: medium
...
# LeetCode 大小为 K 且平均值大于等于阈值的子数组数目 题解

## 1. 题目概述

- **标题 / 题号**：大小为 K 且平均值大于等于阈值的子数组数目（#1343，medium）
- **链接**：https://leetcode.cn/problems/number-of-sub-arrays-of-size-k-and-average-greater-than-or-equal-to-threshold/
- **难度**：中等
- **标签**：数组、滑动窗口、前缀和

**题意**：给你一个整数数组 `arr` 和两个整数 `k` 与 `threshold`。请你返回**长度恰好为 `k`** 且**平均值大于等于 `threshold`** 的子数组数目。
