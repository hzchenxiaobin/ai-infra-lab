...
# LeetCode 圆和矩形是否有重叠 题解

## 1. 题目概述

- **标题 / 题号**：圆和矩形是否有重叠（#1401，medium）
- **链接**：https://leetcode.cn/problems/circle-and-rectangle-overlapping/
- **难度**：中等
- **标签**：几何、数学

**题意**：给定一个以 `(radius, xCenter, yCenter)` 表示的**圆**，和一个与坐标轴平行的**矩形** `(x1, y1, x2, y2)`，其中 `(x1, y1)` 是矩形左下角、`(x2, y2)` 是右上角。若圆与矩形有重叠（含边界相切），返回 `true`，否则返回 `false`。
...
![示例2](https://assets.leetcode.com/uploads/2020/02/20/sample_2_1728.png)
