...
- **标题 / 题号**：旅行终点站（#1436，easy）
- **链接**：https://leetcode.cn/problems/destination-city/
- **难度**：简单
- **标签**：数组、哈希表、字符串

**题意**：给你一份旅游线路 `paths`，其中 `paths[i] = [cityAi, cityBi]` 表示存在一条从 `cityAi` 到 `cityBi` 的直接路径。每个城市名是一个字符串。**所有路径串成一条不含环的旅行线**，且**恰好有一个终点站**——即那个**没有任何出边**的城市。请返回该终点站的城市名。
