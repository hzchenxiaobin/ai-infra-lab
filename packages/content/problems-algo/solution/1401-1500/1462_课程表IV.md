...
type: problem
title: "课程表 IV"
tags: [Floyd-Warshall, 传递闭包, 图, 拓扑排序]
knowledge_points: [floyd-warshall, graph, topological-sort, 传递闭包]
updated: 2026-09-03
source: leetcode
number: 1462
difficulty: medium
...
- **标题 / 题号**：课程表 IV（#1462，medium）
- **链接**：https://leetcode.cn/problems/course-schedule-iv/
- **难度**：中等
- **标签**：图、拓扑排序、传递闭包、Floyd-Warshall

**题意**：你总共需要上 `numCourses` 门课，课程编号 `0` 到 `numCourses-1`。给你一个数组 `prerequisites`，其中 `prerequisites[i] = [ai, bi]` 表示**想选 `bi` 必须先选 `ai`**（即 `ai` 是 `bi` 的先修课）。先修关系可以**间接传递**：若 `a` 是 `b` 的先修、`b` 是 `c` 的先修，则 `a` 也是 `c` 的先修。

再给你一个查询数组 `queries`，`queries[j] = [uj, vj]`，问 `uj` 是否是 `vj` 的先修课（直接或间接均可）。返回布尔数组 `answer`，`answer[j]` 为第 `j` 个查询的答案。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/05/01/courses4-1-graph.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/05/01/courses4-3-graph.jpg)
