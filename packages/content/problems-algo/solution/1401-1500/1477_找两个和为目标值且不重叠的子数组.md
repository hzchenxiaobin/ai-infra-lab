...
tags: [动态规划, 数组, 滑动窗口]
knowledge_points: [array, dynamic-programming, sliding-window]
updated: 2026-09-03
source: leetcode
number: 1477
difficulty: medium
...
# LeetCode 找两个和为目标值且不重叠的子数组 题解

## 1. 题目概述

- **标题 / 题号**：找两个和为目标值且不重叠的子数组（#1477，medium）
- **链接**：https://leetcode.cn/problems/find-two-non-overlapping-sub-arrays-each-with-target-sum/
- **难度**：中等
- **标签**：数组、滑动窗口、动态规划

**题意**：给你一个正整数数组 `arr` 和一个整数值 `target`。请在 `arr` 中找**两个互不重叠的子数组**且它们的和都等于 `target`。可能会有多种方案，请你返回满足要求的两个子数组长度和的**最小值**。如果无法找到这样的两个子数组，请返回 **-1**。
