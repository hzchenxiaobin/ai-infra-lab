...
type: problem
title: 苹果和桔子
tags: ["GROUP BY", IF/CASE, SQL, SUM, 条件聚合, 聚合减法]
knowledge_points: [case-when, conditional-aggregation, group-by, if-case, sql, sum, 聚合减法]
updated: 2026-09-03
source: leetcode
number: 1445
difficulty: medium
...
- **标题 / 题号**：苹果和桔子（#1445，medium）
- **链接**：https://leetcode.cn/problems/apples-oranges/
- **难度**：中等
- **标签**：SQL、GROUP BY、条件聚合、SUM、IF/CASE、聚合减法

**题意**：给定 `Sales` 表，每行记录某天某种水果的销量。`fruit` 是枚举类型，取值仅为 `'apples'` 或 `'oranges'`。编写 SQL 查询，报告**每天苹果销量与桔子销量之差**（`diff = apples − oranges`），结果按 `sale_date` 升序返回。
