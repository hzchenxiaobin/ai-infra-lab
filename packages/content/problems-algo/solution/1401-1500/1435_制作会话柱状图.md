...
type: problem
title: 制作会话柱状图
tags: [CASE/IF, "GROUP BY", "LEFT JOIN", SQL, UNION, 分桶, 区间统计, 数据库]
knowledge_points: [case-if, database, group-by, left-join, sql, union, 分桶, 区间统计]
updated: 2026-09-03
source: leetcode
number: 1435
difficulty: easy
...
- **标题 / 题号**：制作会话柱状图（#1435，easy）
- **链接**：https://leetcode.cn/problems/create-a-session-bar-chart/
- **难度**：简单
- **标签**：数据库、SQL、UNION、CASE/IF、GROUP BY、分桶、LEFT JOIN、区间统计

**题意**：给定 `Sessions` 表，每行记录一次会话的 `session_id` 与 `duration`（**以秒为单位**的访问时长）。现在想把会话时长按**分钟**分到四个桶里，统计每个桶的会话数，画一张「柱状图」的底层数据表：
