...
type: problem
title: 安排邮筒
tags: [中位数, "划分型 DP", 前缀和, 动态规划]
knowledge_points: [dynamic-programming, median, partition-dp, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1478
difficulty: hard
...
- **标题 / 题号**：安排邮筒（#1478，hard）
- **链接**：https://leetcode.cn/problems/allocate-mailboxes/
- **难度**：困难
- **标签**：动态规划、划分型 DP、中位数、前缀和

**题意**：一条街上有 `n` 栋房子，`houses[i]` 是第 `i` 栋的位置。要在街上安排 `k` 个邮筒，使**每栋房子到离它最近的邮筒的距离之和最小**。返回该最小总和。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/06/13/sample_11_1816.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/06/13/sample_2_1816.png)
