...
title: 逐步求和得到正数的最小值
tags: [前缀和, 数组, 贪心]
knowledge_points: [array, greedy, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1413
difficulty: easy
...
1. **如何想到前缀和？**
   ...
   > 每一步的当前值 = `startValue + (nums[0] + … + nums[i])`，括号内正是前缀和。一旦把当前值拆成「偏移 + 前缀和」，问题就从「模拟累加链」变成「找前缀和的最值」——这是 [303. 区域和检索](../0301-0400/303_区域和检索%20-%20数组不可变.md) 以来前缀和的经典应用范式。
...
| 303 | [区域和检索 - 数组不可变](../0301-0400/303_区域和检索%20-%20数组不可变.md) | 前缀和模板题，预处理前缀和数组支持 $O(1)$ 区间和查询，是 1413「把累加链拆成偏移 + 前缀和」的母题 |
