...
title: 用栈操作构建数组
tags: [双指针, 数组, 栈, 模拟]
knowledge_points: [array, simulation, stack, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1441
difficulty: easy
...
# LeetCode 用栈操作构建数组 题解

## 1. 题目概述

- **标题 / 题号**：用栈操作构建数组（#1441，easy）
- **链接**：https://leetcode.cn/problems/build-an-array-with-stack-operations/
- **难度**：简单
- **标签**：栈、数组、模拟、双指针

**题意**：存在一个不含重复元素的整数流 `stream = 1, 2, 3, …, n`（按升序逐个读出）。另有一个**严格递增**的目标数组 `target`，其中元素都取自 `[1, n]`。初始时栈为空，且只允许两种操作：
