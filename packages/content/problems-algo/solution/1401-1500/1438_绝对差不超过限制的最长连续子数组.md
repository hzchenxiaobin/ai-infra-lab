...
tags: [单调队列, 双指针, 数组, 滑动窗口]
knowledge_points: [array, monotonic-queue, sliding-window, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1438
difficulty: medium
...
# LeetCode 绝对差不超过限制的最长连续子数组 题解

## 1. 题目概述

- **标题 / 题号**：绝对差不超过限制的最长连续子数组（#1438，medium）
- **链接**：https://leetcode.cn/problems/longest-continuous-subarray-with-absolute-diff-less-than-or-equal-to-limit/
- **难度**：中等
- **标签**：数组、滑动窗口、单调队列、双指针

**题意**：给定一个整数数组 `nums` 和一个整数 `limit`，请你返回**最长**的**非空连续子数组**的长度，使得该子数组中**任意两个元素**之间的绝对差**小于或等于** `limit`。
