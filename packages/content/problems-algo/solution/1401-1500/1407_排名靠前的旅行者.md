...
type: problem
title: 排名靠前的旅行者
tags: ["GROUP BY", IFNULL/COALESCE, "LEFT JOIN", "ORDER BY", SQL, SUM, 数据库, 聚合]
knowledge_points: [aggregation, database, group-by, ifnull-coalesce, left-join, null-coalescing, order-by, sql, sum]
updated: 2026-09-03
source: leetcode
number: 1407
difficulty: easy
...
# LeetCode 排名靠前的旅行者 题解

## 1. 题目概述

- **标题 / 题号**：排名靠前的旅行者（#1407，easy）
- **链接**：https://leetcode.cn/problems/top-travellers/
- **难度**：简单
- **标签**：数据库、SQL、LEFT JOIN、GROUP BY、SUM、IFNULL/COALESCE、ORDER BY、聚合

**题意**：给定 `Users` 表与 `Rides` 表。`Users` 每行记录一个用户（`id`、`name`）；`Rides` 每行记录一次行程（`id`、`user_id`、`distance`）。请编写 SQL，报告**每个用户的旅行距离**（即该用户所有行程的 `distance` 之和），并按 `travelled_distance` **降序**排列；若距离相同，再按 `name` **升序**排列。
