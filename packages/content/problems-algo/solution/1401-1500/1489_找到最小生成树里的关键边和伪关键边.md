...
tags: ["Kruskal 算法", 图, 并查集, 最小生成树]
knowledge_points: [graph, kruskal, mst, union-find]
updated: 2026-09-03
source: leetcode
number: 1489
difficulty: hard
...
# LeetCode 找到最小生成树里的关键边和伪关键边 题解

## 1. 题目概述

- **标题 / 题号**：找到最小生成树里的关键边和伪关键边（#1489，hard）
- **链接**：https://leetcode.cn/problems/find-critical-and-pseudo-critical-edges-in-minimum-spanning-tree/
- **难度**：困难
- **标签**：图、最小生成树、Kruskal 算法、并查集

**题意**：给定一个 `n` 个节点的带权无向连通图，节点编号 `0` 到 `n-1`，以及边数组 `edges`，其中 `edges[i] = [fromi, toi, weighti]`。**最小生成树（MST）** 是图中边的一个子集，连接所有节点且无环、权值和最小。
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/06/21/ex1.png)
...
![示例2](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/06/21/msts.png)
