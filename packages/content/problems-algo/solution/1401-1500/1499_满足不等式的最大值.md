...
title: 满足不等式的最大值
tags: [单调队列, 数组, 滑动窗口]
knowledge_points: [array, monotonic-queue, sliding-window]
updated: 2026-09-03
source: leetcode
number: 1499
difficulty: hard
...
# LeetCode 满足不等式的最大值 题解

## 1. 题目概述

- **标题 / 题号**：满足不等式的最大值（#1499，hard）
- **链接**：https://leetcode.cn/problems/max-value-of-equation/
- **难度**：困难
- **标签**：数组、单调队列、滑动窗口

**题意**：给定一个点集 `points`，其中 `points[i] = [xi, yi]` 表示二维平面上的一个点。**`points` 已按 $x$ 坐标严格递增排序**。再给定一个整数 $k$。
