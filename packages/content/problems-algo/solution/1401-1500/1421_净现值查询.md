...
type: problem
title: 净现值查询
tags: [COALESCE, IFNULL, "LEFT JOIN", "NULL 处理", SQL]
knowledge_points: [coalesce, ifnull, left-join, null-handling, sql]
updated: 2026-09-03
source: leetcode
number: 1421
difficulty: easy
...
- **标题 / 题号**：净现值查询（#1421，easy）
- **链接**：https://leetcode.cn/problems/npv-queries/
- **难度**：简单
- **标签**：SQL、LEFT JOIN、IFNULL、COALESCE、NULL 处理

**题意**：给定 `NPV` 表和 `Queries` 表。`NPV` 表记录每个项目（`id`）在某一年（`year`）的净现值（`npv`）；`Queries` 表给出若干 `(id, year)` 查询对。编写 SQL 查询，为 `Queries` 表中的**每一行**返回对应的 `npv` 值。如果 `NPV` 表中**没有匹配的 `(id, year)` 记录**，则 `npv` 返回 `0`。
