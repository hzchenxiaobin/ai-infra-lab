...
type: problem
title: 周内每天的销售情况
tags: ["CASE WHEN", "GROUP BY", "LEFT JOIN", SQL, WEEKDAY, 条件聚合, 行转列]
knowledge_points: [case-when, conditional-aggregation, group-by, left-join, pivot, sql, weekday]
updated: 2026-09-03
source: leetcode
number: 1479
difficulty: hard
...
# LeetCode 周内每天的销售情况 题解

## 1. 题目概述

- **标题 / 题号**：周内每天的销售情况（#1479，hard）
- **链接**：https://leetcode.cn/problems/sales-by-day-of-the-week/
- **难度**：困难
- **标签**：SQL、行转列、条件聚合、WEEKDAY、LEFT JOIN、GROUP BY、CASE WHEN

**题意**：给定两张表：`Items` 存储商品信息（名称与品类），`Orders` 存储订单记录（日期、商品、数量）。编写 SQL 查询，统计**每个品类**在一周中**每天**售出的总数量，输出一张「品类 × 星期」的透视矩阵。
