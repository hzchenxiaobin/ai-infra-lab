...
tags: [二分查找, 堆, 数组, 矩阵]
knowledge_points: [array, binary-search, heap, matrix, 矩阵]
updated: 2026-09-03
source: leetcode
number: 1439
difficulty: hard
...
# LeetCode 有序矩阵中的第k个最小数组和 题解

## 1. 题目概述

- **标题 / 题号**：有序矩阵中的第k个最小数组和（#1439，hard）
- **链接**：https://leetcode.cn/problems/find-the-kth-smallest-sum-of-a-matrix-with-sorted-rows/
- **难度**：困难
- **标签**：数组、二分查找、矩阵、堆

**题意**：给定一个 $m \times n$ 的矩阵 `mat`，其中**每一行**都按**非递减**顺序排列，以及一个整数 `k`。你可以从每一行中选出**恰好 1 个**元素形成一个数组。返回所有可能数组中的**第 `k` 个最小**数组和。
