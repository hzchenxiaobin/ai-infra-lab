...
type: problem
title: 最长上传前缀
tags: [数组, 设计, "贪心（指针）/ 并查集"]
knowledge_points: [array, design, 贪心-指针-并查集]
updated: 2026-09-03
source: leetcode
number: 2424
difficulty: medium
...
- **标题 / 题号**：最长上传前缀（#2424，medium）
- **链接**：https://leetcode.cn/problems/longest-uploaded-prefix/
- **难度**：中等
- **标签**：设计、数组、贪心（指针）/ 并查集

**题意**：有 `n` 个视频，编号为 `1` 到 `n` 之间的**不同**整数，按某个顺序依次上传。实现 `LUPrefix` 类，在上传过程中维护**最长上传前缀**：
