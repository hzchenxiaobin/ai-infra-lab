...
title: 让数组不相等的最小总代价
tags: [哈希表, 数组, 计数, 贪心]
knowledge_points: [array, counting, greedy, hash-table]
updated: 2026-09-03
source: leetcode
number: 2499
difficulty: hard
...
# LeetCode 让数组不相等的最小总代价 题解

## 1. 题目概述

- **标题 / 题号**：让数组不相等的最小总代价（#2499，hard）
- **链接**：https://leetcode.cn/problems/minimum-total-cost-to-make-arrays-unequal/
- **难度**：困难
- **标签**：贪心、数组、哈希表、计数

**题意**：给你两个长度均为 $n$、下标从 $0$ 开始的数组 `nums1`、`nums2`。每次操作可交换 `nums1` 中任意两个下标处的值，**开销为两个下标之和**（$i+j$）。要求对任意 $i$ 满足 $\text{nums1}[i] \ne \text{nums2}[i]$，可操作任意次，求**最小总代价**；不能达成返回 $-1$。
