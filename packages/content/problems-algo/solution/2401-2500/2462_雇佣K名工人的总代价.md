...
type: problem
title: "雇佣 K 名工人的总代价"
tags: [双指针, "堆（优先队列）", 数组, 模拟, 贪心]
knowledge_points: [array, greedy, heap, simulation, two-pointers]
updated: 2026-09-03
source: leetcode
number: 2462
difficulty: medium
...
# LeetCode 雇佣 K 名工人的总代价 题解

## 1. 题目概述

- **标题 / 题号**：雇佣 K 名工人的总代价（#2462，medium）
- **链接**：https://leetcode.cn/problems/total-cost-to-hire-k-workers/
- **难度**：中等
- **标签**：数组、堆（优先队列）、双指针、模拟、贪心

**题意**：给定长度为 $n$ 的 0 下标数组 `costs`，`costs[i]` 是雇佣第 $i$ 个工人的代价。再给定整数 $k$ 与 $candidates$，要恰好雇佣 $k$ 名工人。共进行 $k$ 轮，每轮雇佣恰好一人，规则如下：
