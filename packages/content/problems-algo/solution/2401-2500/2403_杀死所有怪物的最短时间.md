...
type: problem
title: 杀死所有怪物的最短时间
tags: [位掩码, 位运算, 动态规划, 数组, 状态压缩]
knowledge_points: [array, bit-manipulation, bitmask, bitmask-dp, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 2403
difficulty: hard
...
# LeetCode 杀死所有怪物的最短时间 题解

## 1. 题目概述

- **标题 / 题号**：杀死所有怪物的最短时间（#2403，hard）
- **链接**：https://leetcode.cn/problems/minimum-time-to-kill-all-monsters/
- **难度**：困难
- **标签**：位运算、数组、动态规划、状态压缩、位掩码

**题意**：你有一个整数数组 `power`，其中 `power[i]` 是第 `i` 个怪物的力量。你从 `0` 点法力值开始，**每天**获取 `gain` 点法力值，最初 `gain = 1`。每天，在获得 `gain` 点法力值后，若你的法力值**大于或等于**某个未击败怪物的力量，你就可以打败它。当你打败一个怪物时：
