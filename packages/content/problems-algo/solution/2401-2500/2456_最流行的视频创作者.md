...
type: problem
title: 最流行的视频创作者
tags: [哈希表, "堆（优先队列）", 字符串, 排序, 数组]
knowledge_points: [array, hash-table, heap, sorting, string]
updated: 2026-09-03
source: leetcode
number: 2456
difficulty: medium
...
# LeetCode 最流行的视频创作者 题解

## 1. 题目概述

- **标题 / 题号**：最流行的视频创作者（#2456，medium）
- **链接**：https://leetcode.cn/problems/most-popular-video-creator/
- **难度**：中等
- **标签**：数组、哈希表、字符串、排序、堆（优先队列）

**题意**：给定三个长度均为 $n$ 的数组 `creators`、`ids`、`views`：平台上第 $i$ 个视频的创作者是 `creators[i]`，分配的视频 id 是 `ids[i]`，播放量为 `views[i]`。
