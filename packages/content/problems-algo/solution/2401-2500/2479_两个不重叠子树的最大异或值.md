...
type: problem
title: 两个不重叠子树的最大异或值
tags: [位运算, "字典树（0-1 Trie）", 树, 深度优先搜索]
knowledge_points: [01-trie, bit-manipulation, dfs, tree]
updated: 2026-09-03
source: leetcode
number: 2479
difficulty: hard
...
# LeetCode 两个不重叠子树的最大异或值 题解

## 1. 题目概述

- **标题 / 题号**：两个不重叠子树的最大异或值（#2479，hard）
- **链接**：https://leetcode.cn/problems/maximum-xor-of-two-non-overlapping-subtrees/
- **难度**：困难
- **标签**：树、深度优先搜索、字典树（0-1 Trie）、位运算

**题意**：给定一棵 **$n$ 个节点**的无向树，节点编号 $0$ 到 $n-1$，**根为节点 $0$**。再给定 `edges`（长度 $n-1$，`edges[i]=[a_i,b_i]` 表示一条边）和 `values`（`values[i]` 为节点 $i$ 的**值**）。
