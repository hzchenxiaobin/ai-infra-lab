...
title: 反转之后的数字和
tags: [数位反转, 数学, 枚举]
knowledge_points: [enumeration, math, 数位反转]
updated: 2026-09-03
source: leetcode
number: 2443
difficulty: medium
...
# LeetCode 反转之后的数字和 题解

## 1. 题目概述

- **标题 / 题号**：反转之后的数字和（#2443，medium）
- **链接**：https://leetcode.cn/problems/sum-of-number-and-its-reverse/
- **难度**：中等
- **标签**：数学、枚举、数位反转

**题意**：给你一个**非负**整数 `num`。如果存在某个**非负**整数 `k` 满足 $k + \text{reverse}(k) = \text{num}$，则返回 `true`；否则返回 `false`。
