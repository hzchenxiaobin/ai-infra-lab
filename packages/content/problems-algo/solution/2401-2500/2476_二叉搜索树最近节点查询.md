...
type: problem
title: 二叉搜索树最近节点查询
tags: [中序遍历, 二分查找, 二叉搜索树, 树]
knowledge_points: [binary-search, bst, inorder-traversal, tree]
updated: 2026-09-03
source: leetcode
number: 2476
difficulty: medium
...
# LeetCode 二叉搜索树最近节点查询 题解

## 1. 题目概述

- **标题 / 题号**：二叉搜索树最近节点查询（#2476，medium）
- **链接**：https://leetcode.cn/problems/closest-nodes-queries-in-a-binary-search-tree/
- **难度**：中等
- **标签**：树、二叉搜索树、中序遍历、二分查找

**题意**：给定一棵 **二叉搜索树（BST）** 的根 `root`，和一个长度为 `n` 的正整数数组 `queries`。对每个 `queries[i]` 求一个二元组 `answer[i] = [mini, maxi]`：
...
![示例1](https://assets.leetcode.com/uploads/2022/09/28/bstreeedrawioo.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/09/28/bstttreee.png)
...
1. **中序遍历 = 严格递增序列**（承接 [98. 验证二叉搜索树](../0001-0100/98_验证二叉搜索树.md) 的中序单调性）：一次 DFS `左→取值→右`，就能把 $n$ 个节点值按升序收集进数组 `a`，耗时 $O(n)$。
...
4. **BST 中序遍历为何天然有序、要不要再 `sort`？**
   ...
   > BST 定义保证「左子树所有值 < 根 < 右子树所有值」，中序 `左→根→右` 的访问顺序即严格递增（见 [98. 验证二叉搜索树](../0001-0100/98_验证二叉搜索树.md)）。因此一次中序直接得到有序数组，**不必再排序**，否则反而抹掉 BST 的核心性质、白付 $O(n\log n)$。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 98 | [验证二叉搜索树](https://leetcode.cn/problems/validate-binary-search-tree/)（[题解](../0001-0100/98_验证二叉搜索树.md)） | **中序单调性母题**，本题「中序得有序数组」正是它的直接推论，理解了 98 就理解了为什么不必再排序 |
