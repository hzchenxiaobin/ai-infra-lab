...
title: 所有数对的异或和
tags: [位运算, 数组, 脑筋急转弯]
knowledge_points: [array, bit-manipulation, brainteaser]
updated: 2026-09-03
source: leetcode
number: 2425
difficulty: medium
...
# LeetCode 所有数对的异或和 题解

## 1. 题目概述

- **标题 / 题号**：所有数对的异或和（#2425，medium）
- **链接**：https://leetcode.cn/problems/bitwise-xor-of-all-pairings/
- **难度**：中等
- **标签**：位运算、数组、脑筋急转弯

**题意**：给你两个下标从 0 开始的数组 `nums1` 和 `nums2`，都只含非负整数。将 `nums1` 中每个整数与 `nums2` 中每个整数**恰好配一次**，得到所有数对的异或和，构成数组 `nums3`（共 $n \cdot m$ 个元素，$n=\text{len}(\text{nums1})$，$m=\text{len}(\text{nums2})$）。返回 `nums3` 中所有整数的**异或和**。
