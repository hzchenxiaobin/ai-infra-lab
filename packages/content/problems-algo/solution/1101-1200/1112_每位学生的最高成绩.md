...
type: problem
title: 每位学生的最高成绩
tags: ["GROUP BY", ROW_NUMBER, SQL, tie-breaking, 分组求最值, 子查询, 窗口函数]
knowledge_points: [group-by, group-extremum, row-number, sql, subquery, tie-breaking, window-function, 分组求最值]
updated: 2026-09-03
source: leetcode
number: 1112
difficulty: medium
...
# LeetCode 每位学生的最高成绩 题解

## 1. 题目概述

- **标题 / 题号**：每位学生的最高成绩（#1112，medium）
- **链接**：https://leetcode.cn/problems/highest-grade-for-each-student/
- **难度**：中等
- **标签**：SQL、窗口函数、ROW_NUMBER、GROUP BY、子查询、分组求最值、tie-breaking

**题意**：给定 `Enrolls` 表，每行记录一名学生在某门课程中获得的成绩。编写 SQL 查询，找出**每个学生成绩最高的那门课**。如果某学生有多门课并列最高分，则返回 `course_id` 最小的那一门。结果按 `student_id` 升序排列。
