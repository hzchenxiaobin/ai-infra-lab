...
type: problem
title: "市场分析 I"
tags: ["COUNT 与 NULL", "GROUP BY", "LEFT JOIN", SQL, 列别名, "条件放 ON"]
knowledge_points: [count-与-null, group-by, left-join, sql, 列别名, 条件放-on]
updated: 2026-09-03
source: leetcode
number: 1158
difficulty: medium
...
- **标题 / 题号**：市场分析 I（#1158，medium）
- **链接**：https://leetcode.cn/problems/market-analysis-i/
- **难度**：中等
- **标签**：SQL、LEFT JOIN、条件放 ON、COUNT 与 NULL、GROUP BY、列别名

**题意**：给定购物网站的 `Users`、`Orders`、`Items` 三张表，编写 SQL 查询，找出**每个用户**的注册日期（`join_date`）和其在 **`2019` 年作为买家**的订单总数。结果以任意顺序返回。
