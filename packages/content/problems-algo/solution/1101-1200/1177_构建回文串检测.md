...
title: 构建回文串检测
tags: [位运算, 前缀和, 哈希表, 数组]
knowledge_points: [array, bit-manipulation, hash-table, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1177
difficulty: medium
...
- **标题 / 题号**：构建回文串检测（#1177，medium）
- **链接**：https://leetcode.cn/problems/can-make-palindrome-from-substring/
- **难度**：中等
- **标签**：位运算、数组、前缀和、哈希表

**题意**：给定字符串 `s` 和若干查询 `queries[i] = [left, right, k]`。对每个查询，取出子串 `s[left..right]`，可以**重新排列**该子串，并**最多替换** `k` 个字符为任意小写字母，判断能否变成回文串。返回布尔数组 `answer`。
