...
title: 用户网站访问行为分析
tags: [哈希表, 排序, 组合枚举]
knowledge_points: [combination-enumeration, hash-table, sorting]
updated: 2026-09-03
source: leetcode
number: 1152
difficulty: medium
...
# LeetCode 用户网站访问行为分析 题解

## 1. 题目概述

- **标题 / 题号**：用户网站访问行为分析（#1152，medium）
- **链接**：https://leetcode.cn/problems/analyze-user-website-visit-pattern/
- **难度**：中等
- **标签**：哈希表、排序、组合枚举

**题意**：给定三个等长数组 `username`、`timestamp`、`website`，其中第 `i` 条记录表示用户 `username[i]` 在时间 `timestamp[i]` 访问了网站 `website[i]`。
