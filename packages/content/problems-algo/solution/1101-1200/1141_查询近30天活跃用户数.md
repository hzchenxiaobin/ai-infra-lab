...
type: problem
title: 查询近30天活跃用户数
tags: ["COUNT(DISTINCT)", DATEDIFF, "GROUP BY", SQL, 日期过滤]
knowledge_points: [count-distinct, date-filter, datediff, group-by, sql, 日期过滤]
updated: 2026-09-03
source: leetcode
number: 1141
difficulty: easy
...
# LeetCode 查询近30天活跃用户数 题解

## 1. 题目概述

- **标题 / 题号**：查询近30天活跃用户数（#1141，easy）
- **链接**：https://leetcode.cn/problems/user-activity-for-the-past-30-days-i/
- **难度**：简单
- **标签**：SQL、GROUP BY、COUNT(DISTINCT)、日期过滤、DATEDIFF

**题意**：给定 `Activity` 表，记录社交媒体网站的用户活动。编写 SQL 查询，统计截至 `2019-07-27`（含当天），近 `30` 天内**每日活跃用户数**。当天只要有一条活动记录，该用户即为活跃用户。
