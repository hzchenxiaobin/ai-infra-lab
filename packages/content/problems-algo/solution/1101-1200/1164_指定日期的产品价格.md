...
type: problem
title: 指定日期的产品价格
tags: [IFNULL, "LEFT JOIN", ROW_NUMBER, SQL, 子查询, 点时查询, 窗口函数]
knowledge_points: [ifnull, left-join, row-number, sql, subquery, window-function, 点时查询]
updated: 2026-09-03
source: leetcode
number: 1164
difficulty: medium
...
# LeetCode 指定日期的产品价格 题解

## 1. 题目概述

- **标题 / 题号**：指定日期的产品价格（#1164，medium）
- **链接**：https://leetcode.cn/problems/product-price-at-a-given-date/
- **难度**：中等
- **标签**：SQL、窗口函数、ROW_NUMBER、LEFT JOIN、IFNULL、子查询、点时查询

**题意**：给定 `Products` 表，记录每个产品在不同日期的改价事件。编写 SQL 查询，找出 **`2019-08-16`** 这一天**所有产品**的价格。产品在首次改价前价格为 `10`；若某产品在 `2019-08-16`（含当天）之前没有任何改价记录，则其当日价格取默认值 `10`。
