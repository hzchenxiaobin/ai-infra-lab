...
type: problem
title: 查询活跃业务
tags: ["AVG 聚合", "GROUP BY", HAVING, SQL, 分组均值比较, "子查询 JOIN", 窗口函数]
knowledge_points: [avg, avg-聚合, group-by, having, sql, window-function, 分组均值比较, 子查询-join]
updated: 2026-09-03
source: leetcode
number: 1126
difficulty: medium
...
- **标题 / 题号**：查询活跃业务（#1126，medium）
- **链接**：https://leetcode.cn/problems/active-businesses/
- **难度**：中等
- **标签**：SQL、子查询 JOIN、窗口函数、GROUP BY、HAVING、AVG 聚合、分组均值比较

**题意**：给定 `Events` 表，每行记录某个业务（`business_id`）的某类事件（`event_type`）发生的次数（`occurences`）。编写 SQL 查询，找出所有**活跃业务**。
