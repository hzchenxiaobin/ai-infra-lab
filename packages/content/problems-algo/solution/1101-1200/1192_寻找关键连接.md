...
type: problem
title: 寻找关键连接
tags: ["Tarjan 算法", 图, 桥, 深度优先搜索]
knowledge_points: [bridge, dfs, graph, tarjan]
updated: 2026-09-03
source: leetcode
number: 1192
difficulty: hard
...
- **标题 / 题号**：寻找关键连接（#1192，hard）
- **链接**：https://leetcode.cn/problems/critical-connections-in-a-network/
- **难度**：困难
- **标签**：图、深度优先搜索、Tarjan 算法、桥

**题意**：有 `n` 台服务器，按从 `0` 到 `n-1` 编号，它们之间通过无向连接 `connections` 互连，其中 `connections[i] = [ai, bi]` 表示服务器 `ai` 和 `bi` 之间有一条连接。**关键连接**（critical connection）是指：如果将该连接删除，会导致某些服务器无法到达其他某些服务器的连接。换句话说，关键连接就是图中的**桥**（bridge）。

请返回所有关键连接，答案可以以任意顺序给出。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/original_images/critical-connections-in-a-network.png)
