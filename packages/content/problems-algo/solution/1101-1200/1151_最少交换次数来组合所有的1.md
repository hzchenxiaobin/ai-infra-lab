...
tags: [数组, 滑动窗口]
knowledge_points: [array, sliding-window]
updated: 2026-09-03
source: leetcode
number: 1151
difficulty: medium
...
# LeetCode 最少交换次数来组合所有的 1 题解

## 1. 题目概述

- **标题 / 题号**：最少交换次数来组合所有的 1（#1151，medium）
- **链接**：https://leetcode.cn/problems/minimum-swaps-to-group-all-1s-together/
- **难度**：中等
- **标签**：数组、滑动窗口

**题意**：给定一个**二进制数组** `data`，其中 `data[i]` 只能取 `0` 或 `1`。要求将数组中**所有的 `1`** 聚集到一个**连续的子数组**中（可以放在数组的任意位置），返回达成此目标所需的**最少交换次数**。一次交换定义为交换数组中任意两个元素的位置。
