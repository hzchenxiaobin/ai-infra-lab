...
type: problem
title: K次串联后最大子数组之和
tags: ["Kadane 算法", 动态规划, 数组]
knowledge_points: [array, dynamic-programming, kadane]
updated: 2026-09-03
source: leetcode
number: 1191
difficulty: medium
...
# LeetCode K次串联后最大子数组之和 题解

## 1. 题目概述

- **标题 / 题号**：K次串联后最大子数组之和（#1191，medium）
- **链接**：https://leetcode.cn/problems/k-concatenation-maximum-sum/
- **难度**：中等
- **标签**：数组、动态规划、Kadane 算法

**题意**：给定一个整数数组 `arr` 和一个整数 `k`，把 `arr` **重复串联 `k` 次**得到新数组（例如 `arr = [1, 2]`、`k = 3` 得到 `[1, 2, 1, 2, 1, 2]`）。返回新数组中**最大子数组之和**。注意子数组长度可以为 `0`（此时和为 `0`）。由于结果可能很大，需对 $10^9 + 7$ 取模。
...
于是 $\text{kadane}_2 = \max(\text{kadane}(arr),\ \text{maxPrefix}+\text{maxSuffix})$。当 $\text{sum}>0$ 时由引理可省去 $\text{kadane}(arr)$ 这一比较，直接用 $\text{maxPrefix}+\text{maxSuffix}$。这种写法把「最大子数组和」拆成「最大前缀和 + 最大后缀和」两个一维扫描，与 [918 环形子数组](../0901-1000/918_最大环形子数组和.md)「用补集 $\text{total}-\text{minSubarray}$ 处理跨界」的思路同源——本质上都是**用前缀/后缀极值刻画「跨界子数组」**。
...
| 918 | [环形子数组的最大和](https://leetcode.cn/problems/maximum-sum-circular-subarray/)（[题解](../0901-1000/918_最大环形子数组和.md)） | Kadane 环形变体，用补集/前缀后缀极值处理跨界 |
