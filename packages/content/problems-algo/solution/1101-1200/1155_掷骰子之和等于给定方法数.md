...
type: problem
title: 掷骰子之和等于给定方法数
tags: [分组背包, 动态规划, "计数 DP"]
knowledge_points: [counting-dp, dynamic-programming, grouped-knapsack]
updated: 2026-09-03
source: leetcode
number: 1155
difficulty: medium
...
# LeetCode 掷骰子之和等于给定方法数 题解

## 1. 题目概述

- **标题 / 题号**：掷骰子之和等于给定方法数（#1155，medium）
- **链接**：https://leetcode.cn/problems/number-of-dice-rolls-with-target-sum/
- **难度**：中等
- **标签**：动态规划、分组背包、计数 DP

**题意**：有 `n` 枚骰子，每枚骰子有 `k` 个面，面值分别为 `1` 到 `k`。给定三个整数 `n`、`k` 和 `target`，返回**掷骰子使所有面值之和恰好等于 `target` 的不同方法数**。由于答案可能很大，返回对 $10^9 + 7$ 取模的结果。
