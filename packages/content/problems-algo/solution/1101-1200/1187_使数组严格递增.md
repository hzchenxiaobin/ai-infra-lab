...
- **标题 / 题号**：使数组严格递增（#1187，hard）
- **链接**：https://leetcode.cn/problems/make-array-strictly-increasing/
- **难度**：困难
- **标签**：动态规划、贪心、二分查找

**题意**：给定两个整数数组 `arr1` 和 `arr2`，每次操作可以选取 `arr1[i]` 并将其替换为 `arr2` 中的任意元素。返回使 `arr1` **严格递增**所需的最少操作数；若无法实现，返回 `-1`。
...
| 198 | [打家劫舍](https://leetcode.cn/problems/house-robber/)（[题解](../0101-0200/198_打家劫舍.md)） | 「保留或跳过」的决策结构，一维 DP 入门，对比本题的「保留或替换」 |
