...
title: 建造街区的最短时间
tags: [哈夫曼思想, 堆, 树, 贪心]
knowledge_points: [greedy, heap, tree, 哈夫曼思想]
updated: 2026-09-03
source: leetcode
number: 1199
difficulty: hard
...
# LeetCode 建造街区的最短时间 题解

## 1. 题目概述

- **标题 / 题号**：建造街区的最短时间（#1199，hard）
- **链接**：https://leetcode.cn/problems/minimum-time-to-build-blocks/
- **难度**：困难
- **标签**：贪心、堆、树、哈夫曼思想

**题意**：给定一个数组 `blocks`，其中 `blocks[i] = t` 表示第 `i` 个街区需要 `t` 单位时间建造。**每个街区只能由一个工人建造**。
