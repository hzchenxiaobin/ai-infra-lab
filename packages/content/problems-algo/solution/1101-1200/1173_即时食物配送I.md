...
type: problem
title: "即时食物配送 I"
tags: [AVG, "CASE WHEN", ROUND, SQL, 布尔表达式, 百分比计算, 聚合函数]
knowledge_points: [aggregate-function, avg, case-when, round, sql, 布尔表达式, 百分比计算]
updated: 2026-09-03
source: leetcode
number: 1173
difficulty: easy
...
# LeetCode 即时食物配送 I 题解

## 1. 题目概述

- **标题 / 题号**：即时食物配送 I（#1173，easy）
- **链接**：https://leetcode.cn/problems/immediate-food-delivery-i/
- **难度**：简单
- **标签**：SQL、聚合函数、AVG、CASE WHEN、ROUND、百分比计算、布尔表达式

**题意**：给定 `Delivery` 表，每行记录一笔外卖订单，包含下单日期 `order_date` 和顾客期望的配送日期 `customer_pref_delivery_date`。编写 SQL 查询，找出**即时订单的占比**（即 `order_date = customer_pref_delivery_date` 的订单所占百分比），结果**保留 2 位小数**。
