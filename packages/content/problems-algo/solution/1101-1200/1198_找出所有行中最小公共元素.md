...
title: 找出所有行中最小公共元素
tags: [二分查找, 哈希表, 数组]
knowledge_points: [array, binary-search, hash-table]
updated: 2026-09-03
source: leetcode
number: 1198
difficulty: easy
...
# LeetCode 找出所有行中最小公共元素 题解

## 1. 题目概述

- **标题 / 题号**：找出所有行中最小公共元素（#1198，easy）
- **链接**：https://leetcode.cn/problems/find-smallest-common-element-in-all-rows/
- **难度**：简单
- **标签**：数组、哈希表、二分查找

**题意**：给定一个 `m × n` 的矩阵 `mat`，其中**每一行都已按严格递增顺序排序**。返回所有行中**最小的公共元素**；如果不存在公共元素，返回 `-1`。
