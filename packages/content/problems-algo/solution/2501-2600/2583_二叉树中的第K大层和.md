...
type: problem
title: 二叉树中的第K大层和
tags: [二叉树, 堆, "广度优先搜索（BFS）", 排序, 树]
knowledge_points: [bfs, binary-tree, heap, sorting, tree]
updated: 2026-09-03
source: leetcode
number: 2583
difficulty: medium
...
# LeetCode 二叉树中的第K大层和 题解

## 1. 题目概述

- **标题 / 题号**：二叉树中的第K大层和（#2583，medium）
- **链接**：https://leetcode.cn/problems/kth-largest-sum-in-a-binary-tree/
- **难度**：中等
- **标签**：树、二叉树、广度优先搜索（BFS）、排序、堆

**题意**：给你一棵二叉树的根节点 `root` 和一个正整数 `k`。树中的**层和**是指**同一层**上节点值的总和。返回树中第 `k` 大的层和（不一定不同）。如果树少于 `k` 层，则返回 `-1`。两个节点与根节点距离相同即认为它们在同一层。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/12/14/binaryytreeedrawio-2.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/12/14/treedrawio-3.png)
