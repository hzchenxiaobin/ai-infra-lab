...
type: problem
title: 两个线段获得的最多奖品
tags: [前缀最大值, 动态规划, 双指针, 数组]
knowledge_points: [array, dynamic-programming, two-pointers, 前缀最大值]
updated: 2026-09-03
source: leetcode
number: 2555
difficulty: medium
...
# LeetCode 两个线段获得的最多奖品 题解

## 1. 题目概述

- **标题 / 题号**：两个线段获得的最多奖品（#2555，medium）
- **链接**：https://leetcode.cn/problems/maximize-win-from-two-segments/
- **难度**：中等
- **标签**：数组、双指针、动态规划、前缀最大值

**题意**：在 **X 轴**上有一些奖品。给定按**非递减**顺序排列的整数数组 `prizePositions`，其中 `prizePositions[i]` 是第 $i$ 件奖品的位置（同一位置可有多件）。再给定整数 $k$。
