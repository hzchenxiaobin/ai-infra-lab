...
type: problem
title: 礼盒的最大甜蜜度
tags: [二分查找, 二分答案, 排序, 数组, 贪心]
knowledge_points: [array, binary-search, binary-search-answer, greedy, sorting]
updated: 2026-09-03
source: leetcode
number: 2517
difficulty: medium
...
# LeetCode 礼盒的最大甜蜜度 题解

## 1. 题目概述

- **标题 / 题号**：礼盒的最大甜蜜度（#2517，medium）
- **链接**：https://leetcode.cn/problems/maximum-tastiness-of-candy-basket/
- **难度**：中等
- **标签**：数组、二分查找、二分答案、贪心、排序

**题意**：给定正整数数组 `price`（`price[i]` 为第 `i` 颗糖的价格）和正整数 `k`。商店出售装 `k` 颗**不同**糖果的礼盒，礼盒的**甜蜜度** = 盒内任意两颗糖果价格差的绝对值中的**最小值**。返回能达到的**最大甜蜜度**。
