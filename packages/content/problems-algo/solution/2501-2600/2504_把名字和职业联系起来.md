...
type: problem
title: 把名字和职业联系起来
tags: [CONCAT, SQL, 字符串拼接, 投影, 数据库, 逐行映射]
knowledge_points: [concat, database, sql, string-concat, 字符串拼接, 投影, 逐行映射]
updated: 2026-09-03
source: leetcode
number: 2504
difficulty: easy
...
# LeetCode 把名字和职业联系起来 题解

## 1. 题目概述

- **标题 / 题号**：把名字和职业联系起来（#2504，easy）
- **链接**：https://leetcode.cn/problems/concatenate-the-name-and-the-profession/
- **难度**：简单
- **标签**：数据库、SQL、`CONCAT`、字符串拼接、投影、逐行映射、LeetCode 锁题

**题意**：给定 `Person` 表，每行记录一个人的 `person_id`、`name`（姓名）与 `profession`（职业，取自固定枚举）。编写解决方案，把每个人的**姓名与职业拼接**成一条字符串返回。
