...
id: "lc:2508"
type: problem
title: 添加边使所有节点度数都为偶数
tags: [哈希表, 图, 数学, 贪心]
knowledge_points: [graph, greedy, hash-table, math]
updated: 2026-09-03
source: leetcode
number: 2508
difficulty: hard
...
# LeetCode 添加边使所有节点度数都为偶数 题解

## 1. 题目概述

- **标题 / 题号**：添加边使所有节点度数都为偶数（#2508，hard）
- **链接**：https://leetcode.cn/problems/add-edges-to-make-degrees-of-all-nodes-even/
- **难度**：困难
- **标签**：图、贪心、哈希表、数学

**题意**：给定 $n$ 个节点（编号 $1\sim n$）的**无向**图与边集 `edges`，图中**无重边、无自环**，且不一定连通。你可以**至多**添加 $2$ 条额外边（同样不能产生重边或自环），目标是使**所有节点的度数都为偶数**。能则返回 `true`，否则 `false`。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/10/26/agraphdrawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/10/26/aagraphdrawio.png)
...
![示例3](https://assets.leetcode.com/uploads/2022/10/26/aaagraphdrawio.png)
