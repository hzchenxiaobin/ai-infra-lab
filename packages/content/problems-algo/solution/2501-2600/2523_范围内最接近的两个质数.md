...
title: 范围内最接近的两个质数
tags: [埃氏筛, 数学, 数组, 数论]
knowledge_points: [array, math, number-theory, sieve]
updated: 2026-09-03
source: leetcode
number: 2523
difficulty: medium
...
# LeetCode 范围内最接近的两个质数 题解

## 1. 题目概述

- **标题 / 题号**：范围内最接近的两个质数（#2523，medium）
- **链接**：https://leetcode.cn/problems/closest-prime-numbers-in-range/
- **难度**：中等
- **标签**：数学、数论、埃氏筛、数组

**题意**：给定两个正整数 `left` 和 `right`，在闭区间 $[\text{left}, \text{right}]$ 内找到两个质数 $\text{num1} < \text{num2}$，使得差值 $\text{num2} - \text{num1}$ 是所有满足条件的质数对中**最小的**。若有多个质数对差值相同，返回 $\text{num1}$ 最小的那对。若区间内质数少于 $2$ 个，返回 $[-1, -1]$。
...
> 💡 **识别信号**：题面"范围内取两个、最小化差值" + 数据有序（质数天然升序） $\Rightarrow$ 想"**相邻扫描**"。这与 2517「排序后只看相邻差」、[611. 有效三角形的个数](../0601-0700/611_有效三角形的个数.md)「固定一端 + 双指针」同属"有序区间只看相邻/两端"的化简范式。
...
| 611 | [有效三角形的个数](https://leetcode.cn/problems/valid-triangle-numbers/)（[站内题解](../0601-0700/611_有效三角形的个数.md)） | 有序数组上"固定一端 + 扫相邻/双指针"化简两两枚举，与本题相邻扫描同范式 |
