...
type: problem
title: 最大子序列的分数
tags: ["堆（优先队列）", 排序, 数组, 贪心]
knowledge_points: [array, greedy, heap, sorting]
updated: 2026-09-03
source: leetcode
number: 2542
difficulty: medium
...
# LeetCode 最大子序列的分数 题解

## 1. 题目概述

- **标题 / 题号**：最大子序列的分数（#2542，medium）
- **链接**：https://leetcode.cn/problems/maximum-subsequence-score/
- **难度**：中等
- **标签**：贪心、数组、排序、堆（优先队列）

**题意**：给定两个长度均为 $n$、下标从 $0$ 开始的整数数组 `nums1` 与 `nums2`，以及正整数 $k$。需从 `nums1` 中选一个长度为 $k$ 的**子序列**（即从 $\{0,1,\dots,n-1\}$ 中选 $k$ 个下标 $i_0,i_1,\dots,i_{k-1}$）。分数定义为：
