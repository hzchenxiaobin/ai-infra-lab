...
title: 子字符串异或查询
tags: [位运算, 哈希表, 字符串, 数组]
knowledge_points: [array, bit-manipulation, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 2564
difficulty: medium
...
# LeetCode 子字符串异或查询 题解

## 1. 题目概述

- **标题 / 题号**：子字符串异或查询（#2564，medium）
- **链接**：https://leetcode.cn/problems/substring-xor-queries/
- **难度**：中等
- **标签**：位运算、数组、哈希表、字符串

**题意**：给定一个**二进制字符串** `s` 和一个整数数组 `queries`，其中 `queries[i] = [first_i, second_i]`。对第 `i` 个查询，找到 `s` 的**最短子字符串**，其对应的**十进制**值 `val` 满足 `val ^ first_i == second_i`。返回 `ans[i] = [left_i, right_i]` 为该子字符串的端点（下标从 `0` 开始）；不存在则 `[-1, -1]`；有多个答案时取 `left_i` 最小者。
