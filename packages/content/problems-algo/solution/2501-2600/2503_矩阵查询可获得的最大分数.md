...
type: problem
title: 矩阵查询可获得的最大分数
tags: [图, 堆/优先队列, 并查集, 广度优先搜索, 排序, 离线查询]
knowledge_points: [bfs, graph, heap, offline-queries, sorting, union-find]
updated: 2026-09-03
source: leetcode
number: 2503
difficulty: hard
...
# LeetCode 矩阵查询可获得的最大分数 题解

## 1. 题目概述

- **标题 / 题号**：矩阵查询可获得的最大分数（#2503，hard）
- **链接**：https://leetcode.cn/problems/maximum-number-of-points-from-grid-queries/
- **难度**：困难
- **标签**：图、堆/优先队列、广度优先搜索、离线查询、排序、并查集

**题意**：给你一个 `m x n` 的整数矩阵 `grid` 和一个大小为 `k` 的数组 `queries`。对每个 `queries[i]`，从矩阵**左上角** `(0,0)` 出发，重复以下过程：
...
![示例1](https://assets.leetcode.com/uploads/2025/03/15/image1.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/10/20/yetgriddrawio-2.png)
