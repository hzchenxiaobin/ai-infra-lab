...
id: "lc:2577"
type: problem
title: 在网格图中访问一个格子的最少时间
tags: [Dijkstra, 图, 堆/优先队列, 最短路径, 贪心]
knowledge_points: [dijkstra, graph, greedy, heap, shortest-path]
updated: 2026-09-03
source: leetcode
number: 2577
difficulty: hard
...
# LeetCode 在网格图中访问一个格子的最少时间 题解

## 1. 题目概述

- **标题 / 题号**：在网格图中访问一个格子的最少时间（#2577，hard）
- **链接**：https://leetcode.cn/problems/minimum-time-to-visit-a-cell-in-a-grid/
- **难度**：困难
- **标签**：图、最短路径、Dijkstra、堆/优先队列、贪心

**题意**：给定 `m x n` 的非负矩阵 `grid`，`grid[row][col]` 表示**最早**能访问格子 `(row, col)` 的时刻——即到达它时已经经过的时间必须 $\geq \text{grid[row][col]}$。从**左上角** `(0,0)` 在时刻 `0` 出发，**必须一直移动**到上下左右相邻格子（不能停留），每次移动耗时 `1`。求**最早**到达右下角 `(m-1, n-1)` 的时刻；不可达返回 `-1`。

> ⚠️ 「必须一直移动」是本题的灵魂约束：当邻居阈值未解锁时，不能傻等，得靠**来回弹跳**消耗时间。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2023/02/14/yetgriddrawio-8.png)
...
![示例2](https://assets.leetcode.com/uploads/2023/02/14/yetgriddrawio-9.png)
