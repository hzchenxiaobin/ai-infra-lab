...
title: 将珠子放入背包中
tags: [前缀和, 排序, 数组, 贪心]
knowledge_points: [array, greedy, prefix-sum, sorting]
updated: 2026-09-03
source: leetcode
number: 2551
difficulty: hard
...
# LeetCode 将珠子放入背包中 题解

## 1. 题目概述

- **标题 / 题号**：将珠子放入背包中（#2551，hard）
- **链接**：https://leetcode.cn/problems/put-marbles-in-bags/
- **难度**：困难
- **标签**：贪心、数组、排序、前缀和

**题意**：有 `k` 个背包与一个下标从 $0$ 开始的整数数组 `weights`（`weights[i]` 为第 $i$ 颗珠子的重量）。把所有珠子按原顺序切成 $k$ 段**连续**子数组分别装入 $k$ 个背包，规则：
