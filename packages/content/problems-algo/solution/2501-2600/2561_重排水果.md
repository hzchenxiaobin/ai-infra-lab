...
title: 重排水果
tags: [哈希表, 排序, 数组, 贪心]
knowledge_points: [array, greedy, hash-table, sorting]
updated: 2026-09-03
source: leetcode
number: 2561
difficulty: hard
...
- **标题 / 题号**：重排水果（#2561，hard）
- **链接**：https://leetcode.cn/problems/rearranging-fruits/
- **难度**：困难
- **标签**：贪心、数组、哈希表、排序

**题意**：两个果篮 `basket1` 与 `basket2`，各有 $n$ 个水果，每个水果有一个成本。每次操作选下标 $i$、$j$，交换 `basket1[i]` 与 `basket2[j]`，**交换成本** $=\min(\text{basket1}[i],\text{basket2}[j])$。目标：使两篮排序后完全相同（即两篮作为多重集合相等），求**最小总交换成本**；不可行返回 $-1$。
