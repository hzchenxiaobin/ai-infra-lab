...
type: problem
title: 奖励最顶尖的K名学生
tags: [Top-K, 哈希表, 字符串, 排序]
knowledge_points: [hash-table, sorting, string, top-k]
updated: 2026-09-03
source: leetcode
number: 2512
difficulty: medium
...
# LeetCode 奖励最顶尖的K名学生 题解

## 1. 题目概述

- **标题 / 题号**：奖励最顶尖的K名学生（#2512，medium）
- **链接**：https://leetcode.cn/problems/reward-top-k-students/
- **难度**：中等
- **标签**：哈希表、排序、字符串、Top-K

**题意**：给定两个字符串数组 `positive_feedback` 与 `negative_feedback`，分别是「正面词」与「负面词」集合，二者**无交集**（不会有一个词同时出现在两表中）。每位学生初始分数为 $0$：
