...
type: problem
title: 公平分发饼干
tags: [动态规划, 回溯, 数组, 状态压缩]
knowledge_points: [array, backtracking, bitmask-dp, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 2305
difficulty: medium
...
- **标题 / 题号**：公平分发饼干（#2305，medium）
- **链接**：https://leetcode.cn/problems/fair-distribution-of-cookies/
- **难度**：中等
- **标签**：数组、回溯、状态压缩、动态规划

**题意**：给你一个整数数组 `cookies`，其中 `cookies[i]` 表示第 `i` 个零食包中的饼干数量；另给一个整数 `k` 表示等待分发零食包的孩子数量。**所有**零食包都需要分发，且同一个包里的所有饼干必须整包分给同一个孩子，不能拆分。
