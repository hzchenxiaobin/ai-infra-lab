...
type: problem
title: 从树中删除边的最小分数
tags: [位运算, 无向连通图, 树, 深度优先搜索]
knowledge_points: [bit-manipulation, dfs, tree, 无向连通图]
updated: 2026-09-03
source: leetcode
number: 2322
difficulty: hard
...
# LeetCode 从树中删除边的最小分数 题解

## 1. 题目概述

- **标题 / 题号**：从树中删除边的最小分数（#2322，hard）
- **链接**：https://leetcode.cn/problems/minimum-score-after-removals-on-a-tree/
- **难度**：困难
- **标签**：树、深度优先搜索、位运算、无向连通图

**题意**：给你一棵 $n$ 个节点（编号 $0 \dots n-1$）的**无向连通树**（$n-1$ 条边），每个节点 $i$ 有权值 $\text{nums}[i]$。**删除两条不同的边**，树被切成**三个连通分量**；每个分量取其所有节点权值的**异或值**，该方案的**分数** $=$ 三个异或值里的 $\max - \min$。求所有删边方案中的**最小分数**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/05/03/ex1drawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/05/03/ex2drawio.png)
