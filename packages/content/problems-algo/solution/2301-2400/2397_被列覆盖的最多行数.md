...
type: problem
title: 被列覆盖的最多行数
tags: [二进制枚举, 位运算, 数组, 矩阵]
knowledge_points: [array, bit-manipulation, bitmask-enumeration, matrix, 矩阵]
updated: 2026-09-03
source: leetcode
number: 2397
difficulty: medium
...
# LeetCode 被列覆盖的最多行数 题解

## 1. 题目概述

- **标题 / 题号**：被列覆盖的最多行数（#2397，medium）
- **链接**：https://leetcode.cn/problems/maximum-rows-covered-by-columns/
- **难度**：中等
- **标签**：位运算、二进制枚举、数组、矩阵

**题意**：给定下标从 **0** 开始、大小为 $m \times n$ 的二进制矩阵 `matrix` 和整数 `numSelect`。你需要从 $n$ 列中选出**恰好** `numSelect` 个**不同**的列，记选中的列集合为 $s$。
...
![示例1](https://assets.leetcode.com/uploads/2022/07/14/rowscovered.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/07/14/rowscovered2.png)
