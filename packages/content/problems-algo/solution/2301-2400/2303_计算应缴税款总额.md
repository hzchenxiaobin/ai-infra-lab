...
# LeetCode 计算应缴税款总额 题解

## 1. 题目概述

- **标题 / 题号**：计算应缴税款总额（#2303，easy）
- **链接**：https://leetcode.cn/problems/calculate-amount-paid-in-taxes/
- **难度**：简单
- **标签**：数组、模拟、数学

**题意**：给定下标从 $0$ 开始的二维整数数组 `brackets`，其中 `brackets[i] = [upper_i, percent_i]`，表示第 $i$ 个税级的**上限**为 `upper_i`、**税率**为 `percent_i`。税级按上限**从低到高**严格递增排列（$0 < i$ 时 `upper_{i-1} < upper_i`）。
