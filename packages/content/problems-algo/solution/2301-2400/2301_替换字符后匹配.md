...
type: problem
title: 替换字符后匹配
tags: ["Shift-AND 匹配", 位运算, "位集（bitset）", 字符串]
knowledge_points: [bit-manipulation, shift-and-匹配, string, 位集-bitset]
updated: 2026-09-03
source: leetcode
number: 2301
difficulty: hard
...
- **标题 / 题号**：替换字符后匹配（#2301，hard）
- **链接**：https://leetcode.cn/problems/match-substring-after-replacement/
- **难度**：困难
- **标签**：字符串、位运算、位集（bitset）、Shift-AND 匹配

**题意**：给你两个字符串 `s` 和 `sub`，以及一个二维字符数组 `mappings`，其中 `mappings[i] = [oldi, newi]` 表示你可以将 `sub` 中**任意数目**的 `oldi` 字符替换为 `newi`。`sub` 中每个字符**不能**被替换超过一次。
