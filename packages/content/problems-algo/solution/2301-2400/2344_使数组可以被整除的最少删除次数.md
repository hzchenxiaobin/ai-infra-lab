...
tags: [排序, 数学, 数组, 数论, 贪心]
knowledge_points: [array, greedy, math, number-theory, sorting]
updated: 2026-09-03
source: leetcode
number: 2344
difficulty: hard
...
# LeetCode 使数组可以被整除的最少删除次数 题解

## 1. 题目概述

- **标题 / 题号**：使数组可以被整除的最少删除次数（#2344，hard）
- **链接**：https://leetcode.cn/problems/minimum-deletions-to-make-array-divisible/
- **难度**：困难
- **标签**：数组、数学、数论、排序、贪心

**题意**：给你两个正整数数组 `nums` 和 `numsDivide`。你可以从 `nums` 中删除任意数目的元素。请返回使 `nums` 中**最小**元素可以整除 `numsDivide` 中所有元素的**最少**删除次数。如果无法得到这样的元素，返回 `-1`。
