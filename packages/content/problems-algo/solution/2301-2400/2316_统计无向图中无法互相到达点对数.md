...
tags: [图, 并查集, 广度优先搜索, 深度优先搜索]
knowledge_points: [bfs, dfs, graph, union-find]
updated: 2026-09-03
source: leetcode
number: 2316
difficulty: medium
...
# LeetCode 统计无向图中无法互相到达点对数 题解

## 1. 题目概述

- **标题 / 题号**：统计无向图中无法互相到达点对数（#2316，medium）
- **链接**：https://leetcode.cn/problems/count-unreachable-pairs-of-nodes-in-an-undirected-graph/
- **难度**：中等
- **标签**：图、并查集、深度优先搜索、广度优先搜索

**题意**：给你一个整数 `n`，表示一张**无向图**中有 `n` 个节点，编号为 `0` 到 `n - 1`。同时给你一个二维整数数组 `edges`，其中 `edges[i] = [ai, bi]` 表示节点 `ai` 和 `bi` 之间有一条**无向**边。

请你返回 **无法互相到达** 的不同**点对数目**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/05/05/tc-3.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/05/05/tc-2.png)
