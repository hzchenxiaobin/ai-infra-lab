...
title: 坐上公交的最晚时间
tags: [二分查找, 双指针, 排序]
knowledge_points: [binary-search, sorting, two-pointers]
updated: 2026-09-03
source: leetcode
number: 2332
difficulty: medium
...
# LeetCode 坐上公交的最晚时间 题解

## 1. 题目概述

- **标题 / 题号**：坐上公交的最晚时间（#2332，medium）
- **链接**：https://leetcode.cn/problems/the-latest-time-to-catch-a-bus/
- **难度**：中等
- **标签**：排序、双指针、二分查找

**题意**：给定 `buses[i]`（第 `i` 辆公交的**出发时间**）、`passengers[j]`（第 `j` 位乘客的**到达时间**）和 `capacity`（每辆车最多容纳人数）。所有公交出发时间互不相同，所有乘客到达时间也互不相同。数组**不一定有序**。
