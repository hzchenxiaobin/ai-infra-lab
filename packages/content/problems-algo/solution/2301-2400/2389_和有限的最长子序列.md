...
type: problem
title: 和有限的最长子序列
tags: [二分查找, 前缀和, 排序, 数组, 贪心]
knowledge_points: [array, binary-search, greedy, prefix-sum, sorting]
updated: 2026-09-03
source: leetcode
number: 2389
difficulty: easy
...
# LeetCode 和有限的最长子序列 题解

## 1. 题目概述

- **标题 / 题号**：和有限的最长子序列（#2389，easy）
- **链接**：https://leetcode.cn/problems/longest-subsequence-with-limited-sum/
- **难度**：简单
- **标签**：贪心、数组、排序、前缀和、二分查找

**题意**：给你一个长度为 $n$ 的整数数组 `nums`，和一个长度为 $m$ 的整数数组 `queries`。返回一个长度为 $m$ 的数组 `answer`，其中 `answer[i]` 是 `nums` 中元素之和小于等于 `queries[i]` 的**子序列**的**最大**长度。
