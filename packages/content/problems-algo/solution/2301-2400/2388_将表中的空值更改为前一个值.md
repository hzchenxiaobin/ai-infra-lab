...
type: problem
title: 将表中的空值更改为前一个值
tags: [COUNT/MAX, LOCF, "NULL 处理", SQL, islands-and-gaps, 数据库, 窗口函数]
knowledge_points: [count-max, database, gaps-and-islands, locf, null-handling, sql, window-function]
updated: 2026-09-03
source: leetcode
number: 2388
difficulty: medium
...
# LeetCode 将表中的空值更改为前一个值 题解

## 1. 题目概述

- **标题 / 题号**：将表中的空值更改为前一个值（#2388，medium）
...
**题意**：给定 `CoffeeShop` 表，每行记录一笔订单的 `id` 与所点饮品 `drink`。`drink` 列可能为 `NULL`，语义为「该顾客点了和**前一笔订单**（按 `id` 升序）相同的饮品」。请编写 SQL 查询，把 `drink` 列中的 `NULL` 改为「按 `id` 升序时其前面最近的那个非空 `drink` 值」，并按 `id` 升序返回结果。
