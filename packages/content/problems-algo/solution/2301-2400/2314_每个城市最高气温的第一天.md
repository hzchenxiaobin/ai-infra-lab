...
type: problem
title: 每个城市最高气温的第一天
tags: ["GROUP BY", "JOIN 子查询", "MAX()", "MIN()", "RANK vs ROW_NUMBER", ROW_NUMBER, SQL, 数据库, 窗口函数, 组级最值的再最值]
knowledge_points: [database, group-by, join-子查询, max, min, rank-vs-row-number, row-number, sql, window-function, 组级最值的再最值]
updated: 2026-09-03
source: leetcode
number: 2314
difficulty: medium
...
# LeetCode 每个城市最高气温的第一天 题解

## 1. 题目概述

- **标题 / 题号**：每个城市最高气温的第一天（#2314，medium）
...
**题意**：给定 `Weather` 表，记录各城市若干天的气温。编写 SQL 查询，找出**每个城市出现最高气温的那一天中最早的一天**——即：先确定每座城市的最高气温 $\max(\text{degree})$，再在该城市所有"气温等于最高气温"的记录里取**最早**的 `day`。返回 `city_id` 与 `day` 两列，按 `city_id` 升序排列。
