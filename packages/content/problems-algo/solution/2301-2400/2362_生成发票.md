...
type: problem
title: 生成发票
tags: ["GROUP BY", SQL, "多表 JOIN", 聚合求和]
knowledge_points: [aggregation, group-by, multi-join, sql, 聚合求和]
updated: 2026-09-03
source: leetcode
number: 2362
difficulty: hard
...
- **标题 / 题号**：生成发票（#2362，hard）
- **链接**：https://leetcode.cn/problems/generate-the-invoice/
- **难度**：困难
- **标签**：SQL、多表 JOIN、GROUP BY、聚合求和、LeetCode 锁题

**题意**：给定 `Products`（商品单价表）与 `Purchases`（采购明细表）两张表，编写 SQL 查询，**为每张发票生成一行报告**：该发票的总金额 = 其中所有采购项的「数量 × 单价」之和。
