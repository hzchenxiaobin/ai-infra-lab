...
type: problem
title: "字母移位 II"
tags: [前缀和, 字符串, 差分数组, 数组]
knowledge_points: [array, difference-array, prefix-sum, string]
updated: 2026-09-03
source: leetcode
number: 2381
difficulty: medium
...
- **标题 / 题号**：字母移位 II（#2381，medium）
- **链接**：https://leetcode.cn/problems/shifting-letters-ii/
- **难度**：中等
- **标签**：数组、字符串、差分数组、前缀和

**题意**：给你一个小写英文字母组成的字符串 `s` 和一个二维整数数组 `shifts`，其中 `shifts[i] = [start_i, end_i, direction_i]`。对每个 `i`，把 `s` 中下标从 `start_i` 到 `end_i`（两端都包含）的所有字符移位：`direction_i = 1` 向后移位（用字母表下一个字母替换，`'z'` 变 `'a'`），`direction_i = 0` 向前移位（用前一个字母替换，`'a'` 变 `'z'`）。返回所有移位操作后的最终字符串。
