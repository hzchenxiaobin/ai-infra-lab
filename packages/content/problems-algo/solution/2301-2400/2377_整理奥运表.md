...
type: problem
title: 整理奥运表
tags: ["ORDER BY", SQL, 多键排序, 数据库, "级联 tie-break"]
knowledge_points: [database, order-by, sql, 多键排序, 级联-tie-break]
updated: 2026-09-03
source: leetcode
number: 2377
difficulty: easy
...
- **标题 / 题号**：整理奥运表（#2377，easy）
- **链接**：https://leetcode.cn/problems/sort-the-olympic-table/
- **难度**：简单
- **标签**：数据库、SQL、`ORDER BY`、多键排序、级联 tie-break

**题意**：给定 `Olympic` 表，每行记录某国在本届奥运获得的金、银、铜牌数。编写 SQL 查询，把这张表按奥运奖牌榜惯例排序后返回：**金多者在前；金同则银多者在前；金银同则铜多者在前；金银铜三键全同则按国家名升序**。
