...
type: problem
title: 预算内的最多机器人数目
tags: [单调性优化, 单调队列, 数组, 滑动窗口]
knowledge_points: [array, monotonic-queue, monotonicity, sliding-window]
updated: 2026-09-03
source: leetcode
number: 2398
difficulty: hard
...
# LeetCode 预算内的最多机器人数目 题解

## 1. 题目概述

- **标题 / 题号**：预算内的最多机器人数目（#2398，hard）
- **链接**：https://leetcode.cn/problems/maximum-number-of-robots-within-budget/
- **难度**：困难
- **标签**：数组、滑动窗口、单调队列、单调性优化

**题意**：你有 `n` 个机器人，给定两个长度均为 `n` 的 0 下标整数数组 `chargeTimes` 和 `runningCosts`。第 `i` 个机器人充电需 `chargeTimes[i]`、运行需 `runningCosts[i]`。再给定整数 `budget`。
