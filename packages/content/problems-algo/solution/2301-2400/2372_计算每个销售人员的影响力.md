...
type: problem
title: 计算每个销售人员的影响力
tags: ["COALESCE 零填充", "GROUP BY", "LEFT JOIN", SQL, "多表 JOIN", 聚合求和]
knowledge_points: [aggregation, coalesce-零填充, group-by, left-join, multi-join, sql, 聚合求和]
updated: 2026-09-03
source: leetcode
number: 2372
difficulty: medium
...
- **标题 / 题号**：计算每个销售人员的影响力（#2372，medium）
...
**题意**：给定 `Salesperson`（销售人员表）、`Customer`（客户表）、`Sales`（销售记录表）三张表。每个客户被分配给一名销售人员，每条销售记录归属于一个客户。编写 SQL 查询，**为每一名销售人员计算其"影响力"**：即归属于该销售人员所有客户的销售记录的 `price` 之和。
