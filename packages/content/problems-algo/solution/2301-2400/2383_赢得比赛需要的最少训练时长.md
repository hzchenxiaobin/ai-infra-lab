...
# LeetCode 赢得比赛需要的最少训练时长 题解

## 1. 题目概述

- **标题 / 题号**：赢得比赛需要的最少训练时长（#2383，easy）
- **链接**：https://leetcode.cn/problems/minimum-hours-of-training-to-win-a-competition/
- **难度**：简单
- **标签**：贪心、数组、模拟

**题意**：你正在参加一场比赛，初始精力为 `initialEnergy`、初始经验为 `initialExperience`（均为正整数）。另给两个长度均为 $n$ 的数组 `energy` 与 `experience`，你将**依次**对上 $n$ 个对手：第 $i$ 个对手精力为 `energy[i]`、经验为 `experience[i]`。
