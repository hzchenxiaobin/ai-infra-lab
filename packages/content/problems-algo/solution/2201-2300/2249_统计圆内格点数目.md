...
title: 统计圆内格点数目
tags: [几何, 哈希表, 数组, 枚举]
knowledge_points: [array, enumeration, geometry, hash-table]
updated: 2026-09-03
source: leetcode
number: 2249
difficulty: medium
...
# LeetCode 统计圆内格点数目 题解

## 1. 题目概述

- **标题 / 题号**：统计圆内格点数目（#2249，medium）
- **链接**：https://leetcode.cn/problems/count-lattice-points-inside-a-circle/
- **难度**：中等
- **标签**：几何、数组、枚举、哈希表

**题意**：给你一个二维整数数组 `circles`，其中 `circles[i] = [xi, yi, ri]` 表示第 $i$ 个圆的圆心 $(x_i, y_i)$ 和半径 $r_i$。

返回**至少存在于一个圆内**的**格点**数目。格点是坐标为整数的点；点在圆内或圆上都算「存在于圆内」（即到圆心距离 $\le r_i$）。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/02/exa-11.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/02/exa-22.png)
