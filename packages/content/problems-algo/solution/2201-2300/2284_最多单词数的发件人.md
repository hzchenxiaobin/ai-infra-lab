...
title: 最多单词数的发件人
tags: [哈希表, 字符串, 数组, 计数]
knowledge_points: [array, counting, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 2284
difficulty: medium
...
# LeetCode 最多单词数的发件人 题解

## 1. 题目概述

- **标题 / 题号**：最多单词数的发件人（#2284，medium）
- **链接**：https://leetcode.cn/problems/sender-with-largest-word-count/
- **难度**：中等
- **标签**：数组、哈希表、字符串、计数

**题意**：给你一个聊天记录，共 $n$ 条信息。两个字符串数组 `messages` 和 `senders`，其中 `messages[i]` 是 `senders[i]` 发出的一条信息。一条信息是若干用**单个空格**连接的单词，开头和结尾没有多余空格。发件人的**单词计数**是这个发件人总共发出的单词数（一个发件人可发出多条信息）。
