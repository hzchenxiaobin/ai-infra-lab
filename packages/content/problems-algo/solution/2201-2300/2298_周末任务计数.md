...
type: problem
title: 周末任务计数
tags: [SQL, 数据库, 日期函数, 条件聚合]
knowledge_points: [conditional-aggregation, database, date-function, sql]
updated: 2026-09-03
source: leetcode
number: 2298
difficulty: medium
...
- **标题 / 题号**：周末任务计数（#2298，medium）
- **链接**：https://leetcode.cn/problems/tasks-count-in-the-weekend/
- **难度**：中等
- **标签**：数据库、SQL、日期函数、条件聚合、LeetCode 锁题

**题意**：给定 `Tasks` 表，每行记录一次任务的提交：是谁提交的（`assignee_id`）、何时提交的（`submit_date`）。编写解决方案，统计并返回：
