...
type: problem
title: 树中最接近路径的节点
tags: [广度优先搜索, 数组, 树, 深度优先搜索]
knowledge_points: [array, bfs, dfs, tree]
updated: 2026-09-03
source: leetcode
number: 2277
difficulty: hard
...
# LeetCode 树中最接近路径的节点 题解

## 1. 题目概述

- **标题 / 题号**：树中最接近路径的节点（#2277，hard）
- **链接**：https://leetcode.cn/problems/closest-node-to-path-in-tree/
- **难度**：困难
- **标签**：树、深度优先搜索、广度优先搜索、数组

**题意**：给定正整数 `n`，表示一棵树中的节点数，编号从 `0` 到 `n - 1`。再给定长度为 `n - 1` 的二维数组 `edges`，其中 `edges[i] = [node1_i, node2_i]` 表示树中一条**双向**边。
