...
# LeetCode 统计值等于子树平均值的节点数 题解

## 1. 题目概述

- **标题 / 题号**：统计值等于子树平均值的节点数（#2265，medium）
- **链接**：https://leetcode.cn/problems/count-nodes-equal-to-average-of-subtree/
- **难度**：中等
- **标签**：树、二叉树、深度优先搜索、后序遍历

**题意**：给定一棵二叉树的根节点 `root`，统计其中满足「**节点值等于其子树所有节点值的平均值（向下取整）**」的节点个数。

> **子树平均值**：以某节点为根的子树中，所有节点值之和 `sum` 除以节点数 `n`，**向下取整**（即 `sum / n` 的整数除法）。该节点的子树包含它自身及全部后代。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/15/image-20220315203925-1.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/26/image-20220326133920-1.png)
