...
id: "lc:2247"
type: problem
title: "K 条高速公路的最大旅行费用"
tags: [位掩码, 位运算, 动态规划, 图, 状态压缩]
knowledge_points: [bit-manipulation, bitmask, bitmask-dp, dynamic-programming, graph]
updated: 2026-09-03
source: leetcode
number: 2247
difficulty: hard
...
# LeetCode K 条高速公路的最大旅行费用 题解

## 1. 题目概述

- **标题 / 题号**：K 条高速公路的最大旅行费用（#2247，hard）
- **链接**：https://leetcode.cn/problems/maximum-cost-of-trip-with-k-highways/
- **难度**：困难
- **标签**：位运算、图、动态规划、状态压缩、位掩码

**题意**：一系列高速公路连接从 `0` 到 `n - 1` 的 $n$ 个城市。给定一个二维整数数组 `highways`，其中 `highways[i] = [city1, city2, toll]` 表示一条**双向**高速公路连接 `city1` 与 `city2`，通行费用为 `toll`。
