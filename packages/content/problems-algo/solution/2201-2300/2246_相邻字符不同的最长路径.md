...
type: problem
title: 相邻字符不同的最长路径
tags: [动态规划, 后序遍历, 树, 深度优先搜索]
knowledge_points: [dfs, dynamic-programming, postorder-traversal, tree]
updated: 2026-09-03
source: leetcode
number: 2246
difficulty: hard
...
# LeetCode 相邻字符不同的最长路径 题解

## 1. 题目概述

- **标题 / 题号**：相邻字符不同的最长路径（#2246，hard）
- **链接**：https://leetcode.cn/problems/longest-path-with-different-adjacent-characters/
- **难度**：困难
- **标签**：树、深度优先搜索、动态规划、后序遍历

**题意**：给定一棵以节点 `0` 为根的 **$n$ 节点树**，用 `parent` 数组描述：`parent[i]` 是节点 $i$ 的父节点（`parent[0] = -1` 表示根）。每个节点 $i$ 上有一个字符 `s[i]`。

要求找出树中一条**最长路径**，满足路径上**相邻节点的字符不同**。路径长度 = 路径上的**节点数**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/25/testingdrawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/25/graph2drawio.png)
