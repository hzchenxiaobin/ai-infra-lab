...
id: "lc:2232"
type: problem
title: 向表达式添加括号后的最小结果
tags: [字符串, 数学, 枚举]
knowledge_points: [enumeration, math, string]
updated: 2026-09-03
source: leetcode
number: 2232
difficulty: medium
...
# LeetCode 向表达式添加括号后的最小结果 题解

## 1. 题目概述

- **标题 / 题号**：向表达式添加括号后的最小结果（#2232，medium）
- **链接**：https://leetcode.cn/problems/minimize-result-by-adding-parentheses-to-expression/
- **难度**：中等
- **标签**：字符串、枚举、数学

**题意**：给定下标从 0 开始的字符串 `expression`，格式为 `"<num1>+<num2>"`，其中 `<num1>` 和 `<num2>` 是正整数。请向其中添加**一对括号**，使得：
...
| 131 | [分割回文串](../0101-0200/131_分割回文串.md)（[题解](../0101-0200/131_分割回文串.md)） | 在串上枚举所有切分并校验段性质，回溯枚举切分点的母题 |
