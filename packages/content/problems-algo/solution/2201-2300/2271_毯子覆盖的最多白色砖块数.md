...
type: problem
title: 毯子覆盖的最多白色砖块数
tags: [二分查找, 前缀和, 排序, 数组, 滑动窗口]
knowledge_points: [array, binary-search, prefix-sum, sliding-window, sorting]
updated: 2026-09-03
source: leetcode
number: 2271
difficulty: medium
...
# LeetCode 毯子覆盖的最多白色砖块数 题解

## 1. 题目概述

- **标题 / 题号**：毯子覆盖的最多白色砖块数（#2271，medium）
- **链接**：https://leetcode.cn/problems/maximum-white-tiles-covered-by-a-carpet/
- **难度**：中等
- **标签**：数组、二分查找、前缀和、排序、滑动窗口

**题意**：给定一个二维整数数组 `tiles`，其中 `tiles[i] = [l_i, r_i]` 表示区间 `[l_i, r_i]` 内**每个位置**都被涂成白色（即第 `i` 段白砖覆盖 $r_i - l_i + 1$ 块）。各段白砖**互不重叠**，但给出顺序任意。再给定整数 `carpetLen`，表示一块可放在**任意位置**的毯子的长度。求这块毯子**最多**能盖住多少块白色砖块。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/25/example1drawio3.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/24/example2drawio.png)
