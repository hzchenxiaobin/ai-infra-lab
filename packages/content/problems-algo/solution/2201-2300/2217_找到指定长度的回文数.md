...
title: 找到指定长度的回文数
tags: [数学, 构造]
knowledge_points: [construction, math]
updated: 2026-09-03
source: leetcode
number: 2217
difficulty: medium
...
# LeetCode 找到指定长度的回文数 题解

## 1. 题目概述

- **标题 / 题号**：找到指定长度的回文数（#2217，medium）
- **链接**：https://leetcode.cn/problems/find-palindrome-with-fixed-length/
- **难度**：中等
- **标签**：数学、构造

**题意**：给你一个整数数组 `queries` 和一个**正**整数 `intLength`，请你返回一个数组 `answer`，其中 `answer[i]` 是长度为 `intLength` 的**正回文数**中第 `queries[i]` 小的数字（1-indexed），如果不存在这样的回文数，则为 `-1`。
