...
title: 统计包含每个点的矩形数目
tags: [二分查找, 排序, 数组, 桶]
knowledge_points: [array, binary-search, sorting, 桶]
updated: 2026-09-03
source: leetcode
number: 2250
difficulty: medium
...
# LeetCode 统计包含每个点的矩形数目 题解

## 1. 题目概述

- **标题 / 题号**：统计包含每个点的矩形数目（#2250，medium）
- **链接**：https://leetcode.cn/problems/count-number-of-rectangles-containing-each-point/
- **难度**：中等
- **标签**：数组、排序、二分查找、桶

**题意**：给定 $n$ 个矩形，第 $i$ 个矩形的**左下角**在原点 $(0, 0)$，**右上角**在 $(l_i, h_i)$（含边界）。再给定 $m$ 个点，点 $j$ 的坐标为 $(x_j, y_j)$。

对每个点，统计**包含它的矩形数目**——矩形 $i$ 包含点 $j$ 当且仅当 $0 \le x_j \le l_i$ 且 $0 \le y_j \le h_i$。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/02/example1.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/02/example2.png)
