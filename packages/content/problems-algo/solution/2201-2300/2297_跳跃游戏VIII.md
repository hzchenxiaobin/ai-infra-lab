...
type: problem
title: "跳跃游戏 VIII"
tags: [动态规划, 单调栈, 图, 数组, 最短路, 栈]
knowledge_points: [array, dynamic-programming, graph, monotonic-stack, shortest-path, stack]
updated: 2026-09-03
source: leetcode
number: 2297
difficulty: medium
...
# LeetCode 跳跃游戏 VIII 题解

## 1. 题目概述

- **标题 / 题号**：跳跃游戏 VIII（#2297，medium）
- **链接**：https://leetcode.cn/problems/jump-game-viii/
- **难度**：中等
- **标签**：栈、图、数组、动态规划、最短路、单调栈

**题意**：给定一个长度为 $n$ 的下标从 $0$ 开始的整数数组 `nums`，初始位置在下标 $0$。当 $i < j$ 时，你可以从下标 $i$ 跳转到下标 $j$，需满足以下两种情况之一：
