...
type: problem
title: "7 天内两次购买的用户"
tags: [DATEDIFF, SQL, 去重, 数据库, 日期差, "窗口函数 LAG", 自连接]
knowledge_points: [database, datediff, dedup, self-join, sql, window-lag, 日期差, 窗口函数-lag]
updated: 2026-09-03
source: leetcode
number: 2228
difficulty: medium
...
# LeetCode 7 天内两次购买的用户 题解

## 1. 题目概述

- **标题 / 题号**：7 天内两次购买的用户（#2228，medium）
...
**题意**：给定 `Purchases` 表，每行记录某用户在某日期的一次购买。编写解决方案，报告在**任意 7 天时间段内**至少进行过**两次**购买的用户。按 `user_id` 升序返回结果。
