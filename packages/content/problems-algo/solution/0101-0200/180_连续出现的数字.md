...
type: problem
title: 连续出现的数字
tags: [LAG, SQL, 窗口函数, 自连接]
knowledge_points: [lag, self-join, sql, window-function]
updated: 2026-09-03
source: leetcode
number: 180
difficulty: medium
...
- **标题 / 题号**：连续出现的数字（#180，medium）
- **链接**：https://leetcode.cn/problems/consecutive-numbers/
- **难度**：中等
- **标签**：SQL、自连接、窗口函数、LAG

**题意**：给定一张 `Logs` 表，编写 SQL 查询，找出**至少连续出现三次**的所有数字。返回结果表，列名为 `ConsecutiveNums`，结果可按任意顺序返回。
