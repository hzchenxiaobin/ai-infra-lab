...
# LeetCode 三角形最小路径和 题解

## 1. 题目概述

- **标题 / 题号**：三角形最小路径和（#120，medium）
- **链接**：https://leetcode.cn/problems/triangle/
- **难度**：中等
- **标签**：数组、动态规划、网格 DP

**题意**：给定一个非空二维数组 `triangle`，第 `i` 行有 `i+1` 个元素。自顶向下走，每一步只能移动到**下一行相邻**的格子：从 `(i, j)` 可以走到 `(i+1, j)`（正下方）或 `(i+1, j+1)`（右下方）。求自顶向下的**最小路径和**。
