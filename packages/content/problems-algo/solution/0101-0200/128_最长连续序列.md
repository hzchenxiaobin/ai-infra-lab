...
title: 最长连续序列
tags: [哈希表, 并查集, 数组]
knowledge_points: [array, hash-table, union-find]
updated: 2026-09-03
source: leetcode
number: 128
difficulty: medium
...
judge: none
related_learn: ["learn:w01d01", "learn:w01d07", "learn:w07d01"]
---
...
- **标题 / 题号**：最长连续序列（#128，medium）
- **链接**：https://leetcode.cn/problems/longest-consecutive-sequence/
- **难度**：中等
- **标签**：哈希表、数组、并查集

**题意**：给定一个未排序的整数数组 `nums`，找出数字连续的最长序列的长度。要求算法时间复杂度为 `O(n)`。
...
> 💡 与 [Week7 Day1 并发引擎](../../../learn/daily/week7/day1/README.md) 的 request_id 管理同构——引擎用 dict（哈希表）`O(1)` 索引 `running[request_id]`，正如集合 `O(1)` 查元素存在性。两者都是用哈希实现 `O(1)` 查找的核心模式。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 128 | [最长连续序列](https://leetcode.cn/problems/longest-consecutive-sequence/)（题解） | 哈希集合 |
