...
type: problem
title: 超过经理收入的员工
tags: [SQL, "Self JOIN", 相关子查询]
knowledge_points: [correlated-subquery, self-join, sql]
updated: 2026-09-03
source: leetcode
number: 181
difficulty: easy
...
# LeetCode 超过经理收入的员工 题解

## 1. 题目概述

- **标题 / 题号**：超过经理收入的员工（#181，easy）
- **链接**：https://leetcode.cn/problems/employees-earning-more-than-their-managers/
- **难度**：简单
- **标签**：SQL、Self JOIN、相关子查询

**题意**：给定一张 `Employee` 表，表中的每一行包含员工的 `id`、`name`、`salary` 以及其经理的 `managerId`。要求编写 SQL 查询，找出**薪资严格高于其直属经理**的员工姓名。结果列名为 `Employee`，可按任意顺序返回。
