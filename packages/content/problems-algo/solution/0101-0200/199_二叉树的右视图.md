...
- **标题 / 题号**：二叉树的右视图（#199，medium）
- **链接**：https://leetcode.cn/problems/binary-tree-right-side-view/
- **难度**：中等
- **标签**：树、BFS、DFS

**题意**：给定一棵二叉树的根节点 `root`，想象自己站在树的**右侧**，按**从顶到底**的顺序，返回能看到的节点值组成的列表。

> 站在右侧看，每层只能看到该层**最右**的节点——左边的节点被右边的挡住了。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2024/11/24/tmpd5jn43fs-1.png)
...
![示例2](https://assets.leetcode.com/uploads/2024/11/24/tmpkpe40xeh-1.png)
...
> 💡 这是 [102. 层序遍历](102_二叉树的层序遍历.md) 的直接应用——每层只取最右节点。它同时可用 BFS（层序取最后）和 DFS（根右左首达即记录）两种解法，是「同一问题两种遍历视角」的典型题。掌握它，你就理解了「层序遍历的变种」与「DFS 按层记录」的等价性。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 102 | [二叉树的层序遍历](https://leetcode.cn/problems/binary-tree-level-order-traversal/) | BFS 骨架，本题基础（[题解](102_二叉树的层序遍历.md)） |
| 199 | [二叉树的右视图](https://leetcode.cn/problems/binary-tree-right-side-view/)（题解） | 本题，层序取末尾 |
| 513 | [找树左下角的值](https://leetcode.cn/problems/find-bottom-left-tree-value/) | BFS 最底层第一个 |
| 116 | [填充每个节点的下一个右侧节点指针](https://leetcode.cn/problems/populating-next-right-pointers-in-each-node/)（[题解](116_填充每个节点的下一个右侧节点指针.md)） | 层序思想，同层串联 |
