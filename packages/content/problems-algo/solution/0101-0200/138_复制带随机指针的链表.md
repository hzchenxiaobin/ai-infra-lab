...
title: 复制带随机指针的链表
tags: [哈希表, 链表]
knowledge_points: [hash-table, linked-list]
updated: 2026-09-03
source: leetcode
number: 138
difficulty: medium
...
- **标题 / 题号**：复制带随机指针的链表（#138，medium）
- **链接**：https://leetcode.cn/problems/copy-list-with-random-pointer/
- **难度**：中等
- **标签**：链表、哈希表

**题意**：给定一个长度为 `n` 的链表，每个节点除了 `next` 指针外还有一个 `random` 指针，可指向链表中任意节点或 `null`。请**深拷贝**整个链表——构造一个全新链表，节点关系（next + random）与原链表完全一致，返回新链表的头节点。
...
| 138 | [复制带随机指针的链表](https://leetcode.cn/problems/copy-list-with-random-pointer/)（题解） | 哈希映射 |
