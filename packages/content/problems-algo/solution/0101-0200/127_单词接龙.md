...
type: problem
title: 单词接龙
tags: ["双向 BFS", 哈希表, 字符串, 广度优先搜索]
knowledge_points: [bfs, bidirectional-bfs, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 127
difficulty: hard
...
**题意**：给定两个单词 `beginWord` 和 `endWord`，以及一个词典 `wordList`。找到从 `beginWord` 到 `endWord` 的**最短转换序列**的长度，规则：
