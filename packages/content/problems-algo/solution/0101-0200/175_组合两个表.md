...
title: 组合两个表
tags: [JOIN, SQL, 数据库]
knowledge_points: [database, join, sql]
updated: 2026-09-03
source: leetcode
number: 175
difficulty: easy
...
- **标题 / 题号**：组合两个表（#175，easy）
- **链接**：https://leetcode.cn/problems/combine-two-tables/
- **难度**：简单
- **标签**：SQL、JOIN、数据库

**题意**：给定两张表 `Person`（人员）和 `Address`（地址），要求编写 SQL 查询，报告**每个人的** `firstName`、`lastName`、`city`、`state`。如果某人在 `Address` 表中没有地址记录，则 `city` 和 `state` 报告为 `NULL`。
