...
- **标题 / 题号**：环形链表 II（#142，medium）
- **链接**：https://leetcode.cn/problems/linked-list-cycle-ii/
- **难度**：中等
- **标签**：链表、双指针

**题意**：给定一个链表的头节点 `head`，返回链表**开始入环的第一个节点**。如果链表无环，则返回 `null`。
...
> 💡 这是 [141. 环形链表](141_环形链表.md) 的进阶版。141 只问"**有没有环**"，本题问"**环入口在哪**"。同一套 Floyd 快慢指针，但需要多走一个"第二阶段"——把数学关系 `a = (k-1)(b+c) + c` 翻译成代码。一旦想通这个等式，本题就是 141 的"加两行"。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 141 | [环形链表](https://leetcode.cn/problems/linked-list-cycle/)（[题解](141_环形链表.md)） | 本题前置题，只判环不找入口，同一套快慢指针阶段一 |
