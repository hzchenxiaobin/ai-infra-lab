...
title: 分数到小数
tags: [哈希表, 字符串, 数学, 模拟]
knowledge_points: [hash-table, math, simulation, string]
updated: 2026-09-03
source: leetcode
number: 166
difficulty: medium
...
- **标题 / 题号**：分数到小数（#166，medium）
- **链接**：https://leetcode.cn/problems/fraction-to-recurring-decimal/
- **难度**：中等
- **标签**：哈希表、数学、字符串、模拟

**题意**：给定两个整数 `numerator`（分子）和 `denominator`（分母），以**字符串**形式返回对应的小数。如果小数部分**循环**，则把循环节用括号括起来；如果存在多种答案，返回任意一个即可。
