...
# LeetCode 求根节点到叶节点数字之和 题解

## 1. 题目概述

- **标题 / 题号**：求根节点到叶节点数字之和（#129，medium）
- **链接**：https://leetcode.cn/problems/sum-root-to-leaf-numbers/
- **难度**：中等
- **标签**：树、深度优先搜索、二叉树

**题意**：给定一棵二叉树的根节点 `root`，树上每个节点存放一位数字 `0`~`9`。每条「从根节点到叶子节点」的路径代表一个数字：把路径上从根到叶的各节点值**按顺序拼接**成一个整数。求所有根到叶路径所表示的**数字之和**。
