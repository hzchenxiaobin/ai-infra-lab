...
type: problem
title: 行程和用户
tags: ["CASE WHEN", SQL, "多表 JOIN", 聚合]
knowledge_points: [aggregation, case-when, multi-join, sql]
updated: 2026-09-03
source: leetcode
number: 262
difficulty: hard
...
- **标题 / 题号**：行程和用户（#262，hard）
- **链接**：https://leetcode.cn/problems/trips-and-users/
- **难度**：困难
- **标签**：SQL、多表 JOIN、聚合、CASE WHEN

**题意**：给定 `Trips`（行程）和 `Users`（用户）两张表，编写 SQL 查询，找出 **2013-10-01 至 2013-10-03** 期间，**客户端和司机端均未被封禁** 的行程中，每天的 **取消率**（取消行程数 / 有效行程总数），结果保留两位小数。
