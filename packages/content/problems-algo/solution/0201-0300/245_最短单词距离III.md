...
type: problem
title: 最短单词距离III
tags: [单遍扫描, 双指针, 字符串, 数组]
knowledge_points: [array, one-pass, string, two-pointers, 单遍扫描]
updated: 2026-09-03
source: leetcode
number: 245
difficulty: medium
...
# LeetCode 最短单词距离III 题解

## 1. 题目概述

- **标题 / 题号**：最短单词距离III（#245，medium）
- **链接**：https://leetcode.cn/problems/shortest-word-distance-iii/
- **难度**：中等
- **标签**：数组、字符串、双指针、单遍扫描

**题意**：给定一个字符串数组 `wordsDict` 和两个字符串 `word1`、`word2`（二者都保证出现在数组中），返回这两个单词在数组中的**最短距离**——即各自出现位置下标之差的绝对值的最小值 $|i - j|$。
