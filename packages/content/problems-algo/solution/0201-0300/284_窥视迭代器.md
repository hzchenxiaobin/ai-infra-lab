...
title: 窥视迭代器
tags: [装饰器模式, 设计, 迭代器]
knowledge_points: [decorator-pattern, design, iterator]
updated: 2026-09-03
source: leetcode
number: 284
difficulty: medium
...
- **标题 / 题号**：窥视迭代器（#284，medium）
- **链接**：https://leetcode.cn/problems/peeking-iterator/
- **难度**：中等
- **标签**：设计、迭代器、装饰器模式

**题意**：给定一个已有的整数迭代器 `Iterator`（支持 `next()` 与 `hasNext()`），请设计一个 `PeekingIterator`，在原有功能基础上**额外支持 `peek()` 操作**——返回下一个元素但**不移动指针**。实现以下接口：
...
> ⚠️ **这就是装饰器模式**（Decorator Pattern）：`PeekingIterator` 包裹一个 `Iterator`，在不修改原迭代器的前提下「贴」上一层 `peek` 能力。缓存就是这层「装饰」的全部成本——仅 $O(1)$ 空间。这个模式与 [232. 用栈实现队列](./232_用栈实现队列.md) 的「用一种结构模拟另一种语义」同属「包装 + 转发」家族，区别在于装饰器不改变原有接口，只**增加**新接口。
...
| 232 | [用栈实现队列](./232_用栈实现队列.md) | 用栈模拟队列的倒换，均摊 $O(1)$ 分析的经典题，与本题「用缓存模拟 peek」同属「用一种结构包装另一种语义」家族 |
