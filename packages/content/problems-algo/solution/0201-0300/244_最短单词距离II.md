...
title: 最短单词距离II
tags: [双指针, 哈希表, 数组, 设计]
knowledge_points: [array, design, hash-table, two-pointers]
updated: 2026-09-03
source: leetcode
number: 244
difficulty: medium
...
| 981 | [基于时间的键值存储](https://leetcode.cn/problems/time-based-key-value-store/)（[题解](../0901-1000/981_基于时间的键值存储.md)） | 同为「设计题 + 预处理哈希表」范式——`HashMap<key, 有序数组>` + 二分查找，对照本题的 `HashMap<word, 有序下标列表>` + 双指针归并，预处理结构相同、查询策略因问题而异 |
