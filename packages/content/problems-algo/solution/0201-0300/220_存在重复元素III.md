...
type: problem
title: "存在重复元素 III"
tags: [哈希表, 数组, 桶排序, 滑动窗口]
knowledge_points: [array, bucket-sort, hash-table, sliding-window]
updated: 2026-09-03
source: leetcode
number: 220
difficulty: hard
...
# LeetCode 存在重复元素 III 题解

## 1. 题目概述

- **标题 / 题号**：存在重复元素 III（#220，hard）
- **链接**：https://leetcode.cn/problems/contains-duplicate-iii/
- **难度**：困难
- **标签**：数组、哈希表、滑动窗口、桶排序

**题意**：给定一个整数数组 `nums` 和两个整数 `indexDiff` 与 `valueDiff`，判断是否存在**两个不同下标** `i` 和 `j`，使得 $\text{abs}(i - j) \leq \text{indexDiff}$ 且 $\text{abs}(\text{nums}[i] - \text{nums}[j]) \leq \text{valueDiff}$。存在则返回 `true`，否则返回 `false`。
