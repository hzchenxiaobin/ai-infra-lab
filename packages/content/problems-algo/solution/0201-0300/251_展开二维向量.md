...
title: 展开二维向量
tags: [双指针, 数组, 设计, 迭代器]
knowledge_points: [array, design, iterator, two-pointers]
updated: 2026-09-03
source: leetcode
number: 251
difficulty: medium
...
5. **均摊 $O(1)$ 的分析依据是什么？**
   ...
   > `hasNext()` 中的 `row++` 是不可逆操作——`row` 从 0 单调递增到 $n$，总共最多 $n$ 次。设总共调用 $k$ 次 `next()`/`hasNext()`，所有 `row++` 的总次数不超过 $n$，分摊到 $k$ 次调用，每次均摊 $O(n/k)$。当 $k$ 与 $n$ 同阶时为 $O(1)$。这与 [232. 用栈实现队列](./232_用栈实现队列.md) 的双栈倒换均摊分析一脉相承。
...
| 232 | [用栈实现队列](./232_用栈实现队列.md) | 均摊 $O(1)$ 分析的经典题，与本题的 `row++` 均摊同属「摊还分析」家族 |
