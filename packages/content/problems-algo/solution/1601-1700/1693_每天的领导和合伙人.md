...
type: problem
title: 每天的领导和合伙人
tags: ["COUNT(DISTINCT)", "GROUP BY", SQL, 单表查询, 去重计数, 多列分组, 数据库]
knowledge_points: [count-distinct, database, distinct-count, group-by, single-table-query, sql, 单表查询, 多列分组]
updated: 2026-09-03
source: leetcode
number: 1693
difficulty: easy
...
# LeetCode 每天的领导和合伙人 题解

## 1. 题目概述

- **标题 / 题号**：每天的领导和合伙人（#1693，easy）
- **链接**：https://leetcode.cn/problems/daily-leads-and-partners/
- **难度**：简单
- **标签**：数据库、SQL、`GROUP BY`、`COUNT(DISTINCT)`、多列分组、去重计数、单表查询

**题意**：给定 `DailySales`（每日销售）表，编写 SQL 查询，对于每个 `date_id` 和 `make_name` 的组合，统计**不同**（distinct）`lead_id` 的数量和**不同** `partner_id` 的数量。以任意顺序返回结果。
...
| 1484 | [按日期分组销售产品](https://leetcode.cn/problems/group-sold-products-by-the-date/)（[题解](../1401-1500/1484_按日期分组销售产品.md)） | `GROUP BY` + `COUNT(DISTINCT)` + `GROUP_CONCAT(DISTINCT)`——单列分组 + 去重计数 + 去重拼接，与 1693 共享"`COUNT(DISTINCT)` 去重计数"骨架，但多了字符串聚合维度 |
