...
title: 使字符串平衡的最少删除次数
tags: [前缀和, 动态规划, 字符串]
knowledge_points: [dynamic-programming, prefix-sum, string]
updated: 2026-09-03
source: leetcode
number: 1653
difficulty: medium
...
# LeetCode 使字符串平衡的最少删除次数 题解

## 1. 题目概述

- **标题 / 题号**：使字符串平衡的最少删除次数（#1653，medium）
- **链接**：https://leetcode.cn/problems/minimum-deletions-to-make-string-balanced/
- **难度**：中等
- **标签**：字符串、动态规划、前缀和

**题意**：给定一个**只含 `'a'` 和 `'b'`** 的字符串 `s`，你可以删除任意数目的字符，使 `s` 变成**平衡**的。所谓平衡，是指不存在下标对 $(i, j)$ 满足 $i < j$ 且 `s[i] = 'b'`、`s[j] = 'a'`——换句话说，**所有 `'a'` 都在所有 `'b'` 之前**，即串形如 `aaa...bbb...`。返回使 `s` 平衡的**最少删除次数**。
