...
type: problem
title: 找到处理最多请求的服务器
tags: ["堆（优先队列）", 数组, 有序集合, 模拟]
knowledge_points: [array, heap, ordered-set, simulation]
updated: 2026-09-03
source: leetcode
number: 1606
difficulty: hard
...
# LeetCode 找到处理最多请求的服务器 题解

## 1. 题目概述

- **标题 / 题号**：找到处理最多请求的服务器（#1606，hard）
- **链接**：https://leetcode.cn/problems/find-servers-that-handled-most-number-of-requests/
- **难度**：困难
- **标签**：数组、有序集合、模拟、堆（优先队列）

**题意**：有 `k` 个服务器，编号 `0` 到 `k-1`，每个服务器计算能力无穷但**同一时刻只能处理一个请求**。第 `i` 个请求在 `arrival[i]` 时刻到达，需要 `load[i]` 的时间来完成。请求分配规则：
...
![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2020/10/03/load-1.png)
