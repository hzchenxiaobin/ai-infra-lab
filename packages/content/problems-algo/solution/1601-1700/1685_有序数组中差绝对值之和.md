...
title: 有序数组中差绝对值之和
tags: [前缀和, 数学, 数组]
knowledge_points: [array, math, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 1685
difficulty: medium
...
# LeetCode 有序数组中差绝对值之和 题解

## 1. 题目概述

- **标题 / 题号**：有序数组中差绝对值之和（#1685，medium）
- **链接**：https://leetcode.cn/problems/sum-of-absolute-differences-in-a-sorted-array/
- **难度**：中等
- **标签**：数组、前缀和、数学

**题意**：给你一个 **非递减** 有序整数数组 `nums`。请建立并返回一个整数数组 `result`，它跟 `nums` 长度相同，且 `result[i]` 等于 `nums[i]` 与数组中所有其他元素差的绝对值之和：
...
| 303 | [区域和检索 - 数组不可变](https://leetcode.cn/problems/range-sum-query-immutable/)（[题解](../0301-0400/303_区域和检索%20-%20数组不可变.md)） | 前缀和基础模板，巩固 $\text{sum}(l, r) = \text{prefix}[r+1] - \text{prefix}[l]$ 的用法 |
