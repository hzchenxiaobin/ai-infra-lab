...
tags: [哈希表, 字符串, 广度优先搜索]
knowledge_points: [bfs, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 1625
difficulty: medium
...
# LeetCode 执行操作后字典序最小的字符串 题解

## 1. 题目概述

- **标题 / 题号**：执行操作后字典序最小的字符串（#1625，medium）
- **链接**：https://leetcode.cn/problems/lexicographically-smallest-string-after-applying-operations/
- **难度**：中等
- **标签**：广度优先搜索、字符串、哈希表

**题意**：给你一个字符串 `s` 以及两个整数 `a` 和 `b`。其中，字符串 `s` 的长度为偶数，且仅由数字 `0` 到 `9` 组成。
