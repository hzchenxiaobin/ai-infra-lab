...
tags: [二叉树, 双指针, 哈希表, 树]
knowledge_points: [binary-tree, hash-table, tree, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1650
difficulty: medium
...
**公共祖先 / 最近公共祖先**的定义与 [236. 二叉树的最近公共祖先](../0201-0300/236_二叉树的最近公共祖先.md) 一致：若节点 `x` 是 `p` 的祖先（`p` 在 `x` 的子树中，含 `p == x`）且是 `q` 的祖先，则 `x` 是公共祖先；**最近**指深度最大的那个。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 236 | [二叉树的最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/)（[题解](../0201-0300/236_二叉树的最近公共祖先.md)） | 无 parent 指针的母题，后序 DFS，对照本题的双指针 |
