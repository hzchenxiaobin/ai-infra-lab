...
type: problem
title: 各赛事的用户注册率
tags: [COUNT, "GROUP BY", "ORDER BY 多列排序", ROUND, SQL, 数据库, 整数除法陷阱, 标量子查询]
knowledge_points: [count, database, group-by, order-by-多列排序, round, sql, 整数除法陷阱, 标量子查询]
updated: 2026-09-03
source: leetcode
number: 1633
difficulty: easy
...
# LeetCode 各赛事的用户注册率 题解

## 1. 题目概述

- **标题 / 题号**：各赛事的用户注册率（#1633，easy）
...
**题意**：给定 `Users`（用户）和 `Register`（注册）两张表，编写 SQL 查询，统计**每个赛事**的**用户注册百分率**（注册该赛事的用户数 ÷ 总用户数 × 100），结果保留两位小数。返回结果按 `percentage` **降序**排序，若相同则按 `contest_id` **升序**排序。
