...
type: problem
title: 每台机器的进程平均运行时间
tags: [AVG, "GROUP BY", ROUND, SQL, 数据库, "条件聚合（CASE/MAX）", "自连接（self-join）", 配对行折叠]
knowledge_points: [avg, database, group-by, round, self-join, sql, 条件聚合-case-max, 配对行折叠]
updated: 2026-09-03
source: leetcode
number: 1661
difficulty: easy
...
# LeetCode 每台机器的进程平均运行时间 题解

## 1. 题目概述

- **标题 / 题号**：每台机器的进程平均运行时间（#1661，easy）
...
**题意**：给定 `Activity`（活动）表，记录工厂中每台机器上每个进程的「开始 / 结束」时间戳。同一台机器、同一个进程一定成对出现一条 `start` 和一条 `end`，且 `start` 时间戳严格小于 `end`。每台机器上运行的进程数相同。编写 SQL 查询，计算**每台机器**完成一个进程的**平均耗时**（`end` 时间戳 − `start` 时间戳的平均值），结果四舍五入保留 3 位小数，返回 `machine_id` 与 `processing_time`。
