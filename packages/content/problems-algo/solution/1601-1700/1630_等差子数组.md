...
title: 等差子数组
tags: [排序, 数组, 枚举]
knowledge_points: [array, enumeration, sorting]
updated: 2026-09-03
source: leetcode
number: 1630
difficulty: medium
...
- **标题 / 题号**：等差子数组（#1630，medium）
- **链接**：https://leetcode.cn/problems/arithmetic-subarrays/
- **难度**：中等
- **标签**：数组、排序、枚举

**题意**：如果一个数列由至少两个元素组成，且任意相邻两项之差恒为同一个常数，则称该数列为**等差数列**。更形式化地，序列 $s$ 为等差数列当且仅当 $s[i+1] - s[i] = s[1] - s[0]$ 对所有合法 $i$ 成立。
