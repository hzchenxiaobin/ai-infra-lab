...
type: problem
title: 发票中的产品金额
tags: ["GROUP BY", IFNULL/COALESCE, JOIN, "LEFT JOIN vs INNER JOIN", "ORDER BY", SQL, "SUM 聚合", 数据库]
knowledge_points: [database, group-by, ifnull-coalesce, join, left-join-vs-inner-join, null-coalescing, order-by, sql, sum-聚合]
updated: 2026-09-03
source: leetcode
number: 1677
difficulty: easy
...
# LeetCode 发票中的产品金额 题解

## 1. 题目概述

- **标题 / 题号**：发票中的产品金额（#1677，easy）
...
**题意**：给定 `Product`（产品）与 `Invoice`（发票）两张表，编写 SQL 查询，对**所有产品**返回每个产品的产品名称，以及全部发票累计的**总应缴款项**、**总已支付金额**、**总已取消金额**、**总已退款金额**。结果按 `product_name`（即 `name`）排序。
