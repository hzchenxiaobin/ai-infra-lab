...
type: problem
title: 纠正二叉树
tags: [二叉树, 哈希表, "广度优先搜索（BFS）", 树]
knowledge_points: [bfs, binary-tree, hash-table, tree]
updated: 2026-09-03
source: leetcode
number: 1660
difficulty: medium
...
- **标题 / 题号**：纠正二叉树（#1660，medium）
- **链接**：https://leetcode.cn/problems/correct-a-binary-tree/
- **难度**：中等
- **标签**：树、二叉树、广度优先搜索（BFS）、哈希表

**题意**：给定一棵二叉树的根节点 `root`，树中存在**一个错误节点** `u`：它的 `right` 指针被错误地设置为指向树中另一个与 `u` **同一深度**的节点 `v`（而非 `u` 的下一层子节点）。这违反了二叉树的结构约束——子节点应比父节点深一层。
