...
type: problem
title: 最小不兼容性
tags: [位运算, 动态规划, 子集枚举, 状态压缩]
knowledge_points: [bit-manipulation, bitmask-dp, dynamic-programming, subset-enumeration]
updated: 2026-09-03
source: leetcode
number: 1681
difficulty: hard
...
- **标题 / 题号**：最小不兼容性（#1681，hard）
- **链接**：https://leetcode.cn/problems/minimum-incompatibility/
- **难度**：困难
- **标签**：动态规划、状态压缩、位运算、子集枚举

**题意**：给定一个整数数组 `nums` 和一个整数 `k`，需要将数组划分为 `k` 个**大小相同**的子集，使得同一子集内**没有两个相同的元素**。每个子集的**不兼容性**定义为该子集中最大值与最小值之差。求所有子集不兼容性之和的**最小值**，若无法划分则返回 `-1`。
