...
title: 字符频次唯一的最小删除次数
tags: [哈希表, 排序, 贪心]
knowledge_points: [greedy, hash-table, sorting]
updated: 2026-09-03
source: leetcode
number: 1647
difficulty: medium
...
# LeetCode 字符频次唯一的最小删除次数 题解

## 1. 题目概述

- **标题 / 题号**：字符频次唯一的最小删除次数（#1647，medium）
- **链接**：https://leetcode.cn/problems/minimum-deletions-to-make-character-frequencies-unique/
- **难度**：中等
- **标签**：贪心、哈希表、排序

**题意**：给定一个字符串 `s`，如果 `s` 中**没有两个不同字符的出现次数相同**，则称 `s` 是「好字符串」。每次操作可以删除 `s` 中的一个字符，问最少需要删除多少个字符才能使 `s` 变为好字符串。
