...
type: problem
title: 最长乘积等价子数组
tags: [数学, 数组, 数论, 枚举, 滑动窗口]
knowledge_points: [array, enumeration, math, number-theory, sliding-window]
updated: 2026-09-03
source: leetcode
number: 3411
difficulty: easy
...
# LeetCode 最长乘积等价子数组 题解

## 1. 题目概述

- **标题 / 题号**：最长乘积等价子数组（#3411，easy）
- **链接**：https://leetcode.cn/problems/maximum-subarray-with-equal-products/
- **难度**：简单
- **标签**：数组、数学、数论、枚举、滑动窗口

**题意**：给你一个由**正整数**组成的数组 `nums`。如果一个数组 `arr` 满足 $\prod(\text{arr}) = \mathrm{lcm}(\text{arr}) \times \gcd(\text{arr})$，则称其为**乘积等价数组**，其中：
