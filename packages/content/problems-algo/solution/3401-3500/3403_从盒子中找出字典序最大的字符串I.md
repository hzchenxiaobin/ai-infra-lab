...
id: "lc:3403"
type: problem
title: "从盒子中找出字典序最大的字符串 I"
tags: [字符串, 枚举, 贪心]
knowledge_points: [enumeration, greedy, string]
updated: 2026-09-03
source: leetcode
number: 3403
difficulty: medium
...
- **标题 / 题号**：从盒子中找出字典序最大的字符串 I（#3403，medium）
- **链接**：https://leetcode.cn/problems/find-the-lexicographically-largest-string-from-the-box-i/
- **难度**：中等
- **标签**：字符串、枚举、贪心

**题意**：给定字符串 `word` 和整数 `numFriends`。游戏进行多个回合，每一回合把 `word` 分割成 `numFriends` 个**非空**字符串（每种分割方式与之前所有回合都不完全相同），分割出的所有字符串都放入一个盒子。所有回合结束后，返回盒子中**字典序最大**的字符串。
