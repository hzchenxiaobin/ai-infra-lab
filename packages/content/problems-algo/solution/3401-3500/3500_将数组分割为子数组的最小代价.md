...
id: "lc:3500"
type: problem
title: 将数组分割为子数组的最小代价
tags: [前缀和, 动态规划, 数组, 李超线段树]
knowledge_points: [array, dynamic-programming, li-chao-tree, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 3500
difficulty: hard
...
# LeetCode 将数组分割为子数组的最小代价 题解

## 1. 题目概述

- **标题 / 题号**：将数组分割为子数组的最小代价（#3500，hard）
- **链接**：https://leetcode.cn/problems/minimum-cost-to-divide-array-into-subarrays/
- **难度**：困难
- **标签**：数组、动态规划、前缀和、李超线段树

**题意**：给定两个长度均为 $n$ 的整数数组 $\text{nums}$、$\text{cost}$ 与整数 $k$。将 $\text{nums}$ 划分成若干个**连续非空**子数组，第 $i$ 个子数组（覆盖 $\text{nums}[l..r]$）的代价为
