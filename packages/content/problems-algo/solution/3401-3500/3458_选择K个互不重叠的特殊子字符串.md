...
id: "lc:3458"
type: problem
title: 选择K个互不重叠的特殊子字符串
tags: [动态规划, 哈希表, 字符串, 排序, 贪心]
knowledge_points: [dynamic-programming, greedy, hash-table, sorting, string]
updated: 2026-09-03
source: leetcode
number: 3458
difficulty: medium
...
# LeetCode 选择K个互不重叠的特殊子字符串 题解

## 1. 题目概述

- **标题 / 题号**：选择K个互不重叠的特殊子字符串（#3458，medium）
- **链接**：https://leetcode.cn/problems/select-k-disjoint-special-substrings/
- **难度**：中等
- **标签**：贪心、哈希表、字符串、动态规划、排序

**题意**：给定长度为 $n$ 的字符串 `s` 和整数 $k$，判断能否选出 $k$ 个**互不重叠**的**特殊子字符串**。
