...
type: problem
title: 最短匹配子字符串
tags: [二分查找, 双指针, 字符串, 字符串匹配]
knowledge_points: [binary-search, string, string-matching, two-pointers]
updated: 2026-09-03
source: leetcode
number: 3455
difficulty: hard
...
# LeetCode 最短匹配子字符串 题解

## 1. 题目概述

- **标题 / 题号**：最短匹配子字符串（#3455，hard）
- **链接**：https://leetcode.cn/problems/shortest-matching-substring/
- **难度**：困难
- **标签**：字符串、字符串匹配、双指针、二分查找

**题意**：给你一个字符串 `s` 和一个模式字符串 `p`，其中 `p` **恰好包含两个** `'*'` 字符。`'*'` 匹配**零个或多个**字符的任意序列。返回 `s` 中与 `p` 匹配的**最短**子字符串的长度；不存在这样的子字符串时返回 `-1`。空子字符串也被视为合法的匹配对象。
