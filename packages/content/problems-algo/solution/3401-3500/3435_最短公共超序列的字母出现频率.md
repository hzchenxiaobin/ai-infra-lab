...
id: "lc:3435"
type: problem
title: 最短公共超序列的字母出现频率
tags: [位运算, 图, 字符串, 拓扑排序, 数组, 枚举]
knowledge_points: [array, bit-manipulation, enumeration, graph, string, topological-sort]
updated: 2026-09-03
source: leetcode
number: 3435
difficulty: hard
...
# LeetCode 最短公共超序列的字母出现频率 题解

- **关联**：1092（SCS 经典 DP）、207/802（拓扑判环）、1494（n≤16 位运算子集枚举）

## 1. 题目概述

- **标题 / 题号**：最短公共超序列的字母出现频率（#3435，hard）
- **链接**：https://leetcode.cn/problems/frequencies-of-shortest-supersequences/
- **难度**：困难
- **标签**：位运算、图、拓扑排序、数组、字符串、枚举

**题意**：给你一个字符串数组 `words`，其中每个字符串长度均为 2，且互不相同。**最短公共超序列**（SCS）指的是这样一个字符串：`words` 中所有字符串都是它的**子序列**，且它的长度**最短**。
