...
type: problem
title: "DNA 模式识别"
tags: ["LIKE 前缀/后缀/子串", Pandas, SQL, 字符串匹配, 布尔标志列, 数据库]
knowledge_points: [database, like-前缀-后缀-子串, pandas, sql, string-matching, 布尔标志列]
updated: 2026-09-03
source: leetcode
number: 3475
difficulty: medium
...
# LeetCode DNA 模式识别 题解

## 1. 题目概述

- **标题 / 题号**：DNA 模式识别（#3475，medium）
- **链接**：https://leetcode.cn/problems/dna-pattern-recognition/
- **难度**：中等
- **标签**：数据库、SQL、Pandas、字符串匹配、`LIKE` 前缀/后缀/子串、布尔标志列

**题意**：给定 `Samples` 表（每行一个 DNA 样本：主键 `sample_id`、由 `A/T/G/C` 组成的序列 `dna_sequence`、采集物种 `species`）。编写解决方案，为**每一个样本**判定四种生物学模式是否存在，输出 4 个标志列（存在为 `1`，不存在为 `0`）：
