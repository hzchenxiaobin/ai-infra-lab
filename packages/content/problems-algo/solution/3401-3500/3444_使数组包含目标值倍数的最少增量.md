...
id: "lc:3444"
type: problem
title: 使数组包含目标值倍数的最少增量
tags: [位运算, 动态规划, 数学, 数组, 数论, 状态压缩]
knowledge_points: [array, bit-manipulation, bitmask-dp, dynamic-programming, math, number-theory]
updated: 2026-09-03
source: leetcode
number: 3444
difficulty: hard
...
# LeetCode 使数组包含目标值倍数的最少增量 题解

## 1. 题目概述

- **标题 / 题号**：使数组包含目标值倍数的最少增量（#3444，hard）
- **链接**：https://leetcode.cn/problems/minimum-increments-for-target-multiples-in-an-array/
- **难度**：困难
- **标签**：位运算、数组、数学、动态规划、状态压缩、数论

**题意**：给你两个数组 `nums` 和 `target`。一次操作中，你可以将 `nums` 中的**任意一个元素递增 1**。返回要使 `target` 中的**每个元素**在 `nums` 中**至少存在一个倍数**所需的最少操作次数。
