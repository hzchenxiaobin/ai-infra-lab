...
type: problem
title: "奇偶频次间的最大差值 II"
tags: [前缀和, 字符串, 枚举, 滑动窗口]
knowledge_points: [enumeration, prefix-sum, sliding-window, string]
updated: 2026-09-03
source: leetcode
number: 3445
difficulty: hard
...
# LeetCode 奇偶频次间的最大差值 II 题解

## 1. 题目概述

- **标题 / 题号**：奇偶频次间的最大差值 II（#3445，hard）
- **链接**：https://leetcode.cn/problems/maximum-difference-between-even-and-odd-frequency-ii/
- **难度**：困难
- **标签**：字符串、枚举、前缀和、滑动窗口

**题意**：给你一个字符串 `s` 和一个整数 `k`。请你找出 `s` 的子字符串 `subs` 中两个字符的出现频次之间的**最大**差值 $\text{freq}[a] - \text{freq}[b]$，其中：
