...
type: problem
title: 给边赋权值的方案数I
tags: [广度优先搜索, 数学, 树, 深度优先搜索]
knowledge_points: [bfs, dfs, math, tree]
updated: 2026-09-03
source: leetcode
number: 3558
difficulty: medium
...
# LeetCode 给边赋权值的方案数I 题解

## 1. 题目概述

- **标题 / 题号**：给边赋权值的方案数I（#3558，medium）
- **链接**：https://leetcode.cn/problems/number-of-ways-to-assign-edge-weights-i/
- **难度**：中等
- **标签**：树, 深度优先搜索, 广度优先搜索, 数学

**题意**：给你一棵 $n$ 个节点的无向树，节点从 $1$ 到 $n$ 编号，以节点 $1$ 为根。树由长度为 $n-1$ 的二维整数数组 `edges` 给出，`edges[i] = [u_i, v_i]` 表示节点 $u_i$ 和 $v_i$ 之间有一条边。
