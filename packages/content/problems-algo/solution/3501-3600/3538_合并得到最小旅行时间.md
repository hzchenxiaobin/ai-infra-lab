...
# LeetCode 合并得到最小旅行时间 题解

## 1. 题目概述

- **标题 / 题号**：合并得到最小旅行时间（#3538，hard）
- **链接**：https://leetcode.cn/problems/merge-operations-for-minimum-travel-time/
- **难度**：困难
- **标签**：数组、动态规划、前缀和

**题意**：给定一条长 $l$ 公里的直路、$n$ 个路标和整数 $k$。数组 `position` 严格升序列出路标位置（`position[0] = 0`，`position[n-1] = l`）；`time[i]` 表示**从 `position[i]` 开到 `position[i+1]` 这一路段**每行驶 1 公里所需的时间（分钟）。
