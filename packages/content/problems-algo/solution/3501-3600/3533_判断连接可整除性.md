...
# LeetCode 判断连接可整除性 题解

## 1. 题目概述

- **标题 / 题号**：判断连接可整除性（#3533，hard）
- **链接**：https://leetcode.cn/problems/concatenated-divisibility/
- **难度**：困难
- **标签**：位运算、数组、动态规划、状态压缩

**题意**：给定正整数数组 `nums` 和正整数 $k$。把 `nums` 的某个**排列**按顺序**连接十进制表示**得到一个大数，若该数能被 $k$ 整除，则称该排列形成**可整除连接**。返回能形成可整除连接且**字典序最小**的排列（以整数列表表示）；若不存在这样的排列，返回空列表。
