...
type: problem
title: "最小回文排列 II"
tags: [哈希表, 字符串, 数学, 组合数学, 计数]
knowledge_points: [combinatorics, counting, hash-table, math, string]
updated: 2026-09-03
source: leetcode
number: 3518
difficulty: hard
...
# LeetCode 最小回文排列 II 题解

## 1. 题目概述

- **标题 / 题号**：最小回文排列 II（#3518，hard）
- **链接**：https://leetcode.cn/problems/smallest-palindromic-rearrangement-ii/
- **难度**：困难
- **标签**：哈希表、数学、字符串、组合数学、计数

**题意**：给你一个**回文**字符串 `s` 和一个整数 `k`。把 `s` 的字符任意重排，所有结果中**仍是回文**的那些字符串构成「回文排列」集合——**产生相同回文串的不同重排只计一次**。返回该集合中按**字典序**排列的第 $k$ 小者；若不同回文排列不足 $k$ 个，返回空字符串 `""`。
