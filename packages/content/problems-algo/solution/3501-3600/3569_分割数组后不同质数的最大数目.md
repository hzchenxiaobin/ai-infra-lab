...
tags: [哈希表, 数论, 有序集合, 线段树]
knowledge_points: [hash-table, number-theory, ordered-set, segment-tree]
updated: 2026-09-03
source: leetcode
number: 3569
difficulty: hard
...
# LeetCode 分割数组后不同质数的最大数目 题解

## 1. 题目概述

- **标题 / 题号**：分割数组后不同质数的最大数目（#3569，hard）
- **链接**：https://leetcode.cn/problems/maximize-count-of-distinct-primes-after-split/
- **难度**：困难
- **标签**：线段树、数论、哈希表、有序集合

**题意**：给定长度为 $n$ 的整数数组 `nums` 与二维数组 `queries`，其中 `queries[i] = [idx, val]`。对每个查询依次执行：
