...
tags: [动态规划, 双指针, 字符串, 枚举]
knowledge_points: [dynamic-programming, enumeration, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 3503
difficulty: medium
...
# LeetCode 子字符串连接后的最长回文串 I 题解

## 1. 题目概述

- **标题 / 题号**：子字符串连接后的最长回文串 I（#3503，medium）
- **链接**：https://leetcode.cn/problems/longest-palindrome-after-substring-concatenation-i/
- **难度**：中等
- **标签**：双指针、字符串、动态规划、枚举

**题意**：给你两个字符串 $s$ 和 $t$。你可以从 $s$ 中选择一个**子串**（可以为空）以及从 $t$ 中选择一个**子串**（可以为空），然后将它们**按顺序**连接（$s$ 的子串在前），得到一个新字符串。返回能构造出的**最长回文串**的长度。
