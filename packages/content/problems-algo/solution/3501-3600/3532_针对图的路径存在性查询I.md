...
type: problem
title: "针对图的路径存在性查询 I"
tags: [二分查找, 哈希表, 图, 并查集, 数组]
knowledge_points: [array, binary-search, graph, hash-table, union-find]
updated: 2026-09-03
source: leetcode
number: 3532
difficulty: medium
...
# LeetCode 针对图的路径存在性查询 I 题解

## 1. 题目概述

- **标题 / 题号**：针对图的路径存在性查询 I（#3532，medium）
- **链接**：https://leetcode.cn/problems/path-existence-queries-in-a-graph-i/
- **难度**：中等
- **标签**：并查集、图、数组、哈希表、二分查找

**题意**：给定 $n$ 个节点（编号 $0$ 到 $n-1$）、一个**非递减**整数数组 `nums` 和整数 `maxDiff`。图中的边由值决定：当且仅当 $|\text{nums}[i] - \text{nums}[j]| \le \text{maxDiff}$ 时，节点 $i$ 与 $j$ 之间存在一条**无向边**。

再给一个查询数组 `queries`，其中 $\text{queries}[i] = [u_i, v_i]$，要求判断 $u_i$ 与 $v_i$ 之间**是否存在路径**。返回布尔数组 `answer`。

**示例 1**：

![示例1](https://pic.leetcode.cn/1745660506-eNVQtC-screenshot-2025-03-26-at-122249.png)
