...
tags: [位运算, 动态规划, 图, 拓扑排序, 数组, 状态压缩]
knowledge_points: [array, bit-manipulation, bitmask-dp, dynamic-programming, graph, topological-sort]
updated: 2026-09-03
source: leetcode
number: 3530
difficulty: hard
...
# LeetCode 有向无环图中合法拓扑排序的最大利润 题解

## 1. 题目概述

- **标题 / 题号**：有向无环图中合法拓扑排序的最大利润（#3530，hard）
- **链接**：https://leetcode.cn/problems/maximum-profit-from-valid-topological-order-in-dag/
- **难度**：困难
- **标签**：位运算、图、拓扑排序、动态规划、状态压缩、数组

**题意**：给定 $n$ 个节点组成的**有向无环图（DAG）**，节点编号 $0$ 到 $n-1$，边由二维数组 `edges` 给出，`edges[i] = [u_i, v_i]` 表示一条 $u_i \to v_i$ 的有向边。每个节点有得分 `score[i]`。
...
![示例1](https://pic.leetcode.cn/1745660258-BXXGjv-screenshot-2025-03-11-at-021131.png)
...
![示例2](https://pic.leetcode.cn/1745660268-mJrEKY-screenshot-2025-03-11-at-023558.png)
