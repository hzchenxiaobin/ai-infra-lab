...
type: problem
title: 所有人渡河所需的最短时间
tags: [位运算, 图, "堆（优先队列）", 最短路, 状态压缩]
knowledge_points: [bit-manipulation, bitmask-dp, graph, heap, shortest-path]
updated: 2026-09-03
source: leetcode
number: 3594
difficulty: hard
...
# LeetCode 所有人渡河所需的最短时间 题解

## 1. 题目概述

- **标题 / 题号**：所有人渡河所需的最短时间（#3594，hard）
- **链接**：https://leetcode.cn/problems/minimum-time-to-transport-all-individuals/
- **难度**：困难
- **标签**：位运算、图、状态压缩、最短路、堆（优先队列）

**题意**：$n$ 名人员在营地，要用一艘**最多载 $k$ 人**的船渡河到对岸。环境条件以 $m$ 个阶段**周期性循环**，阶段 $j$ 有速度倍率 $mul[j]$：大于 $1$ 渡河变慢，小于 $1$ 变快。每个人 $i$ 的划船能力为 $time[i]$（倍率为 $1$ 时单独渡河的分钟数）。规则：
