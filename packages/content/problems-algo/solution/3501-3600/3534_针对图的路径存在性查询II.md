...
tags: [二分查找, 位运算, 双指针, 图, 排序, 贪心]
knowledge_points: [binary-search, bit-manipulation, graph, greedy, sorting, two-pointers]
updated: 2026-09-03
source: leetcode
number: 3534
difficulty: hard
...
# LeetCode 针对图的路径存在性查询 II 题解

## 1. 题目概述

- **标题 / 题号**：针对图的路径存在性查询 II（#3534，hard）
- **链接**：https://leetcode.cn/problems/path-existence-queries-in-a-graph-ii/
- **难度**：困难
- **标签**：贪心、位运算、图、排序、双指针、二分查找

**题意**：给定 $n$ 个节点（编号 $0$ 到 $n-1$）、整数数组 `nums` 和整数 `maxDiff`。图中的边由**值**决定：当且仅当 $|\text{nums}[i] - \text{nums}[j]| \le \text{maxDiff}$ 时，节点 $i$ 与 $j$ 之间存在一条**无向边**。

再给一个查询数组 `queries`，其中 $\text{queries}[i] = [u_i, v_i]$，要求求出 $u_i$ 与 $v_i$ 之间的**最短距离**（边无权重，即最少边数）；若两节点之间不存在路径，返回 $-1$。

**示例 1**：

![示例1](https://pic.leetcode.cn/1745660620-PauXMH-4149example1drawio.png)
...
![示例2](https://pic.leetcode.cn/1745660627-mSVsDs-4149example2drawio.png)
