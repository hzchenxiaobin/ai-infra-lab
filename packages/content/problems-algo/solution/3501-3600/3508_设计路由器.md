...
type: problem
title: 设计路由器
tags: [二分查找, 哈希表, 有序集合, 设计, 队列]
knowledge_points: [binary-search, design, hash-table, ordered-set, queue]
updated: 2026-09-03
source: leetcode
number: 3508
difficulty: medium
...
- **标题 / 题号**：设计路由器（#3508，medium）
- **链接**：https://leetcode.cn/problems/implement-router/
- **难度**：中等
- **标签**：设计、队列、哈希表、二分查找、有序集合

**题意**：设计一个网络路由器 `Router`，在固定内存限制下管理数据包。每个数据包由三元组 `(source, destination, timestamp)` 描述，需支持四个操作：
