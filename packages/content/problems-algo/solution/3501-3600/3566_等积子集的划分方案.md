...
title: 等积子集的划分方案
tags: [位运算, 数组, 枚举, 递归]
knowledge_points: [array, bit-manipulation, enumeration, recursion]
updated: 2026-09-03
source: leetcode
number: 3566
difficulty: medium
...
# LeetCode 等积子集的划分方案 题解

## 1. 题目概述

- **标题 / 题号**：等积子集的划分方案（#3566，medium）
- **链接**：https://leetcode.cn/problems/partition-array-into-two-equal-product-subsets/
- **难度**：中等
- **标签**：位运算, 递归, 数组, 枚举

**题意**：给定一个**正整数互不相同**的数组 `nums` 和一个整数 `target`，判断能否把 `nums` 划分成两个**非空**、**互不相交**的子集，使每个元素**恰好属于其中一个**子集，且**两个子集的元素乘积都等于 target**。存在这样的划分返回 `true`，否则返回 `false`。
