...
# LeetCode 相同颜色的最大子树 题解

## 1. 题目概述

- **标题 / 题号**：相同颜色的最大子树（#3004，medium）
...
**题意**：有一棵 $n$ 个节点的无向树，节点编号 $0$ 到 $n-1$，根节点为 $0$。`edges` 给出全部 $n-1$ 条边（`edges[i] = [u_i, v_i]` 表示 $u_i$ 与 $v_i$ 之间有一条边）；另有长度为 $n$ 的数组 `colors`，`colors[i]` 是节点 $i$ 的颜色。
