...
type: problem
title: 最长公共后缀查询
tags: ["字典树（Trie）", 字符串, 数组]
knowledge_points: [array, string, trie]
updated: 2026-09-03
source: leetcode
number: 3093
difficulty: hard
...
# LeetCode 最长公共后缀查询 题解

- **核心套路**：反转化「后缀」为「前缀」+ 前缀 Trie 每节点维护最短单词下标（升序插入、严格更短才更新，平局自动取最早）

## 1. 题目概述

- **标题 / 题号**：最长公共后缀查询（#3093，hard）
- **链接**：https://leetcode.cn/problems/longest-common-suffix-queries/
- **难度**：困难
- **标签**：字典树（Trie）、数组、字符串

**题意**：给你两个字符串数组 `wordsContainer` 和 `wordsQuery`。对于每个 `wordsQuery[i]`，需要从 `wordsContainer` 中找到一个与 `wordsQuery[i]` 有**最长公共后缀**的字符串，返回其下标。多条规则依次破平局：
