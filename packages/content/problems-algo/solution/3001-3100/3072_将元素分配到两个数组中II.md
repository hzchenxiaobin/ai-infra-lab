...
tags: [数组, 树状数组, 模拟, 线段树]
knowledge_points: [array, fenwick-tree, segment-tree, simulation]
updated: 2026-09-03
source: leetcode
number: 3072
difficulty: hard
...
# LeetCode 将元素分配到两个数组中II 题解

## 1. 题目概述

- **标题 / 题号**：将元素分配到两个数组中 II（#3072，hard）
- **链接**：https://leetcode.cn/problems/distribute-elements-into-two-arrays-ii/
- **难度**：困难
- **标签**：树状数组、线段树、数组、模拟

**题意**：给你一个下标从 **1** 开始、长度为 $n$ 的整数数组 `nums`。定义函数 `greaterCount(arr, val)`：返回数组 `arr` 中**严格大于** `val` 的元素数量。
