...
type: problem
title: 快照分析
tags: ["GROUP BY", JOIN, ROUND, SQL, 条件聚合]
knowledge_points: [conditional-aggregation, group-by, join, round, sql]
updated: 2026-09-03
source: leetcode
number: 3056
difficulty: medium
...
**题意**：Snap 记录了用户发送（`send`）和打开（`open`）快照的活动流水，每个用户属于一个年龄组（`age_bucket`）。编写解决方案，计算**每个年龄组**花在**发送**和**打开**快照上的总时间**百分比**，四舍五入到小数点后 `2` 位，以**任意顺序**返回结果表。
