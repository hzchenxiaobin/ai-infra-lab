...
type: problem
title: 披萨配料成本分析
tags: ["ORDER BY", SQL, 上三角去重, 数据库, 组合枚举, "自连接（Self Join）"]
knowledge_points: [combination-enumeration, database, order-by, sql, 上三角去重, 自连接-self-join]
updated: 2026-09-03
source: leetcode
number: 3050
difficulty: medium
...
- **标题 / 题号**：披萨配料成本分析（#3050，medium）
- **链接**：https://leetcode.cn/problems/pizza-toppings-cost-analysis/
- **难度**：中等
- **标签**：SQL、数据库、自连接（Self Join）、组合枚举、`ORDER BY`、上三角去重

**题意**：你是一家披萨店的老板。店里的每种配料都有名称与单价，记录在 `Toppings` 表中。顾客点披萨时会**恰好选择两种不同的配料**。你想做一次成本分析：**枚举所有可能的配料两两组合，报告每种组合的总成本**。编写 SQL 查询，列出每一种「两种不同配料」的组合：
