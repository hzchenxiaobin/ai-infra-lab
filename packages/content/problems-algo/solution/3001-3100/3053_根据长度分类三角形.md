...
type: problem
title: 根据长度分类三角形
tags: ["CASE WHEN", SQL, 三角形不等式, 多分支分类, 数据库]
knowledge_points: [case-when, database, sql, 三角形不等式, 多分支分类]
updated: 2026-09-03
source: leetcode
number: 3053
difficulty: easy
...
# LeetCode 根据长度分类三角形 题解

## 1. 题目概述

- **标题 / 题号**：根据长度分类三角形（#3053，easy）
- **链接**：https://leetcode.cn/problems/classifying-triangles-by-lengths/
- **难度**：简单
- **标签**：数据库、SQL、`CASE WHEN`、多分支分类、三角形不等式、LeetCode 锁题

**题意**：给定 `Triangles` 表，每行存放三条线段的长度 `A`、`B`、`C`。编写解决方案，对每行按边长把三角形分类，输出原三列并追加一列分类标签，结果顺序不限。
