...
tags: [前缀和, 数组, 滑动窗口, 贪心]
knowledge_points: [array, greedy, prefix-sum, sliding-window]
updated: 2026-09-03
source: leetcode
number: 3086
difficulty: hard
...
# LeetCode 拾起K个1需要的最少行动次数 题解

## 1. 题目概述

- **标题 / 题号**：拾起 K 个 1 需要的最少行动次数（#3086，hard）
- **链接**：https://leetcode.cn/problems/minimum-moves-to-pick-k-ones/
- **难度**：困难
- **标签**：贪心，数组，前缀和，滑动窗口

**题意**：给定下标从 **0** 开始、长度为 $n$ 的二进制数组 `nums`，以及一个**正整数** $k$ 和一个**非负整数** `maxChanges`。
