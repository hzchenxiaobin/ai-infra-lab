...
id: "lc:3036"
type: problem
title: "匹配模式数组的子数组数目 II"
tags: [哈希函数, 字符串匹配, 数组, 滚动哈希]
knowledge_points: [array, hash-function, rolling-hash, string-matching]
updated: 2026-09-03
source: leetcode
number: 3036
difficulty: hard
...
# LeetCode 匹配模式数组的子数组数目II 题解

## 1. 题目概述

- **标题 / 题号**：匹配模式数组的子数组数目 II（#3036，hard）
- **链接**：https://leetcode.cn/problems/number-of-subarrays-that-match-a-pattern-ii/
- **难度**：困难
- **标签**：数组、字符串匹配、哈希函数、滚动哈希

**题意**：给你一个下标从 **0** 开始长度为 $n$ 的整数数组 `nums`，和一个下标从 0 开始长度为 $m$ 的整数数组 `pattern`，`pattern` 数组只包含整数 $-1$、$0$ 和 $1$。
