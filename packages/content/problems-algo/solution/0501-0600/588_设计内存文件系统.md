...
title: 设计内存文件系统
tags: [哈希表, 字典树, 字符串, 设计]
knowledge_points: [design, hash-table, string, trie]
updated: 2026-09-03
source: leetcode
number: 588
difficulty: hard
...
# LeetCode 设计内存文件系统 题解

## 1. 题目概述

- **标题 / 题号**：设计内存文件系统（#588，hard）
- **链接**：https://leetcode.cn/problems/design-in-memory-file-system/
- **难度**：困难
- **标签**：设计、字典树、哈希表、字符串

**题意**：设计一个**内存文件系统**，支持四类操作：列目录、建目录、写文件、读文件。所有路径都是**绝对路径**（以 `/` 开头），目录与文件名仅含字母数字与 `.`、`_`、`-`。
...
| 460 | [LFU 缓存](https://leetcode.cn/problems/lfu-cache/)（[题解](../0401-0500/460_LFU缓存.md)） | 更复杂的设计题，频率桶 + 哈希，对照本题体会「结构蕴含关系」的设计原则 |
