...
type: problem
title: 寻找用户推荐人
tags: ["NULL 处理", SQL, WHERE, 三值逻辑]
knowledge_points: [null-handling, sql, three-valued-logic, where, where-filter]
updated: 2026-09-03
source: leetcode
number: 584
difficulty: easy
...
- **标题 / 题号**：寻找用户推荐人（#584，easy）
- **链接**：https://leetcode.cn/problems/find-customer-referee/
- **难度**：简单
- **标签**：SQL、WHERE、NULL 处理、三值逻辑

**题意**：给定 `Customer` 表，每行记录一个客户的 `id`、姓名 `name` 以及推荐他的客户的 `referee_id`。编写 SQL 查询，返回满足下列**任一条件**的客户姓名：
