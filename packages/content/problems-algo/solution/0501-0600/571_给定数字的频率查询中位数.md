...
type: problem
title: 给定数字的频率查询中位数
tags: [SQL, "SUM OVER", 中位数, 双向累计, 窗口函数, 累计求和]
knowledge_points: [median, running-sum, sql, sum-over, window-function, window-sum, 双向累计, 累计求和]
updated: 2026-09-03
source: leetcode
number: 571
difficulty: hard
...
# LeetCode 给定数字的频率查询中位数 题解

## 1. 题目概述

- **标题 / 题号**：给定数字的频率查询中位数（#571，hard）
- **链接**：https://leetcode.cn/problems/find-median-given-frequency-of-numbers/
- **难度**：困难
- **标签**：SQL、中位数、窗口函数、累计求和、SUM OVER、双向累计

**题意**：给定 `Numbers` 表，每行记录一个数值 `num` 及其出现的次数 `frequency`。编写 SQL 查询，返回所有数字的**中位数**。
