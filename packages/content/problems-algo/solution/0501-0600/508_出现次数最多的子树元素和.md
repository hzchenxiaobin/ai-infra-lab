...
type: problem
title: 出现次数最多的子树元素和
tags: [二叉树, 后序遍历, 哈希表, 树, 深度优先搜索]
knowledge_points: [binary-tree, dfs, hash-table, postorder-traversal, tree]
updated: 2026-09-03
source: leetcode
number: 508
difficulty: medium
...
# LeetCode 出现次数最多的子树元素和 题解

## 1. 题目概述

- **标题 / 题号**：出现次数最多的子树元素和（#508，medium）
- **链接**：https://leetcode.cn/problems/most-frequent-subtree-sum/
- **难度**：中等
- **标签**：树、二叉树、深度优先搜索、后序遍历、哈希表

**题意**：给你一棵二叉树的根节点 `root`，返回出现次数最多的**子树元素和**。如果多个子树和出现次数相同且并列最高，则返回**全部**这些值（顺序任意）。

> **子树元素和**：以某节点为根的子树（包含该节点本身）中**所有节点值之和**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/04/24/freq1-tree.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/04/24/freq2-tree.jpg)
