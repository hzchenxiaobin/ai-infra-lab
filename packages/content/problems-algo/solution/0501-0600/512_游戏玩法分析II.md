...
type: problem
title: "游戏玩法分析 II"
tags: ["GROUP BY", JOIN, SQL, 子查询, 窗口函数, 聚合]
knowledge_points: [aggregation, group-by, join, sql, subquery, window-function]
updated: 2026-09-03
source: leetcode
number: 512
difficulty: easy
...
# LeetCode 游戏玩法分析 II 题解

## 1. 题目概述

- **标题 / 题号**：游戏玩法分析 II（#512，easy）
- **链接**：https://leetcode.cn/problems/game-play-analysis-ii/
- **难度**：简单
- **标签**：SQL、聚合、GROUP BY、子查询、JOIN、窗口函数

**题意**：给定 `Activity` 表（结构与 [511. 游戏玩法分析 I](../0501-0600/511_游戏玩法分析I.md) 完全相同），每行记录一名玩家在某个日期使用某台设备登录并玩过若干局游戏。编写 SQL 查询，报告**每个玩家首次登录那天所使用的 `device_id`**。
