...
type: problem
title: 当选者
tags: ["GROUP BY", JOIN, LIMIT, "ORDER BY", SQL, 子查询, 聚合]
knowledge_points: [aggregation, group-by, join, limit, order-by, sql, subquery]
updated: 2026-09-03
source: leetcode
number: 574
difficulty: medium
...
- **标题 / 题号**：当选者（#574，medium）
- **链接**：https://leetcode.cn/problems/winning-candidate/
- **难度**：中等
- **标签**：SQL、JOIN、GROUP BY、聚合、ORDER BY、LIMIT、子查询

**题意**：给定 `Candidate` 表（候选人）与 `Vote` 表（选票），每张选票记录投给某位候选人的 `CandidateId`。编写 SQL 查询，返回**得票最多**的候选人姓名（即「当选者」）。题目保证当选者的票数**严格多于**所有其他候选人（无并列）。
