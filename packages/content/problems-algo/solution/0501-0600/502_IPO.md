...
- **标题 / 题号**：IPO（#502，medium）
- **链接**：https://leetcode.cn/problems/ipo/
- **难度**：中等
- **标签**：贪心、堆、优先队列、排序

**题意**：给定 `n` 个项目，每个项目 `i` 有**纯利润** `profits[i]` 和**启动所需最低资本** `capital[i]`。你拥有初始资本 `w`，最多可完成 `k` 个**不同**项目。每完成一个项目，资本 `w` 增加 `profits[i]`（利润为正，资本只增不减）。求完成至多 `k` 个项目后的**最终最大资本**。
