...
tags: [BST, 中序遍历, 树, 父指针]
knowledge_points: [bst, inorder-traversal, tree, 父指针]
updated: 2026-09-03
source: leetcode
number: 510
difficulty: medium
...
# LeetCode 二叉搜索树中的中序后继 II 题解

## 1. 题目概述

- **标题 / 题号**：二叉搜索树中的中序后继 II（#510，medium）
- **链接**：https://leetcode.cn/problems/inorder-successor-in-bst-ii/
- **难度**：中等
- **标签**：树、BST、中序遍历、父指针

**题意**：给定一棵**二叉搜索树（BST）**中的某个节点 `node`（**不是根节点**），每个节点除了 `val`、`left`、`right` 之外还额外持有 **`parent` 指针**指向其父节点。请找出 `node` 在 BST 中的**中序后继**——即中序遍历序列中紧跟在 `node` 之后的那个节点（值大于 `node.val` 的最小节点）。若 `node` 已是中序最后一个节点，返回 `null`。
