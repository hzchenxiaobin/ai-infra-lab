...
tags: [双指针, 子序列判定, 字符串, 排序, 数组]
knowledge_points: [array, sorting, string, two-pointers, 子序列判定]
updated: 2026-09-03
source: leetcode
number: 524
difficulty: medium
...
# LeetCode 通过删除字母匹配到字典里最长单词 题解

## 1. 题目概述

- **标题 / 题号**：通过删除字母匹配到字典里最长单词（#524，medium）
- **链接**：https://leetcode.cn/problems/longest-word-in-dictionary-through-deleting/
- **难度**：中等
- **标签**：数组、字符串、双指针、排序、子序列判定

**题意**：给定一个字符串 `s` 和一个字符串数组 `dictionary`，返回 `dictionary` 中**最长**的、可以通过从 `s` 中**删除若干字符**（也可不删）得到的字符串。若存在多个长度相同的结果，返回**字典序最小**的那个；若不存在，返回空字符串 `""`。
