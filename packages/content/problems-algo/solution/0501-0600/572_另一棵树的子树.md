...
- **标题 / 题号**：另一棵树的子树（#572，easy）
- **链接**：https://leetcode.cn/problems/subtree-of-another-tree/
- **难度**：简单
- **标签**：树、二叉树、深度优先搜索

**题意**：给你两棵二叉树 `root` 和 `subRoot`。检验 `root` 中是否包含和 `subRoot` 具有**相同结构和节点值**的子树。如果存在这样的子树，返回 `true`，否则返回 `false`。

> **子树定义**：二叉树 `tree` 的一棵子树包括 `tree` 中**某个节点**和这个节点的**所有后代**。`tree` 也可以看做它自身的一棵子树。

**示例 1**：

![示例1](https://pic.leetcode.cn/1724998676-cATjhe-image.png)
...
![示例2](https://pic.leetcode.cn/1724998698-sEJWnq-image.png)
