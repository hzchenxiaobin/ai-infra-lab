...
type: problem
title: 非重叠矩形中的随机点
tags: [二分查找, 前缀和, 数学, 概率与随机]
knowledge_points: [binary-search, math, prefix-sum, probability]
updated: 2026-09-03
source: leetcode
number: 497
difficulty: medium
...
# LeetCode 非重叠矩形中的随机点 题解

## 1. 题目概述

- **标题 / 题号**：非重叠矩形中的随机点（#497，medium）
- **链接**：https://leetcode.cn/problems/random-point-in-non-overlapping-rectangles/
- **难度**：中等
- **标签**：数学、概率与随机、前缀和、二分查找

**题意**：给定一组**两两不重叠**的轴对齐矩形 `rects`，其中 `rects[i] = [xi, yi, ai, bi]`，`(xi, yi)` 是左下角坐标、`(ai, bi)` 是右上角坐标。请实现 `Solution` 类：
...
![示例1](https://assets.leetcode.com/uploads/2021/07/24/lc-pickrandomrec.jpg)
