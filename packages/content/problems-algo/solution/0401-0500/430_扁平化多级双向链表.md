...
title: 扁平化多级双向链表
tags: [DFS, 链表]
knowledge_points: [dfs, linked-list]
updated: 2026-09-03
source: leetcode
number: 430
difficulty: medium
...
judge: none
related_learn: ["learn:w03d04", "learn:w03d07"]
---

# LeetCode 扁平化多级双向链表 题解

## 1. 题目概述

- **标题 / 题号**：扁平化多级双向链表（#430，medium）
- **链接**：https://leetcode.cn/problems/flatten-a-multilevel-doubly-linked-list/
- **难度**：中等
- **标签**：链表、DFS

**题意**：给定一个**多级双向链表**的头节点 `head`。链表中每个节点除了 `prev` 和 `next` 指针外，还可能有 **`child` 指针**，指向另一条双向链表（子链）的头节点。子链本身也可能有 `child`，形成**多级嵌套**结构。

请将链表**扁平化**：所有子链都插入到对应节点的 `next` 位置，并把 `child` 指针置空。扁平化后是一条**没有 child**的双向链表，子链整体插在父节点与其原 `next` 之间。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/11/09/flatten11.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/11/09/flatten12.jpg)
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 114 | [二叉树展开为链表](https://leetcode.cn/problems/flatten-binary-tree-to-linked-list/)（[题解](../0101-0200/114_二叉树展开为链表.md)） | 树的 right 展开，本题的"树版"，同 DFS 思路 |
| 138 | [复制带随机指针的链表](https://leetcode.cn/problems/copy-list-with-random-pointer/)（[题解](../0101-0200/138_复制带随机指针的链表.md)） | 链表 + 多指针，哈希/拼接拆分 |
| 146 | [LRU 缓存](https://leetcode.cn/problems/lru-cache/)（[题解](../0101-0200/146_LRU缓存.md)） | 双向链表 + 哈希，双向指针操作综合题 |
| 92 | [反转链表 II](https://leetcode.cn/problems/reverse-linked-list-ii/)（[题解](../0001-0100/92_反转链表 II.md)） | 链表区间指针操作，头插法重连 |
