...
title: 供暖器
tags: [二分查找, 排序, 数组]
knowledge_points: [array, binary-search, sorting]
updated: 2026-09-03
source: leetcode
number: 475
difficulty: medium
...
- **标题 / 题号**：供暖器（#475，medium）
- **链接**：https://leetcode.cn/problems/heaters/
- **难度**：中等
- **标签**：数组、二分查找、排序

**题意**：给定分布在**水平直线**上的若干房屋 `houses` 与供暖器 `heaters` 的位置坐标，请你为供暖器设计一个**统一的最小供暖半径** `radius`，使得每个房屋都落在**至少一个**供暖器的供暖范围内（即 $|house - heater| \le radius$）。返回这个最小的 `radius`。
