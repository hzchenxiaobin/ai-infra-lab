...
- **标题 / 题号**：目标和（#494，medium）
- **链接**：https://leetcode.cn/problems/target-sum/
- **难度**：中等
- **标签**：数组、动态规划、背包

**题意**：给你一个非负整数数组 `nums` 和一个整数 `target`。向数组中每个整数前添加 `+` 或 `-`，串联起所有整数构造一个表达式，返回运算结果等于 `target` 的不同表达式数目。
...
| 1049 | [最后一块石头的重量 II](https://leetcode.cn/problems/last-stone-weight-ii/)（[题解](../1001-1100/1049_最后一块石头的重量II.md)） | 同样转化为「尽量装满一半」，但是求最值而非计数 |
