...
title: 迷宫
tags: [BFS, DFS, 图, 滚动模拟]
knowledge_points: [bfs, dfs, graph, 滚动模拟]
updated: 2026-09-03
source: leetcode
number: 490
difficulty: medium
...
**题意**：给定一个 `m x n` 的二维矩阵 `maze`，其中 `0` 表示**空地**、`1` 表示**墙**。矩阵中有一个球，给定它的**起始位置** `start = [startRow, startCol]` 和**目标位置** `destination = [destRow, destCol]`。
