...
type: problem
title: 分割数组的最大值
tags: [二分查找, 动态规划, 数组, 贪心]
knowledge_points: [array, binary-search, dynamic-programming, greedy]
updated: 2026-09-03
source: leetcode
number: 410
difficulty: hard
...
# LeetCode 分割数组的最大值 题解

## 1. 题目概述

- **标题 / 题号**：分割数组的最大值（#410，hard）
- **链接**：https://leetcode.cn/problems/split-array-largest-sum/
- **难度**：困难
- **标签**：数组、二分查找、贪心、动态规划

**题意**：给定一个非负整数数组 `nums` 和一个整数 `k`，将 `nums` 分成 `k` 个**非空连续子数组**，使得这 `k` 个子数组各自求和后，**最大和尽可能小**。返回这个最小化的最大和。
