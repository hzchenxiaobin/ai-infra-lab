...
title: 火柴拼正方形
tags: [剪枝, 回溯, 数组]
knowledge_points: [array, backtracking, pruning]
updated: 2026-09-03
source: leetcode
number: 473
difficulty: medium
...
- **标题 / 题号**：火柴拼正方形（#473，medium）
- **链接**：https://leetcode.cn/problems/matchsticks-to-square/
- **难度**：中等
- **标签**：数组、回溯、剪枝

**题意**：给定一个整数数组 `matchsticks`，其中 `matchsticks[i]` 是第 $i$ 根火柴棒的长度。要用**所有**火柴拼成一个正方形，**不能折断**任何一根，每根必须**使用一次**（可首尾相连成一条边）。能拼成正方形返回 `true`，否则返回 `false`。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/04/09/matchsticks1-grid.jpg)
...
| 416 | [分割等和子集](https://leetcode.cn/problems/partition-equal-subset-sum/)（[题解](./416_分割等和子集.md)） | $k=2$ 的特例，可用 0-1 背包 DP 求解，对照「2 组等分」与「4 组等分」的规模差异 |
