...
type: problem
title: 按位与为零的三元组
tags: ["Meet in the Middle", 位运算, 动态规划, 数组]
knowledge_points: [array, bit-manipulation, dynamic-programming, meet-in-the-middle]
updated: 2026-09-03
source: leetcode
number: 982
difficulty: medium
...
# LeetCode 按位与为零的三元组 题解

## 1. 题目概述

- **标题 / 题号**：按位与为零的三元组（#982，medium）
- **链接**：https://leetcode.cn/problems/triples-with-bitwise-and-equal-to-zero/
- **难度**：中等
- **标签**：位运算、数组、Meet in the Middle、动态规划

**题意**：给定一个整数数组 `nums`，返回满足 `nums[i] & nums[j] & nums[k] == 0` 的**三元组** `(i, j, k)` 的个数，其中 `0 <= i, j, k < nums.length`，`&` 表示按位与运算符。三元组的三个下标**有序且可重复**（`i`、`j`、`k` 互不影响，可取相同下标）。
