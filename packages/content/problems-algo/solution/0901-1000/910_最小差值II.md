...
title: "最小差值 II"
tags: [排序, 数学, 数组, 贪心]
knowledge_points: [array, greedy, math, sorting]
updated: 2026-09-03
source: leetcode
number: 910
difficulty: medium
...
- **标题 / 题号**：最小差值 II（#910，medium）
- **链接**：https://leetcode.cn/problems/smallest-range-ii/
- **难度**：中等
- **标签**：贪心、数组、数学、排序

**题意**：给定一个整数数组 `nums` 和一个整数 `k`。对于每个下标 `i`，将 `nums[i]` 变成 `nums[i] + k` 或 `nums[i] - k`。数组的**分数**是最大元素与最小元素的差值。在更改每个下标对应的值之后，返回 `nums` 的最小分数。
...
| 435 | [无重叠区间](https://leetcode.cn/problems/non-overlapping-intervals/)（[站内题解](../0401-0500/435_无重叠区间.md)） | 贪心区间调度，排序 + 交换论证证明贪心正确性的典型 |
