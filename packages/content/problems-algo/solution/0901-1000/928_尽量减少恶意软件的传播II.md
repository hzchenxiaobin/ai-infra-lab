...
tags: [BFS, 图, 连通分量]
knowledge_points: [bfs, connected-components, graph]
updated: 2026-09-03
source: leetcode
number: 928
difficulty: hard
...
# LeetCode 尽量减少恶意软件的传播 II 题解

## 1. 题目概述

- **标题 / 题号**：尽量减少恶意软件的传播 II（#928，hard）
- **链接**：https://leetcode.cn/problems/minimize-malware-spread-ii/
- **难度**：困难
- **标签**：图、BFS、连通分量

**题意**：给定 `n` 个节点的网络，用 `n × n` 邻接矩阵 `graph` 表示（`graph[i][j] = 1` 表示节点 `i` 与 `j` 直接相连）。部分节点 `initial` 被恶意软件感染，感染规则：只要两个节点直接相连且至少一个被感染，则两个都感染，传播直到停止。
