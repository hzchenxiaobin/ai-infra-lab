...
title: 子序列最大优雅度
tags: [哈希表, 排序, 数组, 栈, 贪心]
knowledge_points: [array, greedy, hash-table, sorting, stack]
updated: 2026-09-03
source: leetcode
number: 2813
difficulty: hard
...
# LeetCode 子序列最大优雅度 题解

## 1. 题目概述

- **标题 / 题号**：子序列最大优雅度（#2813，hard）
- **链接**：https://leetcode.cn/problems/maximum-elegance-of-a-k-length-subsequence/
- **难度**：困难
- **标签**：贪心、数组、栈、哈希表、排序

**题意**：给定一个长度为 `n` 的二维整数数组 `items` 和一个整数 `k`。`items[i] = [profit_i, category_i]`，分别表示第 `i` 个项目的利润和类别。
