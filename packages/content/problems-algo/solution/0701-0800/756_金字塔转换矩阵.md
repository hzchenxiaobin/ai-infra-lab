...
title: 金字塔转换矩阵
tags: [DFS, 回溯, 记忆化搜索]
knowledge_points: [backtracking, dfs, memoization]
updated: 2026-09-03
source: leetcode
number: 756
difficulty: medium
...
![示例1](https://assets.leetcode.com/uploads/2021/08/26/pyramid1-grid.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/08/26/pyramid2-grid.jpg)
...
> 💡 **一句话总结**：756 题是"分层回溯 + 记忆化"的招牌——外层 `dfs(row)` 做层间跳跃，内层 `build(idx, next_row)` 做行内回溯，`memo` 缓存行字符串的成败结果剪掉重复子问题。核心建模是 `trans[(left, right)] = [tops]` 转换映射，将 `allowed` 列表转化为 $O(1)$ 查询。与 [22. 括号生成](../0001-0100/22_括号生成.md)（回溯构造合法序列）、[139. 单词拆分](../0101-0200/139_单词拆分.md)（记忆化判定）的思想一脉相承。
...
| 139 | [单词拆分](https://leetcode.cn/problems/word-break/)（[题解](../0101-0200/139_单词拆分.md)） | 记忆化 DFS 判定可行性，同样用 memo 缓存中间状态避免重复 |
