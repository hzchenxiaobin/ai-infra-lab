...
- **标题 / 题号**：句子相似性（#734，easy）
- **链接**：https://leetcode.cn/problems/sentence-similarity/
- **难度**：简单
- **标签**：哈希表

**题意**：给定两个句子 `sentence1` 和 `sentence2`（均以字符串数组形式给出），以及一个**相似词对**列表 `similarPairs`。判断这两个句子是否**相似**。
...
| 290 | [单词规律](https://leetcode.cn/problems/word-pattern/)（[题解](../0201-0300/290_单词规律.md)） | 逐位匹配 + 双向哈希双射，与本题逐位相似判定结构相近 |
