...
title: 图像渲染
tags: [BFS, DFS, 图, 连通分量]
knowledge_points: [bfs, connected-components, dfs, graph]
updated: 2026-09-03
source: leetcode
number: 733
difficulty: easy
...
**题意**：给你一个由整数组成的二维网格 `image`，其中 `image[i][j]` 表示像素的颜色值。再给定起点 `(sr, sc)` 和新颜色 `color`。请从起点开始进行**泛洪填充（Flood Fill）**：把起点以及所有与起点**4 方向连通且颜色相同**的像素，统一改成 `color`，返回修改后的图像。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/06/01/flood1-grid.jpg)
