...
title: "第 K 个最小的质数分数"
tags: [二分查找, 双指针, 数组]
knowledge_points: [array, binary-search, two-pointers]
updated: 2026-09-03
source: leetcode
number: 786
difficulty: medium
...
# LeetCode 第 K 个最小的质数分数 题解

## 1. 题目概述

- **标题 / 题号**：第 K 个最小的质数分数（#786，medium）
- **链接**：https://leetcode.cn/problems/k-th-smallest-prime-fraction/
- **难度**：中等
- **标签**：数组、二分查找、双指针

**题意**：给定一个**严格递增**排序的数组 `arr` 和一个整数 `k`。数组 `arr` 由 `1` 和若干**质数**组成，且所有整数互不相同。对于每对满足 `0 <= i < j < arr.length` 的 `i` 和 `j`，可以得到分数 `arr[i] / arr[j]`。返回**第 `k` 个最小**的分数，以长度为 2 的整数数组 `[arr[i], arr[j]]` 返回。
