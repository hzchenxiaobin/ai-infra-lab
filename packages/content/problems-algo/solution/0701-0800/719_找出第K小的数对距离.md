...
title: "找出第 K 小的数对距离"
tags: [二分查找, 双指针, 排序, 数组]
knowledge_points: [array, binary-search, sorting, two-pointers]
updated: 2026-09-03
source: leetcode
number: 719
difficulty: hard
...
judge: none
related_learn: ["learn:w08d04", "learn:w08d07"]
---

# LeetCode 找出第 K 小的数对距离 题解

## 1. 题目概述

- **标题 / 题号**：找出第 K 小的数对距离（#719，hard）
- **链接**：https://leetcode.cn/problems/find-k-th-smallest-pair-distance/
- **难度**：困难
- **标签**：数组、二分查找、双指针、排序

**题意**：给定一个整数数组 `nums`，定义数对距离为 $|a - b|$。在所有 $\binom{n}{2}$ 个数对 `(nums[i], nums[j])`（$i < j$）的距离中，返回**第 k 小**的距离（k 从 1 开始）。
...
> 💡 **识别信号**：题目求「第 k 小/大」且能高效统计「$\le$ 某值的元素个数」→ 想「二分答案」。本题与 [分割数组的最大值](../0401-0500/410_分割数组的最大值.md)、[爱吃香蕉的珂珂](../0801-0900/875_爱吃香蕉的珂珂.md) 同骨架，只是 `check()` 换成了「双指针数对计数」。
...
即可套用：找最小的 $x$ 使 $\text{count}(x) \ge k$。不同题目的差异只在 `count()` 的实现：本题用**排序 + 双指针**，[410. 分割数组的最大值](../0401-0500/410_分割数组的最大值.md) 用**贪心划段**，[378. 有序矩阵中第 K 小元素](https://leetcode.cn/problems/kth-smallest-element-in-a-sorted-matrix/) 用**矩阵双指针**。
