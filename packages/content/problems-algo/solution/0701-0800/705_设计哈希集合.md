...
title: 设计哈希集合
tags: [哈希表, 设计, 链表]
knowledge_points: [design, hash-table, linked-list]
updated: 2026-09-03
source: leetcode
number: 705
difficulty: easy
...
- **标题 / 题号**：设计哈希集合（#705，easy）
- **链接**：https://leetcode.cn/problems/design-hashset/
- **难度**：简单
- **标签**：设计、哈希表、链表

**题意**：不使用任何内建的哈希表库，设计一个哈希集合（`HashSet`）。实现 `MyHashSet` 类：
...
| 146 | [LRU 缓存](../0101-0200/146_LRU缓存.md) | 哈希表 + 双向链表的设计题，哈希表负责 `O(1)` 定位，链表维护访问顺序（题解已写） |
