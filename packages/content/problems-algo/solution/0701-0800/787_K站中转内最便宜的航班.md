...
type: problem
title: "K 站中转内最便宜的航班"
tags: [Bellman-Ford, 动态规划, 图, 最短路径]
knowledge_points: [bellman-ford, dynamic-programming, graph, shortest-path]
updated: 2026-09-03
source: leetcode
number: 787
difficulty: medium
...
# LeetCode K 站中转内最便宜的航班 题解

## 1. 题目概述

- **标题 / 题号**：K 站中转内最便宜的航班（#787，medium）
- **链接**：https://leetcode.cn/problems/cheapest-flights-within-k-stops/
- **难度**：中等
- **标签**：图、最短路径、Bellman-Ford、动态规划

**题意**：有 `n` 个城市通过若干航班连接。给定航班列表 `flights`，其中 `flights[i] = [from, to, price]` 表示从 `from` 到 `to` 的一趟航班，价格为 `price`。再给定起点 `src`、终点 `dst` 和整数 `k`，求从 `src` 到 `dst` 且**中转次数最多 `k` 次**的最便宜价格；若不存在这样的路线返回 `-1`。

> 💡 **「中转 `k` 次」=「最多经过 `k+1` 条边」**：每次中转意味着在中间城市换一趟航班，因此 `k` 次中转等价于路径上最多有 `k+1` 条边。这是本题区别于普通最短路的关键约束——不仅要便宜，还要「边数受限」。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2022/03/18/cheapest-flights-within-k-stops-3drawio.png)
...
![示例2](https://assets.leetcode.com/uploads/2022/03/18/cheapest-flights-within-k-stops-1drawio.png)
...
![示例3](https://assets.leetcode.com/uploads/2022/03/18/cheapest-flights-within-k-stops-2drawio.png)
