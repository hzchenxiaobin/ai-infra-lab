...
title: "我的日程安排表 II"
tags: [区间, 数组, 设计]
knowledge_points: [array, design, interval]
updated: 2026-09-03
source: leetcode
number: 731
difficulty: medium
...
# LeetCode 我的日程安排表 II 题解

## 1. 题目概述

- **标题 / 题号**：我的日程安排表 II（#731，medium）
- **链接**：https://leetcode.cn/problems/my-calendar-ii/
- **难度**：中等
- **标签**：设计、数组、区间

**题意**：实现一个日程安排类 `MyCalendarTwo`，支持往里添加半开区间 `[startTime, endTime)` 表示的事件。**允许双重预订**（同一时刻最多被两个事件覆盖），但**禁止三重预订**（同一时刻被三个事件覆盖）。
