...
# LeetCode 循环有序链表的插入 题解

## 1. 题目概述

- **标题 / 题号**：循环有序链表的插入（#708，medium）
- **链接**：https://leetcode.cn/problems/insert-into-a-cyclic-sorted-list/
- **难度**：中等
- **标签**：链表

**题意**：给定一个**循环有序链表**中的某个节点 `head`（链表按**非降序**排列成环，但 `head` 不一定是最小值节点），以及一个待插入值 `insertVal`。请将 `insertVal` 插入链表，使插入后链表**仍保持循环有序**。返回 `head`（原给定节点）。
