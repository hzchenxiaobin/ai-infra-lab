...
title: 香槟塔
tags: [动态规划, 模拟]
knowledge_points: [dynamic-programming, simulation]
updated: 2026-09-03
source: leetcode
number: 799
difficulty: medium
...
- **标题 / 题号**：香槟塔（#799，medium）
- **链接**：https://leetcode.cn/problems/champagne-tower/
- **难度**：中等
- **标签**：动态规划、模拟

**题意**：把玻璃杯摆成金字塔形：第 `0` 行有 1 个杯子，第 `1` 行有 2 个杯子，……，第 `i` 行有 `i + 1` 个杯子。从顶部杯子倒入 `poured` 体积的香槟。每个杯子容量为 1——当杯子里的香槟超过 1 时，多余的部分会**等量**流入下方左右两个杯子（各分一半）。如此层层下溢，直到所有超出的香槟都流出底层（底层杯子下方没有杯子，多余部分直接流到地上）。

返回第 `query_row` 行第 `query_glass` 个杯子中的香槟量。

**示例 1**：

![示例1](https://s3-lc-upload.s3.amazonaws.com/uploads/2018/03/09/tower.png)
