...
# LeetCode 在二叉树中增加一行 题解

## 1. 题目概述

- **标题 / 题号**：在二叉树中增加一行（#623，medium）
- **链接**：https://leetcode.cn/problems/add-one-row-to-tree/
- **难度**：中等
- **标签**：树、二叉树、广度优先搜索（BFS）、深度优先搜索（DFS）

**题意**：给定一棵二叉树的根 `root` 和两个整数 `val` 与 `depth`，在深度 `depth` 处**增加一行**值为 `val` 的节点。根节点位于深度 `1`。
...
![示例1](https://assets.leetcode.com/uploads/2021/03/15/addrow-tree.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/03/11/add2-tree.jpg)
