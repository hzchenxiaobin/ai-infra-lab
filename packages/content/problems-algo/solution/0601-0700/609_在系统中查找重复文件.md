...
title: 在系统中查找重复文件
tags: [哈希表, 字符串, 数组]
knowledge_points: [array, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 609
difficulty: medium
...
# LeetCode 在系统中查找重复文件 题解

## 1. 题目概述

- **标题 / 题号**：在系统中查找重复文件（#609，medium）
- **链接**：https://leetcode.cn/problems/find-duplicate-file-in-system/
- **难度**：中等
- **标签**：数组、哈希表、字符串

**题意**：给定一个目录信息列表 `paths`，每个字符串描述一个目录及其下的所有文件（含文件内容）。要求按路径返回文件系统中所有**内容完全相同**的重复文件组，每组至少包含两个文件。
...
将文件分块读取，逐块更新滚动哈希（如 [187. 重复的 DNA 序列](../0101-0200/187_重复的DNA序列.md) 的滚动哈希思想）：
...
> 💡 **面试建议**：进阶问题考察的是「从理想模型到工程落地」的思维迁移。核心思路：**用更小的代理（大小、哈希摘要）替代完整内容**，在精度与效率间取衡。这与 [187. 重复的 DNA 序列](../0101-0200/187_重复的DNA序列.md) 用滚动哈希替代子串比较是同一思想。
