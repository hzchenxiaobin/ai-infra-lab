...
type: problem
title: 员工的重要性
tags: [哈希表, 广度优先搜索, 树, 深度优先搜索]
knowledge_points: [bfs, dfs, hash-table, tree]
updated: 2026-09-03
source: leetcode
number: 690
difficulty: medium
...
- **标题 / 题号**：员工的重要性（#690，medium）
- **链接**：https://leetcode.cn/problems/employee-importance/
- **难度**：中等
- **标签**：深度优先搜索、广度优先搜索、哈希表、树

**题意**：你有一个保存员工信息的数据结构，它包含了员工唯一的 `id`、重要度 `importance` 和直系下属的 `id` 列表 `subordinates`。
...
![示例1](https://pic.leetcode.cn/1716170448-dKZffb-image.png)
...
![示例2](https://pic.leetcode.cn/1716170929-dkWpra-image.png)
...
1. **聚合操作可替换**：把 `total += importance` 换成「收集 id 进列表」即 [582. 杀死进程](../0501-0600/582_杀死进程.md)；换成「取 `max`」即 [559. N 叉树的最大深度](../0501-0600/559_N叉树的最大深度.md)；换成「计数」即统计子树节点数。**遍历骨架不变，只换聚合动作**。
...
| 559 | [N 叉树的最大深度](https://leetcode.cn/problems/maximum-depth-of-n-ary-tree/)（[题解](../0501-0600/559_N叉树的最大深度.md)） | 子树聚合的「取 max」版本，遍历骨架不变只换聚合动作 |
| 590 | [N 叉树的后序遍历](https://leetcode.cn/problems/n-ary-tree-postorder-traversal/)（[题解](../0501-0600/590_N叉树的后序遍历.md)） | N 叉树子树遍历模板，先子后根的后序天然适合「子树聚合」 |
