...
title: 重复叠加字符串匹配
tags: [字符串, 字符串匹配]
knowledge_points: [string, string-matching]
updated: 2026-09-03
source: leetcode
number: 686
difficulty: medium
...
# LeetCode 重复叠加字符串匹配 题解

## 1. 题目概述

- **标题 / 题号**：重复叠加字符串匹配（#686，medium）
- **链接**：https://leetcode.cn/problems/repeated-string-match/
- **难度**：中等
- **标签**：字符串、字符串匹配

**题意**：给定两个字符串 `a` 和 `b`，求**最少**需要将 `a` 重复多少次，才能使 `b` 成为重复后字符串的**子串**。如果无论重复多少次都不可能，返回 $-1$。
