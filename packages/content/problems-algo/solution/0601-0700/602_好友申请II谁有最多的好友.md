...
tags: ["GROUP BY", LIMIT, "ORDER BY", SQL, "UNION ALL", 聚合]
knowledge_points: [aggregation, group-by, limit, order-by, sql, union-all]
updated: 2026-09-03
source: leetcode
number: 602
difficulty: medium
...
# LeetCode 好友申请 II ：谁有最多的好友 题解

## 1. 题目概述

- **标题 / 题号**：好友申请 II ：谁有最多的好友（#602，medium）
- **链接**：https://leetcode.cn/problems/friend-requests-ii-who-has-the-most-friends/
- **难度**：中等
- **标签**：SQL、UNION ALL、GROUP BY、聚合、ORDER BY、LIMIT

**题意**：给定 `RequestAccepted` 表，每行记录一次被通过的好友申请：谁（`requester_id`）向谁（`accepter_id`）发送申请及通过日期。编写 SQL 查询，找出**拥有最多好友的人及其好友数目**。
