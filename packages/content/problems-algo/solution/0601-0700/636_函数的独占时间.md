...
title: 函数的独占时间
tags: [数组, 栈, 模拟]
knowledge_points: [array, simulation, stack]
updated: 2026-09-03
source: leetcode
number: 636
difficulty: medium
...
- **标题 / 题号**：函数的独占时间（#636，medium）
- **链接**：https://leetcode.cn/problems/exclusive-time-of-functions/
- **难度**：中等
- **标签**：栈、数组、模拟

**题意**：在一个**单核 CPU**上运行 `n` 个函数（编号 `0` 到 `n-1`）。CPU 同一时刻只能执行一个函数。函数可以调用其他函数（甚至递归调用自己），调用时当前函数被**暂停**，被调函数执行完毕后再**恢复**。
...
![示例1](https://assets.leetcode.com/uploads/2019/04/05/diag1b.png)
