...
title: "子数组最大平均数 II"
tags: [二分查找, 前缀和, 数组]
knowledge_points: [array, binary-search, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 644
difficulty: hard
...
# LeetCode 子数组最大平均数 II 题解

## 1. 题目概述

- **标题 / 题号**：子数组最大平均数 II（#644，hard）
- **链接**：https://leetcode.cn/problems/maximum-average-subarray-ii/
- **难度**：困难
- **标签**：数组、二分查找、前缀和

**题意**：给定一个由 $n$ 个整数组成的数组 `nums` 和一个整数 `k`，找出**长度大于等于 $k$ 的连续子数组**中**平均数最大**的那个，返回该最大平均数。答案误差不超过 $10^{-5}$ 即视为正确。
...
| 719 | [找出第 K 小的数对距离](https://leetcode.cn/problems/find-k-th-smallest-pair-distance/)（[题解](../0701-0800/719_找出第K小的数对距离.md)） | 二分答案 + 双指针计数，实数域二分的整数变体 |
