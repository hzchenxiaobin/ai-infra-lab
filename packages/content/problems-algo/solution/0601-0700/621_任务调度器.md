...
title: 任务调度器
tags: [数组, 计数, 贪心]
knowledge_points: [array, counting, greedy]
updated: 2026-09-03
source: leetcode
number: 621
difficulty: medium
...
- **标题 / 题号**：任务调度器（#621，medium）
- **链接**：https://leetcode.cn/problems/task-schedule/
- **难度**：中等
- **标签**：贪心、数组、计数

**题意**：给定一个用字符数组 `tasks` 表示的 CPU 任务列表（每个字母代表一种任务），以及冷却时间 `n`。**同一类任务执行后必须间隔** `n` **个单位时间**才能再执行同类任务。每个单位时间可执行一个任务或待命（idle）。求完成所有任务的最少时间。
