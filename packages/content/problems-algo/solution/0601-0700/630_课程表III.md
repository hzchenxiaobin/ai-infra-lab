...
- **标题 / 题号**：课程表 III（#630，hard）
- **链接**：https://leetcode.cn/problems/course-schedule-iii/
- **难度**：困难
- **标签**：贪心、堆（优先队列）、排序

**题意**：这里有 `n` 门不同的在线课程，按从 `1` 到 `n` 编号。给你一个数组 `courses`，其中 `courses[i] = [duration_i, lastDay_i]` 表示第 `i` 门课将会持续上 `duration_i` 天课，并且必须在不晚于 `lastDay_i` 的时候完成。
