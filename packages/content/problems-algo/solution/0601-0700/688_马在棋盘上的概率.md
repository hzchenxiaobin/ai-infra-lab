...
title: 马在棋盘上的概率
tags: [动态规划]
knowledge_points: [dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 688
difficulty: medium
...
# LeetCode 马在棋盘上的概率 题解

## 1. 题目概述

- **标题 / 题号**：马在棋盘上的概率（#688，medium）
- **链接**：https://leetcode.cn/problems/knight-probability-in-chessboard/
- **难度**：中等
- **标签**：动态规划

**题意**：已知一个 `n x n` 的国际象棋棋盘，一颗骑士（马）初始位于格子 `(row, column)`。骑士按国际象棋规则走「日」字形（L 形），每一步在 **8 个可能方向中等概率随机选择一个**（每个方向概率 $1/8$）。骑士连续走 `k` 步，一旦某一步走出棋盘则立刻停止移动（后续不再回到棋盘）。求骑士走完 `k` 步后**仍然留在棋盘上**的概率。

**示例 1**：

![示例1](https://assets.leetcode.cn/aliyun-lc-upload/uploads/2018/10/12/knight.png)
