...
type: problem
title: "矩形面积 II"
tags: [坐标压缩, 扫描线, 数组, 线段树]
knowledge_points: [array, coordinate-compression, segment-tree, sweep-line]
updated: 2026-09-03
source: leetcode
number: 850
difficulty: hard
...
- **标题 / 题号**：矩形面积 II（#850，hard）
- **链接**：https://leetcode.cn/problems/rectangle-area-ii/
- **难度**：困难
- **标签**：扫描线、坐标压缩、数组、线段树

**题意**：给定一个轴对齐矩形的列表 `rectangles`，其中 `rectangles[i] = [xi1, yi1, xi2, yi2]` 表示第 `i` 个矩形的左下角和右上角坐标。计算平面内所有矩形**覆盖的总面积**（即并集面积），结果对 $10^9 + 7$ 取模。

**示例 1**：

![示例1](https://s3-lc-upload.s3.amazonaws.com/uploads/2018/06/06/rectangle_area_ii_pic.png)
