...
type: problem
title: 最大平均值和的分组
tags: ["划分型 DP", 前缀和, 动态规划, 数组]
knowledge_points: [array, dynamic-programming, partition-dp, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 813
difficulty: medium
...
# LeetCode 最大平均值和的分组 题解

## 1. 题目概述

- **标题 / 题号**：最大平均值和的分组（#813，medium）
- **链接**：https://leetcode.cn/problems/largest-sum-of-averages/
- **难度**：中等
- **标签**：数组、动态规划、前缀和、划分型 DP

**题意**：给定数组 `nums` 和整数 `k`，将 `nums` 分成**最多** `k` 个**非空且连续**的子数组。**分数** = 各子数组平均值之和。要求用到 `nums` 中每一个数，返回能得到的最大分数（误差在 $10^{-6}$ 内即正确）。
