...
title: 最低加油次数
tags: [动态规划, "堆（优先队列）", 贪心]
knowledge_points: [dynamic-programming, greedy, heap]
updated: 2026-09-03
source: leetcode
number: 871
difficulty: hard
...
- **标题 / 题号**：最低加油次数（#871，hard）
- **链接**：https://leetcode.cn/problems/minimum-number-of-refueling-stops/
- **难度**：困难
- **标签**：贪心、堆（优先队列）、动态规划

**题意**：汽车从起点 `0` 出发驶向目的地，目的地位于出发位置东面 `target` 英里处。沿途有加油站，用数组 `stations` 表示，其中 `stations[i] = [position_i, fuel_i]` 表示第 `i` 个加油站位于出发位置东面 `position_i` 英里处，有 `fuel_i` 升汽油。
