...
title: 车队
tags: [单调栈, 排序, 贪心]
knowledge_points: [greedy, monotonic-stack, sorting]
updated: 2026-09-03
source: leetcode
number: 853
difficulty: medium
...
**题意**：在一条单行道上，有 `n` 辆车开往同一目的地，目的地在 `target` 英里以外。给定两个整数数组 `position` 和 `speed`，长度均为 `n`，其中 `position[i]` 是第 `i` 辆车的起始位置，`speed[i]` 是其速度（英里/小时）。
