...
type: problem
title: 喧闹和富有
tags: [图, 拓扑排序, 有向无环图, 深度优先搜索]
knowledge_points: [dfs, graph, topological-sort, 有向无环图]
updated: 2026-09-03
source: leetcode
number: 851
difficulty: medium
...
- **标题 / 题号**：喧闹和富有（#851，medium）
- **链接**：https://leetcode.cn/problems/loud-and-rich/
- **难度**：中等
- **标签**：深度优先搜索、图、拓扑排序、有向无环图

**题意**：有 $n$ 个人，编号 $0$ 到 $n-1$，每个人有不同数目的钱和不同的安静值。给定数组 `richer`，其中 `richer[i] = [ai, bi]` 表示 person $a_i$ **比** person $b_i$ **更有钱**；另给数组 `quiet`，`quiet[i]` 是 person $i$ 的安静值。题目保证 `richer` 的数据**逻辑自洽**（不会同时出现 $x$ 比 $y$ 富且 $y$ 比 $x$ 富，即无矛盾环）。
