...
title: 使序列递增的最小交换次数
tags: [动态规划, 数组, "状态机 DP"]
knowledge_points: [array, dynamic-programming, state-machine-dp]
updated: 2026-09-03
source: leetcode
number: 801
difficulty: hard
...
# LeetCode 使序列递增的最小交换次数 题解

## 1. 题目概述

- **标题 / 题号**：使序列递增的最小交换次数（#801，hard）
- **链接**：https://leetcode.cn/problems/minimum-swaps-to-make-sequences-increasing/
- **难度**：困难
- **标签**：数组、动态规划、状态机 DP

**题意**：给定两个**长度相同**的整数数组 `nums1` 和 `nums2`。一次操作可以选择某个下标 `i`，**交换** `nums1[i]` 与 `nums2[i]`（同一列上下互换）。问最少需要多少次交换，才能使 `nums1` 和 `nums2` **都严格递增**。题目保证输入一定有解。
