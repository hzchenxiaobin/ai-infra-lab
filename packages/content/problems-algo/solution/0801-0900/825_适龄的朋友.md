...
type: problem
title: 适龄的朋友
tags: [前缀和, 哈希表, 数组, 计数排序]
knowledge_points: [array, counting-sort, hash-table, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 825
difficulty: medium
...
- **标题 / 题号**：适龄的朋友（#825，medium）
- **链接**：https://leetcode.cn/problems/friends-of-appropriate-ages/
- **难度**：中等
- **标签**：数组、哈希表、计数排序、前缀和

**题意**：有 `n` 个人，`ages[i]` 是第 `i` 个人的年龄。人们会互相发送好友请求。**人物 `x` 不会向人物 `y`（`x != y`）发送好友请求**，若满足以下任一条件：
