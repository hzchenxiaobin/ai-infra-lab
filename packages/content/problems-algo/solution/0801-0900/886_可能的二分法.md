...
type: problem
title: 可能的二分法
tags: [二分图, 图, 并查集, 广度优先搜索, 深度优先搜索]
knowledge_points: [bfs, bipartite-graph, dfs, graph, union-find]
updated: 2026-09-03
source: leetcode
number: 886
difficulty: medium
...
- **标题 / 题号**：可能的二分法（#886，medium）
- **链接**：https://leetcode.cn/problems/possible-bipartition/
- **难度**：中等
- **标签**：深度优先搜索、广度优先搜索、并查集、图、二分图

**题意**：给定 `n` 个人（编号 `1` 到 `n`），想把每个人分进**任意**大小的两组。某些人互相讨厌，不能分在同一组。给定整数 `n` 和数组 `dislikes`，其中 `dislikes[i] = [ai, bi]` 表示 `ai` 和 `bi` 不能同组。判断是否能把所有人分成满足条件的两组。
