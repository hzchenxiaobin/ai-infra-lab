...
**题意**：帮派里有 `n` 名成员，给定若干可实施的犯罪。第 `i` 桩犯罪需要 `group[i]` 人参与，产生 `profit[i]` 的利润。每名成员**最多参与一桩**犯罪（即每桩犯罪要么整体选、要么不选，人员不重叠）。求在**成员总数不超过 `n`** 且**总利润至少为 `minProfit`** 的前提下，共有多少种**犯罪选择方案**。答案对 $10^9 + 7$ 取模。
