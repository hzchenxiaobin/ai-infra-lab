...
type: problem
title: 访问所有节点的最短路径
tags: [位运算, 动态规划, 哈希表, 图, 广度优先搜索, 状态压缩]
knowledge_points: [bfs, bit-manipulation, bitmask-dp, dynamic-programming, graph, hash-table]
updated: 2026-09-03
source: leetcode
number: 847
difficulty: hard
...
# LeetCode 访问所有节点的最短路径 题解

## 1. 题目概述

- **标题 / 题号**：访问所有节点的最短路径（#847，hard）
- **链接**：https://leetcode.cn/problems/shortest-path-visiting-all-nodes/
- **难度**：困难
- **标签**：位运算、广度优先搜索、图、动态规划、状态压缩、哈希表

**题意**：给定一个 **无向连通图**，有 `n` 个节点，编号 `0` 到 `n-1`。`graph[i]` 是与节点 `i` 相邻的所有节点列表（边无权，长度均为 1）。
...
![示例1](https://assets.leetcode.com/uploads/2021/05/12/shortest1-graph.jpg)
...
![示例2](https://assets.leetcode.com/uploads/2021/05/12/shortest2-graph.jpg)
