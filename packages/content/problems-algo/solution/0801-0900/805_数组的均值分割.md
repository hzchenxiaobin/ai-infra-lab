...
type: problem
title: 数组的均值分割
tags: ["Meet in the Middle", 动态规划, 数学, 数组]
knowledge_points: [array, dynamic-programming, math, meet-in-the-middle]
updated: 2026-09-03
source: leetcode
number: 805
difficulty: hard
...
- **标题 / 题号**：数组的均值分割（#805，hard）
- **链接**：https://leetcode.cn/problems/split-array-with-same-average/
- **难度**：困难
- **标签**：数组、数学、动态规划、Meet in the Middle

**题意**：给定一个整数数组 `nums`，需要将每个元素移动到数组 `A` 或数组 `B` 中，使得 `A` 和 `B` 都不为空，并且 `average(A) == average(B)`。可以完成则返回 `true`，否则返回 `false`。
