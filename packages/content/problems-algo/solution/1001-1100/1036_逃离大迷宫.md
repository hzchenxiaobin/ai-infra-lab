...
type: problem
title: 逃离大迷宫
tags: [BFS, 图, 广度优先搜索, 数学]
knowledge_points: [bfs, graph, math]
updated: 2026-09-03
source: leetcode
number: 1036
difficulty: hard
...
- **标题 / 题号**：逃离大迷宫（#1036，hard）
- **链接**：https://leetcode.cn/problems/escape-a-large-maze/
- **难度**：困难
- **标签**：图、BFS、广度优先搜索、数学

**题意**：在一个 $10^6 \times 10^6$ 的网格中，每个方格坐标为 `(x, y)`。从 `source = [sx, sy]` 出发，意图到达 `target = [tx, ty]`。给定封锁列表 `blocked`（其中每个方格禁止通行），每次可走上、下、左、右四个方向的相邻格，不能走出网格、不能踩封锁格。判断能否从 `source` 到达 `target`。
