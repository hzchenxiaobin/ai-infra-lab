...
title: 距离顺序排列矩阵单元格
tags: [BFS, 几何, 排序]
knowledge_points: [bfs, geometry, sorting]
updated: 2026-09-03
source: leetcode
number: 1030
difficulty: easy
...
# LeetCode 距离顺序排列矩阵单元格 题解

## 1. 题目概述

- **标题 / 题号**：距离顺序排列矩阵单元格（#1030，easy）
- **链接**：https://leetcode.cn/problems/matrix-cells-in-distance-order/
- **难度**：简单
- **标签**：BFS、排序、几何

**题意**：给定四个整数 `rows`、`cols`、`rCenter` 和 `cCenter`。有一个 $rows \times cols$ 的矩阵，你在单元格上的坐标是 $(rCenter, cCenter)$。
