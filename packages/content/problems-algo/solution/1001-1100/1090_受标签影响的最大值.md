...
# LeetCode 受标签影响的最大值 题解

## 1. 题目概述

- **标题 / 题号**：受标签影响的最大值（#1090，medium）
- **链接**：https://leetcode.cn/problems/largest-values-from-labels/
- **难度**：中等
- **标签**：贪心、数组、排序、哈希表

**题意**：给定两个等长整数数组 `values` 和 `labels`（长度均为 $n$），以及两个整数 `numWanted` 和 `useLimit`。`values[i]` 与 `labels[i]` 共同描述第 $i$ 个物品的**值**与**标签**。需要从中选出一个子集，使值的和最大，且满足：
