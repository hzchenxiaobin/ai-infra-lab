...
type: problem
title: 最长重复子串的长度
tags: [二分查找, 动态规划, 字符串, 滚动哈希]
knowledge_points: [binary-search, dynamic-programming, rolling-hash, string]
updated: 2026-09-03
source: leetcode
number: 1062
difficulty: medium
...
# LeetCode 最长重复子串的长度 题解

## 1. 题目概述

- **标题 / 题号**：最长重复子串的长度（#1062，medium）
- **链接**：https://leetcode.cn/problems/longest-repeating-substring/
- **难度**：中等
- **标签**：字符串、动态规划、二分查找、滚动哈希

**题意**：给定字符串 `s`，找出其中**出现至少两次**的最长子串的**长度**。若不存在重复子串，返回 `0`。
