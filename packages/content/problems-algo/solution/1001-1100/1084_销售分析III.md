...
type: problem
title: "销售分析 III"
tags: ["GROUP BY", HAVING, MAX, MIN, SQL, 反连接, 数据库, 聚合函数]
knowledge_points: [aggregate-function, anti-join, database, group-by, having, max, min, sql]
updated: 2026-09-03
source: leetcode
number: 1084
difficulty: easy
...
# LeetCode 销售分析 III 题解

## 1. 题目概述

- **标题 / 题号**：销售分析 III（#1084，easy）
- **链接**：https://leetcode.cn/problems/sales-analysis-iii/
- **难度**：简单
- **标签**：SQL、数据库、GROUP BY、HAVING、聚合函数、MIN、MAX、反连接

**题意**：给定 `Product` 表（产品信息）与 `Sales` 表（销售记录），编写 SQL 查询，报告**仅在 2019 年春季**售出的产品——即所有销售记录的 `sale_date` 都落在 2019-01-01（含）至 2019-03-31（含）之间的产品。
