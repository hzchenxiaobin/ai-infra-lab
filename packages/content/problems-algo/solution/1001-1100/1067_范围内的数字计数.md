...
title: 范围内的数字计数
tags: [按位计数, "数位 DP", 数学]
knowledge_points: [bit-counting, digit-dp, math, 按位计数]
updated: 2026-09-03
source: leetcode
number: 1067
difficulty: hard
...
# LeetCode 范围内的数字计数 题解

## 1. 题目概述

- **标题 / 题号**：范围内的数字计数（#1067，hard）
- **链接**：https://leetcode.cn/problems/digit-count-in-range/
- **难度**：困难
- **标签**：数学、数位 DP、按位计数

**题意**：给定一个数字 $d$（$0 \le d \le 9$）与两个整数 $low$、$high$（$low \le high$），统计 **$[low, high]$ 范围内所有整数**中数字 $d$ 出现的**总次数**。
