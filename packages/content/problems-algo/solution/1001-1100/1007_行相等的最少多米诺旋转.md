...
# LeetCode 行相等的最少多米诺旋转 题解

## 1. 题目概述

- **标题 / 题号**：行相等的最少多米诺旋转（#1007，medium）
- **链接**：https://leetcode.cn/problems/minimum-domino-rotations-for-equal-row/
- **难度**：中等
- **标签**：贪心、数组

**题意**：一排 `n` 张多米诺骨牌，`tops[i]` 和 `bottoms[i]` 分别表示第 `i` 张牌的上半部分和下半部分的数字（每个数字都在 `1~6` 之间）。一次「旋转」操作可交换第 `i` 张牌的 `tops[i]` 与 `bottoms[i]`。

返回**最小旋转次数**，使得**上排所有值相同**或者**下排所有值相同**。若不可能，返回 `-1`。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2021/05/14/domino.png)
