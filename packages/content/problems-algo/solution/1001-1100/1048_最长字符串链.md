...
title: 最长字符串链
tags: [动态规划, 哈希表, 排序, 数组]
knowledge_points: [array, dynamic-programming, hash-table, sorting]
updated: 2026-09-03
source: leetcode
number: 1048
difficulty: medium
...
- **标题 / 题号**：最长字符串链（#1048，medium）
- **链接**：https://leetcode.cn/problems/longest-string-chain/
- **难度**：中等
- **标签**：数组、哈希表、动态规划、排序

**题意**：给出一个单词数组 `words`，每个单词由小写英文字母组成。若**不改变其他字符的顺序**，在 `wordA` 的任意位置添加**恰好一个**字母使其变成 `wordB`，则称 `wordA` 是 `wordB` 的**前身**。例如 `"abc"` 是 `"abac"` 的前身，而 `"cba"` 不是 `"bcad"` 的前身。
...
> 💡 若把「前身」改为「子序列」关系（不要求长度差 1），则变为 [139. 单词拆分](../0101-0200/139_单词拆分.md) 的变体，需更复杂的状态定义。
...
| 139 | [单词拆分](https://leetcode.cn/problems/word-break/)（[题解](../0101-0200/139_单词拆分.md)） | 字符串 DP + 哈希查表，但判定方向相反（拆分而非拼接） |
