...
title: 驼峰式匹配
tags: [双指针, 字符串, 贪心]
knowledge_points: [greedy, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1023
difficulty: medium
...
- **标题 / 题号**：驼峰式匹配（#1023，medium）
- **链接**：https://leetcode.cn/problems/camelcase-matching/
- **难度**：中等
- **标签**：双指针、字符串、贪心

**题意**：给定一个字符串数组 `queries` 和一个模式串 `pattern`，返回一个布尔数组 `answer`。`answer[i]` 为 `true` 当且仅当可以通过在 `pattern` 的**任意位置插入小写字母**（也可不插入）得到 `queries[i]`。
