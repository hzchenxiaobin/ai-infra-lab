...
type: problem
title: "项目员工 III"
tags: ["GROUP BY", JOIN, MAX, RANK, SQL, 子查询, 数据库, 窗口函数]
knowledge_points: [database, group-by, join, max, rank, sql, subquery, window-function]
updated: 2026-09-03
source: leetcode
number: 1077
difficulty: medium
...
# LeetCode 项目员工 III 题解

## 1. 题目概述

- **标题 / 题号**：项目员工 III（#1077，medium）
- **链接**：https://leetcode.cn/problems/project-employees-iii/
- **难度**：中等
- **标签**：SQL、数据库、JOIN、GROUP BY、MAX、子查询、窗口函数、RANK

**题意**：给定项目表 `Project` 和员工表 `Employee`，编写 SQL 查询，选出**每个项目中经验年限最高的员工**的 `project_id` 和 `employee_id`。若一个项目有多名员工并列最高经验，则**全部返回**。
