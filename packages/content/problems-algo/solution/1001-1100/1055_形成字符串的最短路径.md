...
type: problem
title: 形成字符串的最短路径
tags: [动态规划, 双指针, 字符串, 贪心]
knowledge_points: [dynamic-programming, greedy, string, two-pointers]
updated: 2026-09-03
source: leetcode
number: 1055
difficulty: medium
...
# LeetCode 形成字符串的最短路径 题解

## 1. 题目概述

- **标题 / 题号**：形成字符串的最短路径（#1055，medium）
- **链接**：https://leetcode.cn/problems/shortest-way-to-form-string/
- **难度**：中等
- **标签**：贪心、双指针、字符串、动态规划

**题意**：给定两个仅由小写字母组成的字符串 `source` 和 `target`。每次操作可以从 `source` 中选出**一个子序列**（保持相对顺序、可不连续），将这次选出的子序列**拼接到结果末尾**。问**最少需要多少次操作**才能拼出 `target`；若无法拼出则返回 `-1`。
