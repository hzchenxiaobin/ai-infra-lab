...
title: "可被 K 整除的最小整数"
tags: [哈希, 数学]
knowledge_points: [hash-table, math]
updated: 2026-09-03
source: leetcode
number: 1015
difficulty: medium
...
# LeetCode 可被 K 整除的最小整数 题解

## 1. 题目概述

- **标题 / 题号**：可被 K 整除的最小整数（#1015，medium）
- **链接**：https://leetcode.cn/problems/smallest-integer-divisible-by-k/
- **难度**：中等
- **标签**：数学, 哈希

**题意**：给定正整数 $k$，找出**仅包含数字 1** 的最小正整数 $n$ 的**长度**，使得 $k \mid n$。若不存在这样的 $n$，返回 $-1$。
...
| # | 题目 | 与本题的关联 |
|---|------|-------------|
| 166 | [分数到小数](https://leetcode.cn/problems/fraction-to-recurring-decimal/)（[题解](../0101-0200/166_分数到小数.md)） | 长除法只跟踪余数，余数重复即循环节。与本题「不构造大数，只跟踪余数」同构，是模运算避免溢出的经典双题 |
| 202 | [快乐数](https://leetcode.cn/problems/happy-number/)（[题解](../0201-0300/202_快乐数.md)） | 数位平方和序列 + Floyd 判圈。与本题「序列到达目标 or 陷入循环」结构同构，都需判断序列是否终止 |
...
| 172 | [阶乘后的零](https://leetcode.cn/problems/factorial-trailing-zeroes/)（[题解](../0101-0200/172_阶乘后的零.md)） | 勒让德公式数因子。数论入门，与本题 $\gcd(k, 10)$ 判可行性对照，都是因子分析驱动 |
