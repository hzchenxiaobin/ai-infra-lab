...
type: problem
title: "项目员工 II"
tags: [ALL, "GROUP BY", RANK, SQL, 子查询, 数据库, 窗口函数, 聚合]
knowledge_points: [aggregation, all, database, group-by, rank, sql, subquery, window-function]
updated: 2026-09-03
source: leetcode
number: 1076
difficulty: easy
...
- **标题 / 题号**：项目员工 II（#1076，easy）
- **链接**：https://leetcode.cn/problems/project-employees-ii/
- **难度**：简单
- **标签**：SQL、数据库、GROUP BY、聚合、子查询、ALL、窗口函数、RANK

**题意**：给定 `Project` 表（项目—员工参与关系）与 `Employee` 表（员工信息），编写 SQL 查询，返回**员工数量最多**的所有项目 `project_id`。结果无顺序要求。
