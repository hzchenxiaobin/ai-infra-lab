...
title: 不相交的线
tags: ["二维 DP", 动态规划, 数组]
knowledge_points: [2d-dp, array, dynamic-programming]
updated: 2026-09-03
source: leetcode
number: 1035
difficulty: medium
...
- **标题 / 题号**：不相交的线（#1035，medium）
- **链接**：https://leetcode.cn/problems/uncrossed-lines/
- **难度**：中等
- **标签**：动态规划、二维 DP、数组

**题意**：给定两个整数数组 `nums1` 和 `nums2`，将它们分别写在两条水平线上。可以画**连接线**：一条连接线连接 `nums1[i]` 和 `nums2[j]`，要求 `nums1[i] == nums2[j]`，且该线不与任何其他连接线相交（端点也不能重合，每个数最多属于一条连线）。返回能画出的**最大连接线数**。

**示例 1**：

![示例1](https://assets.leetcode.com/uploads/2019/04/26/142.png)
