...
id: "lc:3784"
type: problem
title: 使所有字符相等的最小删除代价
tags: [哈希表, 字符串, 数组, 枚举]
knowledge_points: [array, enumeration, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 3784
difficulty: medium
...
# LeetCode 使所有字符相等的最小删除代价 题解

## 1. 题目概述

- **标题 / 题号**：使所有字符相等的最小删除代价（#3784，medium）
- **链接**：https://leetcode.cn/problems/minimum-deletion-cost-to-make-all-characters-equal/
- **难度**：中等
- **标签**：数组、哈希表、字符串、枚举

**题意**：给你一个长度为 $n$ 的字符串 $s$ 和一个整数数组 $\text{cost}$，其中 $\text{cost}[i]$ 表示**删除**字符串 $s$ 中第 $i$ 个字符的代价。
