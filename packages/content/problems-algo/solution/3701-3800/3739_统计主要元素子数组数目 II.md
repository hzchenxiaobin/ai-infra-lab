...
tags: [分治, 前缀和, 哈希表, 归并排序, 数组, 树状数组]
knowledge_points: [array, divide-and-conquer, fenwick-tree, hash-table, merge-sort, prefix-sum]
updated: 2026-09-03
source: leetcode
number: 3739
difficulty: hard
...
# LeetCode 统计主要元素子数组数目 II 题解

## 1. 题目概述

- **标题 / 题号**：统计主要元素子数组数目 II（#3739，hard）
- **链接**：https://leetcode.cn/problems/count-subarrays-with-majority-element-ii/
- **难度**：困难
- **标签**：数组、哈希表、前缀和、树状数组、归并排序、分治

**题意**：给你一个整数数组 `nums` 和一个整数 `target`。返回 `nums` 中满足 `target` 是**主要元素**的**子数组**数目。
