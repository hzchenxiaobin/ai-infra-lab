...
type: problem
title: 最大划分因子
tags: [二分查找, 图, 并查集, 广度优先搜索]
knowledge_points: [bfs, binary-search, graph, union-find]
updated: 2026-09-03
source: leetcode
number: 3710
difficulty: hard
...
- **标题 / 题号**：最大划分因子（#3710，hard）
- **链接**：https://leetcode.cn/problems/maximum-partition-factor/
- **难度**：困难
- **标签**：二分查找、图、广度优先搜索、并查集

**题意**：给定二维整数数组 `points`，`points[i] = [x_i, y_i]` 表示平面上第 $i$ 个点。两点间的**曼哈顿距离**为 $|x_i - x_j| + |y_i - y_j|$。
