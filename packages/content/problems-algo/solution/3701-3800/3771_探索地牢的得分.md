...
- **标题 / 题号**：探索地牢的得分（#3771，medium）
- **链接**：https://leetcode.cn/problems/total-score-of-dungeon-runs/
- **难度**：中等
- **标签**：数组、前缀和、二分查找

**题意**：给定一个正整数 `hp` 和两个正整数数组 `damage` 与 `requirement`，数组下标从 1 开始。地牢中有 $n$ 个陷阱房间，编号 $1$ 到 $n$：进入房间 $i$ 会使生命值减少 $\text{damage}[i]$；减少之后，若剩余生命值**至少**为 $\text{requirement}[i]$，则可以从该房间获得 1 分。
