...
title: 查找树的直径端点
tags: [图, 广度优先搜索, 树]
knowledge_points: [bfs, graph, tree]
updated: 2026-09-03
source: leetcode
number: 3787
difficulty: medium
...
# LeetCode 查找树的直径端点 题解

## 1. 题目概述

- **标题 / 题号**：查找树的直径端点（#3787，medium）
...
**题意**：给你一棵 $n$ 个节点（编号 $0 \sim n-1$）、$n-1$ 条边的无向树。树的**直径**是任意两节点间最长简单路径的**边数**。若某条直径的路径为 $u \to \cdots \to v$，则称 $u$、$v$ 是**直径端点**。由于最远点可能并列，树的直径往往不止一条，端点也可能不止两个。
