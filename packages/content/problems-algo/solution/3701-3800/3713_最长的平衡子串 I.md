...
title: "最长的平衡子串 I"
tags: [哈希表, 字符串, 枚举, 计数]
knowledge_points: [counting, enumeration, hash-table, string]
updated: 2026-09-03
source: leetcode
number: 3713
difficulty: medium
...
# LeetCode 最长的平衡子串 I 题解

## 1. 题目概述

- **标题 / 题号**：最长的平衡子串 I（#3713，medium）
- **链接**：https://leetcode.cn/problems/longest-balanced-substring-i/
- **难度**：中等
- **标签**：哈希表、字符串、计数、枚举

**题意**：给你一个仅由小写英文字母组成的字符串 `s`。如果一个**子串**中所有**不同**字符出现的次数都**相同**，则称该子串为**平衡**子串。请返回 `s` 的**最长平衡子串**的**长度**。
