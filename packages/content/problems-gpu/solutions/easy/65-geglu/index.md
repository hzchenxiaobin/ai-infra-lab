...
> 💡 这道题是 **kernel fusion 的经典练习**——GeGLU 本质是 3 步串联操作（split → GELU → multiply），朴素实现需要 3 个 kernel + 2 个临时数组。融合为单个 kernel 后，split/GELU/mul 在寄存器中完成，只需 1 次 HBM 读 + 1 次 HBM 写。与 [#54 SwiGLU](/solutions/easy/54-swiglu/) 是姊妹题——SwiGLU 用 SiLU 门控，GeGLU 用 GELU 门控，结构完全同构。
...
> 📎 完整可编译代码已整理到 <a href="./65-geglu.cu" download><code>65-geglu.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 65-geglu.cu -o geglu
./geglu
```
