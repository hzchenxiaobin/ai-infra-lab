...
> 📎 完整可编译代码已整理到 <a href="./7-color-inversion.cu" download><code>7-color-inversion.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 7-color-inversion.cu -o invert
./invert 4096 5120
```
...
```bash
# 分别编译两个版本
nvcc -O3 -arch=sm_120 -DUSE_NAIVE 7-color-inversion.cu -o invert_naive
nvcc -O3 -arch=sm_120                7-color-inversion.cu -o invert_vec
