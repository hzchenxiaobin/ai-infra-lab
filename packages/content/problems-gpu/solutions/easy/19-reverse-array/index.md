...
> 💡 与 [Vector Reversal（#32）](/solutions/easy/108-vector-reversal/) 的区别：Vector Reversal 是 `input → output`（两个数组，读 input 写 output，天然 coalesced）；Reverse Array 是 **in-place**（单个数组，交换对称位置的元素）。in-place 的核心挑战是**避免双重交换**——如果每个 thread 都执行 `swap(i, N-1-i)`，对称的两个 thread 会把交换做两遍，结果等于没换。
...
> 📎 完整可编译代码已整理到 <a href="./19-reverse-array.cu" download><code>19-reverse-array.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 19-reverse-array.cu -o reverse_array
