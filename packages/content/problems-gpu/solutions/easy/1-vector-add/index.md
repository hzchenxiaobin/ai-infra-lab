...
> 📎 完整可编译代码已整理到 <a href="./1-vector-add.cu" download><code>1-vector-add.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
nvcc -O3 -arch=sm_120 1-vector-add.cu -o vector_add
