...
> 📎 完整可编译代码已整理到 <a href="./68-sigmoid.cu" download><code>68-sigmoid.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 68-sigmoid.cu -o sigmoid
./sigmoid 50000000
```
...
nvcc -O3 -arch=sm_120 -DUSE_EXPF 68-sigmoid.cu -o sigmoid_expf
nvcc -O3 -arch=sm_120            68-sigmoid.cu -o sigmoid_fast
