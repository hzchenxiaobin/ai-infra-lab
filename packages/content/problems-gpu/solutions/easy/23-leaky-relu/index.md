...
> 📎 完整可编译代码已整理到 <a href="./23-leaky-relu.cu" download><code>23-leaky-relu.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 23-leaky-relu.cu -o leaky_relu
./leaky_relu 50000000
```
...
```bash
# 分别编译两个版本
nvcc -O3 -arch=sm_120 -DUSE_IFELSE 23-leaky-relu.cu -o leaky_ifelse
nvcc -O3 -arch=sm_120                   23-leaky-relu.cu -o leaky_fmaxf
...
> 注：上面 `23-leaky-relu.cu` 默认是 `fmaxf` 版本；如需 if-else 对比，在文件顶部用 `#ifdef USE_IFELSE` 包一份 `if (x>0) o=x; else o=0.01f*x;` 的 kernel 即可。
