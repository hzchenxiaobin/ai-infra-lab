...
languages: [cpp, cuda]
judge: leetgpu-com
related_learn: ["learn:note:cuda-interview-notes", "learn:w01d02", "learn:w01d07", "learn:w01d07:exercise:leetgpu-week1-review"]
---
...
> 📎 完整可编译代码已整理到 <a href="./21-relu.cu" download><code>21-relu.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 21-relu.cu -o relu
./relu 25000000
```
...
```bash
# 分别编译两个版本
nvcc -O3 -arch=sm_120 -DUSE_IFELSE 21-relu.cu -o relu_ifelse
nvcc -O3 -arch=sm_120                   21-relu.cu -o relu_fmaxf
...
> 注：上面 `21-relu.cu` 默认是 `fmaxf` 版本；如需 if-else 对比，在文件顶部用 `#ifdef USE_IFELSE` 包一份 `if (x<0) o=0; else o=x;` 的 kernel 即可。关键指标解读见下表。
