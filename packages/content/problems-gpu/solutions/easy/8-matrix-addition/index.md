...
tags: [CUDA, Roofline, "coalesced access", element-wise, "float4 向量化", "grid-stride loop", memory-bound]
knowledge_points: [2d-grid-映射, coalesced-access, cuda, elementwise, float4-向量化, grid-stride-loop, memory-bound, parallel-patterns, roofline]
updated: 2026-09-07
source: leetgpu
number: 8
difficulty: easy
languages: [cpp, cuda]
judge: leetgpu-com
related_learn: ["learn:w01d03", "learn:w01d07", "learn:w01d07:exercise:leetgpu-week1-review", "learn:w02d03", "learn:w10d03"]
---
...
> 💡 这是 Week 1 的"毕业考试"题之一（[Day 7 综合练习 1](../../../../learn/daily/week1/day7/README.md)）。它是 **memory-bound** 算子的典型案例——算术强度 `1 FLOP / 12B`（1 次加法 ↔ 读 2 个 float + 写 1 个 float），远低于 GPU 平衡点。优化核心不是"算得快"而是"**读得快**"：用 `float4` 向量化加载把 4 条 `load` 合并为 1 条 128-bit 事务，配合 grid-stride loop 和 coalesced access 把 HBM 带宽榨干。本题综合检验 Week 1 的三大概念：coalesced access（Day 4）、occupancy 调优（Day 2）、Roofline 判定（Day 6）。
...
把 `M×N` 矩阵视为一维 `float` 数组（行主序连续），用 **1D grid-stride loop**（[Day 1 Vector Addition](/solutions/easy/1-vector-add/) 的同款模式）映射线程到元素。在此基础上叠加 `float4` **向量化**：每 thread 一次处理 4 个 float（128-bit），把 4 条 `load` 合并为 1 条 `float4` 加载。
...
> 💡 Matrix Addition 是**纯 element-wise**——每个输出 `C[i]` 只依赖 `A[i]` 和 `B[i]`，无邻域复用。因此**不需要 shared memory**（与 [Matrix Transpose #3](/solutions/easy/3-matrix-transpose/) 的 tiling、[Matrix Multiplication #2](/solutions/easy/2-matrix-multiplication/) 的 tile 复用截然不同）。优化全在 global memory 访问效率上。
...
> 📎 完整可编译代码已整理到 <a href="./8-matrix-addition.cu" download><code>8-matrix-addition.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 8-matrix-addition.cu -o matadd
./matadd 4096 4096
```
...
4. **CUDA Streams 流水线**：极大矩阵可分块多 stream，H2D/compute/D2H 重叠（[Day 3](../../../../learn/daily/week2/day3/README.md) 主题）。本题单 kernel 已够快，streams 适合 host-device 流水线场景。
