...
tags: [CUDA, GEMM, Linear, PyTorch, "batch size", 推理基础]
knowledge_points: [batch-size, cuda, gemm, linear, parallel-patterns, pytorch, 推理基础, 简单-pytorch-算子封装]
updated: 2026-09-07
source: leetgpu
number: 41
difficulty: easy
...
judge: leetgpu-com
related_learn: ["learn:w06d01", "learn:w07d01"]
---
...
> 💡 这道题是 [Week6 Day1](../../../../learn/daily/week6/day1/README.md) Dynamic Batching 的**微缩版**——它直接展示了 batch_size 对推理性能的影响。性能测试用 `batch_size=1000` 而非 1，正是因为大 batch 让 GEMM 的 M 维足够大，充分利用 GPU 并行。Dynamic Batcher 在系统层面做的事就是把多个单请求（M=1）聚合成大 batch（M=N），让模型 forward 的 GEMM 从 memory-bound 逼近 compute-bound。
...
下面给出适配 LeetGPU 官方 starter 签名的 Python 提交版本。
...
| # | 题目 | 难度 | 核心概念 | 与本题的关联 |
|---|------|------|----------|-------------|
| 85 | [LoRA Linear](https://leetgpu.com/challenges/lora-linear) | 中等 | — | LoRA Linear，低秩适配的推理变体 |
| 106 | [Token Embedding Layer](https://leetgpu.com/challenges/token-embedding-layer) | 中等 | — | Token Embedding，推理管线组件 |
| 74 | [GPT-2 Transformer Block](https://leetgpu.com/challenges/gpt-2-transformer-block) | 困难 | — | GPT-2 Block，完整推理模块 |
| 2 | [Matrix Multiplication](https://leetgpu.com/challenges/matrix-multiplication) | 简单 | — | Matrix Multiplication，Linear 的 CUDA 实现 |

> 💡 **选题思路**：PyTorch Linear 前向封装，练习推理管线的最简形态。做完这组练习，即可掌握该 CUDA 模板在不同场景下的迁移应用。
