...
> 💡 这道题是最简**带条件的 elementwise kernel**——与 [#21 ReLU](/solutions/easy/21-relu/)（`max(x, 0)`）同构，但 clamp 有双边界（上界 + 下界）。核心考点是**分支发散（warp divergence）**：朴素 `if-else` 实现在 warp 内边界数据处产生分支串行；**无分支版**用 `fminf`/`fmaxf` 硬件指令消除 divergence。
...
> 📎 完整可编译代码已整理到 <a href="./62-value-clipping.cu" download><code>62-value-clipping.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 62-value-clipping.cu -o clip
./clip
```
