...
- **标题 / 题号**：Sliding Window Self-Attention（#59，hard）
...
> 📎 完整可编译代码已整理到 <a href="./59-sliding-window-attn.cu" download><code>59-sliding-window-attn.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 59-sliding-window-attn.cu -o swa
