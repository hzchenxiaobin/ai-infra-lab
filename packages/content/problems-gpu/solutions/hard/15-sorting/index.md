...
> 📎 完整可编译代码已整理到 <a href="./15-sorting.cu" download><code>15-sorting.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```cuda
// 15-sorting.cu —— LeetGPU Sorting 提交版
...
```bash
nvcc -O3 -arch=sm_120 15-sorting.cu -o sorting
./sorting 1000000
```
