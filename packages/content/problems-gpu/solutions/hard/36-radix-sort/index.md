...
> 📎 完整可编译代码已整理到 <a href="./36-radix-sort.cu" download><code>36-radix-sort.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```cuda
// 36-radix-sort.cu —— LeetGPU Radix Sort 提交版
...
```bash
nvcc -O3 -arch=sm_120 36-radix-sort.cu -o radixsort
./radixsort 1000000
```
