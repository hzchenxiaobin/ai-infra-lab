...
tags: [CUDA, element-wise, memory-bound, "ncu profiling 对象", "分块 scale", 量化推理]
knowledge_points: [cuda, elementwise, memory-bound, ncu-profiling-对象, quantization, quantized-inference, weight-dequantization, 分块-scale]
updated: 2026-09-07
source: leetgpu
number: 64
difficulty: medium
...
> 💡 这道题是 [Week5 Day6](../../../../learn/daily/week5/day6/README.md) 讲的 **ncu profiling 的理想分析对象**——一个典型的 memory-bound element-wise kernel。每元素只做 1 次乘法（FLOPs 极少），但要读 X + 读 S + 写 Y（HBM 流量大），算术强度极低。用 ncu 测它的 `dram__throughput`（接近峰值）和 `sm__throughput`（很低），验证"AI 极低 → memory-bound"的判据。推理系统里它是 INT8/INT4 量化推理的必经步骤——量化权重从 HBM 读出后反量化再 GEMM。
...
> 📎 完整可编译代码已整理到 <a href="./64-weight-dequantization.cu" download><code>64-weight-dequantization.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 64-weight-dequantization.cu -o dequant -lineinfo
...
> ⚠️ **关键观察**：`dram__throughput` 高 + `sm__throughput` 低 = memory-bound。这正是 [Day6 profiling](../../../../learn/daily/week5/day6/README.md) 讲的 ncu 判据。Weight Dequant 是教学最清晰的 memory-bound 例子——AI≈0.17 远低于 Ridge，瓶颈纯粹在搬运数据。优化方向只能是"减数据量"（量化、压缩）或"提升带宽利用"（coalescing、vector load）。
...
> 💡 **一句话总结**：Weight Dequantization 是 memory-bound element-wise kernel 的教科书例子——每元素 1 次乘法，AI≈0.17 ≪ Ridge。它是 [Day6 ncu profiling](../../../../learn/daily/week5/day6/README.md) 的理想分析对象：`dram__throughput` 接近峰值、`sm__throughput` 极低，确证 memory-bound。推理系统里它是 INT8/INT4 量化推理的必经步骤，优化方向是 vector load + 与 GEMM 融合消除 Y 物化。
