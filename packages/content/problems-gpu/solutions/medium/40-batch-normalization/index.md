...
> 📎 完整可编译代码已整理到 <a href="./40-batch-normalization.cu" download><code>40-batch-normalization.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
# 编译带 lineinfo
nvcc -o batchnorm_profile 40-batch-normalization.cu -O3 -arch=sm_120 -g -lineinfo
# ncu profiling
