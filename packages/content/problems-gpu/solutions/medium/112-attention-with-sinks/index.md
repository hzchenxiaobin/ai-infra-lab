...
> 📎 完整可编译代码已整理到 <a href="./112-attention-with-sinks.cu" download><code>112-attention-with-sinks.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```cuda
// 112-attention-with-sinks.cu —— LeetGPU Attention with Sinks 提交版
...
```bash
nvcc -O3 -arch=sm_120 112-attention-with-sinks.cu -o attn_sinks -lineinfo
