...
languages: [cpp, cuda]
judge: leetgpu-com
related_learn: ["learn:note:cuda-interview-notes", "learn:w06d03"]
---
...
> 💡 这道题是 **LLM 解码的完整 microcosm**：一次 top-p 采样串联了 softmax（[Softmax](/solutions/medium/5-softmax/)）、并行排序（[Top K Selection](/solutions/medium/29-top-k-selection/)）、前缀扫描（[Prefix Sum](/solutions/medium/16-prefix-sum/)）和 CDF 采样四个 CUDA 模板。它考察的不是单个技巧，而是**如何将多个 kernel 融合为一个完整的推理管线**——这正是 LLM 推理引擎（vLLM、TensorRT-LLM）在 decode 阶段的核心工作。
...
2. **五阶段串联**：
   - **Softmax**：block 内 reduction 求 max 和 sum（同 [Softmax](/solutions/medium/5-softmax/) 两遍扫描）
   - **Bitonic sort**：对 (prob, idx) pair 做并行排序（同 [Top K Selection](/solutions/medium/29-top-k-selection/) 的 bitonic 网络）
   - **Cumsum**：warp scan + block scan（同 [Prefix Sum](/solutions/medium/16-prefix-sum/) 三阶段模板）
