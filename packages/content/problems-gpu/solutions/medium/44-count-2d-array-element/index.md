...
tags: ["2D 索引", CUDA, atomicAdd, memory-bound, predicate, "warp shuffle", "归约（reduction）"]
knowledge_points: [2d-索引, atomic-add, cuda, memory-bound, predicate, reduction, reduction-scan, warp-shuffle, 计数-直方图]
updated: 2026-09-07
source: leetgpu
number: 44
difficulty: medium
...
> 📎 完整可编译代码已整理到 <a href="./44-count-2d-array-element.cu" download><code>44-count-2d-array-element.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_75 44-count-2d-array-element.cu -o count_2d
