...
> 💡 这道题是 [2D Convolution（#10）](/solutions/medium/10-2d-convolution/) 的"max reduction"变体。两者都是滑窗 stencil：每个输出对应输入中一个 `K×K` 邻域。差异在于：卷积是**加权求和**（reduce by sum），池化是**取最大值**（reduce by max）。由于 `K=3` 小且 max 操作不可分离（不像 Gaussian blur 可行列分离），朴素滑窗 + 直接 global 访存是标准做法；边界 padding 用 `-∞` 跳过即可。
...
> 📎 完整可编译代码已整理到 <a href="./42-2d-max-pooling.cu" download><code>42-2d-max-pooling.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 42-2d-max-pooling.cu -o max_pool2d
