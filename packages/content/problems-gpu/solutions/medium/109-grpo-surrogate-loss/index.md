...
> 💡 GRPO 是 DeepSeek-Math / DeepSeek-R1 的核心训练算法。它把 PPO 的"critic 估 advantage"换成"组内奖励标准化"，省掉 value 网络。本题的**前向损失**正是这个 advantage 计算 + PPO clip + KL 惩罚的融合——它和 [Mean Squared Error](/solutions/medium/27-mean-squared-error/)、[Categorical Cross Entropy](/solutions/medium/25-categorical-cross-entropy-loss/) 同属"**fused element-wise + global reduction**"损失类 kernel 模板，但多了一层"组内归约（advantage）+ 全局归约（mean）"的两级结构。
...
> ⚠️ **致命问题**：朴素版要分配 **5~6 个 `B·G·S` 大小的临时数组**（`ratio`/`clipped`/`surrogate`/`kl_diff`/`kl_penalty`/`term`），每个 16 MB（FP32, `N=4.19M`）。HBM 读写量爆炸：每个中间结果写一次、下一步读一次，**总 HBM 流量 ≈ 12 遍 `N·4B` ≈ 200 MB**，而真正有用的输入只有 `log_pi/log_pi_old/log_ref` 三个数组共 48 MB。这是典型的"算子粒度太细导致 memory-bound 雪崩"——和 [Day 5 Softmax](/solutions/medium/5-softmax/) 的三遍扫描、[Cross Entropy](/solutions/medium/25-categorical-cross-entropy-loss/) 的多 kernel 链同病。
...
> 📎 完整可编译代码已整理到 <a href="./109-grpo-surrogate-loss.cu" download><code>109-grpo-surrogate-loss.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 109-grpo-surrogate-loss.cu -o grpo -lineinfo
./grpo                       # 性能 shape
...
> 💡 **一句话总结**：GRPO Surrogate Loss 是"**两级归约 + 全融合**"的 RL 损失模板——组内 warp shuffle 求 advantage，全局 block reduce + atomicAdd 求均值，中间 6 个逐 token 算子全压进寄存器。它把 [Reduction](/solutions/medium/4-reduction/) 的归约骨架、[Softmax](/solutions/medium/5-softmax/) 的数值稳定性、[Cross Entropy](/solutions/medium/25-categorical-cross-entropy-loss/) 的 fused-loss 思想融到一道题，是 RL 训练 kernel 的微缩实战。掌握它，迁移到任何"逐 token 算子 + 标量归约"的损失（REINFORCE、DPO、PPO 全家桶）都是同一套骨架。
