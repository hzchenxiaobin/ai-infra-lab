...
tags: [CUDA, memory-bound, "prefix sum", "segmented scan", "warp shuffle"]
knowledge_points: [cuda, memory-bound, prefix-sum-gpu, reduction-scan, segmented-scan, warp-shuffle, 分段扫描]
updated: 2026-09-07
source: leetgpu
number: 70
difficulty: medium
languages: [cpp, cuda]
judge: leetgpu-com
related_learn: ["learn:note:cuda-interview-notes", "learn:w06d04", "learn:w07d03"]
---
...
> 💡 这道题的**段内独立扫描 + 段边界处理**与 [Week6 Day4](../../../../learn/daily/week6/day4/README.md) 的 Chunked Prefill 把长 prompt 拆成多个 chunk 同构——每个 chunk 像一个段，段内独立做 attention，段间通过 KV Cache 累积（carry）。segmented prefix sum 段间**不 carry**，chunked prefill 段间**carry**（KV 状态传递），对比学习能加深理解。
...
> 📎 完整可编译代码已整理到 <a href="./70-segmented-prefix-sum.cu" download><code>70-segmented-prefix-sum.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 70-segmented-prefix-sum.cu -o segmented_prefix_sum
