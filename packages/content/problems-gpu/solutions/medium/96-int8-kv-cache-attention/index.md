...
tags: [Attention, CUDA, Decode-phase, "INT8 量化", "KV Cache", memory-bound, "online softmax", "per-token scale"]
knowledge_points: [attention, cuda, decode-phase, int8-kv-cache-attention, int8-quantization, kv-cache, memory-bound, online-softmax, per-token-scale]
updated: 2026-09-07
source: leetgpu
number: 96
difficulty: medium
...
> 💡 这道题就是 [Week5 Day1](../../../../learn/daily/week5/day1/README.md) 讲的 **Decode 阶段核心算子**：单 query 对 KV Cache 做 1×L 的 attention，是典型 **memory-bound**。题目把 KV 存成 int8 + per-token scale，正是 Day1 "减少 KV Cache 读取"优化方向的落地——int8 相比 fp32 把 KV 的 HBM 流量直接砍到 1/4。生产级推理系统（TensorRT-LLM、vLLM）都用这套。
...
> ⚠️ 正确做法是**把反量化流式地融在 attention kernel 里**：从 HBM 读 int8 K/V → 寄存器里乘 scale 成 fp32 → 立刻用于点积/加权 → 丢弃。fp32 K/V 永不落 HBM。这就是"kernel fusion 消除中间矩阵物化"的又一次应用（与 [Week4 Softmax Attention](/solutions/medium/6-softmax-attention/) 消除 S/P 物化同源）。
...
2. **online softmax 三公式**（与 [Week4 Softmax Attention](/solutions/medium/6-softmax-attention/) 完全一致）：遍历 `L` 个 token，用 `m/l/o` 增量更新，`S=QK^T` 和 `P=softmax(S)` 永不落 HBM。
...
> 📎 完整可编译代码已整理到 <a href="./96-int8-kv-cache-attention.cu" download><code>96-int8-kv-cache-attention.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
__global__ void int8_kv_attention_kernel(const float* __restrict__ Q, const int8_t* __restrict__ K_int8,
    ...
    for (int s = 0; s < seq_len; ++s) {
        const int8_t* Ks = K_int8 + (h * seq_len + s) * head_dim;
        const int8_t* Vs = V_int8 + (h * seq_len + s) * head_dim;
        const float ks = k_scale[h * seq_len + s];
        const float vs = v_scale[h * seq_len + s];
...
extern "C" void solve(const float* Q, const int8_t* K_int8, const int8_t* V_int8,
                      ...
                      int num_heads, int seq_len, int head_dim) {
    int8_kv_attention_kernel<<<num_heads, BLOCK_SIZE>>>(Q, K_int8, V_int8, k_scale, v_scale, output,
...
```bash
nvcc -O3 -arch=sm_120 96-int8-kv-cache-attention.cu -o int8_kv_attn -lineinfo
