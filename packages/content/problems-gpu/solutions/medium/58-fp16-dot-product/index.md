...
> 💡 这道题是 [Dot Product（#17）](/solutions/medium/17-dot-product/) 的半精度变体。核心难点不在归约结构（与 FP32 版完全一致），而在于**精度保证**：FP16 只有 10 bit 尾数（约 3 位有效十进制），直接累加大数组会迅速丢失精度。标准做法是**读 half、转 FP32、FP32 累加、最后再转回 half**——即 `__half2float()` / `__float2half()` 两次类型转换。
...
> 📎 完整可编译代码已整理到 <a href="./58-fp16-dot-product.cu" download><code>58-fp16-dot-product.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 58-fp16-dot-product.cu -o fp16_dot
