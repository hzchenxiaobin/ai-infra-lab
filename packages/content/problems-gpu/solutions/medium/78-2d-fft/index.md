...
```cuda
// 78-2d-fft.cu —— LeetGPU 2D FFT 提交版
...
> 📎 完整可编译代码已整理到 <a href="./78-2d-fft.cu" download><code>78-2d-fft.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_75 78-2d-fft.cu -o 2d_fft
