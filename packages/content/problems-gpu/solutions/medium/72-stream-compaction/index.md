...
> 📎 完整可编译代码已整理到 <a href="./72-stream-compaction.cu" download><code>72-stream-compaction.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
# 编译
nvcc -O3 -arch=sm_120 72-stream-compaction.cu -o stream_compaction
# ncu profiling
