...
> 📎 完整可编译代码已整理到 <a href="./28-gaussian-blur.cu" download><code>28-gaussian-blur.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 28-gaussian-blur.cu -o gaussian_blur
