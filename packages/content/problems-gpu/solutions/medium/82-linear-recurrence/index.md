...
**关键洞察**：虽然递推本身是串行的，但每一步的操作（乘 $a$ 加 $x$）是一个**仿射变换**（affine transform）$h \mapsto a \cdot h + x$，而仿射变换的**复合**满足**结合律**。这意味着我们可以用**并行前缀扫描**（parallel prefix scan）来加速，就像 [Prefix Sum](/solutions/medium/16-prefix-sum/) 用加法结合律做并行扫描一样。
...
2. **三阶段分块扫描**（同 [Prefix Sum](/solutions/medium/16-prefix-sum/) 模板）：
