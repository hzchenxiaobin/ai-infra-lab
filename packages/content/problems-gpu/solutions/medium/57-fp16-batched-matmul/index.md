...
> 💡 这道题是 [#30 Batched Matrix Multiplication](/solutions/medium/30-batched-matrix-multiplication/) 的半精度变体。核心新概念是 **FP16 存储 + FP32 累加**的精度保证策略——FP16 只有 10 bit 尾数（~3.3 位十进制），直接累加会精度灾难；转 FP32（23 bit 尾数）后累加，最终转回 FP16。而 FP16 输入 + FP32 累加恰好与 **WMMA Tensor Core** 的 fragment 类型天然契合——`half` 输入 fragment + `float` 累加器 fragment，一条 `mma.sync` 指令即可完成 `16×16×16` 矩阵乘加（8192 FLOP），吞吐比 CUDA Core 高一个数量级。与 [#22 GEMM](/solutions/medium/22-gemm/) 的 WMMA 范式完全一致，只是多了 batch 维度（`blockIdx.z`）。
...
> 📎 完整可编译代码已整理到 <a href="./57-fp16-batched-matmul.cu" download><code>57-fp16-batched-matmul.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 57-fp16-batched-matmul.cu -o fp16_bmm_wmma
./fp16_bmm_wmma
```
...
```bash
nvcc -O3 -arch=sm_120 -Xptxas -v 57-fp16-batched-matmul.cu -o fp16_bmm_wmma 2>&1 | rg "registers|spill|stack|smem"
```
