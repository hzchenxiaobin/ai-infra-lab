...
languages: [cpp, cuda]
judge: leetgpu-com
related_learn: ["learn:note:cuda-interview-notes", "learn:w03d06"]
---
...
> 💡 RMSNorm 是 **Llama / T5 / Gopher** 等现代大模型替代 LayerNorm 的首选归一化层。它和 [Day 2 Softmax](/solutions/medium/5-softmax/) 一样是"归约 + 归一化"组合，但**只做一次归约**（sum of squares），比 LayerNorm 的两次（mean + variance）更省。本题是练习"单次块归约 + elementwise"的经典模板，也是 Day 20 端到端 Profiling 的完美 memory-bound 靶点。
...
RMSNorm 只需一次 sum 归约。直接复用 [Day 4 Reduction](/solutions/medium/4-reduction/) 的 `warp_reduce_sum` + `block_reduce_sum` 模板：
...
> 💡 对比 [Day 2 Softmax](/solutions/medium/5-softmax/) 的**两次块归约**（max + sum），RMSNorm 只需**一次**。这是它比 LayerNorm/Softmax 更省 HBM 读的根本原因——归约次数直接决定 global memory 读取遍数。
...
> 📎 完整可编译代码已整理到 <a href="./50-rms-normalization.cu" download><code>50-rms-normalization.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 50-rms-normalization.cu -o rmsnorm -lineinfo
./rmsnorm 128 8192
```
