...
tags: ["CDF 查找", CUDA, "accept/reject 采样", "batched kernel", "scan（找首次拒绝）", "vLLM Worker", 投机解码, 推理系统]
knowledge_points: [accept-reject-采样, batched-kernel, cdf-查找, cuda, inference-system, scan-找首次拒绝, speculative-decoding, transformer-inference, vllm-worker]
updated: 2026-09-07
source: leetgpu
number: 87
difficulty: medium
...
> 💡 这道题是 [Week5 Day3](../../../../learn/daily/week5/day3/README.md) 讲的 **vLLM Worker 执行的验证 kernel**。Speculative Decoding 是推理系统的调度层优化——Scheduler 编排"draft 批量生成 + target 批量验证"迭代，Worker 跑验证 kernel。本题就是那个 kernel：对 B 个序列并行做 accept/reject + resample。B 序列的并行度正是 Continuous Batching 拼出来的 batch。
...
> 📎 完整可编译代码已整理到 <a href="./87-speculative-decoding-verification.cu" download><code>87-speculative-decoding-verification.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 87-speculative-decoding-verification.cu -o spec_decode -lineinfo
