...
> 💡 **数值稳定**：所有 `exp` 都减去 running max `m_new`，保证指数 ≤ 0，永不溢出。这正是 [Day 4 Softmax](/solutions/medium/5-softmax/) "减最大值"思想的在线版。
>
...
> 📎 完整可编译代码（naive 对比版 + fused 版）已整理到 <a href="./6-softmax-attention.cu" download><code>6-softmax-attention.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
> 📎 本地自测版完整代码已整理到 <a href="./6-softmax-attention-fa2-tiling.cu" download><code>6-softmax-attention-fa2-tiling.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。

**编译运行**：

```bash
nvcc -O3 -arch=sm_120 6-softmax-attention-fa2-tiling.cu -o fa2_tiling -lineinfo
./fa2_tiling 1024 1024 64        # M=1024 N=1024 d=64
./fa2_tiling 4096 4096 64        # 大序列测试
# 查看 SRAM 使用量：
nvcc -Xptxas -v -O3 -arch=sm_120 6-softmax-attention-fa2-tiling.cu -o fa2_tiling
