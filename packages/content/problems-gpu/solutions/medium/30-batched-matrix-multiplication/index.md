...
tags: [CUDA, "batched GEMM", "register blocking", "shared memory tiling"]
knowledge_points: [batched-gemm, cuda, gemm, register-blocking, shared-memory-tiling]
updated: 2026-09-07
source: leetgpu
number: 30
difficulty: medium
...
> 💡 这道题的 **batched GEMM** 与 [Week6 Day5](../../../../learn/daily/week6/day5/README.md) Mini Engine v1 的多请求并发 forward 同构——v1 每轮把多个请求拼 batch 送 model forward，其中 attention/FFN 的核心计算就是 batched GEMM（batch=请求数）。batched matmul 的"每个 batch 独立计算、共享 kernel launch"正是 v1 Scheduler"每轮选多请求组 batch、一次 forward"的底层映射。
...
> 📎 完整可编译代码已整理到 <a href="./30-batched-matrix-multiplication.cu" download><code>30-batched-matrix-multiplication.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 30-batched-matrix-multiplication.cu -o batched_matmul
...
```bash
nvcc -O3 -arch=sm_120 -Xptxas -v 30-batched-matrix-multiplication.cu -o batched_matmul 2>&1 | rg "registers|spill|stack|smem"
```
