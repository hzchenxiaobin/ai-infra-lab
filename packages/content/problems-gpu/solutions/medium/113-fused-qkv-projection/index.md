...
下面给出适配 LeetGPU 官方 starter 签名的提交版本，单个融合 kernel 在 tiled GEMM 写回阶段直接完成 Q/K/V 的 head 分离布局映射，彻底消除中间 qkv 张量。
...
extern "C" void solve(const float* x, const float* W_qkv,
                      float* Q, float* K, float* V,
                      int M, int num_heads, int head_dim) {
    int D = num_heads * head_dim;
    dim3 grid((3*D + TILE-1)/TILE, (M + TILE-1)/TILE);
    dim3 block(TILE, TILE);
    fused_qkv_kernel<<<grid, block>>>(x, W_qkv, Q, K, V, M, D, num_heads, head_dim);
    cudaDeviceSynchronize();
}
```

> 📎 完整可编译代码已整理到 <a href="./113-fused-qkv-projection.cu" download><code>113-fused-qkv-projection.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
# 编译
nvcc -O3 -arch=sm_80 113-fused-qkv-projection.cu -o fused_qkv
