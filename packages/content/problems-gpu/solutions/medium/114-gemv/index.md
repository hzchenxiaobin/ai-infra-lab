...
> 💡 本题是 [Simple Inference (#41)](/solutions/easy/41-simple-inference/) 的**极致反面**：#41 用大 `batch_size` 把 GEMM 推到 compute-bound，而 GEMV 是 `batch_size=1` 的极端——**算术强度只有 0.5 FLOP/byte，深度 memory-bound**。LLM 自回归解码（逐 token 生成）阶段几乎每个 `Linear` 都是 GEMV，这正是解码阶段被带宽卡住的根因，也是 Continuous Batching / speculative decoding 等系统优化的动机。
...
下面给出适配 LeetGPU 官方 starter 签名的提交版本，采用「1 block 负责一行 + 256 线程沿 K 维 grid-stride + 块归约 + float4 向量化」使 W 的读取完全合并。
...
extern "C" void solve(const float* W, const float* x, const float* bias,
                      float* y, int N, int K) {
    gemv_kernel<<<N, BLOCK_DIM>>>(W, x, bias, y, N, K);
    cudaDeviceSynchronize();
}
```

> 📎 完整可编译代码已整理到 <a href="./114-gemv.cu" download><code>114-gemv.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
# 编译
nvcc -O3 -arch=sm_80 114-gemv.cu -o gemv
...
W 在 GEMM 中被 M 个输入行复用 → 算术强度随 M 线性增长 → 从带宽受限翻转为算力受限。这正是 [Simple Inference (#41)](/solutions/easy/41-simple-inference/) 中 `batch_size` 从 1 到 1000 吞吐提升 ~60 倍的根因。
...
3. **权重量化（INT8/INT4）**：把 W 压到 1 字节甚至 4 bit，带宽下界降 4×–8×，是解码阶段最有效的加速（参考 [INT8 Quantized MatMul (#32)](/solutions/medium/32-int8-quantized-matmul/)、[INT4 Weight-Only (#81)](/solutions/medium/81-int4-matmul/)）。
