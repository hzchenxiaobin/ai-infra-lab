...
> 📎 完整可编译代码已整理到 <a href="./34-logistic-regression.cu" download><code>34-logistic-regression.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_75 34-logistic-regression.cu -o logreg
./logreg
```
