...
tags: [CUDA, atomicAdd, memory-bound, predicate, "warp shuffle", "归约（reduction）"]
knowledge_points: [atomic-add, cuda, memory-bound, predicate, reduction, reduction-scan, warp-shuffle, 计数-直方图]
updated: 2026-09-07
source: leetgpu
number: 43
difficulty: medium
...
核心思想：**把"计数"重写成"判定（1/0）后求和"**，从而完全复用 [#4 Reduction](/solutions/medium/4-reduction/) 的两阶段归约骨架，**全程不发任何 atomic**。
...
> 📎 完整可编译代码已整理到 <a href="./43-count-array-element.cu" download><code>43-count-array-element.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 43-count-array-element.cu -o count_array
