...
id: "gpu:m:061"
type: problem
title: "LeetGPU Rotary Positional Embedding 题解"
tags: [CUDA, elementwise, memory-bound, rotate_half, 位置编码]
knowledge_points: [cuda, elementwise, memory-bound, normalization-embedding, rope, rotate-half, 位置编码]
updated: 2026-09-07
source: leetgpu
number: 61
difficulty: medium
...
> 💡 RoPE 是 LLaMA 架构的核心位置编码组件。与 [Week8 Day2 架构图](../../../../learn/daily/week8/day2/README.md) 直接对应——在五层架构图中，RoPE 位于第④层（模型层）的 Attention 之前，是位置信息注入点。理解 RoPE 的数据流（Q 分两路：一路乘 cos，一路 rotate_half 后乘 sin，再相加）就是画一张"微型数据流图"，与系统级数据流图同构。
...
> 📎 完整可编译代码已整理到 <a href="./61-rope-embedding.cu" download><code>61-rope-embedding.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 61-rope-embedding.cu -o rope
