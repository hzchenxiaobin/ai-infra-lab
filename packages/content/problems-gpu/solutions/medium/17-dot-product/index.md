...
type: problem
title: "Dot Product"
tags: [CUDA, "block 归约", "kernel 融合", "warp shuffle", "归约（reduction）"]
knowledge_points: [block-reduction, cuda, dot-product, kernel-fusion, reduction, reduction-scan, warp-shuffle]
updated: 2026-09-07
source: leetgpu
number: 17
difficulty: medium
...
> 💡 这道题的**两级归约**（block 内 + 跨 block）是 [Week6 总结](../../../../learn/daily/week6/day7/README.md) 中所有"累加/统计"操作的底层模板——benchmark 的 `percentile()` 统计、Scheduler 的 token budget 累加、Continuous Batcher 的 batch 聚合，本质都是 dot product 式的归约。
...
> 📎 完整可编译代码已整理到 <a href="./17-dot-product.cu" download><code>17-dot-product.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 17-dot-product.cu -o dot_product
