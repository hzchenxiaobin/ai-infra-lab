...
tags: ["2D 索引映射", CUDA, memory-bound, "warp shuffle", "归约（reduction）", 范围求和]
knowledge_points: [2d-索引映射, cuda, memory-bound, range-sum, reduction, reduction-scan, warp-shuffle, 子数组和-最大子数组, 范围求和]
updated: 2026-09-07
source: leetgpu
number: 48
difficulty: medium
...
> 📎 完整可编译代码已整理到 <a href="./48-2d-subarray-sum.cu" download><code>48-2d-subarray-sum.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 48-2d-subarray-sum.cu -o 2d_subarray_sum
./2d_subarray_sum
```
