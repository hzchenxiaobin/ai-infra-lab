...
> 📎 完整可编译代码已整理到 <a href="./108-dpo-sequence-loss.cu" download><code>108-dpo-sequence-loss.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```cuda
// 108-dpo-sequence-loss.cu —— LeetGPU DPO Sequence Loss 提交版
...
```bash
nvcc -O3 -arch=sm_120 108-dpo-sequence-loss.cu -o dpo
./dpo 65536 0.1
```
