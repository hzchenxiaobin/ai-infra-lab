...
tags: [CUDA, "bitonic sort", reduction, selection, 堆归约]
knowledge_points: [bitonic-sort, cuda, heap-reduction, reduction, sampling-sorting-search, selection, top-k-selection]
updated: 2026-09-07
source: leetgpu
number: 29
difficulty: medium
...
> 💡 这道题的 **top-k 选择**与 [Week6 Day6](../../../../learn/daily/week6/day6/README.md) benchmark 的 P99 latency 计算同构——P99 就是"找出延迟排第 99 百分位的值"，本质是 top-k selection（k=N×0.01，选第 k 小）。benchmark 的 `percentile()` 是串行排序版，这道题用 GPU 并行加速。
...
> 📎 完整可编译代码已整理到 <a href="./29-top-k-selection.cu" download><code>29-top-k-selection.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 29-top-k-selection.cu -o top_k
