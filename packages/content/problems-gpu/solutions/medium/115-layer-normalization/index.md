...
> 💡 LayerNorm 是 **Transformer（GPT/BERT）** 的标配归一化层，沿 **feature 维度**（最后一维 `D`）归一化，与 BatchNorm 沿 batch 维度归一化形成对照。它和 [RMS Normalization (#50)](/solutions/medium/50-rms-normalization/) 是「归约 + 归一化」组合的两种形态：LayerNorm **两次归约**（先 mean，再 variance，且 variance 依赖 mean），RMSNorm 省掉 mean-centering 只需 **一次归约**。本题是练习「**串行两次块归约** + mean-centering 归一化」的经典模板，也是理解 Llama 为何用 RMSNorm 替代 LayerNorm 的性能参照。
...
> ⚠️ LayerNorm 的核心难点和 [Softmax](/solutions/medium/5-softmax/)、[RMSNorm](/solutions/medium/50-rms-normalization/) 一样：它是「**归约（mean）→ 依赖归约结果的归约（variance）→ 依赖归约结果的归一化**」三阶段，必须用**块内协作**——一个 block 的线程共同求 mean、再共同求 variance、再一起写输出。每 thread 独立扫整行会导致 `O(D²)` 重复读。
...
> 💡 **为什么是「串行两次归约」而非一次？** variance 公式 $\sigma^2 = \text{mean}((x-\mu)^2)$ 中的 $\mu$ 来自 Pass 1 的归约结果——归约结果必须先经 `__syncthreads` 广播到所有 thread，Pass 2 才能开始算 $(x-\mu)^2$。这正是 [RMSNorm](/solutions/medium/50-rms-normalization/) 省掉 mean-centering 后只需一次归约的根本差异。
...
LayerNorm 需要两次 sum 归约（mean、variance）。直接复用 [Reduction](/solutions/medium/4-reduction/) 的 `warp_reduce_sum` + `block_reduce_sum` 模板：
...
> 💡 对比 [RMSNorm](/solutions/medium/50-rms-normalization/) 的**一次块归约**（sum of squares），LayerNorm 是**两次**。归约次数直接决定 global memory 读取遍数：RMSNorm 读 `x` 2 遍，LayerNorm 读 `x` 3 遍。
...
下面给出适配 LeetGPU 官方 starter 签名的提交版本，一个 block 负责一行，串行两次块归约（mean → variance）后完成 mean-centering 归一化写回。
...
extern "C" void solve(const float* x, const float* gamma, const float* beta,
                      float* y, int M, int D, float eps) {
    layernorm_kernel<<<M, BLOCK_SIZE>>>(x, gamma, beta, y, M, D, eps);
    cudaDeviceSynchronize();
}
```

> 📎 完整可编译代码已整理到 <a href="./115-layer-normalization.cu" download><code>115-layer-normalization.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_80 115-layer-normalization.cu -o layernorm -lineinfo
./layernorm
```
...
Transformer 把 LayerNorm + QKV GEMM 融合成单个 kernel，省去 LayerNorm 输出 `(B,N,d)` 的一次 HBM 读写。LayerNorm 是 memory-bound，融合后收益最大——这正是 [Fused QKV Projection](/solutions/medium/113-fused-qkv-projection/) 等「归一化 + 投影」融合 kernel 的动机。
...
> 💡 **一句话总结**：LayerNorm 是「两次串行归约（mean → variance，后者依赖前者）+ mean-centering 归一化」的经典模板——它比 [RMSNorm](/solutions/medium/50-rms-normalization/)（一次归约）多一遍 HBM 读，根因正是 variance 公式里的 $\mu$ 必须先归约出来。掌握了 [Reduction 的 `block_reduce`](/solutions/medium/4-reduction/) 积木后，本题就是把它调用两次并串起依赖链。它的 memory-bound 本质（AI ≈ 0.56）让它成为 profiling 的好靶点——用 ncu 看 `DRAM% >> SM%` 就能一眼判定，而 shared 缓存把读遍数从 3 降到 1 是最直接的优化。
