...
> 📎 完整可编译代码已整理到 <a href="./110-gae-reverse-scan.cu" download><code>110-gae-reverse-scan.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```cuda
// 110-gae-reverse-scan.cu —— LeetGPU Parallel Reverse Scan (GAE) 提交版
...
```bash
nvcc -O3 -arch=sm_120 110-gae-reverse-scan.cu -o gae
