...
> 📎 完整可编译代码已整理到 <a href="./67-moe-topk-gating.cu" download><code>67-moe-topk-gating.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 67-moe-topk-gating.cu -o moe_topk
