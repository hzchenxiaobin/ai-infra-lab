...
tags: [CUDA, LayerNorm, embedding, gather, 推理引擎第一个算子, "融合 kernel"]
knowledge_points: [cuda, embedding, gather, kernel-fusion, layernorm, normalization-embedding, token-embedding, 推理引擎第一个算子]
updated: 2026-09-07
source: leetgpu
number: 106
difficulty: medium
...
> 💡 这道题是 [Week5 Day5](../../../../learn/daily/week5/day5/README.md) 讲的 **Mini 推理引擎 v0 的第一个算子**——`self.embedding(input_ids)` 在 v0 里是 PyTorch 的 `nn.Embedding`，本题就是它的手写 CUDA 版。引擎 `model.forward` 第一步把 token id 转成向量，本题拆成 `gather token emb + gather pos emb + 相加 + LayerNorm` 四步融合。Week7 替换 PyTorch 后端时，引擎的 embedding 层就换成这个手写 kernel。
...
void embed_cpu(const int* token_ids, const int* pos_ids, const float* tok_emb, const float* pos_emb, const float* gamma,
               const float* beta, float* output, int B, int T, int V, int P, int D, float eps) {
    for (int b = 0; b < B; ++b)
        for (int t = 0; t < T; ++t) {
...
> ⚠️ 正确做法：**一个 kernel 内完成 gather + 加 + LayerNorm**，中间的 `s[D]` 只活在寄存器/shared memory，永不落 HBM。这与 [Week4 Softmax Attention](/solutions/medium/6-softmax-attention/) 消除 S/P 物化同理——"融合消除中间矩阵"。
...
> 📎 完整可编译代码已整理到 <a href="./106-token-embedding-layer.cu" download><code>106-token-embedding-layer.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 106-token-embedding-layer.cu -o token_emb -lineinfo
...
> 💡 **一句话总结**：Token Embedding Layer 是推理引擎的第一个算子——token id → 向量。用融合 kernel 把 gather + 加 + LayerNorm 一次完成（中间 `s[D]` 不落 HBM）。它是 [Day5 Mini 引擎](../../../../learn/daily/week5/day5/README.md) `self.embedding` 的手写版，Week7 替换 PyTorch 后端时直接用。瓶颈在 embedding 大表的 gather 随机读，优化靠行对齐 + L2 友好访问。
