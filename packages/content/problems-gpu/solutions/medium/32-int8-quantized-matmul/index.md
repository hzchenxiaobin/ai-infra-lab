...
> 📎 完整可编译代码已整理到 <a href="./32-int8-quantized-matmul.cu" download><code>32-int8-quantized-matmul.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 32-int8-quantized-matmul.cu -o int8_matmul
