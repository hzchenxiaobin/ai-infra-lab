...
> 📎 完整可编译代码已整理到 <a href="./37-matrix-power.cu" download><code>37-matrix-power.cu</code></a>（含 host 端测试 harness，编译与运行命令见文件头注释，用于本地自测与 profiling）。
...
```bash
nvcc -O3 -arch=sm_120 37-matrix-power.cu -o matrix_power
./matrix_power 512 3
```
